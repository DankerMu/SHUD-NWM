...
readonly COMPRESSION_ENV_FILE PRECHECK_REPO_ROOT
...
if [ ! -f "$PRECHECK_SCRIPT" ] || [ -L "$PRECHECK_SCRIPT" ]; then
  fail "compression budget preflight is unavailable or a symlink"
fi

# The preflight reads the compression env as inert descriptor-bound data and
# builds the runner environment itself.  -E ignores caller PYTHONPATH for
# preflight imports while retaining it for the final runner environment it
# constructs.
