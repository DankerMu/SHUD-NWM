...
def build_run_plan(
    *,
    ...
    for label, value in (("repo", repo), ("root", root), ("schema_dump_host", schema_dump_host)):
        ...
        # pg_dump argv (:181) and into that command's ``artifact_associations`` (:183),
        # and all three conjuncts carry for it.  An interior ``//`` false-refuses at the
        # verbatim-vs-normalized association comparison (live_evidence.py :1439,
        ...
        # (:196's command records none, so :1439 never sees it), and its ENTIRE consumer
        # set compares it textually with zero normalization on either side: the verifier's
        # prefix+shape argv gate (live_evidence.py :744-749) and the same prefix check on
        # the captured listing (:1892), plus the supervisor's mirror gates -- the identical
        # prefix+shape check in ``_assert_exact_argv`` (supervisor.py :350-364) and
        ...
        # and ``docker exec sha256sum``s that exact string.  The verbatim-vs-normalized
        # false refusal therefore cannot occur for it; an adjudication test pins that
        # ruling so a future change guarding it must flip the test consciously.
