...
-- primary, in FK order, in ONE statement (the DO block) of ONE transaction.
--
-- Run ONLY through scripts/ops/node27_oneshot_sql.py, AFTER
-- node27_1729_delete_evidence_basin_backup.sql wrote its backup into the same
-- --copy-dir, and ONLY after the user confirmed the dry-run
-- (tasks.md 6.3). Dry-run is the runner default (rolled back; NOTICEs carry
-- every count and per-step timing); --apply commits. The restore is
-- node27_1729_delete_evidence_basin_rollback.sql.
--
...
-- Hypertables are deliberately NOT scanned (.workplans/k3/PROBE-NOTE.txt: a
-- scan of their station/key columns ran >10 min on the live primary):
--   * met.forcing_station_timeseries(station_key) and
--     met.forcing_station_timeseries_legacy(station_id / forcing_version_id)
--     are covered by NO ACTION FKs -- a referencing row makes the DELETE fail
...
BEGIN
    ...
    -- 7. Delete, children first; every step must remove exactly the inventory.
