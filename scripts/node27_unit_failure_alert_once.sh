...
# tick every 30 min. No second channel is introduced.
#
# Exit discipline: a MISCONFIGURED alerter must not add a second failed unit
# on top of the failure it is reporting, so the unconfigured-recipient and
# absent-transport paths log the reason and exit 0. A transport that is
# present and REJECTS the message is not a soft failure — it exits non-zero so
# a silently broken alert lane is itself visible.

set -euo pipefail

HOST=$(hostname 2>/dev/null || echo node-27)
SENDMAIL="${NHMS_FRONTIER_SENDMAIL:-/usr/sbin/sendmail}"
RECIPIENT="${NHMS_ALERT_EMAIL_TO:-}"
SENDER="${NHMS_ALERT_EMAIL_FROM:-NHMS Node-27 Alert <nwm@$HOST>}"
