...
CALLER_PYTHON_OVERRIDE=${NODE27_COLD_RESIDENCY_PYTHON-}
CALLER_SCRIPT_OVERRIDE=${NODE27_COLD_RESIDENCY_SCRIPT-}
readonly CALLER_PYTHONPATH CALLER_PYTHON_OVERRIDE CALLER_SCRIPT_OVERRIDE
ENV_FILE=${NODE27_COLD_RESIDENCY_ENV_FILE:-/home/nwm/NWM/infra/env/node27-cold-residency.env}
...
# Wrapper wall. Same env file, same name, same empty-string handling as the
# runner's Python-side default injection. There is no reserve-byte default.
WALL=${NODE27_COLD_RESIDENCY_WRAPPER_WALL_SECONDS:-3901}
case "$WALL" in
...
REPO_ROOT=${NODE27_COLD_RESIDENCY_REPO_ROOT:-/home/nwm/NWM}
case "$REPO_ROOT" in
...
SCRIPT=${CALLER_SCRIPT_OVERRIDE:-$REPO_ROOT/scripts/node27_cold_residency.py}
...
if [ ! -f "$SCRIPT" ] || [ -L "$SCRIPT" ]; then
  echo '{"status":"failed","reason":"cold residency entrypoint is unavailable or a symlink"}' >&2
  exit 1
fi
