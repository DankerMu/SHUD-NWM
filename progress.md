...
- 证据命令：`cd apps/frontend && corepack pnpm run test:e2e:m15-visual`。Manifest：`.codex/evidence/issue-176/manifest.json`；截图：`.codex/evidence/issue-176/screenshots/`。本地 manifest SHA 解析为运行时当前 git `HEAD`；CI 证据必须等于 `GITHUB_SHA`，PR 证据必须在 final commit/frozen PR head 后重新生成，禁止 `local-uncommitted` 等 placeholder。
