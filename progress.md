...
- M21 #214 smoke/evidence freeze 使用 [`docs/runbooks/qhh-mvp-smoke-evidence.md`](docs/runbooks/qhh-mvp-smoke-evidence.md) 作为 readiness claim 索引。每条 claim 需要 stable ID、mode、command、artifact path 和 claim boundary；新增 `/hydro-met` deterministic Playwright smoke 只证明 mocked latest-product、station inventory、station-series、`q_down` forecast-series、GFS/IFS 和 IFS 144h shorter-horizon UI wiring，不证明 live backend 或 final production readiness。
...
2. **MVP P0 opt-in live smoke**：在目标环境准备好 target DB、对象存储、Slurm、source credentials、共享日志、IdP/operator 身份和浏览器入口后，跑一轮 QHH GFS/IFS live smoke，验收下载 -> canonical -> forcing -> SHUD -> parse -> station series -> forecast-series -> `/hydro-met` 展示 -> `/ops` 日志/重启；未跑前保持 skipped/blocked。
