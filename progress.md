...
当前新重点已完成标准多周期 Slurm 闭环：以 `data/Basins/qhh` 为基准流域完成 GFS/IFS 从下载、canonical、forcing、SHUD、parse 到 display product 的真实后端链路，补齐前端展示所需 API 数据产品；`scripts/run_qhh_backend_smoke.sh` 保留首轮 GFS 标准复现入口，真实 run `qhh_gfs_2026052100_smoke` 已解析入库 11431 行、1633 个 SHUD 输出河段，展示发布后 run 状态为 `frequency_done`。新增 `scripts/run_qhh_cycle.sh`、`scripts/run_qhh_cycle.sbatch` 与 `scripts/run_qhh_continuous.py`，以 `.nhms-runs/qhh-continuous/` 为项目内运行根，支持 GFS 与 IFS 多周期自动扫描、单周期真实全链路运行、Slurm 提交/等待、锁文件、状态 JSON、已完成周期跳过和 IFS unavailable 显式记录；Slurm 作业执行完整 `run_qhh_cycle.sh`，SHUD runtime 也在 compute node 内运行；持续运行 runbook 见 `docs/runbooks/qhh-continuous.md`。2026-05-21 00Z/06Z 的 GFS/IFS 四个 run 均已达到 `frequency_done`，forcing version 均为 386 站点；06Z 两个周期在 Slurm job `5743`/`5744` 上完成，batch MaxRSS 约 75-79GB。已完成 forcing producer 第一阶段 streaming 优化，按 valid_time 逐时次读取 canonical field 并只保留 IDW 所需 grid cell 值，后续需用新周期 Slurm 实测优化后 MaxRSS。qhh 暂无洪水频率曲线，因此 return-period 产品只发布 `no_frequency_curve` 质量状态，不伪造重现期或预警等级。
...
- 真实 qhh Slurm workload 已完成 GFS/IFS 00Z/06Z 单轮闭环；仍需纳入正式 orchestrator、array 化、长日志回收、accounting 汇总和失败重试长链路。
...
1. **QHH Slurm 多周期生产化收口**：把已跑通的专用 qhh GFS/IFS full-chain 纳入正式 orchestrator/integration lane，沉淀 reset/seed/manifest/display-product 生成合同，补 array 化、失败重试、长日志归档和与原始历史 forcing 同时段的数值复现对比；过程记录维护在 `docs/runbooks/qhh-continuous.md`。forcing producer 已完成第一阶段 streaming 优化，下一步是用新 GFS/IFS 周期在 Slurm 上重跑并对比 MaxRSS，再决定是否继续拆 lead-hour array。
2. **M12 后续数据合同**：为 `/segments/:segmentId` 补 station/forcing/weather/frequency-curve/run lineage 合同后接入真实 PRCP/TEMP/RH/wind/Press、forcing 小图、qhh 可用频率曲线和 lineage；继续禁止伪造未接入数据。
3. **效果图 5/6：气象数据产品页**：把 M13 前端合同夹具升级为后端 API/OpenAPI 合同和真实 tile/query/area-stat/station-series 数据源；继续禁止伪造未接入图层。
4. **效果图 7 / M18 模型资产操作**：readonly 浏览已扩展为经 M17 RBAC、preflight、审计保护的 registry lifecycle mutation；支持 activate、deactivate、switch version、rollback version、supersede、deprecate，canonical 状态为 `inactive`/`active`/`deprecated`/`superseded`，`active_flag` 仅保留兼容。仍不支持任意模型包上传、生产对象存储 delete/upload、模型创建/编辑/发布 UI。
5. **真实 MVT 与全国规模性能**：M16 已完成 measured deterministic MVT contract、metadata、frontend vector consumption、bounded GeoJSON compatibility guardrail 和 performance evidence；canonical `.pbf` production success 仍依赖目标环境 live PostGIS。下一步补 live PostGIS/national-data/browser proof，移除 `m16-live-postgis-national-proof` release blocker。
6. **真实生产验证**：在目标环境跑 live auth、alert sink、rollback、真实 Slurm、真实对象存储、真实气象下载。
7. **CLDAS 接入**：实现 adapter、授权数据质量检查、best_available 生产路径。
8. **生产身份认证/授权**：M17 已建立 backend auth context / RBAC matrix；pipeline retry/cancel、hindcast rerun-cycle、model activate/deactivate 现由后端在 mutation 前统一判定。缺失/无效认证、角色不满足、live IdP 未证明分别稳定为 `401 AUTH_REQUIRED`、`403 RBAC_FORBIDDEN`、`503 RELEASE_BLOCKED`；deterministic/backend/live/readiness evidence 明确区分 `policy_simulated`、`backend_route_executed`、`live_proof`、`release_blocked`。后续仍需在目标环境补 opt-in live IdP proof 并移除 release blocker。
