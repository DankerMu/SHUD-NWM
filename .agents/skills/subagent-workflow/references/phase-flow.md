...
```json
{"issue":<N>,"pr":<N>,"date":"<merge-date>","fixture":"none|compact|expanded|high|broad-expanded","rounds":<comprehensive-rounds>,"gate_net_catch":<n>,"verdicts":{"confirmed":<n>,"plausible":<n>,"refuted":<n>},"residual_deferred":<n>,"premerge_skip_blocks":<n>,"rotation_intent":"deliberate|incidental","round_lenses":[["<lens>","..."],["..."]],"catches":[{"round":<n>,"lens":"<lens>","class":"<failure class>","severity":"<sev>"}],"phase7_catches":[{"lens":"<lens>","class":"<failure class>","severity":"<sev>"}]}
```
...
- `round_lenses`: the lens mix actually used per round (index 0 = round 1) — the instrumentation for the pinned-core + rotating-free-slots trial. **Every element is a non-empty list of lens names** and is machine-checked by `evidence_check.py --loop-log-entry`; a bare string (the historical flat shape, one lens name per round) makes `round_lenses[0]` a set of characters, which pins core at 0 and forces every catch into `rotated` — the audit drops those lines from the rotation sample as unattributable.
...
- `phase7_catches`: findings from the **Phase 7 final review**, same item shape minus `round` (it runs after the numbered rounds). Final review is a round role, not a rotated-in lens: it is counted as its own bucket and never as core/rotated. Writing it into `catches` under a pseudo-lens (`final-review`, `gap-sweep`, …) is rejected by `evidence_check.py --loop-log-entry`.
- `rotation_intent` (optional): `deliberate` when free slots were actually rotated in, `incidental` when the lens mix moved for an unrelated reason (reviewer availability, scope change). Omit it and the audit infers rotation from the round-1/later lens-set difference; a later round that only *narrows* round 1 is a contraction and leaves the rotation denominator either way.
...
**Consume the log, don't just append to it**: after appending, run `scripts/loop_log_audit.py --log docs/review-loop-log.jsonl`. Exit 2 means a decision is now owed — a fixture level has reached the keep/cut sample with zero net catch, or the rotation evidence now **contradicts** the decision recorded in `docs/adr/0003-review-lens-rotation-keep.md`. Record the ADR in `docs/adr/` (or a one-line recorded deferral with reason) before starting the next issue. `--rotation-decision keep|cut` names the recorded decision (default `keep`); consistent evidence prints as a NOTE, because re-asking a settled question on every closure is noise, not accountability. `--rotation-decision none` restores the unconditional "sample reached, decide" behaviour.

Rotation keep/cut criterion (same human-call mechanism as below, minimum sample ~8 merged multi-round PRs that **actually rotated** — PRs whose later rounds only narrowed round 1 are not in the denominator, and Phase 7 final review is its own bucket): if later-round catches concentrate in rotated-in lenses, rotation is buying real union recall — keep. If they come almost entirely from pinned-core lenses on fix-touched code, rotation buys nothing — record the cut in `docs/adr/` and revert follow-up rounds to the round-1 mix.
