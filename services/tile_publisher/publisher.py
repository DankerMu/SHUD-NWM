...
_COPYBACK_MAX_FILE_BYTES = 512 * 1024 * 1024
_COPYBACK_MAX_DEPTH = 32
...
class _ForcingPackageValidationError(RuntimeError):
    def __init__(self, ref: _ForcingPackageRef, error: BaseException) -> None:
...
class TilePublisher:
    """Publish q_down display products and shared object-store run products."""
    ...
    def publish_cycle(self, cycle_id: str) -> PublishResult:
        return self.publish_qdown_cycle(cycle_id)

    # ------------------------------------------------------------------
    # q_down display publication
    # ------------------------------------------------------------------
    def publish_qdown_cycle(self, cycle_id: str) -> PublishResult:
        """Publish q_down display products from parsed or published hydro runs."""
    ...
    def _upsert_qdown_layer(
        ...
        *,
        ...
        DB. Mirrors the table's column-filtering / ON CONFLICT pattern.
        """
    ...
    def _copyback_batch_mutex(self, copyback_root: Path, *, object_store_root: Path) -> Iterator[int]:
        ...
        absorbs ``SQLAlchemyError | OSError | ValueError`` (its
        ``except (SQLAlchemyError, OSError, ValueError)`` arm), so a
        ...
        if same_root:
            return {
                ...
                "forcing_packages": [
                    ...
                    for object_key in _forcing_refs_by_key(forcing_refs)
                ],
            }
    ...
    def _forcing_package_refs_for_runs(
        ...
        *,
        ...
        refs_by_key: dict[str, list[_ForcingPackageRef]] = {}
        for run in runs:
            try:
            ...
            existing_refs = refs_by_key.setdefault(ref.object_key, [])
            if existing_refs and any(existing_ref.checksum != ref.checksum for existing_ref in existing_refs):
                raise PublishError(
                    "OBJECT_STORE_COPYBACK_FAILED",
        ...
        return [ref for key in sorted(refs_by_key) for ref in refs_by_key[key]]
    ...
    def _forcing_package_ref_for_run(self, run: dict[str, Any]) -> _ForcingPackageRef:
        ...
        _validate_forcing_package_identity(object_key, run)
        ...
        *,
        ...
        for ref in forcing_refs:
            try:
                ...
                self._validate_forcing_source_tree(ref, source_tree, self.object_store)
            except FileNotFoundError as error:
                raise PublishError(
                    "OBJECT_STORE_COPYBACK_SOURCE_MISSING",
                    "Forcing package is missing from the object-store staging root.",
    ...
    def _validate_forcing_source_tree_for_refs(
        self,
        refs: list[_ForcingPackageRef],
        source_tree: _CopybackSourceTree,
        store: LocalObjectStore,
    ) -> None:
        for ref in refs:
            try:
                self._validate_forcing_source_tree(ref, source_tree, store)
            except (ObjectStoreError, OSError, SafeFilesystemError, ValueError, json.JSONDecodeError) as error:
                raise _ForcingPackageValidationError(ref, error) from error
        ...
        try:
            ...
            if rollback_log is None:
                ...
                if validate_target_tree is not None:
                    validate_target_tree(_collect_copyback_source_tree(target_store, key))
            else:
                rollback_entry = _replace_directory_tree_for_qdown_batch(
                    temp_dir,
                    target_dir,
                    containment_root=target_store.root,
                )
    ...
    def _validate_forcing_source_tree(
        self,
...
def _validate_forcing_package_identity(key: str, run: dict[str, Any]) -> None:
    parts = PurePosixPath(key).parts
    if len(parts) != 5:
        raise ValueError(f"Forcing package key must use forcing/<source>/<cycle>/<basin>/<model>: {key!r}")
    actual = {
        "source_id": parts[1],
        "cycle_time": parts[2],
        "basin_version_id": parts[3],
        "model_id": parts[4],
    }
    expected = {
        "source_id": str(run.get("source_id") or "").strip().lower(),
        "cycle_time": _compact_cycle_time_for_forcing_identity(run.get("cycle_time")),
        "basin_version_id": str(run.get("basin_version_id") or "").strip(),
        "model_id": str(run.get("model_id") or "").strip(),
    }
    mismatches = []
    if not expected["source_id"] or actual["source_id"].lower() != expected["source_id"].lower():
        mismatches.append("source_id")
    for field in ("cycle_time", "basin_version_id", "model_id"):
        if not expected[field] or actual[field] != expected[field]:
            mismatches.append(field)
    if mismatches:
        raise ValueError(
            "Forcing package key identity does not match q_down run lineage: "
            f"{key!r}; mismatched fields: {', '.join(mismatches)}"
        )


def _compact_cycle_time_for_forcing_identity(value: Any) -> str:
    if value is None:
        return ""
    try:
...
def _forcing_refs_by_key(refs: list[_ForcingPackageRef]) -> dict[str, list[_ForcingPackageRef]]:
    refs_by_key: dict[str, list[_ForcingPackageRef]] = {}
    for ref in refs:
        refs_by_key.setdefault(ref.object_key, []).append(ref)
    return {key: refs_by_key[key] for key in sorted(refs_by_key)}
...
def _collect_copyback_source_tree(store: LocalObjectStore, key: str) -> _CopybackSourceTree:
    ...
    try:
        ...
        _collect_copyback_source_entries(
            fd,
            key,
            directories,
            files,
        ...
        return _CopybackSourceTree(
            directories=tuple(sorted(directories)),
            files=tuple(sorted(files)),
            file_sizes=tuple(sorted(file_sizes)),
        )
    finally:
        os.close(fd)
        if fd != root_fd:
...
def _collect_copyback_source_entries(
    ...
    *,
    ...
    for name in names:
        ...
        _enforce_copyback_tree_depth(entry_key, root_depth=root_depth)
        try:
        ...
        if stat.S_ISREG(entry_stat.st_mode):
            ...
            _enforce_copyback_tree_count("files", counters["files"], _COPYBACK_MAX_FILES, tree_key=directory_key)
            _enforce_copyback_tree_count("file bytes", file_size, _COPYBACK_MAX_FILE_BYTES, tree_key=directory_key)
            _enforce_copyback_tree_count(
                "total bytes",
                counters["bytes"],
            ...
            file_sizes.append((entry_key, file_size))
            continue
        raise SafeFilesystemError(
...
def _enforce_copyback_tree_depth(entry_key: str, *, root_depth: int) -> None:
    depth = len(PurePosixPath(entry_key).parts) - root_depth
    if depth > _COPYBACK_MAX_DEPTH:
        raise SafeFilesystemError(
            f"{_copyback_tree_label_title(entry_key)} tree exceeds maximum depth "
            f"{_COPYBACK_MAX_DEPTH}: {entry_key}"
        )


def _enforce_copyback_tree_count(label: str, observed: int, limit: int, *, tree_key: str) -> None:
    if observed > limit:
        raise SafeFilesystemError(
            f"{_copyback_tree_label_title(tree_key)} tree exceeds maximum {label} limit "
            f"{limit}: observed {observed}"
        )
...
def _replace_directory_tree_no_follow(temp_dir: Path, target_dir: Path, *, containment_root: Path) -> None:
    ...
    try:
        if _directory_entry_exists(parent_fd, target_dir.name, target_dir):
            _verify_copyback_target_tree_clean(target_dir, containment_root=containment_root)
...
def _replace_directory_tree_for_qdown_batch(
    temp_dir: Path,
    target_dir: Path,
    *,
    ...
    try:
        ...
        try:
            ...
            pass
        return _CopybackRollbackEntry(
            target_dir=target_dir,
            backup_dir=backup_dir if target_backed_up else None,
        )
    except SafeFilesystemError:
        if target_backed_up and not promoted:
            _restore_copyback_backup(
...
def _rollback_qdown_copyback_batch(
    rollback_log: list[_CopybackRollbackEntry],
    *,
    containment_root: Path,
) -> None:
    ...
    rollback_log.clear()
    if errors:
        raise SafeFilesystemError(f"Failed to roll back q_down copyback batch: {'; '.join(errors)}", kind="io")


def _commit_qdown_copyback_batch(
    rollback_log: list[_CopybackRollbackEntry],
    *,
    containment_root: Path,
    directory_mode: int = 0o755,
...
def _verify_copyback_target_tree_clean(target_dir: Path, *, containment_root: Path) -> None:
    verify_directory_no_follow(target_dir)
    for entry in sorted(target_dir.rglob("*")):
        entry.relative_to(containment_root)
        entry_stat = entry.lstat()
        if stat.S_ISLNK(entry_stat.st_mode):
            raise SafeFilesystemError(f"Copyback target tree entry must not be a symlink: {entry}")
        if not (stat.S_ISDIR(entry_stat.st_mode) or stat.S_ISREG(entry_stat.st_mode)):
            raise SafeFilesystemError(f"Copyback target tree entry must be a file or directory: {entry}")
...
def _same_package_output_file_checksum(output_file: Any) -> str | None:
    if not isinstance(output_file, dict):
        return None
    for field in ("checksum", "sha256", "sha256_checksum"):
        value = output_file.get(field)
        if not isinstance(value, str) or not value.strip():
            continue
        checksum = value.strip()
        if checksum.startswith("sha256:"):
            checksum = checksum.split(":", 1)[1].strip()
