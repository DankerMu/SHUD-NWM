...
def _marker_recovered_candidate_stage(state: Mapping[str, Any]) -> str | None:
    ...
    marker's target lineage (``entity_id`` and recorded previous/failed job id) and the
    ``stage_retry_attempt_floor_sources`` contributor records.  The marker's recorded
    ``details.failed_stage``, when present, must agree with the contributor's stage and may
    disambiguate.  A unique agreeing canonical stage is returned; a foreign, stale,
    non-canonical, disagreeing, or multi-stage match keeps the existing stage-less fallback
    rather than guessing (no job-id stage-token parsing, no "largest floor" choice).
...
def _marker_target_identifiers(event: Mapping[str, Any]) -> set[str]:
    """The exact target row identifiers a marker records (#1308), for lineage intersection."""
    ...
    if isinstance(details, Mapping):
        for key in ("previous_job_id", "failed_job_id", "job_id"):
            value = details.get(key)
            if value not in (None, ""):
...
def _floor_source_identifiers(row: Mapping[str, Any]) -> set[str]:
    """The exact row identifiers a floor-source record carries (#1179), for lineage intersection."""
