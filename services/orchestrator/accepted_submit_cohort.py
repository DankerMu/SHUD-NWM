"""Canonical accepted-submit forecast cohort membership and digest helpers.

The accepted-submit authority's cohort-membership projection, cohort digest and
identity validation are self-contained (they depend only on the shared source /
cycle / scenario helpers), so they live here to keep
:mod:`services.orchestrator.accepted_submit_identity` under the large-file
guard.  The original module re-exports every name so all existing importers
