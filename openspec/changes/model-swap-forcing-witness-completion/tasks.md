...
- [ ] 1.1 `services/orchestrator/scheduler_candidates.py:613-624`：在 `state_decision = identity_quarantine`
...
- [ ] 1.2 红测试（`tests/test_production_scheduler.py`，E2E 形状）：stale journal lineage + 本模型无 forcing 见证，
      断言以下三点：
      ...
      实现前先确认它在未改源码时为红。
- [ ] 1.3 修改钉住测试 `test_completed_forecast_cycle_stale_journal_identity_is_quarantined_end_to_end`（`:11125`），
      按 design D2 补前提：
      ...
      **不得删除或放宽任何原断言。**
- [ ] 1.4 可排空性（D1）：断言 1.2 的 `blocked` 满足 `_decision_is_stable_missing_forcing_blocker`。
      **不**补修复策略调用。
- [ ] 1.5 负向测试：`manual_retry_requested` 形状的候选不会被 quarantine 见证闸改写（钉住结构排斥）。
- [ ] 1.6 （fixture review 第二轮删除：「strict 车道只探一次」没有行为可观察量，只是性能细节，不入 spec；
      D1 的车道限定由代码注释说明。）
- [ ] 1.7 既有 breaker 与白名单测试原样全绿，包括：
...
- [ ] 2.1 公开入口红测试（`tests/test_orchestration_chain.py`），按 D4 复现配方构造：
      ...
      **不 spy `_next_retry_attempt_for_stage`**。docstring 标注证据是构造的。
      实现前先确认为红（得到 `retry_2`）。
- [ ] 2.2 `services/orchestrator/chain.py` `_next_retry_attempt_for_stage`：保留前缀过滤；逐行序号改为
...
- [ ] 2.4 DB 与 FileJournal producer 回归：确认既有测试覆盖以下两处，并在 PR 中列出测试名；缺失时补。
      - `retry.py:465` 的叠加 id；
      - `file_orchestration_journal.py:10708-10730` 的 accepted-submit / 非 accepted-submit 两形。

## 3. #1846 — 裁决落档（design D3）

- [ ] 3.1 `docs/runbooks/current-production-ops.md` hop 5（`:350` 起）补充：
...
- [ ] 4.1 在 #1845 下发评论：写入 design「Descoped: #1845」的核查结论，注明经用户裁决降为 follow-up，保持 open。
- [ ] 4.2 越界 follow-up：`cycle_download_success_missing_raw_manifest` 不可达，经 issue-scribe 立单。
