...
- [x] 1.1 `SUPPORT_MODULE_TEST_RULES` tuple (4 exact-path entries:
...
- [x] 1.2 Every routed target verified existing + file-level
...
- [x] 2.1 Closure guard: every tracked non-suite `tests/**/*.py`
...
- [x] 2.4 Red evidence via injectable mapping/selection seams
...
- [x] 2.5 Domain comment: this guard covers `tests/` support modules;
...
- [x] 2.6 Stale-target guard union extension (Note-7):
...
- [x] 3.1 Third-mode line amended in `instructions/agents/shared.md`
...
- [x] 4.1 ADDED requirement (SHALL first line, 4 scenarios: routed
...
- [x] 5.2 Red evidence per task 2.4 (honest labels for
      naturally-green pins).
- [x] 5.3 `git ls-files '*.py' | xargs uv run ruff check` clean.
- [x] 5.4 `openspec validate ci-selector-support-module-importer-routing
...
- [x] 5.6 Issue #1487 acceptance checkboxes each mapped to a test or
