...
- [x] 4.6 Proof-specific live validators extraction.
  ...
  - Invariant Matrix: owner module `services.production_closure.readiness_live_proofs`; stable facade `services.production_closure.readiness_validation`; retained facade imports/re-exports include `PROOF_CONTRACTS`, `REQUIRED_AUTH_ACTIONS`, `EXPECTED_TARGET_ENVIRONMENT`, `PROOF_SPECIFIC_KEYS`, `SURFACE_PROOF_KEYS`, `_auth_live_item`, `_surface_live_item`, `_surface_live_receipt_errors`, `_common_live_receipt_errors`, `_is_live_proof_mode`, `_has_artifact_or_evidence_refs`, `_non_empty_string`, `_has_meaningful_value`, `_has_meaningful_ref`, `_target_environment_name`, `_string_set`, `_provider_metadata_is_meaningful`, `_role_mapping_is_meaningful`, `_alert_sink_metadata_is_meaningful`, `_alert_delivery_metadata_is_meaningful`, `_rollback_command_metadata_is_meaningful`, `_rollback_result_is_meaningful`, `_target_env_config_metadata_is_meaningful`, `_first_meaningful_mapping`, `_has_any_key_value`, and `_value_from` until inventory-backed caller migration; facade wrappers must inject current facade globals so old monkeypatch/import compatibility remains faithful; `_auth_live_item` owns only auth, owner surface helpers support only alert, rollback, and target-env and fail closed for auth/dependency/scheduler keys, and `_live_proof_items` remains a `readiness_validation` aggregator wrapper that only delegates auth, alert, rollback, and target-env proof-specific builders to `readiness_live_proofs` in 4.6; `validate_readiness(config)` still composes deterministic items, dependency summaries, scheduler evidence, proof-specific live proof items, dependency/scheduler live proof items, exclusions, `_validate_items`, `_release_blockers`, `_final_ready`, summary, and CLI behavior in the existing validator boundary; dependency proof binding `_dependency_receipt_errors` and scheduler proof binding `_scheduler_receipt_errors` remain in the validator boundary until 4.7/4.8 but may share the retained common live receipt contract.
...
- [x] 4.7 Dependency live-proof binder extraction.
  ...
  - Focused Verification: `uv run pytest -q tests/test_production_readiness_validation.py -k "dependency_live_proof_owner or dependency_receipt_facade or live_receipt_items_delegate"`;
    `uv run pytest -q tests/test_production_readiness_validation.py -k "dependency_receipt or slurm_proof"`;
  ...
  - Risk Packs: Evidence schema / producer identity: selected - dependency live receipts must bind proof type, dependency surface, producer issue/schema/run id, artifact ref, checksum/receipt id, and provenance before any public redaction; Path safety / redaction: selected narrowly - raw path aliases may be validated internally but public receipt/item/summary/stdout must stay redacted and bounded by the shared live-proof loader; Auth / readonly boundary: selected narrowly - the source proof represents live provider/credential evidence but does not execute credentials or mutate node-27/node-22 state in validation; Slurm production lifecycle / mock-vs-real parity: selected narrowly - Slurm proof binds to producer evidence, but scheduler live proof remains out of scope; Run manifest / QC provenance: selected - dependency summary bindings, when consumed, must compare current producer run id, artifact ref, and checksum before the receipt can be accepted; Published NHMS artifacts / display identity: selected narrowly - MVT proof remains live performance proof and deterministic MVT summary alone must not claim display performance readiness; PostGIS / TimescaleDB domain behavior: not selected - no DB reads/writes or schema behavior change; Frontend / browser behavior: not selected.
  - Invariant Matrix: owner module `services.production_closure.readiness_dependency_live_proofs`; stable facade `services.production_closure.readiness_validation`; retained facade imports/re-exports include `DEPENDENCY_BINDING_ALIAS_GROUPS`, `DEPENDENCY_BINDING_ALIAS_ERROR_SUFFIXES`, `DEPENDENCY_PROOF_KEYS`, `_dependency_receipt_errors`, `_coalesced_binding_value`, `_dependency_producer_binding`, `_binding_values`, `_binding_canonical_value`, `_normalized_binding_value`, `_dependency_binding_alias_errors`, `_dependency_binding_consistency_errors`, `_dependency_binding_summary_errors`, `_issue_matches`, `_contains_placeholder_value`, `_non_empty_string`, and `_has_meaningful_value` until inventory-backed caller migration proves old import paths are safe to remove; facade export/parity tests must cover every retained name, while facade monkeypatch injection is required specifically for lower-level predicate globals `_issue_matches`, `_contains_placeholder_value`, `_non_empty_string`, and `_has_meaningful_value`; `readiness_live_proofs.PROOF_CONTRACTS` remains the shared common live-proof contract table and is consumed by the owner without moving scheduler proof binding; `_surface_live_item` remains the `readiness_validation` aggregator/facade wrapper and delegates only dependency proof keys (`slurm`, `object_store`, `source`, `e2e`, `mvt`) to the dependency owner in 4.7, while auth/alert/rollback/target-env remain with the proof-specific owner and scheduler remains in the validator boundary until 4.8; `validate_readiness(config)` still composes deterministic items, dependency summaries, scheduler evidence, proof-specific live proof items, dependency live proof items, optional scheduler live proof items, exclusions, `_validate_items`, `_release_blockers`, `_final_ready`, summary, and CLI behavior in the existing validator boundary; deterministic dependency summary acceptance and `_dependency_bindings` ownership remain in `readiness_dependency_summaries`.
  - Boundary Surfaces: receipt load surfaces `slurm`, `object_store`, `source`, `e2e`, and `mvt`; optional summary-binding inputs from consumed deterministic dependency summaries; top-level and provenance alias groups for dependency name/surface, producer issue, producer schema, producer run id, producer artifact ref, and producer checksum/receipt id; raw alias comparison before public redaction; downstream consumers final required-live-proof item counts, release blockers, summary status, public readiness artifacts, and redacted stdout; unchanged sibling surfaces auth/alert/rollback/target-env proof-specific validators, optional scheduler live proof binding, deterministic summary readers, scheduler evidence readers, exclusions, and final aggregation.
  - Regression Rows: owner direct tests and facade parity tests prove every retained facade export above has object-identity or equivalent-output parity, common contract reuse, equivalent dependency receipt errors, and equivalent dependency live item outputs for Slurm, object-store, source, E2E, and MVT; direct owner tests preserve dependency-specific issue/schema/surface checks; missing producer fields, missing/placeholder provenance, sibling nested provenance, contradictory top-level/provenance aliases, lower-priority alias mismatches, summary-binding run/artifact/checksum mismatches, and raw path alias mismatches produce stable dependency binding errors before redaction; accepted receipts with agreeing top-level/provenance aliases and optional consumed-summary bindings become `passed`/`live_proof` without executing live side effects; deterministic dependency summaries alone never satisfy `_final_ready` or final `summary.status=ready`; facade monkeypatch tests prove retained lower-level predicate compatibility for `_issue_matches`, `_contains_placeholder_value`, `_non_empty_string`, and `_has_meaningful_value`; owner fail-closed tests prove `auth`, `alert`, `rollback`, `target_env`, `scheduler`, and unknown proof keys cannot be accepted by the dependency owner; public receipt/item/summary/stdout artifacts do not leak raw secrets, private paths, raw payload, or unbounded nested data.
  - Verification Floor: focused pytest commands above; `uv run pytest -q tests/test_production_readiness_validation.py -k "dependency_live_proof_owner or dependency_receipt_facade or live_receipt_items_delegate"`; `uv run pytest -q tests/test_production_readiness_validation.py -k "dependency_receipt or dependency_bindings or existing_m19"`; `uv run pytest -q tests/test_production_readiness_validation.py`; `uv run ruff check services/production_closure/readiness_validation.py services/production_closure/readiness_dependency_live_proofs.py tests/test_production_readiness_validation.py`; `openspec validate governance-8-module-deepening --strict --no-interactive`; `git diff --check`.
- [x] 4.8 Scheduler live-proof binder extraction.
  ...
  - Inventory/Evidence Update: update readiness inventory row `Optional live scheduler proof` plus guard rows `Live proof loader and receipts` and `Scheduler proof binding`; record the new owner, retained facade exports, focused verification, and facade-removal condition.
  - Risk Packs: Evidence schema / producer identity: selected - scheduler live receipts must bind proof type, surface, producer schema, scheduler pass id, artifact ref, checksum/receipt id, and provenance before any public redaction; Slurm production lifecycle / mock-vs-real parity: selected - only scheduler evidence produced with live-eligible production orchestration status may satisfy the optional live scheduler proof; Run manifest / QC provenance: selected - accepted receipts must exactly match one consumed scheduler evidence binding and must block missing, mismatched, or ambiguous bindings; Path safety / redaction: selected narrowly - raw aliases may be validated internally but public receipt/item/summary/stdout remains redacted and bounded by the shared live-proof loader; Published NHMS artifacts / display identity: not selected - no display artifact or MVT semantics change; PostGIS / TimescaleDB domain behavior: not selected - no DB reads/writes or schema behavior change; Frontend / browser behavior: not selected.
  - Invariant Matrix: owner module `services.production_closure.readiness_scheduler_live_proof`; stable facade `services.production_closure.readiness_validation`; retained facade imports/re-exports include `SCHEDULER_BINDING_ALIAS_GROUPS`, `SCHEDULER_BINDING_ALIAS_ERROR_SUFFIXES`, `_scheduler_receipt_errors`, `_coalesced_scheduler_binding_value`, `_scheduler_producer_binding`, `_scheduler_binding_values`, `_scheduler_binding_canonical_value`, `_scheduler_binding_alias_errors`, `_scheduler_binding_consistency_errors`, and `_scheduler_binding_summary_errors` until inventory-backed caller migration proves old import paths are safe to remove; facade export/parity tests must cover every retained scheduler binding name, while facade monkeypatch injection is required for lower-level predicate globals `_contains_placeholder_value`, `_non_empty_string`, `_has_meaningful_value`, and shared binding normalization if the owner consumes it through the facade; `readiness_live_proofs.PROOF_CONTRACTS` remains the shared common live-proof contract table; `readiness_scheduler_evidence` remains the owner for deterministic scheduler evidence, scheduler binding tuples, live-eligible producer mode/status constants, evidence discovery, evidence safe reads, and review-only deterministic scheduler item semantics; `_live_proof_items` remains in `readiness_validation` and continues to insert the optional `live-scheduler-evidence` item only when scheduler evidence root/file is configured or a scheduler proof receipt is supplied; `_surface_live_item` remains the `readiness_validation` aggregator/facade wrapper and delegates only `scheduler` proof binding to the scheduler live-proof owner in 4.8, while auth/alert/rollback/target-env remain with `readiness_live_proofs` and dependency proof keys remain with `readiness_dependency_live_proofs`; `validate_readiness(config)` still composes deterministic items, dependency summaries, scheduler evidence, proof-specific live proof items, dependency live proof items, optional scheduler live proof items, exclusions, `_validate_items`, `_release_blockers`, `_final_ready`, summary, and CLI behavior in the existing validator boundary.
  - Boundary Surfaces: receipt load surface `scheduler`; optional scheduler evidence bindings produced by `_scheduler_bindings`; top-level and provenance alias groups for producer schema, producer run/pass id, producer artifact ref, and producer checksum/receipt id; raw alias comparison before public redaction; optional item insertion and final required-live-proof count behavior; downstream consumers release blockers, summary status, public readiness artifacts, and redacted stdout; unchanged sibling surfaces deterministic scheduler evidence reader, auth/alert/rollback/target-env proof-specific validators, dependency live-proof binders, deterministic dependency summaries, exclusions, and final aggregation.
  - Regression Rows: owner direct tests and facade parity tests prove every retained facade export above has object-identity or equivalent-output parity, common contract reuse, equivalent scheduler receipt errors, and equivalent scheduler live item outputs; direct owner tests prove only `scheduler` is accepted and `auth`, `alert`, `rollback`, `target_env`, `slurm`, `object_store`, `source`, `e2e`, `mvt`, and unknown proof keys fail closed; missing scheduler evidence binding, missing producer fields, missing/placeholder provenance, contradictory top-level/provenance aliases, lower-priority alias mismatches, producer schema/pass/artifact/checksum mismatches, ambiguous exact matches, non-live scheduler execution modes, and non-live scheduler statuses produce stable scheduler binding errors before redaction; accepted receipts with agreeing aliases and exactly one live-eligible consumed scheduler evidence binding become `passed`/`live_proof`; configured scheduler evidence without accepted scheduler proof creates the optional required final `live-scheduler-evidence` blocker but deterministic scheduler evidence alone never satisfies `_final_ready` or final `summary.status=ready`; facade monkeypatch tests prove retained lower-level predicate compatibility for `_contains_placeholder_value`, `_non_empty_string`, and `_has_meaningful_value`; public receipt/item/summary/stdout artifacts do not leak raw secrets, private paths, raw payload, or unbounded nested data.
  - Verification Floor: focused pytest command above; `uv run pytest -q tests/test_production_readiness_validation.py -k "scheduler_live_proof_owner or scheduler_receipt or live_receipt_items_delegate"`; `uv run pytest -q tests/test_production_readiness_validation.py -k "scheduler and evidence"`; `uv run pytest -q tests/test_production_readiness_validation.py -k "final or release_blocker or existing_lane"`; inventory diff check proves `Optional live scheduler proof`, `Live proof loader and receipts`, and `Scheduler proof binding` rows no longer describe scheduler binder ownership as remaining in `readiness_validation` beyond 4.8; `uv run pytest -q tests/test_production_readiness_validation.py`; `uv run ruff check services/production_closure/readiness_validation.py services/production_closure/readiness_scheduler_live_proof.py tests/test_production_readiness_validation.py`; `openspec validate governance-8-module-deepening --strict --no-interactive`; `git diff --check`.
- [x] 4.9 Scoped exclusion extraction.
  ...
  - Risk Packs: Evidence schema / producer identity: not selected - scoped exclusions are static product-scope records with no producer receipt or external evidence input; Path safety / redaction: not selected - no filesystem or raw payload reads; Slurm production lifecycle / mock-vs-real parity: not selected; Published NHMS artifacts / display identity: selected narrowly - incomplete real national data remains an explicit scoped exclusion and must not be treated as a live MVT/display proof; PostGIS / TimescaleDB domain behavior: not selected; Frontend / browser behavior: not selected.
  - Invariant Matrix: owner module `services.production_closure.readiness_scope_exclusions`; stable facade `services.production_closure.readiness_validation`; retained facade imports/re-exports include `_exclusion_items` until inventory-backed caller migration proves old import paths are safe to remove; `_summary_exclusions` remains in `readiness_validation` until 4.10 final aggregation extraction because it aggregates exclusions from all validated items into public exclusion fields; `validate_readiness(config)` still composes deterministic items, dependency summaries, scheduler evidence, live proof items, scoped exclusions, `_validate_items`, `_release_blockers`, `_final_ready`, summary, and CLI behavior in the existing validator boundary; item validation remains owned by `readiness_item_contracts`.
  - Boundary Surfaces: exactly two static item surfaces `cldas_restricted_source` and `incomplete_real_national_data`; item ids `scope-exclusion-cldas` and `scope-exclusion-national-data`; exclusion ids `cldas-restricted` and `real-national-data-incomplete`; status and execution mode `not_executed`; `required_for_final=false`; `live_proof_accepted=false`; artifact refs `readiness_items.json` and `summary.json`; residual risk, action, and removal criteria stay non-empty and product-scope specific; exclusion inclusion still occurs through `_summary_exclusions` into `summary.exclusions` and the independent `release_blockers.exclusions` field; release blocker lists and final readiness must not treat exclusions as failures or accepted live proofs.
  - Regression Rows: owner direct tests and facade parity tests prove equivalent scoped exclusion items and retained facade behavior; item contract tests prove every scoped exclusion item validates under `readiness_item_contracts`; integration tests prove summary excludes no existing exclusion record, exclusion records include item surface/status, release blockers contain no scoped exclusion blockers, final readiness remains controlled by required live proofs rather than exclusions, and exclusions do not increase `live_proof_item_count`, `required_live_proof_count`, or `accepted_live_proof_count`; deterministic item count semantics stay unchanged until 4.10 final aggregation extraction; negative guard tests prove changing or adding exclusion ids/surfaces requires inventory update or explicit test fixture change.
  - Verification Floor: focused pytest command above; `uv run pytest -q tests/test_production_readiness_validation.py -k "scope_exclusion_owner or exclusions"`; `uv run pytest -q tests/test_production_readiness_validation.py -k "final or release_blocker or existing_lane"`; `uv run pytest -q tests/test_production_readiness_validation.py`; `uv run ruff check services/production_closure/readiness_validation.py services/production_closure/readiness_scope_exclusions.py tests/test_production_readiness_validation.py`; `openspec validate governance-8-module-deepening --strict --no-interactive`; `git diff --check`.
- [x] 4.10 Readiness final aggregation extraction.
  ...
  - Inventory/Evidence Update: update readiness inventory rows `Final aggregation and release blockers` and `Scoped exclusions`, including the guard hook row that currently says `_summary_exclusions` remains in `readiness_validation` until 4.10.
  - Risk Packs: Evidence schema / producer identity: selected narrowly - summary and release-blocker public artifact schemas, final-ready status, item counts, artifact refs, and blocker identities must remain byte-shape compatible; Path safety / redaction: selected narrowly - final summary evidence_dir and writer behavior must keep using shared safe output/redaction helpers and must not expose raw private paths; Slurm production lifecycle / mock-vs-real parity: not selected; Published NHMS artifacts / display identity: selected narrowly - deterministic and historical/read-only evidence must not become accepted live proof; PostGIS / TimescaleDB domain behavior: not selected; Frontend / browser behavior: not selected.
  - Invariant Matrix: owner module `services.production_closure.readiness_final_aggregation`; stable public entrypoint remains `services.production_closure.readiness_validation.validate_readiness(config)`; retained facade imports/re-exports include `_release_blockers`, `_final_ready`, and `_summary_exclusions` until inventory-backed caller migration proves old import paths are safe to remove; `validate_readiness(config)` still owns lane item collection, `EvidenceWriter` preparation, preflight/live receipt/readiness item/environment writes, and CLI behavior, then delegates final release-blocker and summary payload construction to the owner; `readiness_shared_artifacts` remains the safe output/redaction owner; `readiness_item_contracts` remains item validation owner; scoped exclusion item production remains in `readiness_scope_exclusions`.
  - Boundary Surfaces: release-blocker payload schema `nhms.production_readiness.release_blockers.v1`, issue `181`, run id, generated_at, final readiness claim, blocker list, and public exclusions; summary payload schema `nhms.production_readiness.summary.v1`, issue `181`, run id, status `ready`/`release_blocked`, redacted evidence_dir, generated_at, final readiness claim, deterministic/live/required/accepted item counts, release blocker list, public exclusions, exact artifact refs, and interpretation text; `_final_ready(items)` returns true only when no item status is `failed`, `blocked`, or `release_blocked` and every `required_for_final` item is `passed` with `live_proof_accepted=true`; release blockers include failed/blocked/release_blocked items and required-final items not accepted, but scoped exclusions remain separate public exclusions rather than blockers; deterministic item count remains every non-`live_proof` execution mode until a later explicitly scoped aggregation semantic change.
  - Regression Rows: owner direct tests and facade parity tests prove equivalent `_release_blockers`, `_final_ready`, and `_summary_exclusions` behavior; integration tests prove `validate_readiness(config)` writes the same `release_blockers.json` and `summary.json` schemas, exact artifact refs, redacted evidence_dir, final status, blocker list, exclusion records, and count semantics for missing live proofs, all accepted live proofs, scheduler-proof-required runs, invalid item validation failures, existing lane output, and unsafe/existing evidence paths; negative guard tests prove deterministic summaries/scheduler evidence and scoped exclusions never satisfy final live readiness; inventory diff check proves `Final aggregation and release blockers` has owner `readiness_final_aggregation` and `Scoped exclusions` rows no longer describe `_summary_exclusions` as remaining in `readiness_validation` after 4.10; environment/preflight/live receipt/readiness item artifact ownership and write ordering remain compatible.
  - Verification Floor: focused pytest commands above; `uv run pytest -q tests/test_production_readiness_validation.py -k "final_aggregation_owner or final or release_blocker or existing_lane"`; inventory diff check for `Final aggregation and release blockers` and `Scoped exclusions`; `uv run pytest -q tests/test_production_readiness_validation.py`; `uv run pytest -q tests/test_entropy_audit_script.py`; `uv run ruff check services/production_closure/readiness_validation.py services/production_closure/readiness_final_aggregation.py tests/test_production_readiness_validation.py`; `openspec validate governance-8-module-deepening --strict --no-interactive`; `openspec validate --all --strict --no-interactive`; `git diff --check`.
- [x] 4.11 Readiness group verification and evidence closeout.
  ...
  - Fixture Level: expanded integration closeout. This slice is evidence-only but high impact because it records the readiness validation group's task-to-issue-to-PR chain and verifies that the stable validator entrypoints did not drift after all readiness owner-family slices.
  - Risk Pack Selection: Selected: Public API / stable facade (`validate_readiness(config)`, `validate_readiness_item(item)`, validate-readiness CLI behavior, and retained `readiness_validation` compatibility helpers remain stable); Legacy compatibility (readiness inventory owner/facade rows and guard hooks must stay inventoried after 4.1-4.10); Test / evidence coverage (full readiness validation tests, scoped production-closure ruff, entropy guard suite, OpenSpec validation, and diff-check form the group gate); Documentation / migration notes (implementation evidence records the readiness issue/PR mapping); CI / release governance (closeout evidence must line up with merged PR state). Not Selected: moving owner behavior, changing live proof requirements, changing production deployment, changing DB/schema/role, changing Slurm scheduling, changing API routes, changing frontend/display surfaces, or deleting compatibility symbols.
  - Invariant Matrix: Governing invariant: readiness tasks 4.1-4.10 are complete before this closeout is checked. Source-of-truth identity/contract: `docs/review-loop-log.jsonl`, GitHub issue/PR state, and `docs/governance/READINESS_VALIDATION_LANE_INVENTORY.md` agree on the readiness task-to-issue-to-PR mapping. Surfaces: Producers: completed PRs #804-#813 and this closeout PR; Compatibility/stable entrypoints: `services.production_closure.readiness_validation.validate_readiness`, `validate_readiness_item`, validate-readiness CLI, and retained facade helpers; Validators: full `tests/test_production_readiness_validation.py`, scoped `services/production_closure` ruff, entropy inventory guard, OpenSpec validation, and diff-check; Evidence/audit/readiness: readiness lane inventory closeout map and post-merge review-loop log.
  - Regression Rows: every governed readiness owner family remains present in the inventory, including item validation, shared artifacts/safe writes, live proof receipts, dependency summaries, scheduler evidence, proof-specific validators, dependency live-proof binders, scheduler live-proof binder, scoped exclusions, and final aggregation/release blockers; the final readiness evidence map records tasks 4.1-4.11 with issue/PR references; focused verification commands pass after the evidence update; no readiness runtime code, live service mutation, production deploy behavior, live proof requirement, DB/schema/role, Slurm scheduling, API route, frontend/display, scheduler, chain, or two-node group changes in this slice.
  - Verification Floor: focused verification commands above; `uv run pytest -q tests/test_entropy_audit_script.py`; inventory diff check proves a `Readiness Group Closeout Evidence` map exists with tasks 4.1-4.11, issues #745-#755, PRs #804-#813 plus this closeout PR, and the closeout row; `openspec validate --all --strict --no-interactive`; `git diff --check`.

## 5. API Bootstrap Deepening

- [x] 5.1 API OpenAPI patch owner-module extraction.
  ...
  - Fixture Level: expanded OpenAPI owner-module extraction. This slice creates a focused owner for runtime OpenAPI patch generation while keeping `apps.api.main:create_app()` and existing `apps.api.main` helper imports compatible during the API bootstrap deepening group.
  - Risk Pack Selection: Selected: Public API / generated contract (`/openapi.json`, `openapi/nhms.v1.yaml` parity-sensitive runtime schema patches, and frontend type generation inputs stay equivalent); Stable facade / legacy compatibility (`create_app()`, global `app.openapi`, `custom_openapi()`, `_custom_openapi_factory()`, and existing `_patch_*` imports from `apps.api.main` remain callable); Role-boundary / runtime mode (display-readonly still excludes Slurm routes and does not gain route registration changes); Test / evidence coverage (API tests, OpenAPI drift tests, role-boundary/runtime checks, scoped ruff, OpenSpec, and diff-check); Documentation / ownership inventory (structural inventory records `apps/api/openapi_patching.py` as owner and `apps/api/main.py` as facade). Not Selected: route registration extraction, auth policy, request-body validation, frontend UI behavior, DB/schema changes, Slurm scheduling, or generated type rewrites unless the OpenAPI output actually changes.
  - Invariant Matrix: Governing invariant: OpenAPI patching moves out of the app bootstrap without changing generated schema semantics. Owner module: `apps/api/openapi_patching.py` owns patch ordering, runtime/pipeline/station-series/QHH latest-product/MVT/flood/layer-metadata/met-stations schema helpers, and OpenAPI factory construction. Stable facade: `apps/api/main.py` keeps app construction, route inclusion, middleware/static/health/cache wiring, and compatibility exports for existing tests/importers. Contract surfaces: runtime `/openapi.json`; static `openapi/nhms.v1.yaml` comparisons; frontend generated type checks; route-boundary behavior for `compute_control`, `display_readonly`, and `shared_contract`. Validators: `tests/test_openapi_drift.py`, `tests/test_api.py`, `tests/test_runtime_mode.py`, `tests/test_role_boundary_static.py`, scoped ruff, OpenSpec validation, and diff-check.
  - Regression Rows: runtime OpenAPI remains equivalent for runtime config, pipeline status/jobs/logs/retry/cancel operations, station series query and response envelopes, QHH latest-product availability/quality/query index, MVT tile parameter bounds/enums and error responses, flood return-period duration/product-quality schemas, met-stations list, and layer metadata/valid-times/catalog schemas; `display_readonly` still does not document Slurm routes; tests that import `_patch_mvt_tile_openapi` or other retained helpers from `apps.api.main` continue to pass; structural inventory records the new owner module and focused verification; no route handler registration, auth guard, protected mutation policy, request-body parsing, frontend UI, DB/schema, Slurm, scheduler, chain, or two-node behavior changes.
  - Verification Floor: focused verification commands above; `uv run pytest -q tests/test_runtime_mode.py tests/test_role_boundary_static.py tests/test_api.py`; `uv run ruff check apps/api/main.py apps/api/openapi_patching.py tests/test_api.py tests/test_openapi_drift.py`; `openspec validate governance-8-module-deepening --strict --no-interactive`; `openspec validate --all --strict --no-interactive`; `git diff --check`; `cd apps/frontend && pnpm check:api-types` only if runtime OpenAPI/static schema/generated frontend type artifacts change.
- [x] 5.2 API role-aware route registry extraction.
  ...
  - Fixture Level: expanded role-aware route registry extraction. This slice creates a focused owner for router inclusion while keeping `apps.api.main:create_app()` as the public app-factory facade and leaving all route handler behavior in existing route modules.
  - Risk Pack Selection: Selected: Runtime role boundary (`display_readonly` excludes `/api/v1/slurm/*`; `compute_control` and `dev_monolith` keep Slurm routes; `slurm_gateway` remains reserved for the standalone bounded gateway app); Stable facade / legacy compatibility (`create_app(env=None)`, runtime config state, `runtime_router` handler, middleware/error registration, OpenAPI factory, static/health/cache startup remain coordinated by `main.py`); Test / evidence coverage (runtime role tests, role-boundary static tests, API smoke tests, scoped API ruff, OpenSpec validation, and diff-check); Documentation / ownership inventory (structural inventory records the route registry owner and retained `create_app` facade). Not Selected: OpenAPI patch internals, protected mutation auth guard, request-body validation, route handler implementation, static/health/cache extraction, frontend UI, DB/schema, Slurm scheduler behavior, or production topology changes.
  - Invariant Matrix: Governing invariant: router inclusion moves behind `apps/api/route_registry.py` without changing registered route sets for supported full-API roles. Owner module: `apps/api/route_registry.py` owns business router ordering, runtime-router inclusion, and conditional Slurm router inclusion from `RuntimeConfig.slurm_routes_enabled`. Stable facade: `apps/api/main.py` continues to load runtime config, set app state, register errors/middleware, call the route registry, mount static/health routes, assign OpenAPI, and start display cache warmup. Contract surfaces: FastAPI route table, runtime `/api/v1/runtime/config`, `/api/v1/slurm/*` visibility by role, OpenAPI path visibility by role, and standalone `services.slurm_gateway.app:create_gateway_app()` route boundary. Validators: `tests/test_runtime_mode.py`, `tests/test_role_boundary_static.py`, `tests/test_api.py`, scoped API ruff, OpenSpec validation, and diff-check.
  - Regression Rows: display-readonly app starts with runtime config, object-store state, health/static routes, non-Slurm business routes, and no `/api/v1/slurm/*` routes or OpenAPI paths; compute-control/dev-monolith apps keep Slurm health route and OpenAPI path visibility; `NHMS_SERVICE_ROLE=slurm_gateway` still refuses full business API startup; standalone slurm gateway continues to expose only gateway routes; route order and public route handlers remain unchanged; no OpenAPI patch internals, auth guard/policy, request-body validation, static/health/cache behavior, route implementation, frontend UI, DB/schema, Slurm scheduling, scheduler, chain, or two-node behavior changes in this slice.
  - Verification Floor: focused verification command above; `uv run pytest -q tests/test_entropy_audit_script.py tests/test_runtime_mode.py tests/test_api.py`; `uv run ruff check apps/api/main.py apps/api/route_registry.py tests/test_runtime_mode.py tests/test_role_boundary_static.py tests/test_api.py`; `openspec validate governance-8-module-deepening --strict --no-interactive`; `openspec validate --all --strict --no-interactive`; `git diff --check`.
- [x] 5.3 API static/health/cache/startup wiring extraction.
  ...
  - Focused Verification: `uv run pytest -q tests/test_static_serving.py tests/test_runtime_mode.py tests/test_api.py tests/test_monitoring_api.py`.
  ...
  - Fixture Level: expanded static/health/cache/startup wiring extraction. This slice creates a focused owner for non-route-registry startup wiring while keeping `apps.api.main:create_app()` as the public app-factory facade and leaving middleware/auth, OpenAPI patching, and role-aware route inclusion in their existing seams.
  - Risk Pack Selection: Selected: Stable facade / app-factory compatibility (`create_app(env=None)`, app state fields, `runtime_config(request)` response envelope, health route, SPA fallback/static asset behavior, cache-control headers, and display-readonly cache warmup remain stable); Runtime role boundary (display cache warmup remains display-readonly only, but route registry role decisions do not move in this slice); Test / evidence coverage (runtime mode tests, API smoke tests, monitoring API tests, scoped API ruff, OpenSpec validation, and diff-check); Documentation / ownership inventory (structural inventory records the startup wiring owner and retained app-factory surface). Not Selected: route registry role decisions, OpenAPI patch internals, protected mutation auth guard, request-body validation, route handler implementation beyond moving the runtime config handler, frontend UI behavior, DB/schema, Slurm scheduling, scheduler, chain, or two-node behavior.
  - Invariant Matrix: Governing invariant: startup/static wiring moves behind `apps/api/startup_wiring.py` without changing app startup behavior. Owner module: `apps/api/startup_wiring.py` owns `configure_app_state`, runtime config router construction/handler, health/static/SPAs mounting, `CacheControlStaticFiles`, frontend dist path constants, and display cache warmup dispatch. Stable facade: `apps/api/main.py` continues to load runtime config, construct FastAPI, register errors/middleware, call the route registry, assign OpenAPI, and call startup/static wiring owner functions. Contract surfaces: `/health`, `/{full_path:path}` SPA fallback behavior, `/assets` cache-control behavior, `/api/v1/runtime/config` envelope/data, `api.state.runtime_config`, `api.state.object_store_root`, display-readonly cache warmer invocation, and non-display roles not starting the warmer. Validators: `tests/test_runtime_mode.py`, `tests/test_api.py`, `tests/test_monitoring_api.py`, scoped API ruff, OpenSpec validation, and diff-check.
  - Regression Rows: health route returns the same status/service/version payload; static asset mount keeps immutable cache-control for Vite assets and no-cache for non-hash static files/index fallback; `/api/v1/runtime/config` continues to return `_ok(request, config.public_dict())` with request id and role flags; display-readonly startup still sets object-store root and starts display cache warmup, while compute/dev do not start display-only warmup; route registry role decisions, OpenAPI patch internals, protected mutation auth guard/policy, request-body validation, route handlers, frontend UI, DB/schema, Slurm scheduling, scheduler, chain, and two-node behavior remain unchanged.
  - Verification Floor: focused verification command above; `uv run pytest -q tests/test_entropy_audit_script.py tests/test_runtime_mode.py tests/test_api.py`; `uv run ruff check apps/api/main.py apps/api/startup_wiring.py tests/test_static_serving.py tests/test_runtime_mode.py tests/test_api.py tests/test_monitoring_api.py`; `openspec validate governance-8-module-deepening --strict --no-interactive`; `openspec validate --all --strict --no-interactive`; `git diff --check`.
- [x] 5.4 API protected mutation seam retention and tests.
  ...
  - Fixture Level: expanded retained protected-mutation seam. This slice does not extract a new owner module; it records `apps/api/main.py` as the current retained seam for pre-body protected mutation policy, bounded request-body classification, and middleware ordering while strengthening the focused API bootstrap evidence.
  - Repair Intensity: high, because the retained seam gates auth/permission and request validation before write-capable route handlers can run.
  - Risk Pack Selection: Selected: Public API / CLI / script entry (`create_app()` keeps protected mutation middleware ordering and stable error envelopes for mutation endpoints); Schema / columns / units / field names (the public error envelope keeps `request_id`, `status`, `error.code`, `error.message`, `error.details.policy_decision`, and `error.details.audit_record` fields stable); Auth / permissions / secrets (`AUTH_REQUIRED`, `RBAC_FORBIDDEN`, and `RELEASE_BLOCKED` decisions keep stable action/target audit details and secret-shaped target IDs stay redacted); Concurrency / shared state / ordering (pre-body middleware must run before route handlers, store writes, gateway calls, and display manual-action mutation paths); Resource limits / large input / discovery (active/lifecycle body classification stays bounded by `_ACTIVE_TOGGLE_PRE_BODY_MAX_BYTES`, including missing or oversized `content-length`); Error handling / rollback / partial outputs (invalid or oversized pre-body requests return shared `VALIDATION_ERROR` with `request_id` and no store/gateway mutation); Runtime role boundary / fail-closed display behavior (`display_readonly` retains manual-action blockers and does not call Slurm gateway or mutate pipeline state); Test / evidence coverage (the focused #759 verification suite includes request id, error shape, auth policy, resource-limit, ordering, and display fail-closed assertions); Documentation / ownership inventory (structural inventory records this as a retained seam with future extraction conditions). Not Selected: Config / project setup (no service-role parsing or environment contract changes); File IO / path safety / overwrite (no file path or artifact IO behavior changes); Legacy compatibility / examples beyond the retained API facade and error envelope selected above; Release / packaging / dependency compatibility (no dependency, packaging, or release artifact change); Geospatial / CRS / basin geometry; Hydro-met time series / forcing windows; SHUD numerical runtime / conservation / NaN; PostGIS / TimescaleDB domain behavior; Slurm production lifecycle / mock-vs-real parity beyond unchanged retry/cancel gateway no-call assertions; External hydro-met providers / snapshot reproducibility; Run manifest / QC provenance; Published NHMS artifacts / display identity. Also not selected: route registry extraction, OpenAPI patch internals, static/health/cache wiring, new auth semantics, route handler refactors, frontend UI, DB/schema migrations, Slurm scheduler behavior, scheduler, chain, or two-node deployment behavior.
  - Invariant Matrix: Governing invariant: protected mutation requests are authorized or rejected on the retained pre-body seam before any write-capable route handler, store, gateway, or display control mutation can run. Source-of-truth contract: `apps/api/main.py:protected_mutation_auth_guard`, `_protected_mutation_policy`, `_PRE_BODY_PROTECTED_MUTATIONS`, bounded active/lifecycle body readers, and `apps/api/auth.py:evaluate_request_action`. Surfaces: Producers: HTTP mutation requests for model registry create/switch/activate/lifecycle and pipeline retry/cancel/manual actions; Validators/preflight: retained middleware seam in `apps/api/main.py`, shared auth helpers, bounded JSON body validation, runtime role config; Storage/cache/query: model registry store and pipeline store must remain untouched on denied/invalid pre-body paths; Public routes/entrypoints: `create_app()`, protected model endpoints, pipeline retry/cancel endpoints, runtime role route table; Frontend/downstream consumers: existing API error envelope consumers and display-readonly manual-action UI/API callers; Failure paths/rollback/stale state: 401/403/422/503 protected mutation responses and display manual-action blockers must leave store/gateway state unchanged; Evidence/audit/readiness: focused tests, structural inventory row, OpenSpec validation, and diff-check.
  - Regression Rows: protected mutation without credentials -> 401 `AUTH_REQUIRED` with stable `request_id`, policy decision/audit details, and no store/gateway mutation; protected mutation with insufficient role -> 403 `RBAC_FORBIDDEN` with the same envelope and no mutation; malformed active-toggle JSON, missing boolean `active`/`active_flag`, or malformed lifecycle `operation` -> 422 `VALIDATION_ERROR` with the incoming `X-Request-ID`, stable error detail fields, and no downstream write; oversized active/lifecycle body with or without a trustworthy `content-length` -> bounded read, 422 `VALIDATION_ERROR`, stable `request_id`, and no store/gateway mutation; display-readonly retry/cancel -> `CONTROL_PLANE_MANUAL_ACTION_REQUIRED` with no gateway calls and no pipeline state change; compute/dev authorized retry/cancel behavior remains unchanged; route registry, OpenAPI patching, static/health/cache wiring, frontend UI, DB/schema, Slurm scheduling, scheduler, chain, and two-node behavior remain unchanged.
  - Verification Floor: focused verification command above; `uv run pytest -q tests/test_entropy_audit_script.py tests/test_runtime_mode.py tests/test_api.py`; `uv run ruff check apps/api/main.py tests/test_runtime_mode.py tests/test_api.py tests/test_role_boundary_static.py tests/test_retry_cancel_consistency.py`; `openspec validate governance-8-module-deepening --strict --no-interactive`; `openspec validate --all --strict --no-interactive`; `git diff --check`.
- [x] 5.5 API group verification and evidence closeout.
  ...
  - Focused Verification: `uv run pytest -q tests/test_static_serving.py tests/test_runtime_mode.py tests/test_api.py tests/test_role_boundary_static.py tests/test_retry_cancel_consistency.py tests/test_monitoring_api.py tests/test_openapi_drift.py`;
  ...
  - Fixture Level: expanded API bootstrap integration closeout. This slice is evidence/documentation only, but it closes a public API bootstrap group whose prior tasks moved OpenAPI patching, role-aware route registration, startup/static/cache wiring, and retained protected-mutation policy behind explicit owner or facade seams.
  - Repair Intensity: high, because the closeout evidence chain must accurately bind task state, issue/PR state, owner inventory, and verification commands without implying unreviewed API behavior changes.
  - Risk Pack Selection: Selected: Public API / CLI / script entry (`create_app()`, runtime `/openapi.json`, runtime config route, health/static routes, Slurm route visibility, and protected mutation middleware remain stable through the closeout); Schema / columns / units / field names (OpenAPI output and generated frontend type inputs are verified not to drift unless explicitly regenerated); Auth / permissions / secrets (protected mutation retained-seam evidence from #759 remains part of the group gate); Concurrency / shared state / ordering (middleware, route inclusion, startup warmup, and no-downstream-write ordering remain represented by the focused suites); Legacy compatibility / examples (`apps.api.main` facade and retained `_patch_*` helper imports stay recorded); Error handling / rollback / partial outputs (API error envelope, validation, and display manual-action blockers stay covered by the group tests); Documentation / migration notes (structural inventory records the final API task-to-issue-to-PR evidence map); Release / packaging / dependency compatibility (closeout evidence must align with merged PR state and CI without dependency or packaging changes). Not Selected: Config / project setup (no service-role parsing or environment contract change); File IO / path safety / overwrite (no file or artifact IO behavior changes beyond docs evidence writes); Resource limits / large input / discovery (resource-limit behavior was covered in #759 and is not changed here); Geospatial / CRS / basin geometry; Hydro-met time series / forcing windows; SHUD numerical runtime / conservation / NaN; PostGIS / TimescaleDB domain behavior; Slurm production lifecycle / mock-vs-real parity beyond unchanged route-boundary and retry/cancel tests; External hydro-met providers / snapshot reproducibility; Run manifest / QC provenance; Published NHMS artifacts / display identity beyond unchanged display-readonly API boundary evidence. Also not selected: public route removal, DB migrations, display-readonly capability expansion, frontend UI behavior, scheduler, chain, two-node deployment behavior, or new source extraction.
  - Invariant Matrix: Governing invariant: API bootstrap tasks 5.1-5.4 are complete and represented before this closeout is checked. Source-of-truth identity/contract: `openspec/changes/governance-8-module-deepening/tasks.md`, GitHub issue/PR state, `docs/review-loop-log.jsonl`, and `docs/governance/STRUCTURAL_FILE_DISPOSITION_INVENTORY.md` agree on the API task-to-issue-to-PR mapping and owner/facade boundaries. Surfaces: Producers: completed PRs #815-#818 and closeout PR #819; Validators/preflight: focused API group pytest suite, scoped API ruff, OpenAPI drift tests, OpenSpec validation, frontend API type check only when schema artifacts change, and diff-check; Public routes/entrypoints: `apps.api.main:create_app`, runtime OpenAPI factory, role-aware route registry, startup/static/cache wiring, runtime config route, health/static routes, Slurm route visibility, and protected mutation middleware; Frontend/downstream consumers: generated frontend API types and existing API error-envelope consumers; Failure paths/rollback/stale state: display-readonly manual-action fail-closed behavior, protected mutation 401/403/422/503 paths, and no downstream mutation on denied or invalid pre-body requests; Evidence/audit/readiness: structural file disposition inventory, PR evidence block, and post-merge review-loop log.
  - Boundary Surfaces: `apps/api/main.py` remains the API bootstrap facade and retained protected-mutation seam; `apps/api/openapi_patching.py` remains runtime OpenAPI patch owner; `apps/api/route_registry.py` remains role-aware router inclusion owner; `apps/api/startup_wiring.py` remains app-state, runtime-config route, health/static/cache, and display cache warmup owner; `docs/governance/STRUCTURAL_FILE_DISPOSITION_INVENTORY.md` owns the final API group evidence map; the closeout must not create a second owner vocabulary, remove compatibility imports, alter route visibility, alter OpenAPI schema output, alter API error shapes, or promote display-readonly write capability.
  - Regression Rows: the final API evidence map records tasks 5.1-5.5 with issue/PR references, owners, closeout evidence, and non-goal boundaries; focused API group tests pass after the evidence update; OpenAPI drift remains unchanged unless the PR explicitly regenerates schema/type artifacts and runs `cd apps/frontend && pnpm check:api-types`; structural inventory still lists OpenAPI patching, route registry, startup wiring, and protected mutation retained seam owners/facade responsibilities; no runtime API code, route removal, authorization semantic change, request-body validation change, frontend UI behavior, DB/schema migration, Slurm scheduling behavior, scheduler, chain, two-node deployment behavior, or display-readonly capability expansion changes in this slice.
  - Verification Floor: focused verification commands above; `uv run pytest -q tests/test_entropy_audit_script.py tests/test_runtime_mode.py tests/test_api.py`; `uv run ruff check apps/api/main.py apps/api/openapi_patching.py apps/api/route_registry.py apps/api/startup_wiring.py tests/test_static_serving.py tests/test_runtime_mode.py tests/test_api.py tests/test_role_boundary_static.py tests/test_retry_cancel_consistency.py tests/test_monitoring_api.py tests/test_openapi_drift.py`; `uv run python -c 'from pathlib import Path; text=Path("docs/governance/STRUCTURAL_FILE_DISPOSITION_INVENTORY.md").read_text(); after=text.split("## API Bootstrap Group Closeout Evidence", 1)[1]; section=after.split("\\n## ", 1)[0]; block=after.split("\\n## Per-File Notes", 1)[0]; rows=[tuple(cell.strip() for cell in line.strip().strip("|").split("|")[:3]) for line in section.splitlines() if line.strip().startswith("|") and not line.strip().startswith("| Task ") and not line.strip().startswith("|---")]; expected=[("5.1 API OpenAPI patch owner-module extraction","#756","#815"),("5.2 API role-aware route registry extraction","#757","#816"),("5.3 API static/health/cache/startup wiring extraction","#758","#817"),("5.4 API protected mutation seam retention and tests","#759","#818"),("5.5 API group verification and evidence closeout","#760","#819")]; assert rows == expected, rows; tasks=[row[0] for row in rows]; issues=[row[1] for row in rows]; prs=[row[2] for row in rows]; assert len(rows) == 5 and len(set(tasks)) == 5 and len(set(issues)) == 5 and len(set(prs)) == 5, rows; assert not [row for row in rows if row not in expected], rows; upper=block.upper(); assert "TBD" not in upper and "TODO" not in upper, block; required_terms=["apps/api/openapi_patching.py","apps/api/route_registry.py","apps/api/startup_wiring.py","protected-mutation","API Bootstrap Closeout Non-Targets"]; missing=[term for term in required_terms if term not in block]; assert not missing, missing'` exits 0 and proves the `API Bootstrap Group Closeout Evidence` Markdown table has exactly five data rows with exact task/issue/PR bindings for tasks 5.1-5.5, no duplicate tasks/issues/PRs, no extra API closeout task row, no placeholder tokens, and adjacent owner/boundary evidence coverage; `openspec validate --all --strict --no-interactive`; `git diff --check`.

## 6. Frontend Map Surface Deepening

- [x] 6.1 M11 pure map builders extraction.
  ...
  - Fixture Level: expanded pure-builder owner extraction. This slice moves deterministic M11 map data/source builders out of `M11MapLibreSurface.tsx` while keeping that component as the stable rendering facade and compatibility export surface for existing tests/pages.
  - Repair Intensity: medium-high, because source keys, MVT metadata checks, GeoJSON budgets, unavailable reasons, and legacy helper imports directly control the display map's honest render/no-render behavior even though this slice must not change MapLibre primitive rendering or interaction dispatch.
  - Risk Pack Selection: Selected: Public API / CLI / script entry (`M11MapLibreSurface`, `M11MapSurface`, legacy helper exports such as `buildM11RegisteredOverlay`, `buildBasinFeatureCollection`, `buildBasinRiverFeatureCollection`, `buildSelectedSegmentFeatureCollection`, and `m11MapStyleUrls` remain import-compatible); Schema / columns / units / field names (GeoJSON feature properties, vector source layer names, MVT metadata identity, and `sourceKey` fields remain stable); Legacy compatibility / examples (existing page/test imports through `@/components/map/M11MapLibreSurface` keep working while the new builder owner becomes the source of truth); Resource limits / large input / discovery (basin and selected-segment geometry budgets, skipped counts, coordinate counts, and serialized byte accounting remain unchanged); Error handling / rollback / partial outputs (malformed, oversized, missing, release-blocked, unregistered, no-run, no-valid-time, and no-MVT states keep the same unavailable reasons and avoid registering fake sources); Documentation / migration notes (structural inventory records the builder owner and retained facade boundary); Release / packaging / dependency compatibility (frontend tests/build run without new runtime dependencies). Domain Selected: Geospatial / CRS / basin geometry for WGS84 GeoJSON collections, basin/national-river filtering, line-string validation, and client rendering budgets; Published NHMS artifacts / display identity for live-vs-mocked display boundary wording and station-MVT non-closure. Not Selected: Config / project setup; File IO / path safety / overwrite; Auth / permissions / secrets; Concurrency / shared state / ordering beyond unchanged React memo inputs; Hydro-met time series / forcing windows; SHUD numerical runtime / conservation / NaN; PostGIS / TimescaleDB behavior; Slurm production lifecycle; External hydro-met providers / snapshot reproducibility; Run manifest / QC provenance; backend station-MVT endpoint completion; visual redesign.
  - Invariant Matrix: Governing invariant: extracting pure builders must not change what M11 registers, renders, skips, labels, or reports as unavailable. Source-of-truth identity/contract: the new builder owner module owns registered overlay construction, vector source keys, GeoJSON feature collection builders, selected-segment unavailable wording, segment filters, national-river filtering, geometry budget summaries, and the component facade re-exports those helpers for legacy imports. Surfaces: Producers: `M11QueryState`, layer metadata from `/api/v1/layers`, overview basin boundaries, basin segment rows, selected-segment geometry, national river GeoJSON, mesh-covered basin ids, and station overlay props; Validators/preflight: MVT metadata validity/run matching, valid-time normalization, basin and selected-segment geometry budget checks, collection-level basin-river budgets, source-key serialization, and unavailable reason builders; Public entrypoints: `M11MapLibreSurface`, `M11MapSurface`, helper exports imported by `M11Shell.test.tsx`, and map data attributes used as regression evidence; Consumers/downstream: MapLibre `<Source>`/`<Layer>` props, M11 controls, station-layer hook type imports, and frontend regression suites; Failure paths/stale state: release-blocked metadata, missing run id except national overlays, missing current valid time, invalid selected segment geometry, oversized basin/river geometry, empty dynamic mesh coverage, and static-river double-line suppression.
  - Boundary Surfaces: This slice may create a builder owner module and update `M11MapLibreSurface.tsx` to consume/re-export it, but it must not extract MapLibre primitive components, change source/layer IDs, change paint/layout, alter click/hover priority, alter popup or selected-station state, change camera/error handling, complete station-MVT backend work, add generated API type changes, or introduce a second vocabulary for M11 layer identity.
  - Regression Rows: registered flood/warning/discharge/national overlays keep the same vector source IDs, source layers, tile URL templates, source keys, hit-layer registration, and metadata mismatch rejection; basin feature collections still filter by visible basin ids, preserve basin properties, and omit over-budget boundaries; basin-river collections still preserve feature properties, layer colors, skipped/coordinate/serialized-byte accounting, and collection-level unavailable reasons; selected-segment collections still omit malformed or oversized geometry and expose the same Chinese unavailable text; national static rivers are filtered only when a dynamic river line is active and mesh-covered basin ids are present; segment filters still match both `river_segment_id` and `segment_id`; station overlay remains GeoJSON-backed and no station-MVT backend completion is claimed; `M11MapLibreSurface.tsx` remains the stable facade for map rendering and existing helper imports.
...
- [x] 6.2 M11 MapLibre primitive extraction.
  ...
  - Fixture Level: expanded MapLibre primitive owner extraction. This slice moves JSX primitives and their source/layer constants into a focused owner while `M11MapLibreSurface.tsx` remains the interaction/camera/popup facade consuming those primitives.
  - Repair Intensity: medium-high, because the extracted primitives define display source IDs, layer IDs, paint/layout expressions, promote IDs, and selected/hovered filter wiring that tests and interaction dispatch depend on.
  - Risk Pack Selection: Selected: Public API / CLI / script entry (`M11MapLibreSurface`, `M11MapSurface`, exported primitive constants/helpers, and legacy map helper exports remain compatible); Schema / columns / units / field names (GeoJSON property names, vector `source-layer`, promote IDs, and MapLibre expression inputs remain unchanged); Legacy compatibility / examples (existing page/test imports through `M11MapLibreSurface.tsx` continue to work, with any new primitive owner export treated as additive); Error handling / rollback / partial outputs (primitive rendering stays honest for empty collections and absent overlay data, without fake sources); Documentation / migration notes (structural inventory records the primitive owner and retained surface boundary); Release / packaging / dependency compatibility (no new frontend runtime dependency). Domain Selected: Geospatial / CRS / basin geometry for source IDs, label markers, national/basin/basin-river/selected-segment line and polygon primitives; Published NHMS artifacts / display identity for preserving visible layer identity and station-MVT non-closure. Not Selected: Config / project setup; File IO / path safety / overwrite; Auth / permissions / secrets; Concurrency / shared state / ordering beyond unchanged React render ordering; Resource limits / large input / discovery beyond consuming #761 builder outputs; Hydro-met time series / forcing windows; SHUD numerical runtime / conservation / NaN; PostGIS / TimescaleDB behavior; Slurm production lifecycle; External hydro-met providers / snapshot reproducibility; Run manifest / QC provenance; backend station-MVT endpoint completion; visual redesign.
  - Invariant Matrix: Governing invariant: extracting primitives must not change the MapLibre source/layer graph or interaction-observable layer IDs. Source-of-truth identity/contract: the primitive owner module owns national river, basin boundary, basin label marker, basin-river, registered overlay, selected-segment, and station cluster primitive components plus source IDs, layer IDs, paint/layout, promote IDs, and selected/hovered filters. Surfaces: Producers: #761 builder collections and registered overlays, station feature collections, basemap choice, selected/hovered segment IDs, selected station ID, and overlay data; Validators/preflight: focused M11Shell layer/source/cluster/highlight/hover tests, frontend build, OpenSpec validation, and inventory evidence; Public entrypoints: `M11MapLibreSurface`, `M11MapSurface`, primitive owner exports, data attributes, and interactive layer IDs; Consumers/downstream: hover/click dispatch still reads the exported layer ID constants, popup/selection/camera still remain in `M11MapLibreSurface.tsx`; Failure paths/stale state: empty national river, empty basin/basin-river/selected-segment collections, overlay without GeoJSON fallback data, missing station collection, and station cluster selected-state filters.
  - Boundary Surfaces: This slice may create a primitive owner module and make `M11MapLibreSurface.tsx` import its primitive components/constants, but it must not move click/hover dispatch functions, rendered-feature fallback, cluster expansion behavior, cursor state, popup rendering/state, fit/fly camera effects, map source-error handling, Tianditu style construction, #761 pure builders, station-MVT backend work, generated API types, or visual semantics.
  - Regression Rows: national river source/layer IDs, paint expressions, dimming/satellite behavior, and no-static-geo honest skip remain unchanged; basin boundary fill/outline IDs, promoteId, label marker DOM/test id, and label anchors remain unchanged; basin-river source ID, promoteId, main/casing/hover/selected layer IDs, paint/layout, subdued opacity, and selected/hovered filters remain unchanged; registered overlay vector/GeoJSON source behavior, casing/main/hit/selected layer ordering, hit-layer ID, promoteId, source key usage, source-layer propagation, and selected halo filters remain unchanged; selected-segment source/layer IDs, promoteId, paint, and no-source behavior for empty collections remain unchanged; station cluster source/layer IDs, cluster options, promoteId, count symbol layer, selected station filter, and GeoJSON-backed station overlay boundary remain unchanged; interaction dispatch stays in `M11MapLibreSurface.tsx` and only consumes exported constants.
...
- [x] 6.3 M11 interaction dispatch extraction.
  ...
  - Fixture Level: expanded interaction-dispatch owner extraction. This slice moves event dispatch and hit-test helpers behind a focused owner while `M11MapLibreSurface.tsx` keeps rendering composition, popup state/rendering, camera effects, map-error handling, and Tianditu style wiring.
  - Repair Intensity: medium-high, because interaction ordering and cursor/event payload behavior are user-visible and depend on source/layer IDs introduced by #762.
  - Risk Pack Selection: Selected: Public API / CLI / script entry (`M11MapLibreSurface`, `M11MapOverlayInteraction`, `M11MapSurface`, and existing page/test imports remain compatible); Schema / columns / units / field names (event payload `layerId`, feature property reads such as `river_segment_id`, `segment_id`, `station_id`, and MapLibre layer IDs remain stable); Legacy compatibility / examples (callbacks keep the same `onOverlayHover` / `onOverlayClick` payload shapes and null-hover semantics); Error handling / rollback / partial outputs (rendered-feature fallback still catches MapLibre query errors and cluster expansion still no-ops when source/cluster/geometry data is missing); Documentation / migration notes (structural inventory records the interaction owner and retained surface boundary); Release / packaging / dependency compatibility (no new frontend runtime dependency). Domain Selected: Geospatial / CRS / basin geometry for map-feature point/line hit priority and station cluster fly-to center handling; Published NHMS artifacts / display identity for preserving display-readonly interaction semantics and station-MVT non-closure. Not Selected: Config / project setup; File IO / path safety / overwrite; Auth / permissions / secrets; Concurrency / shared state / ordering beyond React callback/state update ordering; Resource limits / large input / discovery beyond consuming #761/#762 outputs; Hydro-met time series / forcing windows; SHUD numerical runtime / conservation / NaN; PostGIS / TimescaleDB behavior; Slurm production lifecycle; External hydro-met providers / snapshot reproducibility; Run manifest / QC provenance; popup internals; camera/error helpers; backend station-MVT endpoint completion; visual redesign.
  - Invariant Matrix: Governing invariant: extracting interaction dispatch must not change priority, cursor behavior, callback payloads, or cluster expansion behavior. Source-of-truth identity/contract: the interaction owner module owns interactive layer ID ordering, event-feature lookup, rendered-feature fallback, feature string-property extraction, hover dispatch, mouseleave reset, click dispatch, station cluster expansion, and cursor reset/set behavior. Surfaces: Producers: MapLibre events, `MapRef`, `showStationLayer`, renderable overlay, basin/basin-river feature availability, callbacks, and state setters; Validators/preflight: focused M11Shell priority/click/hover/cluster/rendered-cluster/river-segment tests, frontend build, OpenSpec validation, and inventory evidence; Public entrypoints: `M11MapLibreSurface`, `M11MapOverlayInteraction`, callback props, interactive layer IDs, and data attributes; Consumers/downstream: basin drilldown, river segment selection, MVT overlay click payloads, station popup flow, and cluster expansion fly-to; Failure paths/stale state: station cluster query fallback, missing map source, invalid cluster id, missing point geometry, queryRenderedFeatures exceptions, hover leave, non-hit pixels, and hidden station overlay.
...
- [x] 7.2 Re-run report-only entropy audit after all six groups complete.
  ...
  - Fixture Level: expanded documentation/evidence fixture. Reason: this task
    records a report-only governance audit snapshot after all six module
    deepening groups completed, and the evidence becomes the input for final
    verification and implementation mapping.
  - Risk Packs Selected: Entropy report integrity, because the PR records
    numeric findings and metadata from the audit JSON; baseline/hard-gate
    boundary, because the audit must remain report-only and must not write
    `.entropy-baseline/latest.json`; structural file budget evidence, because
    line-count and mandatory-governance deltas are the core acceptance evidence;
    compatibility/scoped-context evidence, because scheduler/chain facade guard
    and scoped AGENTS context must stay clean. Risk Packs Not Selected:
    runtime behavior, DB/schema, Slurm scheduling, API route behavior,
    frontend UI behavior, generated API types, production topology, station-MVT
    closure, and live node-27 receipts, because this slice changes only
    docs/OpenSpec evidence.
  - Invariant Matrix: Governing invariant: the recorded entropy delta must be a
    faithful report-only snapshot from the current audit command, with no
    detector changes, source/runtime changes, hard-gate enablement, or baseline
    write. Source-of-truth identity/contract:
    `/tmp/nwm-entropy-768.json` generated by
    `scripts/governance/audit_repo_entropy.py --format json`, the
    `governance-4a.entropy-report.v1` metadata, and
    `docs/governance/entropy-burndown-triage.md`. Surfaces: Producers: audit
    script output and current source line counts; Validators/preflight:
    entropy audit tests, OpenSpec validation, markdownlint, baseline diff, and
    diff-check; Storage/cache/query: `.entropy-baseline/latest.json` must not
    be written; Public routes/entrypoints: none - docs-only evidence;
    Frontend/downstream consumers: none - docs-only evidence; Failure
    paths/stale state: stale counts, mismatched report timestamp, missing
    mandatory-governance/compatibility/scoped-context deltas, or accidental
    baseline write; Evidence/audit: Governance-8 Module Deepening Delta section
    in `entropy-burndown-triage.md`.
  - Boundary Surfaces: This slice may update entropy audit evidence docs and
    this task checkbox only. It must not change detector logic, implementation
    files, tests other than evidence references, generated artifacts,
    `.entropy-baseline/latest.json`, CI hard gates, compatibility symbols,
    lane semantics, API/frontend behavior, DB/schema, Slurm scheduling,
    production topology, station-MVT status, or display live evidence.
  - Regression Rows: audit command writes `/tmp/nwm-entropy-768.json` and
    reports `mode=report-only`, `baseline_written=false`, and
    `gate_eligible_count=0`; six-facade line-count delta is recorded as
    29,467 -> 21,449 (-8,018); mandatory-governance count is recorded as 99;
    scheduler and chain compatibility facade signal counts remain 0; four
    scoped AGENTS contexts pass with 0 missing instruction/glossary/stale
    signals; `.entropy-baseline/latest.json` has no diff; OpenSpec validation,
    markdownlint, entropy audit tests, and diff-check pass.
  - Verification Floor: focused verification command above;
    `uv run python scripts/governance/audit_repo_entropy.py --format json
    >/tmp/nwm-entropy-768.json`; `git diff -- .entropy-baseline/latest.json
