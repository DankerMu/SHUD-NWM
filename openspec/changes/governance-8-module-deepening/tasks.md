...
- [x] 5.5 API group verification and evidence closeout.
  ...
  - Verification Floor: focused verification commands above; `uv run pytest -q tests/test_entropy_audit_script.py tests/test_runtime_mode.py tests/test_api.py`; `uv run ruff check apps/api/main.py apps/api/openapi_patching.py apps/api/route_registry.py apps/api/startup_wiring.py tests/test_static_serving.py tests/test_runtime_mode.py tests/test_api.py tests/test_role_boundary_static.py tests/test_retry_cancel_consistency.py tests/test_monitoring_api.py tests/test_openapi_drift.py`; `uv run python -c 'from pathlib import Path; text=Path("docs/governance/STRUCTURAL_FILE_DISPOSITION_INVENTORY.md").read_text(); section=text.split("## API Bootstrap Group Closeout Evidence", 1)[1].split("\\n## ", 1)[0]; required=["5.1 API OpenAPI patch owner-module extraction","5.2 API role-aware route registry extraction","5.3 API static/health/cache/startup wiring extraction","5.4 API protected mutation seam retention and tests","5.5 API group verification and evidence closeout","#756","#757","#758","#759","#760","#815","#816","#817","#818"]; missing=[item for item in required if item not in section]; assert not missing, missing; assert "TBD" not in section and "TODO" not in section'` exits 0 and proves an `API Bootstrap Group Closeout Evidence` map exists with tasks 5.1-5.5, issues #756-#760, PRs #815-#818 plus this closeout PR, and no placeholder tokens; `openspec validate --all --strict --no-interactive`; `git diff --check`.
