...
- [x] 4.9 Scoped exclusion extraction.
  ...
  - Invariant Matrix: owner module `services.production_closure.readiness_scope_exclusions`; stable facade `services.production_closure.readiness_validation`; retained facade imports/re-exports include `_exclusion_items` until inventory-backed caller migration proves old import paths are safe to remove; `_summary_exclusions` remains in `readiness_validation` until 4.10 final aggregation extraction because it aggregates exclusions from all validated items into summary artifacts; `validate_readiness(config)` still composes deterministic items, dependency summaries, scheduler evidence, live proof items, scoped exclusions, `_validate_items`, `_release_blockers`, `_final_ready`, summary, and CLI behavior in the existing validator boundary; item validation remains owned by `readiness_item_contracts`.
  - Boundary Surfaces: exactly two static item surfaces `cldas_restricted_source` and `incomplete_real_national_data`; item ids `scope-exclusion-cldas` and `scope-exclusion-national-data`; exclusion ids `cldas-restricted` and `real-national-data-incomplete`; status and execution mode `not_executed`; `required_for_final=false`; `live_proof_accepted=false`; artifact refs `readiness_items.json` and `summary.json`; residual risk, action, and removal criteria stay non-empty and product-scope specific; summary inclusion still occurs through `_summary_exclusions`; release blockers and final readiness must not treat exclusions as failures or accepted live proofs.
