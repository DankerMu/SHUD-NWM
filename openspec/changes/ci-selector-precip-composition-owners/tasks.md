...
- 两条 selector 精确集钉：加规则前红（缺 4 个降水 suite）、加规则后绿，红→绿输出贴 PR body
...
- [x] 1.4 两条 owner 行均**不加** `stop_on_match`、**不加** `only_when_any_changed`——`apps/api/**` 规则在其后仍需累加三个通用 API suite。
...
- [x] 3.2 `tests/test_openapi_drift.py` 新增隔离 mutation proof，正反两腿同一测试：未 mutate 的 `main.create_app()` schema 在**两处**与 `openapi/nhms.v1.yaml` 一致——`/api/v1/precip/{source}/{cycle}/index` 操作的 response schema，以及**不存在** `PrecipIndexResponse` component；`monkeypatch.setattr(main, "_patch_precip_openapi", <no-op>)` 后新建 app 在**两处都不一致**（`_patch_precip_openapi` 既 pop 掉 `PrecipIndexResponse` 又重写该 operation，而静态 YAML 中 `PrecipIndexResponse` 出现 0 次）。**逐处比较，不做整文档相等**——既有 oracle `test_static_openapi_matches_runtime_schema` 比的是模块单例 `main.app`，新建 app 的 runtime-mode env 差异会让整文档「不等」腿真空为绿。同样**不得**触碰 `main.app`。idiom 先例见同文件 `test_openapi_patch_owner_module_preserves_main_monkeypatch_facade`。
