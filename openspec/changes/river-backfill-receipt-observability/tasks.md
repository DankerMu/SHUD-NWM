...
- [x] 1.1 Shortfall reason text: append the concurrent-DELETE
...
- [x] 1.2 New `_is_lock_contention` (pgcode in {"55P03","40P01"}, no
      message fallback) + `BatchLockContention` exception raised from
      `execute_batch`'s except arm (57014 check byte-unchanged).
- [x] 1.3 `_run_one_batch_with_retry`: catch `BatchLockContention` at
...
- [x] 3.2 `shortfall` bullet: document the double-zero signature and
      the check-parser-re-parse-window-first step.

## 4. Tests

- [x] 4.1 Shortfall double-zero → signature sentence present; stage
      `shortfall`; behavior unchanged (rollback + next_page rewind).
- [x] 4.2 Shortfall with non-zero diagnostics → signature ABSENT.
- [x] 4.3 Fake cursor pgcode 55P03 → stage `lock_contention`, distinct
      advice, no halving (batch not re-attempted), receipt validates.
- [x] 4.4 Fake cursor pgcode 40P01 → same classification.
- [x] 4.5 Existing 57014 tests green unmodified
      (tests/test_node27_river_identity_backfill.py :266/:356/:375).
- [x] 4.6 Extend the EXISTING receipt-shape test at
...
- [x] 4.7 Schema test: stage `lock_contention` validates; unknown stage
      rejected.
- [x] 4.8 AUTHORIZED existing-test modification (fixture-review P1
      enumeration):
...
- [x] 6.1 `uv run pytest -q tests/test_node27_river_identity_backfill.py
      tests/test_node27_river_identity_backfill_receipt.py` green.
- [x] 6.2 Red evidence: new lock-contention and signature tests fail
