...
- Production-lane effect (review round 1): the filter also applies on the candidate-scoped FileJournal path. A nested partial-array retry (`_retry_partial_array_stage`) records the pending SUBSET as `cohort_members` under the same run's `_forcing_retry_N`; on re-entry that subset row is excluded, the bare `partially_failed` master is resumed, and the subset is recomputed (accepted outcome, replacing the pre-PR `FORCING_RESUME_IDENTITY_MISMATCH` wedge). The nested retry id is then floored at the stage snapshot's next free suffix (`_stage_retry_attempt_floor` over the stage-selection `existing_jobs`, including excluded rows, threaded into `_schedule_cycle_stage_retry` → `_retry_job_for_stage_result`), so it never collides with the excluded `_retry_1`. The floor is taken only from the stage-selection snapshot on the resume branch; a fresh submit passes no floor (its id was already minted past every snapshot row), so the post-submit re-query never advances a suffix and the job-retry-mechanism "selected stage snapshot" rule holds.
