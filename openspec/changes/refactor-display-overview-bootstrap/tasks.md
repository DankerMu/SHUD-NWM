...
- [ ] 2.1 `apps/frontend/src/lib/m11/queryState.ts` 删 `'water-level'` 从 `M11Layer` union（line 4、29）。**Canonical 4-layer 集（与 frontend-mvt-layer-consumption spec line 4 对齐）= `'discharge' | 'flood-return-period' | 'warning-level' | 'river-network'`**；`met-stations` / `met-raster` 属于 raster overlay union（如 `M11RasterOverlay`），不在本 hydrology layer 集；本任务只触 `M11Layer`，不动 raster overlay 任何 union
