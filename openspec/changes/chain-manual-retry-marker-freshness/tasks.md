...
- [ ] 1.1 新鲜性判别谓词单处实现（design D1："本 evidence 上存在活跃 manual-retry
      决策"——`decision == "manual_retry"` 或 `reason == "manual_retry_requested"`；
      判别不出 fail-safe 不采信）
- [ ] 1.2 **铸造点 gate（生产主通道）**：`scheduler_candidate_manifest.
      ...
      其余键逐字节不变
- [ ] 1.3 **evidence 分支 gate（纵深）**：`_retry_attempt_from_basins` 的
      `state_evidence.manual_retry` 分支同谓词判别；direct 字段两分支逐字节不变
- [ ] 1.4 丢弃 claim 时结构化 warning（design D4 措辞："no active manual-retry
      decision"，两写点同 schema）
- [ ] 1.5 `_manual_retry_scoped_cycle_execution` 现状保持 + 注释挂账（design D3
      重写后的理由口径）
- [ ] 1.6 spec delta 场景与实现一致（含 D5 的 :467 交叉引用句）

## 2. Evidence Floor（红先行或变异咬红）

- [ ] E1 (主判别，红先行，**双格**): 候选带无活跃决策的 marker claim
      ...
      链算进预期
- [ ] E2 (must-preserve 1): 既有 `test_manual_retry_terminal_stage_submits_new_attempt_identity`
...
- [ ] E3 (must-preserve 2): 运维 API 显式 `basin.retry_attempt`/`manual_retry_attempt`
      仍被采信（不经判别器）两格
- [ ] E4 (消费者审计钉): 陈旧 marker 几何下 `:581` `submission_attempt` 与
      `chain_manifests` 写进 runtime manifest 的 `submission_attempt` 都不再是
      marker 值（两处各一断言）
- [ ] E5 (AC-4 非静默): 丢弃路径 caplog 断言（措辞 "no active manual-retry
      decision"，含 claim 值与实际 decision）
- [ ] E6 (D3 边界钉): scoped 谓词对陈旧 marker 的现状行为钉 + scoped 谓词选择的
      job 集合喂 `_next_retry_attempt_for_stage` 的耦合现状钉
- [ ] E7 (判别器三态参数化): fresh（采信）/ 无活跃决策（丢弃）/ 判别键缺失
      （fail-safe 丢弃）；丢弃格变异（删判别器）咬红
- [ ] E8 (reason-only 形状): evidence 只带 `reason=="manual_retry_requested"` 无
      decision 键 → 采信
- [ ] E9 (**scheduler→chain 接缝腿**，P0 修正核心): 生产投影路径（
...
- [ ] E10 (P1 可达性核查): `resume_after_completed_stage` 等更高优先 lane 抢占 +
