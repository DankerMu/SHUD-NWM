...
- [ ] A-1 `_river_network_source_version` 投影并摘要 `segment_count, checksum, geometry_generation`（D1）
- [ ] A-2 `_run_row` 与 `display_ready_run` 同加 `LEFT JOIN core.river_network_version rnv` 投影 `geometry_generation`；
      `_run_source_version` 的 `revision_basis` 加该字段
- [ ] A-3 单测（值级）：同一 `basin_version_id` 在 generation 0→1、`segment_count` 变、`checksum` 变 三种情况下
      `_river_network_source_version` 各返回不同串；未变时逐字节相等
- [ ] A-4 单测：同一 run 在 generation 0→1 后 `_run_source_version` 变；`_run_row` 与 `display_ready_run` 都返回该字段；
      对同一 run 两者喂出的 `_run_source_version` 相等（目录/瓦片一致）
- [ ] A-5 单测：两条路由的 `cache_key(TileInput(...))` 随之改变（复用 `services/tiles/mvt.py::cache_key` 值级断言）
- [ ] A-6 凡会走到 `_river_network_source_version` / `_run_row` / `display_ready_run` 真 SQL 的 sqlite 夹具：有该表且含
      `geometry_generation`（及 `segment_count`/`checksum`），全绿
- [ ] A-6b 有意重钉（D4 契约变化，不是放宽；每处在 docstring 写明来由）：
...
- [ ] A-7 `feature_limit(layer)` + `NATIONAL_DISCHARGE_FEATURE_LIMIT = 20_000`；bind / 413 判据与 details /
      `MVT_TILE_BUDGET_TRUNCATED.max_features` 同源；`NATIONAL_DISCHARGE_QUERY_VERSION = "fair-network-budget-v6"`
- [ ] A-8 单测：`feature_limit("hydro-national") == 20_000`，其余四层与 `None` == `MVT_MAX_FEATURES`；
...
- [ ] A-9 `MVT_TILE_FEATURE_OVERFLOW_BLANKED` WARNING（D5），上限值取自 bind
- [ ] A-10 单测（桩行 `"tile": None`，真实 overflow 形状）：`feature_coordinate_overflow_count=1` → 一条 BLANKED、
...
- [ ] A-11 `test_every_column_the_tile_route_reads_is_projected_by_every_layer` 继续绿（新读列已五层投影）；
      `postgis_tile_sql(layer)` 五层文本逐字节不变（A 组不改 SQL）
- [ ] A-12 node-27 throwaway-DB 集成（扩 `tests/test_mvt_national_identity_probe_integration.py` 的 generation 形态）：
...
- [ ] B-1 `_lock_river_network_version` helper（D2）；`_backfill_output_segment_geometry` 入口第一条语句调用；
      `seed_qhh_output_segments` 与 bootstrap 主路径在 `_seed_output_segment_rows` 之前调用；两处 docstring 写锁序不变量与其边界
- [ ] B-2 node-27 throwaway-DB 两会话交错测试：A `UPDATE core.river_network_version`（持锁）→ B 调
...
- [ ] B-2b 两会话，逐步：(1) 前置：该网络已提交可填行（NULL geom 或缺 `Type`）；(2) A 显式调 `_lock_river_network_version`
      ...
      随后 B 完成提交。两腿输出贴 PR
- [ ] B-3 空转断言：几何完备网络 `only_missing=True` → 返回 0、恰一条父行 `FOR NO KEY UPDATE` 且它是第一条语句、零条
...
- [ ] B-3b B 组有意重钉（D2 契约变化，不是放宽；docstring 写明）：`tests/test_hhe_mvt_binding.py` 中断言
...
- [ ] B-4 Type 完备性 helper（D3-1）接入 `seed_qhh_output_segments` 与 bootstrap 主路径；code
      `QHH_OUTPUT_SEGMENT_STREAM_TYPE_INCOMPLETE`
- [ ] B-5 真实 DB 红-绿：先回填使 output 行带 `Type`，再让源 reach 的几何退化（`ST_Length = 0`）使尾随 backfill 返回 0，
...
- [ ] B-6 写面扫描：generation 写的全形状计数（D3-2）、`db/` 纳入（D3-3）、两处 rnv 写方不含该列的显式钉、非空性自检含 `db`；
...
- [ ] B-7 扫描的合成用例（对分类函数，不改生产代码）：`SET geometry_generation = 0`、`SET geometry_generation=geometry_generation+2`、
...
- [ ] B-8 D3-4 / D3-5 裁决只在规格落地（本 change 的 spec delta），代码不改

## C. 回收 runner（#2160）— `scripts/node27_mvt_cache_retention{.py,_once.sh}`、`tests/test_node27_mvt_cache_retention.py`

- [ ] C-1 `fstat` 归类；monkeypatch `os.fstat` 抛 `OSError(errno.ESTALE)` → summary JSON 落盘、`counts.failed == 1`、
      `failed[0].error_type == "OSError"`、rc 1
- [ ] C-2 `<hh>` 在父列举后消失（monkeypatch scandir 抛 `FileNotFoundError` / `NotADirectoryError`）→ `failed[] == []`、rc 0；
      `.locks/<hh>` 同理
- [ ] C-3 `<hh>` `EACCES` 与 cache root 不可读两条既有用例不变且绿；cache root 自身 ENOENT 仍进 failed
- [ ] C-4 `<root>/.locks` 自身 `chmod 0o000`（lstat 通过、scandir `EACCES`）→ 一条 `enumeration_unavailable`、rc 1，瓦片车道仍删除
- [ ] C-5 post-flock `lstat` 抛非 ENOENT `OSError` → `failed`；`unlink` ENOENT → `already_gone`，各一测
- [ ] C-6 wrapper `SUMMARY_PATH` / `LOG_FILE` / `LOCK_PATH` 相对值 → blocked（`SUMMARY_PATH_NOT_ABSOLUTE` / `LOG_FILE_NOT_ABSOLUTE` / `LOCK_PATH_NOT_ABSOLUTE`），
      runner 不执行，子进程测试后仓根 `git status --porcelain` 为空；`bash -n` 通过
- [ ] C-7 既有断言无一放宽

## D. 规格、receipt 与收尾（orchestrator）

- [ ] D-1 spec delta：`mvt-tile-contract`（I-geom 扩展 + D3-4/D3-5 边界）、`postgis-tile-clipping-cache`（按层预算 + BLANKED）、
...
- [ ] D-3 node-27 receipt `docs/runbooks/receipts/2026-09-18-batch-10-mvt-followups-node27.md`：
