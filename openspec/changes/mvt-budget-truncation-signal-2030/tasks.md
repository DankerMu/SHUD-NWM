...
- [x] 1.1 `services/tiles/mvt.py::postgis_tile_sql` final SELECT: add
...
- [x] 2.1 `apps/api/routes/hydro_display.py`: `import logging`; module-level
  `logger = logging.getLogger(__name__)` next to the other module constants.
- [x] 2.2 `_fetch_postgis_tile_bytes`: read `intersecting_feature_count`, `intersecting_coordinate_count`,
...
- [x] 3.1 caplog matrix on `_fetch_postgis_tile_bytes(session, "river-network-national", {}, z=3, x=6, y=3)`
...
- [x] 3.2 `test_every_tile_layer_projects_the_prefilter_intersecting_counts`: for each of the five layers,
...
- [x] 3.3 `test_every_column_the_tile_route_reads_is_projected_by_every_layer` (route↔SQL coverage lock):
...
- [x] 3.4 `_budget_row(coordinate_count)` gains `"intersecting_feature_count": 12,
...
- [x] 3.5 Red proof (batched, implementer): with pre-change `services/tiles/mvt.py` + `hydro_display.py`,
...
- [x] 4.2 `uv run pytest -q tests/test_hydro_display_mvt_scaling.py tests/test_hhe_mvt_binding.py
...
- [x] 4.3 `openspec validate mvt-budget-truncation-signal-2030 --strict --no-interactive` → valid.
- [x] 4.4 Five-layer SQL regression (local, orchestrator): for each layer, `difflib.unified_diff` of
...
- [x] 5.0 Setup: isolated worktree at the PR head (never the production checkout), `NHMS_ENABLE_LIVE_POSTGIS_MVT=true`
...
- [x] 5.1 Signal fires: in-process, `hydro_display.collection_coordinate_limit` monkeypatched to 40000 for tile 3/6/3 and to 15000
...
- [x] 5.2 Production bind silent (`river-network-national`): one pass over `xyz_tiles(CHINA_BOUNDS, [3,4,5,6,7])`
...
- [x] 5.3 Bytes unchanged: md5(tile) old-SQL vs new-SQL equal on 0/0/0, 1/1/0, 2/3/1, 3/6/3, 4/12/6, 5/25/12,
...
- [x] 5.4 Receipt `docs/runbooks/receipts/2026-09-08-issue-2030-budget-truncation-signal-node27.md`: method,
