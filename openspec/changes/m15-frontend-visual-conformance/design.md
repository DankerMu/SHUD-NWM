...
- Manifest commit SHA must be the checked-out `git rev-parse HEAD` commit. CI sets `M15_EVIDENCE_SHA` to the pull request head SHA for PR events and to `github.sha` for push/non-PR events, checks out that same SHA, and fails before capture if `M15_EVIDENCE_SHA` differs from `HEAD`. Local `M15_EVIDENCE_SHA`, PR head SHA env, `GITHUB_SHA`, and `CI_COMMIT_SHA` values are assertions that must match `HEAD`; they cannot attest a different tree. PR evidence must be regenerated after the final commit so it cites the frozen PR head SHA under review. Placeholder values such as `local-uncommitted` are not acceptable.
