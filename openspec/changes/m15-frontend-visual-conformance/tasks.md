...
- [x] 1.2 Update shared CSS/components before page-specific styling. Expected output: shared tokens/components carry the final nav height, panel dimensions, timeline height, warning palette, and focus styling used by all required routes.
...
- [x] 2.3 Add loading, empty, error, restricted, RBAC-denied, partial-data, hover/focus, and icon-accessibility checks. Expected output: each required route has at least one non-happy state test or documented screenshot fixture, and key icon controls have accessible names.
...
- [x] 3.3 Capture or generate state evidence for loading, empty, error, restricted/RBAC-denied, and partial-data states. Expected output: each state label has either screenshot evidence or an automated e2e assertion tied to a deterministic fixture.
- [x] 3.4 Store evidence under `.codex/evidence/issue-176/screenshots/` and create `.codex/evidence/issue-176/manifest.json` or documented markdown with route, viewport, fixture mode, SHA, state label, command, and artifact path.
...
- [x] 4.2 Document blocking criteria: text overflow, horizontal scroll, panel/timeline overlap, missing accessible names, lost focus states, inconsistent warning colors, fake success data in error/restricted states, and missing required screenshot metadata.
- [x] 4.3 Run frontend tests, E2E checks, build, and screenshot capture commands. Minimum expected outputs: `openspec validate m15-frontend-visual-conformance --strict --no-interactive` exits 0; `cd apps/frontend && corepack pnpm test` exits 0; `cd apps/frontend && corepack pnpm build` exits 0; the focused Playwright/e2e screenshot command exits nonzero if required evidence, no-overlap assertions, accessible names, or manifest metadata are missing.
- [x] 4.4 Update `progress.md` with visual conformance status, required/extended routes covered, evidence manifest path, commit SHA, and remaining surfaces.
