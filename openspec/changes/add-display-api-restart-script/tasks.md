...
- [ ] 2.2 (defer) Check `scripts/diagnostic/display-cold-waterfall.sh` for inlined `setsid python ...` patterns — if present, replace with `bash scripts/ops/start-display-api.sh` invocation; if absent (likely), record "no-op confirmed" in PR body and skip
