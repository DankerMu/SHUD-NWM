...
- [x] Apply the decision rule in `design.md`: delete the three `not callable(state_provider)` gates in `build_candidates` (the two `active_duplicate_pipeline` gates and the `completed_duplicate_pipeline` gate, located by those reason strings) if no supported deployment or exercised fixture depends on them; otherwise keep them, pin them with a test that names the depending deployment, and record the refusal to delete.
