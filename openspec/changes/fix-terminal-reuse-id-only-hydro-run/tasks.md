...
- [x] Apply the decision rule in `design.md`: delete the three `not callable(state_provider)` gates (`scheduler_candidates.py:389-395`, `:396-407`, `:412-422`) if no supported deployment or exercised fixture depends on them; otherwise keep them, pin them with a test that names the depending deployment, and record the refusal to delete.
