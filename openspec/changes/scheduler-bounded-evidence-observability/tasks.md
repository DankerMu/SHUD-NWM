...
- [x] 1.5 `limit.candidate_lists`:fallback 构造时置 `"summarized"`;droppable 档清空任一 candidate 列表时翻 `"dropped"`;不超限路径不写该键;`_compact_limit` 终极档(仅存 `reason`)行为不变——该档下两个新键随 limit 块压缩消失(既有 fail-closed,豁免)。
...
- [x] 2.3b D2b 命中:超限 payload 携带 `restart_reconcile`(含 `reserved_unbound_error` 与 quarantine outcome 行)→ fallback 产物保留紧凑 `restart_reconcile`,`reserved_unbound_error`、`quarantine_reason`、`quarantine_field` 可读;无该块的 payload 不写该键。
- [x] 2.4 退化清空:max_evidence_bytes 压到摘要也放不下(但高于 `_compact_limit` 终极档阈值)→ 列表清空、`limit.candidate_lists == "dropped"`、体积仍 ≤ 上限;彻底不可满足时仍抛 `SchedulerEvidenceWriteError("evidence_size_limit_exceeded")`。
