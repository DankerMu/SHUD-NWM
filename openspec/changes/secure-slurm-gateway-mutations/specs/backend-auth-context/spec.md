...
#### Scenario: Missing or mismatched service credential
- **WHEN** a Slurm mutation has no otherwise-valid dev/live identity and the service bearer is missing or mismatched
- **THEN** the route returns `401 AUTH_REQUIRED` and records a no-mutation decision
- **AND** it does not parse an invalid mutation body into a gateway call or invoke a gateway method.
