...
4. **Use a route-scoped service bearer.** `SLURM_GATEWAY_SERVICE_TOKEN` authenticates a fixed scheduler actor with `operator` role only at Slurm mutation dependencies. Comparison is constant-time; missing, empty, short/invalid, or mismatched credentials fail closed. Existing dev/test and live-proof identities remain available under their existing environment gates, so an authorized `sys_admin` can exercise enabled reset without granting reset to the scheduler token.
...
3. Resume the scheduler timer and prove its preflight/client path remains healthy. Roll back by stopping the timer, restoring the prior code/unit/env backups, restarting gateway, and only then resuming the timer.
