...
- [ ] 1.1 `services/orchestrator/scheduler_state_failure.py`（design D2）：
...
- [ ] 1.2 消费点收窄（design D3）：`:339` 换用新 helper（`:343` 传参形式
...
- [ ] 2.2 几何 A 主锚（红-绿）：publish 无码失败 + durable SHUD +
...
- [ ] 2.3 几何 B 主锚（红-绿）：同底 + 带候选身份字段的 auto-retry event
...
- [ ] 2.4 对照组：同底无任何历史码 ⇒ `retry_downstream`（如已有等价既有
      测试则指认引用，不重写）。
- [ ] 2.5 #1313 anchors 零改动全绿：`uv run pytest -q
...
- [ ] 2.6 文本面与占位域边界（I6/I7）：几何 A 的 evidence/reason_code 文本
...
- [ ] 3.1 `uv run pytest -q tests/test_production_scheduler.py -k
      "downstream or permanent or resume or code_recorded"` 通过。
- [ ] 3.2 `uv run ruff check .` 通过。
- [ ] 3.3 `openspec validate downstream-resume-code-scope --strict
      --no-interactive` 通过。
- [ ] 3.4 events 裁决复核（结论已闭合于 D2 第 4 点，fixture review P2-6
