...
- **THEN** the persisted event and the 503 body carry `[object-uri]`/`[uri]` for that entry with `present is True`, never `null`, the same applies to `rejected[].value` and `db_free_runtime.resolved.*.value`, and the raw URI text appears nowhere in the public event or the response
