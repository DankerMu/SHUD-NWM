...
- **资格主体只认 canonical 条目(review round 1 修)**:`included_files` 按 `(role, relative_path)` 排序,`CALIB/*.cfg.ic` 排在 canonical `runtime_input` 条目**之前**——"取首个 `*.cfg.ic`" 会拿错 sha256 / 被 0 字节 CALIB 占位误判。判据改为:manifest 带 `shud_input_name` 时取 `<shud_input_name>.cfg.ic`,否则取顶层(无 `/`)条目;`role` 不参与(planned 写 `shud_input`、written 写 `runtime_input`,不是稳定身份)。inventory 内出现 **>1 个** `*.cfg.ic`(任意层级)= `packaged_initial_condition_ambiguous` → block:runtime 是递归搜索 + exactly-one,gate 提前拦下才对称(0 字节 CALIB 占位在 runtime 侧会被 non_empty 过滤,但 gate 保守 block 是 fail-closed 方向的有意取舍——docstring 不得再声称相反理由)。
- **两级资格来源(review round 2 修,D1 生产拓扑)**:生产 registry 当前 36/36 是 direct-grid variant 行,`resource_profile.manifest_uri` 指向的 variant manifest **只有 `direct_grid_forcing` 一个键、无 `included_files`/`shud_input_name`**——只认 inventory 会把全部生产行判 UNQUALIFIED(bootstrap 生产不可达、审计全假阴性)。资格来源改为两级:(i) manifest 带 `included_files` → inventory 判据(basins 形态,首选,零对象 IO);(ii) manifest 读取成功但**无 `included_files`**(dg-variant 形态)→ 以 registry 行 `resource_profile.shud_input_name` + `model_package_uri` 推导 canonical IC 对象 `{model_package_uri}{shud_input_name}.cfg.ic`,做**有界 no-follow stat + sha256 探针**(存在且非空 → QUALIFIED 携实测 sha256;缺失/0 字节 → UNQUALIFIED;读失败 → UNREADABLE;行内缺 `shud_input_name`/`model_package_uri` → UNQUALIFIED 带独立 reason)。此形态歧义检查不可行(无 inventory 可枚举),由 runtime exactly-one(C1 修后恢复可靠)兜底,记入 limits。仅首时次路径新增这一次对象读,warm 主路径仍零对象 IO。审计工具复用同一两级判据。
- 内容级校验(header 可解析、非全零)下沉到 runtime staging fail-closed。
...
- **重复 prepare 必须收敛(review round 1 修,round 2 改法)**:round-1 的"恰好两个同级候选才删"启发式被 round-2 证伪——它无法区分"上次 materialize 残留"与"包本来就带两个顶层 `*.cfg.ic`",会删 canonical、消费 stray(C1)。改为**staging 前清场**:manifest 声明 packaged IC 时,在 `_stage_artifact(model_package_uri, …)` 之前把 staged model input 下所有 `*.cfg.ic`(no-follow)清掉,候选集恒等于**本次 staging 的产物**——幂等对一切包形态成立(含子目录 IC,C2),真歧义包(两个顶层 IC)恢复 exactly-one fail-closed,`_packaged_ic_candidates` 启发式整个删除。不变量(repeated-class 审计):packaged 分支的候选集必须恒等于本次 staging 产物,禁止事后猜测。
