...
- **E5** **live 双 tick 验收（本变更的核心证据）**：node-27 实机跑第一个 tick，
  88 个 run 各被挡一次、写入 88 行 decline、汇总列出、`rc=0`；第二个 tick
  这些 run 不再进入 pending、无新 handoff 尝试、`rc=0`。两次 tick 的 JSON 汇总
  与 `SELECT count(*) FROM ops.ingest_recompute_decline` 结果留档。
  **必须按 status 分别核对**：60 个 `published` 与 28 个 `succeeded` 都要被抑制；
...
- [ ] T1 迁移 `db/migrations/000055_ops_ingest_recompute_decline.sql`：按 D1 建表
      （三列 NOT NULL 主键、`product_mtime DOUBLE PRECISION`、`IF NOT EXISTS`）。
- [ ] T2 `_process_run`：forcing 失败分支识别 `HANDOFF_APPLY_COMPRESSED_CHUNK_BLOCKED`
      reason code，写 decline（`ON CONFLICT DO NOTHING`）并返回 `outcome="declined"`；
      键不全或写入异常时保持 `outcome="failed"`（D2 fail-closed）。
- [ ] T3 `_already_ingested_runs`：单次 `WHERE run_id = ANY(%s)` 取回 decline 行，
      只对这些 run 读 manifest/mtime，三分量全等者并入返回集（与 `retired` 并列）。
      **`_ingested_run_is_current` 不改动**（D2/D3）。
- [ ] T4 （已取消）初版的 `_ingested_run_is_current` 上提 + 单点判定被 fixture 审查
      P1 推翻——它对 28 个 `status='succeeded'` 的 run 是死代码。见 design.md D2。
- [ ] T5 `main`：汇总新增 `runs.declined`、`runs.declined_runs`、`declines_active`。
      **不要**给 `rc` / `publish_eligible` / `stats_guard` 加冗余排除条件——
      它们已按字符串精确匹配，新 outcome 天然不落入（D5）。
- [ ] T6 单测：declined 分流、瞬态仍 rc=1、两条 fail-closed 路径（写入侧键不全、
      写入抛异常）、读取侧键不匹配不抑制、计数器不污染、`declines_active` 出现在
      汇总且等于表行数、object-store 读取次数与 decline 行数同阶
      （E1/E3/E6/E7/E10）。
- [ ] T7 真实 DB 测试：decline 命中 → 不进 pending；产物 mtime 变新 → 重开（E2）。
- [ ] T8 runbook：`docs/runbooks/tier-node27-timeseries-storage.md` §4.1 之后新增
      压缩前置检查清单（可执行 SQL）；§4.3 解压小节补一句部分解压会打开 parse
      侧守卫路径（D6/D7）。
- [ ] T9 本地验证：ruff + 单测 + openspec validate（两个 change 各自 strict）。
