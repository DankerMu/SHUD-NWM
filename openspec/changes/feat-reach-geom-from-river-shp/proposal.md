...
- **BREAKING（数据语义）**：`core.river_segment` 行粒度从 segment 改为 reach；`river_segment_id` ID 命名规范从 `<model>_seg_<segment_order>_ord_<iRiv>_rec_<iEle>` 简化为 `<model>_reach_<iRiv:06d>`（zero-padded，与既有 `_shud_riv_<index:06d>` 风格对齐，见 [basins_registry_import.py:780](../../../workers/model_registry/basins_registry_import.py:780)）。前端 / API 的 segment 级查询语义变更。
