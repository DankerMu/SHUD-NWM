...
`git ls-files | grep -ci subagent` = **0**。准确说法分两层，不要混：`.gitignore:90` 一直忽略整个 `.claude/`，
`.claude/skills/**` **从未被跟踪过**（`git log --all -- '.claude/skills/**'` 零 commit）；
而同一套 skill 的 `.agents/skills/` 副本，由 `002ba4b59`（2026-09-14，"chore(agents): untrack installed
skill copies under .agents/skills*"）**有意 untrack**，理由是每次装包都会产生永久工作树漂移。
该 commit 本身零个 `.claude/` 路径。

**机制上并非不可能**：`git ls-files '.claude/*'` 返回两个 force-add 的文件
（`.claude/hooks/large-file-guard/{large-file-guard,test-large-file-guard}.sh`，本 PR 改了后者），
所以 force-add 是本仓现成手段。降范围的理由不是做不到，而是**没有授权**：force-add
`.claude/skills/subagent-workflow/scripts/*` 是**首次**把它们纳入跟踪，而那与 `002ba4b59` 对姊妹副本
所作判断正相反——同一份脚本、同一个装包覆盖问题。反转一条四天前的治理决定，#2261 没有提出，
用户对本批的预授权是 merge 而不是治理翻案。按仓规，范围外发现**报告，不修**。
...
（1 结构守卫 / 3 `cmd_close` 词表 / 4 `record-round` 挂钩 / 5 `references/*.md` 口径），
**1 条已交付**（2 的 JSON 一次性修复），**1 条以不同实现面满足**（6 的「测试通过 + ruff 干净」——
由新增的仓库侧守卫承担，而非原指的未跟踪 CLI 单测）。不要把第 6 条算进「推回上游」：
那会把「守卫换了位置」说成「守卫缺席」，而位置变化恰恰是需要被评审的实质。
...
**round-trip 逐字节相同**，所以修复 diff 只含 **16 处**真实改动、不含重排噪声：删 2 个裸顶层 key、
改 13 条词表外的 `outcome: "closed"`、把 `issues.1660.gateEntries` 从 0 改为 1。13 条已逐条用 `gh pr view <pr> --json state` 核对，
...
（六条验收里 4 条被 D1 推回上游），而该字段的用途正是给后继 PR 的 ceiling 记忆。已实测七个 issue 当前
...
- **#2105**：选「在 `frontend-build` 里追加 step」而非独立 job——复用同一次 checkout/install，
  ...
  因为 `apps/frontend/playwright.config.ts` 的 `webServer.command` 硬编码 `corepack pnpm dev`。不加 `retries`。**知情接受的成本**：`frontend` filter 含 `openapi/**`，
...
- **#2211**：逐条补显式字面量 target，不改宽规则——该文件每条规则都是显式 tuple，
  ...
  **issue 的核心前提被实测推翻**：它假定 `tests/test_precip_overlay.py` 是缺失的行为 oracle，
  ...
  两处**既有事实**决定了改法：(a) 既有 requirement「the precipitation application-composition owners MUST select the precipitation surface oracles」
  （`openspec/specs/ci-contract-baseline/spec.md`）已按名钉死
  ...
  (b) exact-set 锚**已经存在**（`tests/test_select_ci_tests.py::test_select_tests_maps_openapi_patch_owner_to_drift_plus_api_consumers`，8 元素含
  `WRITE_SURFACE_SCAN_PATH` 与 `FAMILY_GUARD_PATH`），所以是**更新**它，不是再加一条——
  同一路径两条 exact-set 锚正是批次 8 第 3 轮的重复论证块形状。
- **#1827**：与 `tests/test_select_ci_tests.py::test_demote_helper_rule_selects_public_chain_consumer_exactly`
  同构的 exact-set 全等断言。
