...
| Route identity propagation | For run-scoped hydro and flood MVT, the ready run's concrete `basin_version_id` and `river_network_version_id` must bind metadata `source_refs`, source-version/cache identity, preflight existence checks, live SQL source rows, and frontend run matching. Sibling rows that share `run_id`/variable/duration/valid_time but not the selected run identity must be rejected or excluded before cache write/read semantics can claim the selected run identity. | API and SQL-shape tests seed sibling basin/network rows for the same route identity and prove canonical `.pbf` routes only return/cache rows matching the selected ready run identity; metadata/cache source version tests prove identity changes recreate sources and cannot serve stale sibling tiles. |
| SQL and index resource boundaries | Tile SQL uses bounded tile-envelope CTEs before reusable geometry/budget aggregates; metadata valid-time discovery, latest-ready-run discovery, and tile identity checks use indexes whose leading columns match public route identity or ready-run predicates. | SQL-shape tests; migration tests for hydro identity, latest-ready-run discovery, river-network identity, and valid-time discovery indexes; deterministic/live evidence records plan or SQL-shape hashes. |
