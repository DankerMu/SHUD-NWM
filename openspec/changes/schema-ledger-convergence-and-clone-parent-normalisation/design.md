...
- **Production sequencing against the role audit.** The roles script is also the mandatory audit-only check before every superuser write (`docs/runbooks/tier-node27-timeseries-storage.md` §9.6 "run the audit-only invocation BEFORE every superuser-write session", and `production-ops-readiness`). With this change's SQL, that strict audit would flag `jsonb_typeof` as untrusted while the `flood` CHECKs still exist, which is before `000064` has run. So every pre-write audit while `flood` exists reads a **pinned** source, `git show e340dbc10:db/roles/node27_write_roles.sql`: master's SQL, which still trusts `jsonb_typeof`. It does not depend on where node-27's checkout is, so it also works after a later deploy has pulled past this merge, and after an aborted or rolled-back window. The order in the window is: stop timers → audit-only (pinned source) → `git pull --ff-only` → migrate → full `scripts/node27_provision_write_roles.sh` with a clean strict audit (new SQL, `flood` gone) → restart timers. A pre-merge rehearsal against production confirmed the premise: the pinned source is clean, and the new SQL flags exactly `jsonb_typeof` (tasks §5).
...
- **Backup and rollback (production):** before the apply, `pg_dump -n flood` (schema + data, custom format) into `/home/nwm/tmp/m/`. Rollback is `pg_restore` of that dump plus `DELETE FROM public.schema_migrations WHERE version = '000064_…'`. **What that restores:** the three tables with their columns, constraints, indexes and (zero) rows, as plain tables. `pg_dump -n flood` carries neither TimescaleDB's catalog registration nor chunks, so `return_period_result` comes back as an ordinary table, not a hypertable. For a retired, empty, unread table that is enough; it is recorded so that nobody expects the hypertable back. Rolling back `000062` is impossible (and unnecessary: on production it is a no-op). `000063` has no meaningful rollback, since the old predicates are the defect.
