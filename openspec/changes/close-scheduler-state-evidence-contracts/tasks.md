...
- [ ] 5.3 Push the frozen implementation head, then on node-27 perform the clean-tree/`git pull --ff-only` guard and run the issue #1572 real-DB oracle as `NHMS_RUN_INTEGRATION=1 NHMS_INTEGRATION_DATABASE_URL=<node-27 test DSN> uv run pytest -q -m integration tests/ -k "candidate_state and (truncat or job_limit or stage_attempt)"`. The selected test MUST be the real-repository test from 1.3 (live `integration_database_url`, real `PsycopgOrchestratorRepository`, real `ops.pipeline_job` inserts) and MUST NOT be satisfied by existing local `CapturingRepository`/SQL-stub candidate-state tests. Record exact pushed SHA, selected test node id, command, totals, stage attempts, and results; do not connect to node-22 archived PostgreSQL.
