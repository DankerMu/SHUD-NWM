...
| Legacy compatibility / examples | selected | `systemd.err` retirement covers the retention unit (#1712) and the governance unit (#1765); the other 6 `append:` siblings stay byte-identical; `off` keeps working; DSN `connect_timeout` precedence preserved |
...
- [x] T7 (#1647) `_connect`: `connect_timeout=10` + `options="-c statement_timeout=600000"` defaults, Python-caller override, DSN `connect_timeout` precedence (kwarg omitted when the DSN carries it), stats guard keeps its own; tests for the `_connect` kwarg rows (defaults, guard budget, Python override, DSN precedence, `application_name` merge unchanged) and for every cancellation shape by call path: per-run (ingest apply and `_record_recompute_decline`), display-ready seed (`_ensure_seeded_basin_display_ready` → `seed_failed`, summary emitted, rc≠0), pre-loop/publish (`_basin_seeded`, `_already_ingested_runs`, `_publish_display_runs`, and `_seed_basin`'s direct `_backfill_output_geometry`/`_activate_model` calls — propagate, no summary), the decline count, and both guard levels (fake backend raising `QueryCanceled`).
...
- [x] T9 node-27 receipts, pre-merge half (queued session, receipts on d1529894, 2026-09-02 23:35Z, `/home/nwm/tmp/receipts-2328/b23{a,b,c,d}`; quoted in the PR receipt comment): (a) pre-merge: forced refused retention tick through the wrapper from a detached worktree with scratch receipt/lock -> wrapper `RC` = runner rc, `retention.log` complete (brackets + diagnostic line), wrapper stderr carries the line; post-merge (deployed unit): `journalctl --user -u … -n 30` shows the diagnostic line and the received (redacted) `OnFailure` mail body carries the `RETENTION_` code; `systemd-analyze --user verify` on the alert **instances** `nhms-node27-unit-failure-alert@nhms-node27-timeseries-retention.service.service` and `…@nhms-node27-resource-governance.service.service`; confirm the deployed checkout contains `58d69970` (#1766 AC1) and record it in the PR comment; (b) governance run with `--root-free-critical-bytes` raised above current free -> exit 1 and the received (redacted) mail body quoted (`sendmail exit 0` is not evidence); (c) one autopipe dry tick from the worktree: receipt stats-guard section unchanged and the tick's longest statement (receipt timings / `pg_stat_activity`) recorded against the 600 s budget; (d) with `mkdir -p /home/nwm/tmp; export TMPDIR=/home/nwm/tmp`, run a pytest subset (not the 40-minute full suite on the production host — recorded deviation from #1765 AC1; the `pytest-of-nwm` landing assertion is the property the AC tests) and assert `pytest-of-nwm` exists under `/home/nwm/tmp` and not under `/tmp`, plus `df -h /` before/after. Measured: (a) wrapper `RC=2` = runner `rc=2`, `RETENTION_CONFIG_INVALID` in wrapper stderr and in `retention.log` (start/diagnostic/done); (b) `--root-free-critical-bytes` = free + 1 GiB → exit 1 with `RESOURCE_GOVERNANCE_CRITICAL:ROOT_FREE_BELOW_CRITICAL` (the same read-only audit also reports production's real `DATABASE_SIZE_ABOVE_CRITICAL` / `HOME_FREE_BELOW_WARNING` — recorded, out of scope); (c) `--limit 0` tick rc=0, `stats_guard.status=not_triggered reason=no_run_ingested`, whole tick 9.37 s wall (`/usr/bin/time`), `pg_stat_activity` `nhms-autopipe%` empty mid-tick and after — longest statement ≤ 9.37 s against the 600 s budget; (d) `TMPDIR=/home/nwm/tmp` pytest subset 215 passed 1 skipped, `pytest-of-nwm` under `/home/nwm/tmp`, `/` 14G free before and after.
- [ ] T9 post-merge half (deployed unit, same session after merge): unit re-install + `daemon-reload`; `systemd-analyze --user verify` on the two alert instances; forced refusal on the deployed retention unit → `journalctl --user -u … -n 30` shows the diagnostic line and the received (redacted) `OnFailure` mail body carries the `RETENTION_` code; governance mail body quoted; profile `TMPDIR`; old `/tmp/pytest-of-nwm` removed; confirm the deployed checkout contains `58d69970`. Recorded in the post-merge PR.
- [x] T10 `.gitignore` gains `.worktrees/`.
- [x] T11 `scripts/select_ci_tests.py`: routing rows so the PR lane runs the right suites — `infra/systemd/nhms-node27-timeseries-retention.service` → `tests/test_node27_timeseries_retention.py` (exact; the `.timer` row stays wider), `scripts/node27_timeseries_compression.py` and `scripts/node27_autopipeline.py` → `tests/test_node27_autopipeline_connection_bounds.py` (subset), `pyproject.toml` → `tests/test_python_environment_truth.py` (`.python-version` was already routed there by #1571 and is untouched); pinned by `tests/test_select_ci_tests.py` (`:216-225`, `:243-249`, `:257-277`, and `:904-909` for the autopipeline → connection-bounds row).

## Non-goals (explicit)

No change to `lock_timeout`, timers, H5 semantics, the 6 untouched `append:` sibling units, receipt schemas, or any other script's connection timeouts. `RETENTION_CONCURRENT_INVOCATION` / `RETENTION_UNCAUGHT_ERROR` are not counted by the progress criterion (reasons in design Non-Goals).
