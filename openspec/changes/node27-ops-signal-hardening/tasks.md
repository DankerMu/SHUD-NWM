...
- [x] T11 `scripts/select_ci_tests.py`: routing rows so the PR lane runs the right suites — `infra/systemd/nhms-node27-timeseries-retention.service` → `tests/test_node27_timeseries_retention.py` (exact; the `.timer` row stays wider), `scripts/node27_timeseries_compression.py` and `scripts/node27_autopipeline.py` → `tests/test_node27_autopipeline_connection_bounds.py` (subset), `pyproject.toml` and `.python-version` → `tests/test_python_environment_truth.py`; pinned by `tests/test_select_ci_tests.py` (`:216-224`, `:242-249`, `:256-275`).
