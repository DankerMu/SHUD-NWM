...
Evidence floor:
- uv run ruff check .
- uv run pytest -q tests/test_node27_timeseries_retention.py tests/test_node27_resource_governance.py tests/test_node27_timeseries_compression.py tests/test_node27_autopipeline_stats_guard.py (plus the autopipeline handoff suite)
...
| PostGIS / TimescaleDB 域行为 | selected | ANALYZE identifier quoting on chunks; statement_timeout now spans ingest/backfill write statements (600 s) as well as catalog reads |
...
- [x] T8 (#1647) `qualified_chunk` fail-closed regex (module constant, pattern byte-identical to `_STATS_GUARD_IDENT_RE`, identity test) + malformed-name test; stats-guard falsy set + parametrized test; runbook accepted values.
- [ ] T9 node-27 receipts (queued session): (a) pre-merge: forced refused retention tick through the wrapper from a detached worktree with scratch receipt/lock -> wrapper `RC` = runner rc, `retention.log` complete (brackets + diagnostic line), wrapper stderr carries the line; post-merge (deployed unit): `journalctl --user -u … -n 30` shows the diagnostic line and the `OnFailure` mail body carries the `RETENTION_` code; (b) governance run with `--root-free-critical-bytes` raised above current free -> exit 1 + mail; (c) one autopipe dry tick from the worktree: receipt stats-guard section unchanged and the tick's longest statement (receipt timings / `pg_stat_activity`) recorded against the 600 s budget; (d) with `mkdir -p /home/nwm/tmp; export TMPDIR=/home/nwm/tmp`, run a pytest subset and assert `pytest-of-nwm` exists under `/home/nwm/tmp` and not under `/tmp`, plus `df -h /` before/after. Post-merge: unit re-install + `daemon-reload`, profile `TMPDIR`.
