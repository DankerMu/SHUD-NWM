...
The legacy q_down call site is **not** removed by this requirement and remains live in `_publish_qdown_from_database`, still producing the same summary under `lineage["precip_mirror"]` and still covered by its own tests. The publish entry point raises before it connects when no database URL is configured, so on the DB-free profile that call site mirrors nothing. Its summary is not frozen by this requirement: it gains `missing_object_key` exactly as the new caller's does, since both read the same private implementation — the `missing_object_key` payload addition this requirement describes reaches publish lineage too. Removing the call site is tracked as #2068, blocked on evidence from the new seam. Until then both call sites delegate to the same private implementation, and any change to that implementation MUST keep both callers' contracts.
