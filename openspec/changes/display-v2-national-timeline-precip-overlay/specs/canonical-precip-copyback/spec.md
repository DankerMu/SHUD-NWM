...
The mirror MUST NOT fail, block, or roll back the `convert` stage for **any** reason ("block" here carries the sense it has always had in this requirement: the stage MUST proceed regardless of the mirror's outcome. It is a bound on control flow, not on wall time — see the **Known limit** on wall time recorded in this requirement) — missing source products, unsafe entry names, symlinks, tree/byte/depth limits, IO errors, or a failed rollback. Every such failure is swallowed and recorded in the mirror receipt under the top-level key `precip_mirror` as `{"status": "failed", ...}`, carrying `missing_path` when the cause is an absent source directory and `error`/`error_type` otherwise. On success the key records `{"status": "ok", "file_count": <files mirrored across all trees>, "trees": [...]}`; when every tree was skipped it records `{"status": "skipped", ...}`.

**Known limit — the mirror is bounded in size, not in time.** `_COPYBACK_MAX_FILES`, `_COPYBACK_MAX_DIRECTORIES`, `_COPYBACK_MAX_TOTAL_BYTES`, `_COPYBACK_MAX_FILE_BYTES` and `_COPYBACK_MAX_DEPTH` bound how much the mirror may copy; nothing bounds how long it may take, and the two swallows catch raised exceptions rather than elapsed time. On node-22 the copyback root is the shared NFS mount while the journal and object-store roots are local, so a stalled mount blocks the hook. Binding the hook to `convert` moves that exposure from the tail of the chain to its head: what a stall now delays is not one cycle-status write but **every downstream stage of the cycle** — on node-22's `forecast_state_save_qc` terminal contract that is `forcing`, `forecast`, `state_save_qc` (`stages_through` returns no `parse` for that terminal) — and the scheduler pass carrying them. Nothing is lost — the `pipeline_job` terminal status has already landed and the cycle is re-planned on a later pass — but the delay is chain-wide rather than write-local. Tracked as #2070, whose blast radius this requirement widens.

**Ordering (#2070).** Within `_after_cycle_stage_terminal` the mirror SHALL run only after that entry's persisted cycle-status write (`update_forecast_cycle_status` in the `succeeded`, `partially_failed` and failure-tail arms), and it SHALL still run when that write raises (the write's exception propagates unchanged after the mirror returns). A stalled mount therefore can no longer delay the `convert` entry's own cycle-status write; it still delays the stages that follow in the pass. The wall-time known limit above is **retained, not closed**: the copy is synchronous file IO under the copyback batch lock, and abandoning it mid-copy from a timer would leave a promoted-or-not tree whose rollback cannot run under the lock — a worse state than a slow one.
...
A status MUST NOT be written at a point where the corresponding filesystem outcome is not knowable. Therefore, when this step's own rollback raises, **every** tree still at `copied` becomes `rollback_unknown`: the rollback helper collects per-entry errors and raises once at the end, so a tree it did undo is indistinguishable from one it did not, and even a tree whose own rollback raised may be absent from disk. The top-level `rollback_error`/`rollback_error_type` carry the cause. Trees never promoted stay `pending`. `file_count` is present on the `ok` record and on the `trees_already_mirrored` `skipped` record, and MUST be absent from a `failed` record (the same-root `copyback_root_matches_object_store_root` `skipped` record carries none either); the top-level `file_count` is summed after the copy phase, so every `copied` tree contributes the copy helper's own count, while a `skipped` tree contributes its plan-phase count — the only honest number available for it, because nothing was written. `trees` is absent when the failure happened during planning, before the list was attached to the record.

The legacy q_down call site was removed by #2068. `_copyback_canonical_precip` now has exactly one caller, the public `copyback_canonical_precip` entry point this requirement describes, and `PublishResult.lineage` no longer carries a `precip_mirror` key. The `pipeline_event` receipt is therefore the single outlet for this payload.
...
`_after_cycle_stage_terminal` may run more than once for one cycle's `convert` stage — a retried attempt after a `convert` failure, or a later pass resuming the same job through `resume_cycle_stage` — so a cycle MAY carry more than one `canonical_precip_mirror` receipt. The contract is therefore that at least one receipt records the mirror's outcome; a later duplicate MAY record `skipped`, because the mirror is idempotent.
...
The mirror MUST NOT inherit the two gates that guard the sibling run-tree copyback at the same seam, because neither is a property of precipitation products: it MUST run even when the cycle has no active basins (the run-tree copyback early-returns on an empty `run_ids`, but precipitation is a source/cycle-level product), and it MUST run on **every** entry into the terminal-stage hook for the `convert` stage except `reconcile_unverified`, which returns before it — `succeeded` and the failure tail alike. (`partially_failed` is unreachable for `convert`: the hook's partial arm needs an `ArrayAggregation`, and `chain_stage_execution.py` computes one only for `stage.is_array`, which `convert` is not. The "no `succeeded` gate" rule is therefore carried by the failure tail — `failed` (including the `SLURM_JOB_TIMEOUT` poll-timeout arm), `cancelled`, `reservation_lost`, `permanently_failed` — not by `partially_failed`.) The gate is the stage alone: it MUST NOT read `config.terminal_stage`. That predicate encodes the run-tree copyback's chain-terminal semantics, which say nothing about a `convert` product, and it is not the profile fence — `NHMS_OBJECT_STORE_COPYBACK_ROOT` is (the display profile forbids that variable outright, `apps/api/runtime_mode.py`, and the mirror early-returns when it is unset). A terminal entry for any other stage — `forcing`, `forecast`, `parse`, `state_save_qc`, `publish` — MUST NOT mirror; `_stage_should_copyback_run_trees` keeps its own `state_save_qc` + `forecast_state_save_qc` predicate unchanged.

Binding the mirror to `convert` closes the residual **reachability** gap by construction, because `convert` is the stage that **writes** the products (`workers/canonical_converter/converter.py`). Reachability only: the next paragraph states what the mirror does and does not say about product *completeness*. One control-flow shape that never reaches this hook is a `submission_failed` minted in the submit arm (`chain_stage_execution.py` returns from `_record_submission_failure` before the hook call site), or a chain that dies earlier still; there `sbatch` was never accepted, so there is nothing to mirror — no accepted `convert` submission means no `prcp_rate_or_amount` tree under `OBJECT_STORE_ROOT`. Within a pass that runs `convert`, "products exist ⇔ `convert` terminal reached ⇔ hook called" therefore holds — in the weak sense of *exist*, never *complete*. Two shapes fall outside that qualifier, and both are closed by the **chain-exit recovery** below (#2076). (i) A pass entered with `context.restart_stage` downstream of `convert` (`DOWNSTREAM_RESTART_STAGES` in `scheduler_state_types.py`, minted by the scheduler's restart decision) skips `convert` entirely at `_run_cycle_chain`'s `stage_index < start_stage_index` guard, so it reaches no `convert` terminal. (ii) A mirror that recorded `failed` gets no retry from the hook itself.

**Chain-exit recovery (#2076).** On every exit from `_run_cycle_chain` — normal return or raised exception — the orchestrator SHALL run the mirror once more for the cycle when a copyback root is configured, `canonical/<storage_source>/<cycle_token>/prcp_rate_or_amount/` exists as a directory (not a symlink) under the local object-store root, and either (a) the pass's restart index skipped the `convert` stage, or (b) the mirror already ran in this invocation and its last recorded outcome was `failed` (the outcome is recorded before the receipt write, so a failed receipt write does not hide it). It SHALL NOT run when the hook already ran in this invocation with a non-`failed` outcome, when the pass did reach `convert` but the hook never ran (a `convert` that is not yet terminal, or `submission_failed`), or when the local tree is absent (a retention-pruned cycle emits no receipt). The recovery is the same idempotent, fail-open mirror: it never changes the pass's result, never raises, and a raised chain exception propagates unchanged after it. Its receipt is the same `canonical_precip_mirror` event with the same `details` shape; only `message` differs ("Canonical precipitation mirror recovered at chain exit."), so an operator can tell the two triggers apart. The recovery piggy-backs on passes the scheduler already runs for the cycle and adds no per-pass scan of the discovery window; a cycle that receives no further pass after a failed mirror (chain complete, retry exhausted) still relies on the Requirement 2 backfill script. The determination behind this (#2076): before this change no automatic path re-entered the hook for such a cycle except a full-chain pass, which for an already-converted cycle arises on two paths — the raw-manifest-missing repair's `restart_stage = None` re-plan (`scheduler_state_failure.py`), and a cohort in which no basin carries a restart stage (`_restart_stage_from_basins` in `chain_runtime_utils.py`), which runs the full chain and re-enters through `resume_cycle_stage`; neither is routine for an already-converted cycle, and the node-22 receipt `docs/runbooks/receipts/2026-09-06-precip-copyback-backfill.md` §6.1 observed exactly one receipt per source hours after `convert`, consistent with no routine re-entry. Wall-time exposure: a downstream-restart pass now performs one mirror plan (listing/stat of both trees, bytes rewritten only on mismatch) at its chain exit, after that pass's cycle-status writes — the same coverage the pre-#2069 `state_save_qc` binding carried, and bound by the same Known limit. When the copyback tree already matches, such a pass still acquires the copyback batch lock once, lists both trees once and appends one `skipped` (`trees_already_mirrored`) receipt; this is accepted because the predicate MUST NOT read the copyback root or the journal, and downstream-restart passes for one cycle are bounded by the scheduler's retry policy rather than by the discovery window. The local-tree check MUST refuse a symlink at any path component, matching the publisher's own source-tree walk, and a check that raises counts as absent. The #2070 disposition: the ordering defect is closed by the paragraph above; the absence of a wall-time bound remains an accepted known limit; `convert`'s stage timing still includes the mirror's wall time. A `submit_result_ambiguous` is **not** a third residual here, despite being one for the forecast cohort: the submit arm mints it only inside a branch guarded by `is_forecast_cohort_stage(stage)` (`chain_stage_execution.py`), and `convert` is not in `_FORECAST_STAGE_ALIASES`, so that status is unreachable for this stage and no `convert` `sbatch` can be left running behind an ambiguous submit. Besides the chain-exit recovery, a later full-chain re-plan still carries the already-terminal `convert` job back into this hook through `resume_cycle_stage` (`chain_stage_execution.py` skips the poll and, `convert` being non-array, the aggregation branch), and the backfill script of Requirement 2 remains the manual path. In particular, a cycle whose `state_save_qc` never submits successfully on any pass — the hydro terminal stage failing — MUST still carry its precipitation mirror, because the two lifecycles are unrelated. The mirror is also fail-open where the run-tree sibling is fail-closed: a `RunTreeCopybackError` aborts the stage, whereas any mirror failure is swallowed into the receipt. The swallow MUST cover the receipt write itself — `insert_pipeline_event` can raise `FileOrchestrationJournalError`, which is a sibling of `OrchestratorError` rather than a subclass, so a journal fault MUST NOT abort the `convert` stage either.

The mirror describes bytes, not completeness, and on `convert`'s failure tail the two diverge. `convert` writes each forecast lead individually into its final object key (`workers/canonical_converter/converter.py`), with no directory-level atomicity, so a job that dies partway leaves a partial `prcp_rate_or_amount` tree. Worse, the orchestrator never issues a `scancel` of its own on any failure-tail path (the sole `cancel_job` call site under `services/orchestrator/` is the operator entry point in `chain_forecast_control.py`), so an orchestrator-terminal is not a compute-terminal. The sharp case is a poll timeout, which is **not** a distinct status — `timeout` is an event type, never a member of `TERMINAL_JOB_STATUSES` — but a `failed` carrying `error_code == "SLURM_JOB_TIMEOUT"`, minted by `record_cycle_stage_poll_timeout` in `chain_stage_execution.py` while the gateway still reports the job running. There the `sbatch` MAY be writing the very tree the mirror is copying. The other arms differ in mechanism, and not all of them even leave a partial tree: `cancelled` is observed from the gateway, so Slurm already stopped the job and the tree is frozen-incomplete rather than mid-write; `reservation_lost` is minted only on a *confirmed* accounting absence (`reconcile.py`), meaning the `sbatch` never took, so there is nothing written to mirror at all. This is the reason the "no `succeeded` gate" rule above is safe to state, and it MUST NOT be read as a claim that failure-tail products are whole: `ok` means the destination matched the source **at copy time**, never that `convert` succeeded or that the lead set is complete. Completeness is visible only as the receipt's per-tree `file_count`/`byte_count`, and the receipt lives in the scheduler journal on node-22's local disk, so a display-side reader cannot see it and the mirrored tree carries no completeness marker of its own. Two consequences follow. A partial mirror is self-healing at tree granularity but not within the pass: a later full-chain re-plan re-enters this hook through `resume_cycle_stage`, and a later downstream-restart pass runs the chain-exit recovery; either replaces the whole tree whenever any file name or size differs, and it does so only while the cycle is still inside the scheduler's discovery window. And whether a reader may fall back past an incomplete but more recent mirrored cycle is **not** decided here — it is the window resolver's contract in `precipitation-raster-overlay`, which today selects on mirror presence alone. Narrowing this gate to `succeeded` is NOT the remedy: it would contradict the failure-tail rule above, and a tree holding 50 of 56 leads still serves every early-lead window correctly.
...
- **WHEN** the `convert` stage reaches its terminal entry for `gfs` cycle `2026090212` and the source root holds 56 `.nc` files plus one `grid.json`
...
#### Scenario: A failure tail mirrors the truncated tree and says so only through `file_count`
- **WHEN** the `convert` stage reaches a failure-tail terminal (for example a poll timeout, which mints `failed` without cancelling the Slurm job) while the source tree holds only the leads written so far
- **THEN** the copyback root holds exactly those leads and no others
- **AND** the receipt records `precip_mirror.status == "ok"` — the copy itself succeeded — with a `file_count` equal to the truncated count, which is what distinguishes this from a complete cycle
...
#### Scenario: Missing source does not block the `convert` stage
- **WHEN** the source `prcp_rate_or_amount` directory is absent
- **THEN** the `convert` stage still completes
- **AND** the receipt records `precip_mirror.status == "failed"` naming the absent path in `missing_path`

#### Scenario: Mirror failure other than a missing source does not block the `convert` stage
...
- **AND** the `convert` stage still completes and the cycle's copyback products are untouched
...
#### Scenario: Only the `convert` stage triggers the mirror
- **WHEN** the `forcing`, `forecast`, `parse`, `state_save_qc`, or `publish` stage reaches a terminal entry with a copyback root configured
- **THEN** no mirror is attempted and no `canonical_precip_mirror` receipt is emitted for that entry
- **AND** a `convert` terminal entry still mirrors on a deployment whose `config.terminal_stage` is unset or is not `forecast_state_save_qc`

#### Scenario: A cycle whose hydro terminal stage never submits still carries its mirror
- **WHEN** `convert` completes for a cycle and every later `state_save_qc` submission on every pass fails, so the cycle ends `submission_failed`
- **THEN** the cycle still holds at least one `canonical_precip_mirror` receipt recording `status == "ok"` and the mirrored trees are present under the copyback root

#### Scenario: A downstream-restart pass mirrors the cycle at chain exit
- **WHEN** a pass enters `_run_cycle_chain` with `restart_stage == "forecast"` for a cycle whose local `prcp_rate_or_amount` tree exists and whose copyback root lacks it
- **THEN** `convert` is not submitted or resumed, and after the pass's stages the copyback root holds the tree
- **AND** exactly one `canonical_precip_mirror` receipt is appended, with message "Canonical precipitation mirror recovered at chain exit."

#### Scenario: A failed mirror is healed by a later downstream-restart pass
- **WHEN** the `convert` terminal mirror records `failed` (its chain-exit retry in the same pass also fails), and a later pass for the same cycle enters with a downstream `restart_stage` after the fault clears
- **THEN** that later pass's chain-exit recovery leaves the copyback tree identical to the source and records a non-`failed` receipt

#### Scenario: The recovery does not run when there is nothing to recover
- **WHEN** the hook already recorded `ok` or `skipped` in this invocation, or the pass reached `convert` without a terminal entry, or the local `prcp_rate_or_amount` tree is absent, or no copyback root is configured
- **THEN** no chain-exit recovery runs and no extra receipt is appended

#### Scenario: The recovery is fail-open
- **WHEN** the chain-exit recovery's mirror or its receipt write raises
- **THEN** `_run_cycle_chain` returns the same result it would have returned without the recovery, and an exception already propagating from the chain propagates unchanged

#### Scenario: The mirror runs after the entry's cycle-status write
- **WHEN** `convert` reaches a terminal entry (`succeeded` or a failure-tail status) with a copyback root configured
- **THEN** that entry's `update_forecast_cycle_status` call is recorded before the mirror starts
- **AND** if that status write raises, the mirror still runs and the write's exception is what the caller sees
...
> Supersession（原文保留、追加指针）：上段「Every directory the script creates … `0o755`」记录的是出厂形状；change `fix-canonical-mirror-prune-permissions`（#2100）之后，backfill 脚本与 publisher 的 canonical 降水镜像车道对 `canonical/<storage_source>/<cycle>/` 及其拷贝树显式写 `0o2775`（共享组 + setgid），文件仍 `0o644`；`runs/`、`forcing/` 车道保持 `0o755`；publisher 经 traversal helper 创建的上层目录仍落 `0o755`，backfill 只让 `canonical/` 保持 `0o755`、其下各级写 `0o2775`，`canonical/<storage_source>/` 与 `grid/` 由存量 sweep 收敛到 `2775`。当前权威：`openspec/specs/filesystem-permission-determinism/spec.md` 与 `openspec/changes/fix-canonical-mirror-prune-permissions/specs/`。
...
#### Scenario: Standard library only apart from the named copyback-guard import
...
- **THEN** it imports no `services`, no `workers`, and no third-party module, and the only `packages` import is `packages.common.copyback_guard`
- **AND** that module's own transitive import closure is the standard library plus `packages.common.safe_fs`, so the frozen node-22 checkout still needs no environment built
