...
The mirror receipt is a `pipeline_event` on the forecast cycle, `event_type = "canonical_precip_mirror"`, `entity_id` the cycle id, `status_to` the payload's `status`, and `details` exactly `{"precip_mirror": <payload>}` (one level of nesting, so the receipt reads at `.details.precip_mirror`).

This outlet sanitizes filesystem paths, so the payload MUST NOT rely on them. `_public_pipeline_event_payload` renders every key ending `_path`/`_root` and every scalar beginning `/` as `[local-path]`, which erases `root` and `missing_path` and would otherwise make "source `prcp_rate_or_amount` tree absent" and "`canonical/<storage_source>/grid/` absent" byte-identical — the two states Requirement 1 requires be distinguishable. The payload therefore ALSO carries `missing_object_key`, the **relative** object key of the absent directory (`canonical/<storage_source>/<cycle_token>/prcp_rate_or_amount` or `canonical/<storage_source>/grid`), which has no leading `/` and survives sanitization. This is a deliberate payload addition, not a shape-preserving move: `missing_path` remains for the in-process return value and is expected to render as `[local-path]` in the persisted receipt. On the DB-free profile the repository is `FileOrchestrationJournalRepository`, so the event is appended to `<NHMS_SCHEDULER_JOURNAL_ROOT>/journal/<source_id>/<cycle_token>.jsonl`. The payload key naming the cycle MUST be `cycle`, never `cycle_token`: the event passes through `redact_payload()`, whose sensitive-key regex substring-matches `token` and would blank the value.
...
The mirror MUST NOT inherit the two gates that guard the sibling run-tree copyback at the same seam, because neither is a property of precipitation products: it MUST run even when the cycle has no active basins (the run-tree copyback early-returns on an empty `run_ids`, but precipitation is a source/cycle-level product), and it MUST run for **every** terminal status except `reconcile_unverified` (which returns before the hook) — `succeeded`, `partially_failed`, and the failure tail `failed`/`cancelled`/`timeout`/`submission_failed`/`reservation_lost`/`permanently_failed` alike (the products are written by the earlier `convert` stage and are valid regardless of per-basin forecast outcomes; skipping them would 404 the read side for a cycle whose data exists). This closes per-basin outcomes, not stage-level ones: a chain that terminates before `state_save_qc` never reaches this hook and is a known, recorded gap — such a cycle is not mirrored and the read side 404s for it. The mirror is also fail-open where that sibling is fail-closed: a `RunTreeCopybackError` aborts the stage, whereas any mirror failure is swallowed into the receipt. The swallow MUST cover the receipt write itself — `insert_pipeline_event` can raise `FileOrchestrationJournalError`, which is a sibling of `OrchestratorError` rather than a subclass, so a journal fault MUST NOT abort the terminal stage either.
