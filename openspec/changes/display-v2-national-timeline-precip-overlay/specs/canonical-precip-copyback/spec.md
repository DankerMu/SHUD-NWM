...
The legacy q_down call site is **not** removed by this requirement and remains live in `_publish_qdown_from_database`, still producing the same summary under `lineage["precip_mirror"]` and still covered by its own tests. It is unreachable on the production DB-free topology (the publish entry point raises before connecting), so it mirrors nothing there; on a DB-backed profile its behaviour is unchanged by this requirement. Removing it is tracked as #2068 and is blocked on the new seam being proven by a live node-22 receipt. Until then both call sites delegate to the same private implementation, and any change to that implementation MUST keep both callers' contracts.
