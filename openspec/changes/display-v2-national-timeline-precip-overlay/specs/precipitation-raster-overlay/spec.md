...
This requirement is hosted in `precipitation-raster-overlay` because PNGs are part of the envelope, but governs the complete prewarm workload. The I13 fixed lead-window decision is superseded by `isolate-display-cold-generation` (#2121-A / #2346) after #2121-B and the committed 2026-09-17 cold receipt. `scripts/node27_mvt_prewarm.py` SHALL discover each of `gfs` and `ifs` independently from `GET /api/v1/layers/discharge/cycles?source=<source>` and schedule the z3–z4 China discharge tiles for every valid time published by that source's newest cycle, preserving the supplied strings, order and multiplicity. It SHALL schedule the corresponding PNG only when the existing PNG request-shape contract permits it; river-network prewarm remains z3–z5. No fixed lead window or prefix SHALL truncate the published timeline. Full-cycle cost is not inferred from the former limited envelope: a genuine node-27 full-cycle cold receipt SHALL demonstrate the request count, elapsed time, per-request latency, cache growth and absence of discharge failures/deadline skips. The run SHALL retain a total wall-clock deadline, stop issuing work after it expires, report the unissued count, and return nonzero for incomplete work. The deadline plus one per-request timeout SHALL fit within the ingest tick interval, and the planned request count SHALL be asserted from the production grid and URL-building path. The v4 summary SHALL expose `prewarm_scope=full_cycle`, the effective worker count, per-source available/selected time counts, and issued discharge success/failure totals; the old `lead_hours` field is removed. An empty cycle list SHALL contribute zero requests without fabricating a cycle. Discovery failure SHALL remain distinct from an empty source, SHALL preserve the other source's work, and SHALL produce a source error and nonzero exit. Existing PNG reason whitelists, out-of-contract accounting, global failure accounting and always-emitted summaries SHALL remain unchanged. Historical I13 receipts remain evidence of the earlier limited envelope, not proof of this full-cycle requirement.

#### Scenario: Envelope and request count
- **WHEN** prewarm runs and both sources report a newest cycle publishing `W` valid times within the PNG request-shape contract
- **THEN** the request set is unchanged `river-network-national` z3–z5 China tiles plus, per source, `|z3–z4 China tiles| × W` discharge requests and `W` PNG requests
- **AND** the summary reports the total request count and elapsed seconds, and both land in the deployment receipt
- **AND** the summary reports `prewarm_scope=full_cycle`, per-source available/selected time counts and issued outcome buckets; deadline skips remain separately visible

#### Scenario: The whole published timeline is selected without a lead cut
- **WHEN** a source publishes valid times beyond the former twelve-hour window
- **THEN** every published time is scheduled for discharge, in discovery order with the original timestamp strings and multiplicity
- **AND** PNG requestability is classified independently by the existing grid and horizon contract, rather than by dropping those discharge times
- **AND** the wall-clock deadline still bounds execution and exposes any unissued tail instead of reporting it as warmed

#### Scenario: A coverage-clamped cycle still warms the view the frontend opens on
- **WHEN** intersection coverage makes a source's first published valid time later than its cycle instant
- **THEN** every published entry remains selected, including the first, and every warmed URL retains the published `{cycle}` identity
...
- **WHEN** the published valid-time stride or extent, discharge zoom set, river zoom default, or source list changes
- **THEN** an assertion that builds the whole published grid and expands it with the run's tile and URL builders fails unless the planned request count is still the expected one
...
- **AND** these assertions do not prove deadline fit: that requires the node-27 full-cycle cold receipt, not extrapolation from the earlier limited-window receipt
...
- **WHEN** a published discharge valid time is not a 3-hour step from its source cycle, or the cycle itself does not fall on a whole hour
...
- **WHEN** a source publishes a valid time on the 3-hour grid beyond the +168-hour precipitation forecast horizon
