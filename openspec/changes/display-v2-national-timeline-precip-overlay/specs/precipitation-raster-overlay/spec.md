...
- **THEN** only the valid times satisfying `cycle <= valid_time <= cycle + lead window` are warmed, for discharge tiles and precipitation PNGs alike
...
#### Scenario: The run's cost budget is asserted, not asserted-to
- **WHEN** the lead window, the discharge zoom set, the default worker count, the default per-request timeout, or the default deadline changes
- **THEN** an assertion recomputes the worst-case envelope cost from the measured per-tile costs at a stated fraction of the nominal concurrency and fails unless it still fits inside the default deadline
...
- **AND** neither assertion is claimed to establish that the cost model is correct: it is the node-27 deployment receipt that measures the model, and the assertions only prevent a change to the envelope from silently skipping the budget
