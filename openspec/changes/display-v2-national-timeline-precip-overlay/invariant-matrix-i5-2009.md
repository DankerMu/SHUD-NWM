...
15. **Cycle-dimension bound on `national_discharge_cycles` (user decision, review round 1; value and
    ...
    - Index, **as measured on node-27** (receipt `docs/runbooks/receipts/2026-09-05-issue-2009-discharge-cycles-node27.md` §4, `EXPLAIN (ANALYZE, BUFFERS)`, gfs, 12 d):
      the planner picks the partial index `hydro_run_display_ready_candidate_idx` (`db/migrations/000040`,
      `(LOWER(source_id), run_type, basin_version_id, cycle_time DESC, run_id DESC) WHERE cycle_time IS NOT NULL AND status IN (…)`)
      with `Index Cond: lower(source_id) = 'gfs' AND cycle_time >= now() - '12 days'` — the leading
      `LOWER(source_id)` equality is what beats the `cycle_time`-leading `hydro_run_latest_ready_run_idx`
      (`000021`; both indexes carry the same `status IN (…)` partial predicate, so that is not the
      differentiator), and `cycle_time` is applied as an in-index condition on the fourth column
      (800 rows, index-scan node 3.7 ms). The pre-measurement text of this bullet
      named `000021` on the "leading column" argument; that prediction was wrong and is replaced by the
      measurement. What the row needs holds either way: `:since` reaches an `Index Cond`, no seq scan on
      `hydro_run`, no new migration.
...
16. **The intersection compares network *sets*, not cardinalities (review round 2, r2-inv-5).** The
    ...
    **CLOSED, no longer a DEFER:** `fix-national-discharge-coverage-snapshot-order` (#2087) took the
    statement-order swap — the coverage rows are read first and the active set second, so a network
    activated between the reads is in the denominator and in no cycle's numerator, and both branches
    fail closed (row 40d). The swap is a trade, not a pure win: it newly opens the numerator-SHRINK
    class (a covered row that stops being display-ready between the reads), which the pre-swap order
    did catch and which has live writers. That residual, and the reasons the merge / REPEATABLE READ
    alternatives stay rejected, are recorded in that change's `design.md` D2 and in
    `_national_discharge_coverage_rows`' docstring; #2457 later RULED the residual accepted (keep the
    two statements; criteria in `national-discharge-intersection-closure` design D1, row 40f). Slice-pin note (decision 2): the return-type
...
post-gate site rule added 40b/40c and 41–75 (37 rows), and #2087 later added 40d (the read-order row) and 40e (the superset-containment row), so the table has 85 rows with one numbering gap; `national-discharge-intersection-closure` (#2457 #2458 #2459) then added 40f (the #2457 ruling row) and 40g (the no-argument set rule), 87 rows. Each fix pass that changes the oracle mechanism invalidates
...
|36|The lookback predicate actually bounds the **DB scan**|drop `AND (CAST(:since AS timestamptz) IS NULL OR h.cycle_time >= :since)` from the SQL, keep the bind|**node-27 lane, same as row 3.** No local oracle: `_NationalDiscoverySession` filters by bound values and deliberately never by SQL text (`tests/test_hydro_display_mvt_scaling.py:1396-1403`, the #2009 section comment above the fake), so with the bind still passed the fake keeps filtering and nothing about the scan is observable locally. The local signal is a predicate-text assertion in `test_national_cycles_list_only_cycles_inside_the_lookback_window`, recorded below as a **tripwire, not the oracle** (preamble). The node-27 oracle is the integration case `test_national_cycles_skip_a_cycle_older_than_the_lookback`. Row 36b carries the behavioral half; the node-27 receipt (row-count + `EXPLAIN (ANALYZE, BUFFERS)` with `:since` in an `Index Cond`; measured index is `hydro_run_display_ready_candidate_idx`, see decision 15) carries the scan half|`local 371 pass / 1 fail` @ `services/tiles/mvt.py:2047` · node-27: `1 failed, 14 passed` (test_national_cycles_skip_a_cycle_older_than_the_lookback) · receipt §4: `Index Cond` carries `cycle_time >= now() - '12 days'` (the load-bearing evidence); row counts 800 (gfs, 12 d) vs 2250 (no-arg path: unbounded **and** source-less, so not a pure lookback contrast)|
...
|40d|The active-network statement runs AFTER the coverage statement, so the denominator is never the older snapshot (#2087). The set comparison of rows 40/40b is only half the invariant: it can see a mid-flight activation only for the cycles the newcomer already has rows for|restore the pre-#2087 order in `_national_discharge_coverage_rows` — move the `active_networks = (` block and its `active_network_ids` line back ABOVE the `rows = (` block. **Identifiers, not line numbers, deliberately:** this row first cited `:2385` / `:2341`, which the SAME PR's second commit shifted by +21 when it expanded the helper docstring, leaving `:2385` pointing INSIDE the coverage SQL literal — a re-measurer following it literally would have cut mid-SQL and violated this cell's own next clause. SQL text unchanged; order only|Four activation behavior cases driven by `_ActivationBetweenStatementsSession` (order-independent by construction: the membership change lands after the FIRST `execute()`, and the coverage branch filters by the active set of its own execution, which is what `mi.active_flag` does inside the statement) — `test_national_cycles_close_when_a_network_is_activated_with_zero_coverage_rows`, `test_national_per_cycle_valid_times_close_when_a_network_is_activated_with_zero_coverage_rows`, `test_national_cycles_close_both_cycles_when_the_newcomer_covers_only_the_newer_one`, `test_national_per_cycle_valid_times_close_the_older_cycle_the_newcomer_does_not_cover`; the two DIRECT order oracles, `test_national_cycle_coverage_helper_compares_sets_like_the_per_cycle_valid_times` (its `executions` bind sequence, parametrized ×5 since #2459 added `covered-superset`) and `test_canonical_national_tile_serves_a_fully_covered_identity_unchanged` (`_statement_kinds` == `["digest", "coverage", "active", "tile"]`); and the two round-1 additions that pin the swap's OTHER direction — `test_national_cycles_list_a_cycle_when_a_zero_coverage_network_is_deactivated` (the delta D2 records: the new order is more permissive here, so a partial revert that keeps growth closed but restores the old deactivation strictness reds only this one) and `test_national_cycles_still_list_a_cycle_whose_covered_run_stopped_being_display_ready` (characterizes the numerator-SHRINK residual this order newly opens). EIGHT test names, TWELVE node ids (eleven as delivered by #2087; #2459's `covered-superset` parametrize case is the twelfth). Non-vacuity: each activation case asserts a 3-member post-change active set and non-empty served coverage rows|`local 193 pass / 11 fail` @ the `rows = (` / `active_networks = (` pair in `_national_discharge_coverage_rows`. **Baseline note:** measured with `uv run pytest -q tests/test_hydro_display_mvt_scaling.py` at THIS change's baseline — `204 collected`, `204 passed / 0 failed` as delivered (the `:170` order-coupled `session.sql` site IS repaired in the delivered commit, per tasks.md 4b item 4: the statement is selected out of `session.executions` by content, no assertion weakened; that repair makes `:170` order-INDEPENDENT, which is why it is absent from this row's kill list) — not at the `local 372 passed` post-gate baseline the rows above were taken at; the two counts are not comparable and this row is deliberately not back-fitted to the older one **Re-measured by `national-discharge-intersection-closure` (#2457/#2459)** on its PR-head file set (`tests/test_hydro_display_mvt_scaling*.py tests/test_display_publish_status_only.py tests/test_precip_overlay.py tests/test_mvt_national_identity_probe_integration.py tests/test_api_contract.py`, post-split): unmutated `338 passed / 0 failed / 21 skipped`, this mutation `326 pass / 12 fail`, all in `test_hydro_display_mvt_scaling_coverage_order.py`. The numerator-SHRINK trade this order opens is RULED accepted by #2457 (row 40f).|
|40e|The `national_discharge_cycles` comparison is EQUALITY, not superset containment — a covered set that is a strict SUPERSET of the active set must also fail closed (round 1 of #2087). **Reachability, corrected:** the superset state was ALREADY reachable before this PR — the coverage statement carries `mi.active_flag` and evaluates it in its own snapshot, so under the old order a network activated between the reads that already held display-ready rows landed in `covered` and not in `active@T1`. That is exactly how #2073's set comparison caught cycle K. The swap does not create this state; it changes which race routinely produces it, from activation-with-rows to deactivation-with-rows|`services/tiles/mvt.py` `national_discharge_cycles` → `if not covered_networks >= active_networks:`|`test_national_cycles_close_when_a_covered_network_is_deactivated`: a network with coverage rows deactivated between the two reads leaves `covered ⊋ active`, which must still close. Deliberately order-INDEPENDENT (it closes under both statement orders), so row 40d's mutation leaves it green and the two rows do not alias|`local 203 pass / 1 fail` @ the `national_discharge_cycles` site — at #2087 the ONLY case in the file where `covered` is a strict SUPERSET of `active` (it remains the only superset case that reaches `national_discharge_cycles`; #2459 added superset cases for the other two judging sites). Every other set-comparison case is a subset, equal, or INCOMPARABLE (e.g. the `equal-size-mismatch` parametrize case and the `{rn-a, rn-c1, rn-c2}` vs `{rn-b, rn-c1, rn-c2}` fixtures) — on all three of those shapes `>=` and `==` return the same verdict, which is why the mutation survives them; the strict superset is the one shape where they differ. **Site gap, FIXED by #2459** (`national-discharge-intersection-closure`): the twin comparison in `NationalCycleCoverage.complete` (`services/tiles/mvt.py`) had no `covered ⊋ active` oracle, so the same superset mutation survived there (issue #2459 measured `352 passed / 0 failed` on the pre-split file set — history, not comparable). That gap predated #2087 (row 40b's site, from #2073). Now pinned by the `covered-superset` case of `test_national_cycle_coverage_helper_compares_sets_like_the_per_cycle_valid_times` (the helper verdict and the per-cycle valid-times) and `test_canonical_national_tile_refuses_a_covered_superset_of_the_active_set` (route 424 `MVT_NATIONAL_IDENTITY_INCOMPLETE`, counts 3/2, no tile statement). Mutation `complete` → `self.covered_networks >= self.active_networks` on the PR-head post-split set: unmutated `338 passed / 0 failed / 21 skipped`, mutated `336 pass / 2 fail` (exactly those two node ids)|
|40f|#2457 RULING, not a mutation row: the two-statement read (row 40d's order) is KEPT and the numerator-SHRINK residual (a covered run that stops being display-ready between the reads; writers `mark_run_failed`, `node27_refresh_coverage.py --force`, runtime `mark_failed`) is ACCEPTED|n/a — a ruling has no mutant; the merge alternatives (LEFT JOIN, shape-preserving UNION ALL) are costed in `national-discharge-intersection-closure` design D1|`test_national_cycles_still_list_a_cycle_whose_covered_run_stopped_being_display_ready` stays a CHARACTERIZATION (expectation unchanged, docstring points at the ruling). Criteria: frequency unquantifiable from the DB (no run-status transition record); harm same-type and render-safe (the canonical tile route re-reads the pair on a cache miss and answers 424 `MVT_NATIONAL_IDENTITY_INCOMPLETE`), self-healing within `DISPLAY_CATALOG_TTL_SECONDS = 60` / `DISPLAY_CATALOG_STALE_MAX_SECONDS = 600`; cost M-L. Reopen when a transition record shows the class routine, or a consumer renders from the listing without its own coverage re-check|n/a (ruling)|
|40g|The no-argument branch of `national_discharge_valid_times` judges `frozenset(latest_by_network)` against the helper's ACTIVE set by equality (#2458); the denominator is never derived from the rows, and the empty guard before it stays (rows = ∅ ∧ active = ∅ must not reach `max()` of an empty list)|three mutants at the no-argument site in `services/tiles/mvt.py`: (b1) delete the set check; (b2) `!=` → `not (frozenset(latest_by_network) >= active_networks)`; (b3) `len(latest_by_network) != len(active_networks)`|`test_no_argument_national_valid_times_fail_closed_unless_the_covered_set_equals_the_active_set` (`tests/test_hydro_display_mvt_scaling_discovery.py`, parametrized `partial` / `equal-size-mismatch` / `covered-superset`, each `valid_times == []`, `observed_count == 0`; non-vacuity: the same rows judged against their own networks yield a non-empty timeline) + `test_no_argument_national_valid_times_are_empty_with_no_rows_and_no_active_network` (`tests/test_hydro_display_mvt_scaling_catalog.py`, pins the kept empty guard). Regressions unchanged: row 37's `test_no_argument_national_valid_times_keep_a_network_whose_newest_run_predates_the_lookback`, `test_valid_times_route_without_arguments_keeps_serving_the_national_list`|PR-head post-split set, unmutated `338 passed / 0 failed / 21 skipped`: (b1) `335 pass / 3 fail` (all three parametrize ids), (b2) `337 pass / 1 fail` (`covered-superset`), (b3) `337 pass / 1 fail` (`equal-size-mismatch`). node-27 pre-check 2026-09-23: active 48 = covered 48 (production no-op today)|
