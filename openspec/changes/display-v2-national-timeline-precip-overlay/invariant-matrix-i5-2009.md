...
16. **The intersection compares network *sets*, not cardinalities (review round 2, r2-inv-5).** The
    ...
    **CLOSED, no longer a DEFER:** `fix-national-discharge-coverage-snapshot-order` (#2087) took the
    ...
    `_national_discharge_coverage_rows`' docstring. Slice-pin note (decision 2): the return-type
...
post-gate site rule added 40b/40c and 41–75 (37 rows), and #2087 later added 40d (the read-order row) and 40e (the superset-containment row), so the table has 85 rows with one numbering gap. Each fix pass that changes the oracle mechanism invalidates
