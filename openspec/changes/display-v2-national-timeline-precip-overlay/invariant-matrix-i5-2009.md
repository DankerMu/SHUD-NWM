...
post-gate site rule added 40b/40c and 41–75 (37 rows), and #2087 later added 40d (the read-order row), so the table has 84 rows with one numbering gap. Each fix pass that changes the oracle mechanism invalidates
...
|40d|The active-network statement runs AFTER the coverage statement, so the denominator is never the older snapshot (#2087). The set comparison of rows 40/40b is only half the invariant: it can see a mid-flight activation only for the cycles the newcomer already has rows for|restore the pre-#2087 order in `_national_discharge_coverage_rows` — move the `active_networks = (` block (`services/tiles/mvt.py:2385`) and its `active_network_ids` line back ABOVE `rows = (` (`services/tiles/mvt.py:2341`). SQL text unchanged; order only|Four behavior cases driven by `_ActivationBetweenStatementsSession` (order-independent by construction: the activation lands after the FIRST `execute()`, and the coverage branch filters by the active set of its own execution, which is what `mi.active_flag` does inside the statement) — `test_national_cycles_close_when_a_network_is_activated_with_zero_coverage_rows`, `test_national_per_cycle_valid_times_close_when_a_network_is_activated_with_zero_coverage_rows`, `test_national_cycles_close_both_cycles_when_the_newcomer_covers_only_the_newer_one`, `test_national_per_cycle_valid_times_close_the_older_cycle_the_newcomer_does_not_cover`; plus the two DIRECT order oracles, `test_national_cycle_coverage_helper_compares_sets_like_the_per_cycle_valid_times` (its `executions` bind sequence, parametrized ×4) and `test_canonical_national_tile_serves_a_fully_covered_identity_unchanged` (`_statement_kinds` == `["digest", "coverage", "active", "tile"]`). Non-vacuity: each behavior case asserts a 3-member post-activation active set and non-empty served coverage rows|`local 192 pass / 9 fail` @ `services/tiles/mvt.py:2341` (5 assertions, 9 node ids: the `executions` oracle is parametrized ×4). **Baseline note:** measured with `uv run pytest -q tests/test_hydro_display_mvt_scaling.py` at THIS change's baseline — `201 collected` in that file (`200 passed / 1 failed` as delivered: the `:170` order-coupled `session.sql` finding is left unrepaired per the fixture's oracle-integrity rule, `201 passed` once it is repaired) — not at the `local 372 passed` post-gate baseline the rows above were taken at; the two counts are not comparable and this row is deliberately not back-fitted to the older one|
