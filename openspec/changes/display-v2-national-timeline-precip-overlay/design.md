...
- **实测结论（node-27，516 张 × v2/v3）**：v3 让每一级各有 1 张瓦片的 `coordinate_count` 超 50000（z3 86160 / z4 52458 / z5 53432 / z6 56354 / z7 52381），而单要素最大只有 19528、overflow 全 0——spec 原来的两条判据会全判 Go，生产却 413。提高 simplify 容差在 z3/z4/z6 已饱和（z3 2000m→6000m 只降 14 个坐标，低 zoom 的坐标数由段数而非点密度支配），只有 z7 有效。按「该级回退一档」处理则五级全回退、shipped 表等于 v2，本 change 的密度目标归零。
- **因此新增公平集合预算窗口**：`river-network-national` 照搬 `hydro-national` 的 `national_budget_window`（按 network 分区、`"Type" DESC` 交错排名、累计坐标封顶），超预算瓦片砍最细支流而不是 413；并把集合上限按图层化，`river-network-national` = 120000、其余图层保持 `MVT_MAX_COORDINATES` = 50000，单要素上限所有图层不变。不调全局常量：同一个常量同时是五个图层的单要素守卫与集合守卫，全局调大会顺带放宽 `hydro-national` 的公平预算窗口与单流域层的截断行为。热点瓦片在 120000 下拿满 v3 全密度（z3 86160 坐标 / 260 KB，冷生成 2.34 s，`MVT_MAX_BYTES` 5 MB 远未触及），且窗口保证再稠密也只会降级不会失败。三处落地细节不可省：窗口建在现有 `eligible` 过滤之上（改名 `preeligible`），`network_rank` 的 ROW_NUMBER 与窗口的两个 ORDER BY 三处都以 `river_segment_id` 收尾（z≥9 逐段分支上 `"Type"` 海量并列，缺唯一键截断点会随执行计划漂移，而结果按缓存代落盘），单流域层不加窗口。窗口落地后「集合超预算」恒不成立，于是 go/no-go 每张瓦片跑两遍绑定（生产 120000 与 1e9），Go 追加一条「两遍 `coordinate_count` 相等」，否则无法区分「拿满全密度」与「被窗口砍掉支流」。
