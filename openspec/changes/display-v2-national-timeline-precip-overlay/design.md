...
- **Superseded by #2128（2026-09-16）**：底部控制条改为距地图底部 40px（高度仍 64px），保留版权署名带；图例/notice 及错误边界 fallback 同步上移 24px。以下旧偏移仅记录 #2014/#2015 当时方案，现行几何与浏览器红绿证据以 `../display-followup-layout-catalog-contracts/design.md`、`tasks.md` 及本 change 已同步的 layout spec delta 为准，不再用 CSS 类名断言替代实际矩形验收。
