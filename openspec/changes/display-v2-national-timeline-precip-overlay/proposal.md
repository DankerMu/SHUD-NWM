...
- **BREAKING（目录契约）**：`/api/v1/layers` 的 `discharge` 条目 `tile_url_template` 改为带 `{source}/{cycle}` 的新全国路由 `/api/v1/tiles/hydro-national/{source}/{cycle}/{variable}/{valid_time}/{z}/{x}/{y}.pbf`；旧无源路由保留为别名、行为不变。
- 新增 `GET /api/v1/layers/discharge/cycles?source=`（各活动河网交集，fail-closed）；`valid-times` 支持 `source`+`cycle` 查询，返回从起报时刻起 3h 步长的全范围列表。
...
- 预热脚本扩展到 z3–4 × 双源 × 最新周期全部时次 + 降水 PNG。
...
- 测试：`tests/test_hydro_display_mvt_scaling.py:175`、`tests/test_api_contract.py:1409`、`M11Shell.test.tsx` fixture、新增 precip/copyback/cycles 测试与 vitest。
