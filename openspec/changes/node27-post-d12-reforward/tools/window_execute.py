...
Post-D12 re-forward (retained narrow table, ledger already holds 000059; fresh state only):
  window_execute.py reprepare --state /home/nwm/.local/state/NEW-DIR --config PRIVATE.json
  window_execute.py reforward --state ... --go Danker
  Config adds reforward: {provenance_state: prior D12 state dir,
    provenance: {state_sha256, routes_sha256}} and reads.retained.request for one retained run.
  The prior state is read-only provenance, never resumed. Retained facts are reattached, not re-expanded.
...
class Executor:
    def __init__(self, args):
        ...
        if args.command in ("prepare", "reprepare"):
        ...
        if args.command in ("window", "reforward", "recover") and self.s:
            require(
    ...
    def prepare(self):
        reforward = self.args.command == "reprepare"
        require(os.getuid() == 1005, "NODE27_USER_REQUIRED")
        require(("reforward" in self.c) == reforward, "PREPARE_MODE_CONFIG_MISMATCH")
        require(
        ...
        for kind in ("legacy", "narrow", "retained") if reforward else ("legacy", "narrow"):
        ...
        if reforward:
            ...
            admitted = dict(old_oid=cat[0]["oid"], ledger_before=ledger)
        self.save(
            old_branch=old_branch,
            units=units,
            files=files,
            mode="reforward" if reforward else "initial",
            **admitted,
        ...
        self.phase("REPREPARED" if reforward else "PREPARED")

    def provenance(self):
        """Hash-pinned prior D12 evidence: prior directory at reprepare, own copies afterwards."""
        spec = self.c["reforward"]
        pins = spec["provenance"]
        if self.args.command == "reprepare":
            prior = Path(spec["provenance_state"])
            require(
                prior.is_absolute()
                and not any(p.is_symlink() for p in (prior, *prior.parents))
                and prior.resolve() != self.root.resolve(),
                "PROVENANCE_STATE_INVALID",
            )
            # Read-only descriptor: never create, truncate or take over the prior executor lock.
            lock = os.open(prior / "executor.lock", os.O_RDONLY | os.O_NOFOLLOW)
            try:
                try:
                    fcntl.flock(lock, fcntl.LOCK_SH | fcntl.LOCK_NB)
                except BlockingIOError:
                    raise Refusal("PROVENANCE_EXECUTOR_RUNNING")
                state_raw = safe_path(prior / "state.json", True).read_bytes()
                routes_raw = safe_path(prior / "narrow-routes-before-reverse.json", True).read_bytes()
            finally:
                os.close(lock)
        else:
            state_raw = safe_path(self.root / "provenance-state.json", True).read_bytes()
            routes_raw = safe_path(self.root / "provenance-routes.json", True).read_bytes()
        require(
            digest(state_raw) == pins["state_sha256"] and digest(routes_raw) == pins["routes_sha256"],
            "PROVENANCE_HASH_MISMATCH",
        )
        prior_state, routes = json.loads(state_raw), json.loads(routes_raw)
        oids = (prior_state.get("old_oid"), prior_state.get("narrow_oid"))
        require(
            prior_state.get("phase") in ("RECOVERED_OLD_RETAINED", "BLOCKED_FENCED")
            and all(type(x) is int and 0 < x <= 4294967295 for x in oids)
            and oids[0] != oids[1]
            and isinstance(prior_state.get("ledger_before"), list)
            and isinstance(routes, list),
            "PROVENANCE_NOT_RETAINED_D12",
        )
        return prior_state, routes, state_raw, routes_raw

    def readmission(self):
        """Read-only proof that live state is the retained D12 state the provenance describes."""
        prior, routes, state_raw, routes_raw = self.provenance()
        cat = {r["relname"]: r for r in self.catalog()}
        require(set(cat) == {"river_timeseries", "river_timeseries_narrow_rollback"}, "READMISSION_TABLE_SET")
        require(
            cat["river_timeseries"]["oid"] == prior["old_oid"]
            and cat["river_timeseries_narrow_rollback"]["oid"] == prior["narrow_oid"],
            "READMISSION_OID_MISMATCH",
        )
        require(all(r["owner"] == "nhms_ingest_rw" for r in cat.values()), "READMISSION_OWNER_MISMATCH")
        ledger = self.ledger()
        require(ledger == sorted(set(prior["ledger_before"]) | {EXPAND}), "READMISSION_LEDGER_MISMATCH")
        names = sorted(p.name for p in (Path(self.c["staged_new_repo"]) / "db/migrations").glob("*.sql"))
        require(not [n for n in names if n not in ledger], "READMISSION_PENDING_MIGRATIONS")
        self.route_default()
        self.narrow_shape("river_timeseries_narrow_rollback")
        # Loose index scan over the primary key's leading run_key; never a fact-table scan.
        keys = [
            r["run_key"]
            for r in self.rows(
                "WITH RECURSIVE k AS (SELECT min(run_key) AS run_key FROM hydro.river_timeseries_narrow_rollback "
                "UNION ALL SELECT (SELECT min(r.run_key) FROM hydro.river_timeseries_narrow_rollback r "
                "WHERE r.run_key>k.run_key) FROM k WHERE k.run_key IS NOT NULL) "
                "SELECT run_key FROM k WHERE run_key IS NOT NULL ORDER BY run_key"
            )
        ]
        require(all(type(k) is int for k in keys), "RETAINED_RUN_KEYS_INVALID")
        retained = self.rows(
            "SELECT run_key,run_id,status,parsed_at FROM hydro.hydro_run "
            "WHERE run_key=ANY(ARRAY[" + ",".join(str(k) for k in keys) + "]::integer[]) ORDER BY run_key"
        )
        require([r["run_key"] for r in retained] == keys, "RETAINED_RUN_NOT_IN_PROVENANCE")
        snapshot = {r["run_key"]: r for r in routes if r.get("parsed_at") is not None}
        for row in retained:
            before = snapshot.get(row["run_key"])
            require(before is not None, "RETAINED_RUN_NOT_IN_PROVENANCE")
            # Every parser run stamps parsed_at; publication alone moves parsed -> published.
            require(
                row["run_id"] == before["run_id"]
                and row["parsed_at"] == before["parsed_at"]
                and row["status"] in ("parsed", "published"),
                "RETAINED_RUN_CHANGED_SINCE_D12",
            )
        require(set(snapshot) == {r["run_key"] for r in retained}, "PROVENANCE_RUN_FACTS_MISSING")
        require(
            self.c["reads"]["retained"]["request"]["run_id"] in {r["run_id"] for r in retained},
            "RETAINED_READ_IDENTITY_MISMATCH",
        )
        if self.args.command == "reprepare":
            self.save_file("provenance-state.json", state_raw)
            self.save_file("provenance-routes.json", routes_raw)
            self.save_file(
                "routes-before-reforward.json",
                json.dumps(
                    self.rows(
                        "SELECT run_id,run_key,status,parsed_at,timeseries_store FROM hydro.hydro_run ORDER BY run_key"
                    ),
                    default=str,
                ).encode(),
            )
            ...
            require(
                prior["old_oid"] == self.s["old_oid"]
                and prior["narrow_oid"] == self.s["narrow_oid"]
                and ledger == self.s["ledger_before"]
                and retained == self.s["retained_runs"],
                "READMISSION_STATE_DRIFT",
            )
        return dict(
            old_oid=prior["old_oid"],
            narrow_oid=prior["narrow_oid"],
            ledger_before=ledger,
            retained_runs=retained,
            reforward_provenance={
                "state": self.c["reforward"]["provenance_state"],
                **self.c["reforward"]["provenance"],
            },
        )

    def reattach(self):
        keys = [r["run_key"] for r in self.s["retained_runs"]]
        require(all(type(k) is int for k in keys), "RETAINED_RUN_KEYS_INVALID")
        retained = "ARRAY[" + ",".join(str(k) for k in keys) + "]::integer[]"
        route = (
            f"CASE WHEN run_key=ANY({retained}) THEN 'narrow' "
            "WHEN parsed_at IS NOT NULL OR status IN ('parsed','published') THEN 'legacy' ELSE 'narrow' END"
        )
        self.sql(
            "BEGIN; DO $$ BEGIN IF to_regclass('hydro.river_timeseries_legacy') IS NOT NULL "
            f"OR 'hydro.river_timeseries'::regclass::oid<>{int(self.s['old_oid'])} "
            f"OR 'hydro.river_timeseries_narrow_rollback'::regclass::oid<>{int(self.s['narrow_oid'])} "
            "THEN RAISE EXCEPTION 'REATTACH_FENCE'; END IF; END $$; "
            "ALTER TABLE hydro.river_timeseries RENAME TO river_timeseries_legacy; "
            "ALTER TABLE hydro.river_timeseries_narrow_rollback RENAME TO river_timeseries; "
            f"UPDATE hydro.hydro_run SET timeseries_store={route} WHERE timeseries_store IS DISTINCT FROM ({route}); "
            "COMMIT;"
        )
    ...
    def narrow_shape(self, table):
        require(table in ("river_timeseries", "river_timeseries_narrow_rollback"), "CATALOG_TARGET_REFUSED")
        columns = self.rows(
            "SELECT attname,format_type(atttypid,atttypmod) AS type FROM pg_attribute "
            f"WHERE attrelid='hydro.{table}'::regclass AND attnum>0 AND NOT attisdropped ORDER BY attnum"
        )
        expected = {
            "run_key": "integer",
        ...
        dims = self.rows(
            ...
            f"WHERE hypertable_schema='hydro' AND hypertable_name='{table}'"
        )
        ...
        return columns, indexes, settings

    def validate_expand(self):
        reforward = self.s.get("mode") == "reforward"
        cat = {r["relname"]: r for r in self.catalog()}
        require(set(cat) == {"river_timeseries", "river_timeseries_legacy"}, "EXPAND_TABLE_SET")
        require(
            cat["river_timeseries_legacy"]["oid"] == self.s["old_oid"]
            and (
                cat["river_timeseries"]["oid"] == self.s["narrow_oid"]
                if reforward
                else cat["river_timeseries"]["oid"] != self.s["old_oid"]
            ),
            "EXPAND_OID_MISMATCH",
        )
        ...
        columns, indexes, settings = self.narrow_shape("river_timeseries")
        # Re-forward: retained runs own facts only in the reattached table (design D4).
        retained = ""
        if reforward:
            keys = [r["run_key"] for r in self.s["retained_runs"]]
            require(all(type(k) is int for k in keys), "RETAINED_RUN_KEYS_INVALID")
            array = "ARRAY[" + ",".join(str(k) for k in keys) + "]::integer[]"
            retained = f" AND NOT run_key=ANY({array})"
            require(
                not self.rows(
                    "SELECT run_id FROM hydro.hydro_run "
                    f"WHERE run_key=ANY({array}) AND timeseries_store<>'narrow' LIMIT 1"
                ),
                "RETAINED_ROUTE_MISMATCH",
            )
        require(
            not self.rows(
                "SELECT run_id FROM hydro.hydro_run WHERE (parsed_at IS NOT NULL OR status IN ('parsed','published')) "
                "AND timeseries_store<>'legacy'" + retained + " LIMIT 1"
            ),
        ...
        require(
            not self.rows(
                ...
                "AND timeseries_store<>'narrow'" + retained + " LIMIT 1"
            ),
    ...
    def window(self):
        reforward = self.args.command == "reforward"
        require(
            self.args.go == "Danker"
            and self.s.get("phase") == ("REPREPARED" if reforward else "PREPARED")
            and self.s.get("mode", "initial") == ("reforward" if reforward else "initial"),
            "PREPARED_SIGNED_GO_REQUIRED",
        )
        ...
        try:
            ...
            if reforward:
                ...
                self.reattach()
                self.save(ledger_after=self.ledger())
                require(self.s["ledger_after"] == self.s["ledger_before"], "POST_LEDGER_MISMATCH")
            else:
            ...
            readability = {"parse": parsed, "narrow": narrow, "legacy": legacy}
            if reforward:
                readability["retained"] = self.worker("retained")
            self.save(readability=readability)
            self.no_listener()
            self.start_runtime()
    ...
    def recover(self):
        ...
        if legacy:
            require(
                not rollback
                and legacy["oid"] == self.s["old_oid"]
                and canonical["oid"] != self.s["old_oid"]
                and (self.s.get("mode") != "reforward" or canonical["oid"] == self.s["narrow_oid"]),
...
def worker(args):
    ...
    elif args.action in {"legacy", "narrow", "retained"}:
...
def arguments():
    ...
    p.add_argument("command", choices=("prepare", "reprepare", "window", "reforward", "status", "recover", "worker"))
    p.add_argument("--state", required=True)
    p.add_argument("--config")
    p.add_argument("--go", choices=("Danker",))
    p.add_argument("--action", choices=("imports", "preparse", "parse", "legacy", "narrow", "retained", "migrate"))
    p.add_argument("--repo")
    p.add_argument("--sha")
    args = p.parse_args()
    if args.command in ("prepare", "reprepare") and not args.config:
        p.error(args.command + " requires --config")
    return args
...
def main():
    ...
    try:
        ...
        if args.command in ("prepare", "reprepare"):
            executor.prepare()
        elif args.command in ("window", "reforward"):
            executor.window()
