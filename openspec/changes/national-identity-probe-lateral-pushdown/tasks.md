...
- [ ] 1.1 身份发现取**内联 4 列子查询**（design D2 定稿形态：
...
- [ ] 1.2 `services/tiles/mvt.py:592-628` 探针重塑为 D2 形态：
...
- [ ] 1.3 承重钉重钉：`tests/test_river_ts_read_path_surrogate_keys.py:432-441`
...
- [ ] 2.3 既有套件全绿：mvt.py 的 select_ci_tests 选集 +
      `tests/test_sql_shape_helpers.py`。

## Evidence Floor

- [ ] E1 `uv run pytest -q tests/test_river_ts_read_path_surrogate_keys.py
...
- [ ] E3 `openspec validate national-identity-probe-lateral-pushdown --strict
      --no-interactive` PASS
- [ ] E4 **硬门**，node-27 实机（EXPLAIN 协议：安静库、warm 二采取第二采、
      ...
      tile 字节相同；
      (ii) 压缩无覆盖 pin（**z9 407/200 @ 同时次**）：before 数十秒 →
      after <1s 空响应（零 fact 触达）；
