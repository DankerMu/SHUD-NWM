...
- [ ] 1.1 Changed-test branch: `tests/**.py` not matching
...
- [ ] 1.3 New tree-derived invariant test: every tracked
...
- [ ] 2.3 ci.yml `unit-test-targeted` run step: when count != 0 AND
...
- [ ] 2.4 String-coupling pin: selector suite asserts ci.yml's
      targeted job consumes the literal `meta_guard_only`.
- [ ] 2.5 Route-C pins comment (:812-838) updated: deleted-test-file
      class no longer a lost-smoke case; wording matches final
      semantics.

## 3. #1455(1) — one-hop importer closure

- [ ] 3.1 `_one_hop_importer_modules` (non-recursive; domain =
...
- [ ] 3.2 Extend the owning rule(s) so the derived `real_backend`
...
- [ ] 3.3 `display_coverage` one-hop set derived and covered (or
      derivation shows it adds nothing — record either way).
- [ ] 3.4 Function-body exclusion pin :692-700 stays green
      unmodified (top-level-edges-only composition, design
      decision 4).

## 4. #1455(3) — duplicate-pattern guard

- [ ] 4.1 Guard function parameterized over a rule list + test on
...
- [ ] 4.2 #1443 simulation test: constructed list with a second
      `packages/common/display_coverage.py` entry → guard flags it
      by name.

## 5. #1455(4) — gating-marker anchor

- [ ] 5.1 AST-derive conftest auto-skip marker set from the
...
- [ ] 6.1 `specs/ci-contract-baseline/spec.md`: MODIFIED ×3
...
- [ ] 7.1 `uv run pytest -q tests/test_select_ci_tests.py` green
      (report counts before/after).
- [ ] 7.2 Red evidence: each new guard/pin red against the unmodified
      selector via parameter seams or in-memory/out-of-tree copies
      only (honest labels for naturally-green pins).
- [ ] 7.3 `uv run ruff check .` per issue Verification fields
      (tracked-tree form).
- [ ] 7.4 `openspec validate ci-selector-followup-hardening --strict
...
- [ ] 7.5 CLI spot checks:
      ...
      `--github-output`.
- [ ] 7.6 Runtime accounting in PR body: selected-set size
...
- [ ] 7.7 Diff inspection: `count == 0` ci.yml branch byte-identical;
