...
- [x] 1.4 Round 3: enforce the writer-authority closed world — every accepted-submit writer that can receive or synthesize a reconciliation decision (submit-attempt commit with an accepted transition, cohort defer with a raw decision, cohort task projection, generic versioned transition) rejects `operator_verified_absence` with the typed-authority error before any durable mutation or event, with byte-identical journal and zero events, while the existing legitimate decisions keep their current writers and behavior.
