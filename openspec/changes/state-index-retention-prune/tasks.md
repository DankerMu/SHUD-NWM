...
- [x] 1.4 Capacity evidence on `_StateIndexSnapshot.evidence`, `validated_entries_for_renewal` evidence, `state_index_evidence()`, and the `publish_state_snapshot_index` result; one `logger.warning` per load at ≥0.70.
...
- [x] 2.2 Boundary: exactly-at-window kept, 1 s older non-anchor removed; each of K2 (earliest / latest / latest-before-window per generation, incl. the `""` generation), K3 (run starts), K4, K5 keeps an out-of-window entry; unusable non-anchor rows removed; raw mappings and order preserved for 16/19/20-key shapes.
...
- [x] 2.6 Enforce receipt for a ≥5k-removal index stays under `MAX_RECEIPT_BYTES`.
- [x] 2.7 CLI `main(["prune-retention", ...])` dry-run/enforce round trip including `--retention-days`.

## 3. Capacity evidence

- [x] 3.1 Warning true at ≥0.70 / false below for each of entries, nodes, bytes; reads and publishes still succeed at 0.95.
