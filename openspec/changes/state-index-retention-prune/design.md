...
- Surviving entries republished as their raw mappings in their original order (production mixes 20/19/16-key entry shapes), minus only the read-time injected `index_generated_at` / `object_evidence` that `_copyback_raw_entries` strips for every repair op. The reference lane regains those two fields on its next upsert (≈ +2 nodes / +90 B per entry), so reference-lane headroom is computed on the re-injected shape.
...
- Retention predicate — an entry of group g is kept iff at least one holds:
  ...
  - K3 it is the first entry of a maximal run of consecutive entries of the usable subsequence (unusable entries skipped, ordered as above) sharing one generation — this makes the planner a fixed point (a second prune removes nothing);
...
- Capacity evidence: `capacity = {entry_count, max_entries, json_nodes, max_json_nodes, index_bytes, max_bytes, utilization_ratio, warning_threshold: 0.70, warning}` on `state_index_evidence()`, renewal evidence, publish results and repair lanes; NOT on the per-reader nested `state_snapshot_index` blocks (node-22's largest pass evidence is 4,526,683 B of the 5,000,000 B budget with 384 such blocks). `logger.warning` once per load when `warning` is true.
