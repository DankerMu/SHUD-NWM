...
- [ ] 1.1 `_literal_path_consumer_index()` in the selector test
...
- [ ] 2.1 One pytest: derived sample (all 8 support modules + first
      ...
      never hardcoded.
- [ ] 2.2 Red evidence: constructed divergence (injectable seam or
      scratch probe; zero tracked mutation).

## 3. Guard evidence

- [ ] 3.1 Routing pins for mock_shud_omp (⊇ 3 suites + meta-guard,
      targets exist + file-level non-gated).
- [ ] 3.2 Red: rule entry dropped (constructed) → closure guard
      names the pair with consumer-suite wording.
- [ ] 3.3 Whole-tree selection diff vs master: delta = exactly
      tests/mock_shud_omp.py (additive).
- [ ] 3.4 #1455 disposition guard + #1487 carve-outs untouched
      (suite green unmodified there; real_backend byte-identity).

## 4. Spec delta

- [ ] 4.1 MODIFIED "Support-module changes MUST select their
...
- [ ] 5.4 `openspec validate ci-selector-subprocess-consumption-edge
      --strict --no-interactive`; MODIFIED block difflib clean apart
      from the surgical spots.
- [ ] 5.5 Whole-tree diff = exactly mock_shud_omp (list in PR body).
- [ ] 5.6 Both issues' acceptance checkboxes mapped; Verification
      commands green.
