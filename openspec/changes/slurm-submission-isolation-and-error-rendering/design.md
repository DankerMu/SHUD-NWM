...
- **persisted event** — unchanged: secrets-only raw text. Not for anti-laundering reasons — `[local-path]` is deliberately persisted evidence (`openspec/specs/pipeline-job-persistence/spec.md:719`), and #1592/#1630 govern object-URI placeholders only. The reason is that operators diagnose from the raw text, and that the durable event is the record of what the scheduler reported. The runtime-root recovery that reads `("gateway_response", "manifest")` (`services/orchestrator/retry.py:1421`, field set `:89-95`) is the same rule applied elsewhere, **not** a coupling to these two event types: its scan filters `event_type == "submission"` (`:1382-1386`), so it never reads `slurm_cancellation_gap` / `cancel_failed`. Pin the persisted shape with those readers anyway — they are the honest oracle for "a real root, not a placeholder" — but do not claim this route feeds them.
...
`_unproven_slurm_cancellation_payload`'s `gateway_response` (`:946-958`, also fed to an event at `:687`) is **settled, not deferred**: it dumps a `SlurmJobRecord` whose `manifest` carries `workspace_dir`, `manifest_index_path` and `array_log_dir` for array submissions (`services/slurm_gateway/real_backend.py:390-400`), so the response copy carries host paths and must be rendered, while the event copy must stay raw for the diagnosis reason above (`retry.py:1421` reads that same subtree shape, but only on `submission` events, so it is the oracle for the shape rather than a consumer of these rows).
