...
2. **渲染器** `render_river_ts_sql(template, store)`（`packages/common/`）：`legacy` → 表名替换为 `hydro.river_timeseries_legacy`，其余逐字；`narrow` → 表名为正名，删除每个标记行**及其紧邻的下一行**（该行必须是一条 aid 谓词，否则渲染器 fail-closed），随后做结构性检查（仓内无 SQL parser、三种占位符方言并存：括号配平、无 `WHERE AND`/`FROM AND`/`ON AND`、无悬空 `AND`/`OR (`、连接词后不得紧跟子句/连接/join 关键字（`LIMIT|ORDER|…|UNION|EXCEPT|INTERSECT` 与 `JOIN|LEFT|…|WHERE|ON|FROM|SELECT|WITH|VALUES|SET` 同一关键字族）、无 `AND AND`/`OR OR` 之类重复连接词、`WHERE` 后不得紧跟关键字、无残留标记）、断言不含任何事实表 text 身份列（按表归属，`hydro_run` 子查询里的 `run_id` 合法）、legacy 变体中的每个键/枚举谓词仍在；返回 `RenderedSql(sql, removed_placeholders)`，后者列出被删的位置参数 `%s` 下标（具名参数模板为空）。文本身份 matcher 以 PostgreSQL token 语义处理限定引用：bare identifier ASCII lower-fold，quoted identifier 必须完整且精确小写；一条共享 separator 规则接受 `.` 两侧 scanner-code whitespace，block/line comments 先由 outer/scanner normalisation 化为空白，所以不能借合法 qualified-name separator 绕过 narrow refusal；一个 balanced-group classifier 识别精确的一层 whole-row alias field selection，并让 guarded/render preflight 拒绝含 fact alias 的 nested/cast/function/row/multipart text-member selection（fixture decisions 19–22）。Decision 23 / #2114 用同一 scanner-owned comparison-scalar traversal 把 scalar body 交给既有 direct/quoted/separator 与 parenthesized matcher grammar；若 body 出现已归因 outer fact alias 的已知 text-member candidate（含 local alias shadow 的歧义），guarded helper 与两个 render 在既有 inner-fact-read count-delta 门之后以 correlated-scalar reason fail closed，`outer_predicates` 与 unguarded `text_fact_columns` 的 outer-only 契约不变。Decision 24 / #2141 把 PostgreSQL 的未 schema 限定一参 functional field notation `member(alias)` 加入同一个 alias-member analysis：bare name ASCII lower-fold、quoted name 必须完整且精确小写、comments/whitespace 只经 scanner normalisation；精确一参写入 canonical member set，含 fact alias 但超出该有限 grammar 的 known-member 调用由 guarded/两 render fail closed，schema-qualified function 不被误当字段。因为 decision 23 消费同一 analysis，functional notation 在 comparison scalar body 内无需第二套 matcher。Decision 25 / #2082 不改 renderer 行为，只给 `assert_structurally_intact` 的 quoted-identifier 括号跳过语义增加 helper-level direct pin：commutation oracle 继续只负责 t1/m26 的 traversal-family agreement，并明确不承担 structural m25 的杀伤。跨 store 查询的 `UNION ALL` 组合子已于 #1996 第二次 gate 拆出（fixture I1-1980 决策 15，tasks 1.4 re-entry）：由各读方在自己的事实行子关系内按 caller 侧设计实现，不再作为无调用方的文本级 helper。
...
|---|---|---|
| `mvt:postgis_tile_sql_hydro` | `mvt.py` 不持有 API 的 run metadata；在 `source_rows` 事实行来源内放 legacy/narrow 两支，每支在自己的 run-key authority lookup 同时匹配 `:run_id` 与 branch store，所以运行时至多一支可见 | `ST_AsMVT`、统计与 `.first()` 仍只得到一行；禁止整句 union，也不跨 ownership 向 API 增加 route threading |
| `mvt:postgis_tile_sql_hydro_national` | 两条 data LATERAL 和独立 identity-existence LATERAL 各自拥有一份局部两支 union；两处候选-run 查询都投影 `h.timeseries_store`，各分支按对应候选行限 store；每个组合后的 LATERAL 只保留一次既有 `LIMIT 1` | 两个 zoom 腿继续对称，identity 404/424 判定与 tile 聚合不变；既有 zoom-leg `UNION ALL` 不是 store union |
| `mvt:valid_times_named_identity` / `mvt:valid_times_any_identity` | 两支事实行查询先 union，再由唯一外层执行 `DISTINCT valid_time ORDER BY valid_time DESC LIMIT :limit`；named 分支在各自 run-key authority lookup 同时匹配 `:run_id`/store，any 分支按事实 `run_key` join/`EXISTS` 对应 store | `sample_limit + 1` 与 Python 截断行为不变；禁止 union 两个已各自排序/截断的整句 |
| `hydro_display:mvt_source_identity_probe` | `_run_row`（或同一 run metadata seam）携带 `timeseries_store`，探针只渲染并执行该 store 的一个变体；不做 union | 单次 `SELECT 1 ... LIMIT 1`、`.first()` 与 `MVT_SOURCE_IDENTITY_NOT_FOUND` 404 契约不变 |
...
2. **I2 forecast_store（9 条）**、**I3a per-basin MVT（1 读）**、**I3b national MVT（三个耦合 LATERAL 读）**、**I3c valid-time discovery（2 读）**、**I4a hydro_display 单-run 探针**、**I4b display_coverage river CTE** 各自走渲染器；它们可以独立合绿，但 transition SQL 在 I7 前不得部署或执行。
