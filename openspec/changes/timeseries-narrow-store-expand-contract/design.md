...
| 车道 | live lag 2 天（172800 s；模板 `infra/env/node27-timeseries-compression.example` 在 #1985 前赋值为 604800，本 change 起改为 172800）；retention 14 天；watermark 落后约 1 天 |
...
压缩 runner、retention runner、compression supervisor 的 `validate_current_d3`、capture 的 `HYPERTABLE_KEYS`、live-evidence 回放校验器的目录检查、autopipeline 统计守卫的 IN-list、治理采集（`node27_cold_governance_collection.py`）、治理审计的 policy-missing 检查（`node27_resource_governance.py`）——八个消费点——统一改为"正名 + 存在即纳入的 `_legacy`"（`timescaledb_information.hypertables` 存在性判定）；supervisor 的期望态按表分别断言（正名表键形态 / legacy 表 text 形态）。receipt schema：`schemas/timeseries_compression_receipt.schema.json` 的 `per_table_totals` 改为 `patternProperties`（正名两键必需，`_legacy` 键可选）；`schemas/timeseries_retention_receipt.schema.json` 新增 `legacy_chunks`（按 legacy 表名的映射，仅在存在 sibling 时出现；I9/I14 门禁读该表的条目 = 0）。legacy 表 DROP 后集合自动收敛。期望形态的三态规则：有 sibling → 正名键形态 + sibling text 形态；无 sibling → helper 内按表常量的"无 sibling 默认"（两表今天都是 text 形态）。该默认在各自 contract PR（river I9 / forcing I14）里翻成键形态并先于 DROP 部署——每次一行常量改动，不改发现逻辑；不翻则 DROP 后 supervisor 必红（评审已复现）。retention 的 drop 后发现探针与 `_default_measure_chunk_bytes` 同款隔离：探针失败时仍发 `enforced` receipt，并写出显式 `legacy_chunks: null`（round 2 决策 17：缺项只表示目录中没有 sibling，`null` 表示本 tick 无法担保；round 3：`refused` 出口同样传 `null`，使每份归档 receipt 自描述；I9/I14 门禁只认显式 `0`），绝不把已删 chunk 的记录降级成 `refused`。混合 tick：rename 时 `range_end` 仍在未来的那个 legacy chunk 会在 `range_end + lag` 后被选中，与 narrow chunk 同处一个 bound-4 tick（稳态混合 tick ≈3954 s vs 3900 s wall；(508 + 3×75) GB ≈ 4778 s ≈ 80 min 只是积压态最坏值）。**不用 bound-1 规则处理**（round-2 P1，评审仿真已证）：runner 的选择是表名前缀式的（`ORDER BY hypertable_schema, hypertable_name, range_end`，取前 bound 个），bound 1 下每天到达的 river narrow chunk 永远占满唯一槽位，legacy chunk 与整张 forcing 表都永远轮不到——该规则的退出条件不可达（round 3 更正：budget 链 leg 3 的规则是"抬高 statement timeout ⇒ 必须 bound 1"，反向不成立——默认超时下 bound 1 会被 preflight 接受，所以 bound 1 错在前缀选择饥饿，不在 budget 链）。处置改为 **expand 窗口内的守门式手动压缩**：legacy 表自 rename 起写冻结（D6 fail-closed，无任何 post-cutover 写入者），`compress_chunk` 不依赖 lag，因此 expand 迁移与服务重启后，在同一窗口（最迟次日）以 chunk 91 同款 watcher（gate：`hydro.hydro_run` 中 `timeseries_store='legacy'` 的 run 没有一个处于非终态——SQL 写成补集 `status in ('created','staged','pending','submitted','running')`，终态集以 `_TERMINAL_HYDRO_STATUSES` 为准，测试从代码推导而非钉字面量；`setsid nohup` 脱离；session 级抬高 `statement_timeout`；与 timer 互斥——timer 已 stop 或确认不在 tick 时段）对 surviving legacy chunk 执行 `select compress_chunk(...)`，随后核对 `is_compressed` 与 `df`。压完后该 chunk 被 `_CHUNK_QUERY` 的 `is_compressed = false` 过滤，timer 永远看不到混合 tick，`PER_TICK_BOUND=4` 全程不变。守门压缩是**强制步骤，不是"漏了有网兜"**（round 3 仿真更正）：若漏做，`range_end + lag` 那个 tick 在稳态下只会选到 1 个 narrow river + legacy + 1 个 forcing chunk（bound 4 每天跟得上，不会积 3 个 narrow），≈(75 + 508 + 12.7) GB × 6 s/GB + 380 s ≈ 3954 s，只超 3900 s wall 约 54 s，在推导的 ≈38–73 GB narrow chunk 带（7 天 chunk 268–508 GB 的七分之一；expand 前不存在窄表 chunk，无实测）只有下半段不超 wall，与 508 GB 配对隐含的 ≈73 GB 为 ≈3942 s、仍超 wall——**可能 TERM 也可能什么都不发**，即使 TERM，legacy 的 compress 多半已在 wall 内提交，告警最多是一次尾随信号；(508 + 3×75) GB ≈ 4778 s 只是积压态的最坏值。因此完成判据是 `timescaledb_information.chunks.is_compressed = true` 与 receipt 的 `per_table_totals["hydro.river_timeseries_legacy"]["chunks_compressed"]`，不是"没收到告警"；runbook §4 与 I8 清单必须按此写。expand 前的 legacy 存量同理不再用 bound-1 配方：lag 2 天下让日常 timer 的单表 tick 自行吃掉 terminal legacy chunk（单 chunk ≈51 min 在 60 min 超时内），或同样守门手动压缩；进窗口前以 receipt 核实无未压缩 terminal legacy chunk。
per-tick 推导按表分别陈述：正名表 1 天 chunk，每表每天到达 1 个 terminal chunk（两表 2/天）；`_legacy` 表 7 天宽、切换后**零到达**，只剩有限存量（每表 ≤ ⌈retention 窗口 ÷ 7⌉ + 1 个未压缩 7 天 chunk——2026-09-03 在箱上生效的 21 天窗口下实测 river 2 个（239–508 GB；该数反映压缩节奏——一个仍在填充、一个仍在 lag 内——而非窗口），公式给出的是上界 ⌈21 ÷ 7⌉ + 1 = 4；日数以 runbook Current policy 记录的 live 值为准，不写死 14；wall 论证是按 tick 的，不随 chunk 数变化，只有排空时长与 expand 前置期随之变长）。wall 关系按最坏组合算：一个 legacy 7 天 chunk ≈ 6 s/GB × 508 GB ≈ 51 min 已逼近 60 min 单 chunk 超时与 65 min 整 tick wall，因此过渡配方是：**expand 前**以 receipt 核实 terminal legacy chunk 已全部压缩（timer 单表 tick 或守门手动压缩，chunk 91 即后者），expand 后 legacy 只剩 range_end 在未来的最后一个 chunk，由窗口内守门手动压缩收尾（上段）；稳态 bound 4 满足 2/天 < 4 与 4 × 7.5 min = 30 min < 65 min。timer 节奏结论重述：1 天 chunk 把到达率从每周 2 个改成每天 2 个，日频 timer 仍充分，无需改频率。模板 `PER_TICK_BOUND=4` 保持并重钉（注释改写推导），`LAG_SECONDS` 赋值改为 172800、`one chunk width` 注释删除。

### D8 治理：工作集 + 峰值预测，空态与 watermark 缺失有定义
采集（只读目录）：`uncompressed_bytes`、`daily_ingest_bytes`（fixture 决策 25，2026-09-04 用户决定、触顶后在本 PR 内改：逐 canonical hypertable 取 `chunk_compression_stats('<table>')` ⋈ `timescaledb_information.chunks` 中 `compression_status = 'Compressed'`、按 `range_end` 最新的那个 chunk，`before_compression_total_bytes ÷ days(range_end − range_start)`，各表求和；`_legacy` 写冻结贡献 0、不得被借用；canonical 表无已压缩 chunk → `projection_status = "no_compressed_reference"`、`daily_ingest_bytes = null`、峰值 = 存量、warning `NO_COMPRESSED_REFERENCE`、绝不 critical。为什么不再读未压缩集合：`hydro.river_timeseries` 按预报 `valid_time` 分区、horizon 168 h，最新 7 天 chunk 在 watermark 到达它之前就已写入大部分字节，任何“未压缩字节 ÷ 覆盖天数”的读法在 chunk 边界高报 24×、其后仍 4–5×——node-27 receipt 实测 8.4 TB/day 对 ≈78 GB/day、假 critical 峰值 17 TB；已压缩 chunk 是唯一“再也不会收到写入”的目录量。已知极限（记录、不补偿）：速率滞后 lag + 宽度（今日 ≈9 天，expand 后 ≈3 天），流域扩增在下一次压缩前低报。receipt `working_set.ingest_reference` 按表回显分母（chunk、区间、pre-compression 字节、宽度、日增或 null）——上一版本的错误速率只能靠 live receipt 才发现，因为 receipt 没带分母）、`next_compressible_at`（最老未压缩 chunk 的 `range_end + lag`，lag 读自治理车道自己 env 里的同名变量 `NODE27_TIMESERIES_COMPRESSION_LAG_SECONDS`（默认 172800，模板间交叉钉；receipt 回显 `compression_lag_seconds`，node-27 rollout receipt 与部署的 compression env 值比对——live env 漂移只能靠该 receipt 兜住））、`home_free_bytes`、`projected_peak_bytes = uncompressed_bytes + daily_ingest_bytes × max(0, days(next_compressible_at − watermark))`（单位：天，可为小数）。速率按 canonical hypertable 逐表取参考 chunk 再求和（round 3 的逐表原则保留；round 2 的“窗口集合 `watermark − 7 d <= range_start <= watermark`、分子分母同批 chunk”随决策 25 一并退役——不再存在窗口集合）。`/home` 观测 `unavailable` 而工作集已测到（`ok` 或 `no_uncompressed_chunk`）→ `home_free_bytes = null`、`projection_status` 不变、发 critical `HOME_FREE_UNAVAILABLE` 并非零退出（不复用 `working_set_unavailable`，那会错报目录状态；峰值比较本身仍只在 `ok` 下做，空态 `projected = 0` 不得因 `home_free < margin` 误报）。数据库完全不可达（`connection_failed` / `psycopg2_unavailable`）→ critical `POSTGRES_UNAVAILABLE` 非零退出；`database_url_missing` 仍为有意跳过；`query_failed` 只发 `WORKING_SET_UNAVAILABLE` 一条。critical：`projected_peak_bytes > home_free_bytes − safety_margin_bytes`（默认 100 GiB）；warning：`uncompressed_bytes > working_set_warn_bytes`（默认 400 GiB）。空态：无未压缩 chunk → `next_compressible_at = null`、`projected_peak_bytes = uncompressed_bytes`、`projection_status = "no_uncompressed_chunk"`，不报 critical；watermark 不可用 → 与压缩 runner 同一 fail-closed 语义：`projection_status = "watermark_unavailable"`、发 critical `WATERMARK_UNAVAILABLE`（这是车道自身故障，必须到人）。工作集不可采集（timescale 块 `blocked`、或目录对两张正名表返回零行——只读角色失去 `timescaledb_information.*` 可见性时的形状）→ `projection_status = "working_set_unavailable"`、发 critical `WORKING_SET_UNAVAILABLE`，与 `WATERMARK_UNAVAILABLE` 同级：空目录与"全部压完"必须可区分。`DATABASE_SIZE_ABOVE_*` 降为 info。`timeseries-db-retention` 的"critical 即非零退出"契约不变；新 capability 只新增推荐码。上线预期是条件式的：只要 `projected_peak_bytes > home_free_bytes − margin`，每日 tick 就以 `PROJECTED_PEAK_EXCEEDS_HOME_FREE` 非零退出并触发 OnFailure 邮件（2026-09-03 chunk 91 压缩前的实测——未压缩 869 GB、/home 剩 295 GB——就是这一态；chunk 91 压完后数字改变，以最终 head 的 node-27 receipt 为准）——红是真实容量风险不是误报，runbook §8 与 I8 清单记录该条件式预期与运维确认方式，**不得**用抬高 safety margin 的方式消音。
...
- [expand 前 `/home` 先满] → 保命步骤已在本 change 外执行（chunk 91 守门压缩）；expand 迁移不占空间；expand 前按 D7 核实 terminal legacy chunk 已压完，surviving chunk 在窗口内守门手动压缩，bound 4 不变（D7）。
...
- [contract 迁移在 legacy 非空时被误跑] → 迁移内 chunk 计数 fail-closed；runbook 前置 `legacy_chunks["hydro.river_timeseries_legacy"] = 0`；contract 窗口先部署去掉列引用的代码再 DROP 列。
...
7. **I9 river contract**：开门条件 = 14 天 receipt 归档 + `legacy_chunks["hydro.river_timeseries_legacy"] = 0`；contract 迁移、删函数/backfill runner/aid、oracle 收敛、ADR/runbook/glossary、关闭 #1342/#1336。
