...
- [expand 前 `/home` 先满] → 保命步骤已在本 change 外执行（chunk 91 守门压缩）；expand 迁移不占空间；expand 前按 D7 配方压完 legacy 存量，且 `PER_TICK_BOUND=1` 保持到最后一个 legacy chunk 压完（D7）。
