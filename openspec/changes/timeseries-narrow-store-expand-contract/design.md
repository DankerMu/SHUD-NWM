...
采集（只读目录）：`uncompressed_bytes`、`daily_ingest_bytes`（窗口内未压缩 chunk 体量之和 ÷ 这些 chunk 实际覆盖的天数 `max(1/24, days(watermark − 最早窗口内 range_start))`（round 2：下限改为小时级——整天下限会把边界后第一天的 open chunk 除以 1 天、按 1/age 低报速率，7 天 chunk 几何下每 7 次审计有 1 次翻绿；7 天上限由窗口下界隐含）——分母不是固定 7：稳态下未压缩 chunk 只覆盖 lag+1 天，固定除 7 会把日增系统性低估约 3/7，node-27 首份 receipt 实测 `daily_ingest_bytes == uncompressed_bytes // 7` 逐字节吻合；高估是容量守卫的 fail-safe 方向）、`next_compressible_at`（最老未压缩 chunk 的 `range_end + lag`，lag 读自治理车道自己 env 里的同名变量 `NODE27_TIMESERIES_COMPRESSION_LAG_SECONDS`（默认 172800，模板间交叉钉；receipt 回显 `compression_lag_seconds`，node-27 rollout receipt 与部署的 compression env 值比对——live env 漂移只能靠该 receipt 兜住））、`home_free_bytes`、`projected_peak_bytes = uncompressed_bytes + daily_ingest_bytes × max(0, days(next_compressible_at − watermark))`（单位：天，可为小数）。速率按受管 hypertable 逐表计算再求和（round 3：pooled 分母取全集 `min(range_start)`，7 天 forcing chunk 或写冻结的 `_legacy` 幸存 chunk 会把 1 天表的速率稀释近半）；窗口集合是 `watermark − 7 d <= range_start <= watermark`（round 2：分子与分母必须跨同一批 chunk；预报 horizon 写在 watermark 之后的 chunk 只计入 `uncompressed_bytes`，不进速率——否则 post-expand 会把日增高估约 4 倍、翻出假 critical）。`/home` 观测 `unavailable` 而工作集已测到（`ok` 或 `no_uncompressed_chunk`）→ `home_free_bytes = null`、`projection_status` 不变、发 critical `HOME_FREE_UNAVAILABLE` 并非零退出（不复用 `working_set_unavailable`，那会错报目录状态；峰值比较本身仍只在 `ok` 下做，空态 `projected = 0` 不得因 `home_free < margin` 误报）。数据库完全不可达（`connection_failed` / `psycopg2_unavailable`）→ critical `POSTGRES_UNAVAILABLE` 非零退出；`database_url_missing` 仍为有意跳过；`query_failed` 只发 `WORKING_SET_UNAVAILABLE` 一条。critical：`projected_peak_bytes > home_free_bytes − safety_margin_bytes`（默认 100 GiB）；warning：`uncompressed_bytes > working_set_warn_bytes`（默认 400 GiB）。空态：无未压缩 chunk → `next_compressible_at = null`、`projected_peak_bytes = uncompressed_bytes`、`projection_status = "no_uncompressed_chunk"`，不报 critical；watermark 不可用 → 与压缩 runner 同一 fail-closed 语义：`projection_status = "watermark_unavailable"`、发 critical `WATERMARK_UNAVAILABLE`（这是车道自身故障，必须到人）。工作集不可采集（timescale 块 `blocked`、或目录对两张正名表返回零行——只读角色失去 `timescaledb_information.*` 可见性时的形状）→ `projection_status = "working_set_unavailable"`、发 critical `WORKING_SET_UNAVAILABLE`，与 `WATERMARK_UNAVAILABLE` 同级：空目录与"全部压完"必须可区分。`DATABASE_SIZE_ABOVE_*` 降为 info。`timeseries-db-retention` 的"critical 即非零退出"契约不变；新 capability 只新增推荐码。上线预期是条件式的：只要 `projected_peak_bytes > home_free_bytes − margin`，每日 tick 就以 `PROJECTED_PEAK_EXCEEDS_HOME_FREE` 非零退出并触发 OnFailure 邮件（2026-09-03 chunk 91 压缩前的实测——未压缩 869 GB、/home 剩 295 GB——就是这一态；chunk 91 压完后数字改变，以最终 head 的 node-27 receipt 为准）——红是真实容量风险不是误报，runbook §8 与 I8 清单记录该条件式预期与运维确认方式，**不得**用抬高 safety margin 的方式消音。
