...
2. **渲染器** `render_river_ts_sql(template, store)`（`packages/common/`）：`legacy` → 表名替换为 `hydro.river_timeseries_legacy`，其余逐字；`narrow` → 表名为正名，删除每个标记行**及其紧邻的下一行**（该行必须是一条 aid 谓词，否则渲染器 fail-closed），随后做结构性检查（仓内无 SQL parser、三种占位符方言并存：括号配平、无 `WHERE AND`/`FROM AND`/`ON AND`、无悬空 `AND`/`OR (`、连接词后不得紧跟子句/连接/join 关键字（`LIMIT|ORDER|…|UNION|EXCEPT|INTERSECT` 与 `JOIN|LEFT|…|WHERE|ON|FROM|SELECT|WITH|VALUES|SET` 同一关键字族）、无 `AND AND`/`OR OR` 之类重复连接词、`WHERE` 后不得紧跟关键字、无残留标记）、断言不含任何事实表 text 身份列（按表归属，`hydro_run` 子查询里的 `run_id` 合法）、legacy 变体中的每个键/枚举谓词仍在；返回 `RenderedSql(sql, removed_placeholders)`，后者列出被删的位置参数 `%s` 下标（具名参数模板为空）。跨 store 查询用 `render_union_all(template, stores, params)` 组合子（仅接受具名参数模板，位置参数模板直接 raise）：每分支在事实表自身作用域内绑定 `hydro_run.timeseries_store`：作用域已 join `hydro_run` 时按别名列引用，否则用关联 `EXISTS`（以该引用自己的别名限定，无别名时才用分支的物理表名）；链的选择限定在引用自身的 SELECT 段内：FROM 引用只绑自己段的 `WHERE`、JOIN 引用只绑紧随其后的 `ON`，找不到即拒绝，绝不借用兄弟段或别的 join 的链；事实表处于外连接被保留侧（`LEFT/RIGHT/FULL JOIN hydro.river_timeseries`）的引用直接拒绝——`ON` 位置会把 NULL 扩展行复制到两个分支、`WHERE` 位置会把外连接变内连接，两者都不等价，读方需要时改写为子查询（`store_binding_forms` 按引用逐个报告采用哪种，`store_binding_form` 是其折叠：一致时返回该形式、不一致时返回 `"mixed"`）；参数名跨分支共享、`params` 原样返回，输出形状由 oracle 钉住。contract 时渲染器只接受 `narrow`，模板里的标记行与 aid 行物理删除。 同理，父层紧邻 `(` 之前是 `(LEFT|RIGHT|FULL) [OUTER] JOIN LATERAL` 的事实表子查询作用域也拒绝（NULL 扩展行会在两个 UNION 分支各出现一次）；判定只看父层 `(` 之前的文本，不做跨层推理；`CROSS JOIN LATERAL` / `[INNER] JOIN LATERAL` 照常在 lateral 自己的 WHERE 内绑定。（round-3 Review Failure Retro，shape=depth，重设计）组合子改为**正向白名单**：仅当语句是只读（屏蔽注释/字面量后无顶层 DML）、事实表引用的祖先开括号链无外连接/无否定/非 EXCEPT-INTERSECT 右操作数、引用形式为 `FROM … 自身 WHERE` 或 `[INNER] JOIN … 自身 ON`（alias 形式仅在同 scope 以 `run_key` 关联 `hydro_run` 时使用，否则相关 EXISTS）、无 `;`/锁子句、governing chain 非顶层析取、且 registry 条目声明 `union_safe=True` 时才绑定；其余一律拒绝并指名原因。`union_safe` 是声明属性（False 附原因：DISTINCT / ORDER BY+LIMIT / 仅聚合行 / DISTINCT ON / 单行消费者），不由渲染器推断；I1 只交付语句级 UNION，`union_safe=False` 条目的组合由有真实调用方的 wave-2 PR 设计。白名单与拒绝形状表见 `fixtures/I1-1980.md` Invariant Matrix 与 decisions 12–14。
