...
#### Scenario: An unqualified output star over the fact table cannot bypass the output contract
- **WHEN** a SELECT output list at the top level contains a complete bare `*` item, as the first or a later item and with or without an `ALL` / `DISTINCT` / `DISTINCT ON` prefix, and the FROM/JOIN of that query level provably reads `hydro.river_timeseries` unaliased, under a bare alias, or under an `AS` alias
- **THEN** the guarded `fact_table_text_identity_columns` helper reports all seven `TEXT_IDENTITY_COLUMNS`, the alias-scoped helper reports them only for the proven fact alias, and narrow rendering refuses with an entry-named text-identity error before returning SQL
- **AND** an unqualified output star whose fact ownership cannot be proven is refused as unmodelled whole-row star exposure, never reported clean; `COUNT(*)` and other function arguments, table-qualified multipart stars (still refused by the existing reference-count guard with its existing reason), other-relation and CTE `x.*` projections, literals, comments and `"*"` keep their existing outcomes
...
For the largest registered river network (SHJ-NJ pin) and one small network, the per-segment forecast-series SQL SHALL show `river_segment_key` in an `Index Cond` on narrow uncompressed chunks and in segmentby batch pruning on narrow compressed chunks, with `Rows Removed by Filter / rows returned ≤ 10`, `shared hit ≤ 5000`, SQL warm P95 ≤ 300 ms over at least five warm samples, and the node-27 local single-source `forecast-series` warm P95 ≤ 500 ms over at least thirty warm samples measured on a quiescent node — `nhms-node27-autopipe.service` inactive AND 1-minute load average below 2.0. This is a read-path bound, not a capacity bound: at fewer than thirty samples the P95 index lands at or near the maximum and a single blip reads as the percentile, and while a full-cycle ingest saturates the node the **legacy** read path misses the same bound, so in that state the measurement cannot discriminate this change. Contention behaviour is bounded by the separate user-facing river-click P95 requirement, not here. Because the narrow table omits `river_timeseries_valid_time_idx`, the identity-existence probe's interior-gap miss branch SHALL be measured before (legacy) and after (narrow) with `EXPLAIN (ANALYZE, BUFFERS)` and every coverage loss enumerated, as `timeseries-index-hygiene` requires.
