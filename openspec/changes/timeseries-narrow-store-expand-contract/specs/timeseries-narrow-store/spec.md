...
- **THEN** the `legacy` variant of every registered template, with `hydro.river_timeseries_legacy` replaced by `hydro.river_timeseries`, is equal to the pre-normalisation statement modulo whitespace and marker placement, and every pre-existing test on those readers still passes
