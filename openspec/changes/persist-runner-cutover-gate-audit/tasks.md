...
- [ ] 1.2 runner 接线（registry publish 路径——块在 `:811` 无条件构造，覆盖
...
- [ ] 1.3 `schemas/scheduler_file_provider_refresh_receipt.schema.json`：
      ...
      同步补一个合法块。
- [ ] 1.4 `scripts/publish_scheduler_file_registry.py`：在 `:1206`/`:1216-1222`
...
- [ ] 1.5 `_validate_receipt`：`cutover_gate` 存在时校验 mode ∈
...
- [ ] 2.1 runner 级：`tests/test_scheduler_file_provider_refresh.py` 新增
      ...
      KeyError/缺 key 失败。
- [ ] 2.2 缺席钉：gate 构造前的早期失败路径（锁竞争 `already_running` /
...
- [ ] 2.5 既有全量：`uv run pytest -q tests/test_scheduler_file_provider_refresh.py
      ...
      全绿，零删除零弱化。
- [ ] 2.6 runner wiring 钉（mutant-sensitive，与 2.4 对称）：monkeypatch
