...
- [x] A5.3 A-5（覆盖缺口）：拒绝之后的运维恢复路径补测——refusal pass → 修好 canonical readiness → 候选被 admit 且从 `forecast` 重启 → 计数恰好 +1 → 原 pin 返回 `pin_mismatch`。既有 sink 测试补上「拒绝者不产出 run manifest、dispatch 零调用」两条附加断言。**这两条在预算臂上检测力为零**（round 5 的 R5-TE-04，实测）：递给 `_execute_candidates_async` 的正是上一行刚断言为空的那个列表，空列表在 `reservation.py:363-364` 的 `if not submitters: return []` 处直接返回，`orchestrator_factory` 从未被调用；而 `_budget_pass` 只走到候选构建，本就不执行候选，run manifest 无从产生。预算臂真正有牙的是该测试的**拒绝侧**断言——它不在 M6 的 5 个幸存者名单里，即整条在守卫空转时变红（具体是哪一句先失败未测，也不需要测）；端到端的 `submitted_count == 0` 由**断路器兄弟臂**经真实 `run_once()` 断言。保留那两条无妨，但不得记成补偿性测量。
...
- [x] A5.5 A-1/A-2/A-3 + 搜索范围（覆盖缺口）：审计表逐行重判。规格补第 (iii) 类闭合（带「能剥掉确认物块的路径不由它结案」的牙齿）；W11 由假 (i) 改真 (ii)（两个确认物写入者都产出 `forecast` + `native_shud_resubmitted: True`，命中早退；**不外推到 W9**）；W13/W14/W15 改 (iii) 并换掉 C-1 那条被证伪的前提；补漏行 W16；补漏文件 W17/W18（`chain_repository_state.py`——上一版三文件 scope 把它漏在搜索范围外）；W4 澄清「按位置在册、不按取值」。搜索范围实测并逐个写排除理由。「无未结行」那句删除。**round 5 的 R5-TE-01 把这条补齐**：排除理由第一版委派给了 gitignored 的工作底稿（审阅者从 PR 里拿不到，与 A4.4 判过的同一类），现整表搬进 `design.md`；同时口径对齐规格 SHALL 的「contains the key」——三模式 grep 的赋值面是 15 个文件，裸 token 的接触面是 **25** 个，表按后者写满 **22 行**（25 − 3 个真 writer），多出的 10 个逐个实测全为 EXCLUDED-(1)、无 writer。
...
- EF-6 红证据与变异：每个切片贴出源码改动前的失败断言。单独回退以下三处，对应测试变红：
  ...
  - **sink 守卫切片（round 5 实测，本轮核心改动的红证据）**。三个数字都来自**同一条命令、同一棵最终树**：
    ...
    - **M6** `_refuse_confirmed_candidates_off_forecast` 首句插 `return` → **24 failed / 5 passed**。5 个幸存者里**只有白名单测试真的不调用守卫**；另外四个直接调用守卫，它们幸存是因为断言的是**录取侧**结果，而空转的守卫同样产出 keep——对"守卫是否执行"不敏感。这不是覆盖洞：keep 侧按构造不可能在 no-op 下变红，它钉的是反方向（守卫不得过度拒绝），拒绝侧由同一条命令的 24 红钉住。（round 5 的 R5-TE-03：第一版写成"不经守卫路径"，与 round 4 的 A-2 是同一句式的误描述。）五个是（`[forecast]`、`[forecast_with_a_disagreeing_restart_from_stage]`、`non_mapping_confirmation[absent]`、`..._leaves_an_unconfirmed_off_forecast_candidate_alone`、白名单测试）。
