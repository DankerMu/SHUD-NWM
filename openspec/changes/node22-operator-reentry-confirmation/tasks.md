...
- [x] A1.3 c-03 Phase 6.2 审计：枚举确认匹配之后所有重写 retry `decision` 的点，逐一核对块的键路径幸存（见 PR body 表）。发现兄弟泄漏类 `retry_strict_warm_start_retry_run_manifest_mismatch`（同在白名单），由本 option-A 修复顺带关闭。
