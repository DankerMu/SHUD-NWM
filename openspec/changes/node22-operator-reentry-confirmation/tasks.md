...
- [x] A4.6 c-01/c-02（P2，文档）：`tasks.md:141` 的 A2.1 理由与 design.md:170 自相矛盾（已修）；design/tasks/docstring 里 R2 写下的 15 处坐标整体偏移（R2 报告用的是插入前坐标；该 commit 在 `scheduler_candidates.py:1758` 插入 +39 行、`tests/test_production_scheduler.py:15615` 插入 +62 行），按 round-3 verifier 的更正表逐条改；最坏一行 `:2303-2307` 落在了另一个函数里。已推送 commit message 里的同一处错误不改写历史，记入偏离记录。D0 矩阵（design.md:32-44）改钉 `49cf31316c:` 前缀，与 `53c39b99c:` 同办法。
...
- EF-7 node-27 后端 oracle：隔离 worktree `/home/nwm/tmp/wt-b7a`，复用共享 venv，不设 integration 环境变量，`umask 022` + `TMPDIR=/home/nwm/tmp`。跑本分支改过的全部测试文件：`tests/test_production_scheduler.py tests/test_scheduler_backfill_predecessor.py tests/test_file_orchestration_journal.py tests/test_operator_reentry_confirmation.py tests/test_warm_start_chaining.py tests/test_scheduler_generation.py tests/test_monitoring_api.py`（后三个此前漏列，round 3 c-03；均可在本 EF 声明的环境里跑，不需要 integration 环境变量）。
