...
4. Finish the normal lifecycle: trustworthy task identity/accounting/status queries resolve the original new attempt. Controller binding is nonterminal; completion continues through existing full array accounting, aggregation and forecast preparation, not an operator adoption command. The pre-existing strict package-witness guard runs when selecting a forecast restart, not on every normal forcing→forecast transition; preserve that sibling contract without adding a new package-integrity gate. Confirmed nonacceptance retains the immutable-attempt propagation grace and guarded absence path. Genuine unknown remains unresolved.
...
Implementation clarification: new forcing attempts use their existing persisted Slurm Comment to carry an attempt-specific identity, separate from the stable business key. This avoids clock-tolerance or same-second permanent holds while retaining exact current-attempt matching. A single bounded, target-row, bind-only reconciliation at the response-failure boundary can use controller-held identity before retention expires; later global reconciliation remains available. It never submits again or proves absence from a missing controller record.

## Sibling surfaces and references

chain_slurm_client and chain_stage_execution classify/record submit; reservation and file_orchestration_journal own concurrency/current authority; reconcile and scheduler candidate/execution evidence consume it. Normal forcing completion uses existing full task accounting/aggregation and forecast preparation; forecast-restart selection owns the separate strict forcing-witness guard. accepted_submit_cohort owns forecast digest invariants (#1183). Reuse these existing seams, not new operator or package-validation paths.
