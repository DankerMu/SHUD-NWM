...
- **#2504 (p2)** — the #1446 overwrite guard (`display_coverage.py`: `WHERE force OR EXCLUDED.segment_count > 0 OR current = 0`) freezes a populated coverage row forever once its facts legitimately vanish (retention); `--skip-fresh` never rescans it. Display lists the run, the curve is empty. Measured on node-27 today (read-only, bounded, pre-implementation probe): 2616 of 2716 published, populated, >21-day-old runs have zero river facts; the committed 6.5 audit on the refresh's own watermark cutoff (`docs/runbooks/receipts/2026-09-23-issue-2504-populated-empty-audit/README.md`) reads 2616 empty of 2666 out-of-window populated rows, 0 of 4089 in-window.
...
- **#2529** — decided alert shape (design D5, also in the PR body): a new DB-only **residency** observer `scripts/node27_parse_failure_residency_alert.py` + user unit/timer. It reads `hydro.hydro_run` rows with `status='failed'` (any error code — on node-27 the parser is the only `failed` writer, and its bare `OutputParsingError` codes such as `MODEL_RIVER_FILE_MALFORMED` are the permanent #1781 shape; fix pass 1 dropped the first cut's `OUTPUT_PARSE_` prefix filter) whose `updated_at` is within a retry-liveness bound (renewed each tick by the autopipe's register upsert; `mark_run_failed` no-ops on an already-failed run, so the stored code is the first failure's, reported as `first_error_code`), keeps a small state file of first-observed-failing time per run, and exits non-zero only when a run has stayed failing across its observations for ≥ a threshold (default 2 h), with per-run dedup and a 24 h re-alert. Delivery is the existing `OnFailure=nhms-node27-unit-failure-alert@%n.service` handler, with the unit's stdout on the journal (not `append:`), so the handler's journal excerpt carries the observer's report as a non-empty body. The autopipe unit itself gets **no** `OnFailure=` (every transient rc=1 tick would mail, and its `append:` stdio would leave the body empty). `OUTPUT_PARSE_DB_ERROR` is **not** split (the observer does not read the code at all; the retry path does not read it either; splitting changes persisted codes for no behaviour gain). Root-cause receipt from node-27 is committed as a runbook receipt. Stale `OnFailure=` docstrings are corrected.
