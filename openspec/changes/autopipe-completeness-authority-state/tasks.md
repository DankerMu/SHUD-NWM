...
- [x] 1.2 真实 DB 回归（design D3 (i)-(v)）：先在修复前跑 (i) 取红证（粘贴断言失败行），
  再修复取绿。
...
- [ ] E2 node-27 throwaway DB：`NHMS_RUN_INTEGRATION=1 NHMS_INTEGRATION_DATABASE_URL=<superuser throwaway DSN> uv run pytest -q tests/test_river_identity_normalization_integration.py -k already_ingested` (i)-(v) 全绿，receipt 必须显示非零 passed 数（全 skip 不算），附修复前 (i) 的红证。
- [ ] E3 node-27 live receipt（design D4）：连续 ≥2 tick `rc=0`、`already_ingested` 回到全量、阻断计数 ≤ 基线、elapsed 分钟级；`hydro_run` 状态分布复测。
