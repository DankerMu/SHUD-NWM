...
- [x] 1.10 (round-1 cand-spec-2) `tests/test_file_orchestration_journal.py:2008 :2070`: the two #1472 foreign-completion discriminator matrices parametrize over all five active statuses (add `pending`); docstring enumeration corrected (the existing test name already fit once all five statuses are parametrized); the own-row matrices (`:2107 :2156`) stay two-member
