...
- [ ] Identity: `chain`, `chain_repository`, `file_orchestration_journal`, `scheduler_state_decision`, `scheduler_state_manual_retry` each bind `scheduler_state_types.ACTIVE_HYDRO_STATUSES` by identity and the value is `{"created","staged","pending","submitted","running"}`
...
- [ ] 1.5 `tests/test_hydro_status_set_parity.py`: divergence test replaced by the identity + value test over `chain`, `chain_repository`, `file_orchestration_journal`, `scheduler_state_decision`, `scheduler_state_manual_retry` (round-1 cand-test-evidence-1: the two deciding lanes were unpinned); module docstring updated; the enum-membership test keeps its `<= enum_members` lines (add the journal site)
...
- [ ] 1.9 (round-1 cand-integration-2) `tests/test_scheduler_backfill_predecessor.py`: `emit_predecessor_candidates` against a real `FileOrchestrationJournalRepository` whose predecessor run sits at `pending` with no jobs → `skipped / predecessor_backfill_active_pipeline`, zero candidates; `failed` control → emitted (red at `d7fe213b`: the `pending` case is emitted)
- [ ] 1.10 (round-1 cand-spec-2) `tests/test_file_orchestration_journal.py:2008 :2070`: the two #1472 foreign-completion discriminator matrices parametrize over all five active statuses (add `pending`); docstring enumeration and test name corrected; the own-row matrices (`:2107 :2156`) stay two-member
...
- [ ] 3.4 (round-1 cand-spec-1) committed test in `tests/test_hydro_status_set_parity.py` that repoints `_MIGRATIONS_DIR` (monkeypatch) at a `tmp_path` copy of `db/migrations` plus probe files: a quoted-identifier `ADD VALUE 'complete'` enters the member set (enum assertion red), and a `RENAME VALUE` makes the helper raise with the "does not model renames" message naming the file
