...
- **WHEN** the confirmed quarantine rerun was accepted for submission and then failed at the compute layer, and a later ordinary replacement completes while the recorded identity is still stale
- **THEN** the quarantine rerun count SHALL already equal N+1 from the accepted rerun, the confirmation SHALL NOT match, and no further quarantine rerun SHALL be submitted on that confirmation
