...
- **WHEN** the scanned passes contain only blocked candidates of other decisions and no pass dropped its candidate lists
...
- **THEN** the occurrence count SHALL become N+1, the confirmation SHALL no longer match, and the candidate SHALL return to `blocked` with no submission
...
- **THEN** the confirmation SHALL NOT match and the candidate SHALL be `blocked` with no submission
...
- **GIVEN** a strict warm-start candidate blocked with `attempt == retry_limit == L`
- **WHEN** an operator attests a confirmation with pin L
- **THEN** the next pass SHALL submit one retry without any change to `NHMS_SCHEDULER_RETRY_LIMIT`
- **AND** after that rerun the attempt SHALL be L+1 and the candidate SHALL return to `blocked`
