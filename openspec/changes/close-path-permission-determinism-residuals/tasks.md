...
- [x] 6.5 Produce batched red proofs for the Round 1 and depth-retro closure tests; run the full eight-file suite, tracked-Python ruff, strict OpenSpec, Python 3.11/3.12 matrices, and Phase 6.2 invariant audit. Final-head node-27 at `99debf9dae975b0a0cb362f4548a0fcba058a1e7`: pinned Python 3.11.15 full focused suite -> `2665 passed, 26 skipped`; Python 3.12.13 initial core run exposed one pre-existing master-reproducing retry TOCTOU (`#1356`), then the transparent supplement with exactly that node deselected -> `2548 passed, 8 skipped, 1 deselected`; tracked-Python ruff -> pass; tracked diff empty.
