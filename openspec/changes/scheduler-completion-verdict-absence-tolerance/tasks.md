...
- [x] 1.1 **统一比对 helper**:抽出单一 `terminal_init_state_match(...) -> {"match","absent","conflict"}`,语义 = 逐在场字段比对(见 design 修订:absent=零身份字段;在场字段全合=match;任一在场字段矛盾=conflict;**禁止**部分字段即 conflict);helper 为**纯字段比对**,candidate 侧 `candidate_state` 分支与 `COLD_START_QUARANTINED` 逃生门**留在 candidate wrapper 内、先于 helper 短路返回 match,不进 helper、不上提 verdict 路径**;`scheduler_discovery.py:328-340` 与 `scheduler_candidates.py:1842-1868` 均消费它;legacy hydro_run 行(仅 `init_state_id`)匹配行为逐字节不变。
