...
- **THEN** reconciliation SHALL reject the receipt with `receipt_classification_invalid` unless the bucket's model ids are a subset of `removed` AND `declared_retirements.total` does not exceed `removed.total` (the total-level inequality holds even when item lists are truncated, so an inflated total cannot deflate the refused lower bound), the previous-side equality `unchanged + package_changed + removed == previous_model_count` still holds with removals counting declared retirements, and the refused lower bound accounts for declared retirements as non-refused removals; a legacy receipt without the bucket SHALL keep validating with the bucket read as empty
