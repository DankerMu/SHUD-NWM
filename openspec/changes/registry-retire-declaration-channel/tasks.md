...
- [x] 1.1 schema（design D1）：`scheduler_registry_package_cutover.schema.json`
...
- [x] 1.2 加载层（D1）：refresh 侧 `CUTOVER_TRANSITION_MODES` 加 `"retire"`
...
- [x] 1.3 判据层（D2）：`_classify_registry` 规则 1 retire 豁免（membership
...
- [x] 1.4 证据层（D3，F1 路线）：`publish_all_basin_scheduler_registry` 加
      **out-sink 可选参数**（默认 None，与 cutover_gate/
      ...
      （D3，实现时指认并记录）。
- [x] 1.5 对账层（D4 全枚举）：`ClassificationResult.declared_retirements`
...
- [x] 1.6 runbook（D5）：`current-production-ops.md:508-510` 触发面补全 +
      retire declaration 恢复顺序 + `--allow-uncovered-cutover` 降级标注
      （保持红旗定性）+ `:500-504` 桶枚举加新桶 + 共享消费者一句提示。
- [x] 1.7 共享消费者（D2/F2）：`services/orchestrator/scheduler_generation.py`
...
- [x] 2.1 端到端双分支主锚：previous={alpha,bravo}、bravo 包 invalid 被
...
- [x] 2.5 既有锚零改动确认：`:3198` removal 用例、undeclared 族、audit 块
      族、publish 既有断言 diff 级不动全绿。
- [x] 2.6 证据判别格：目录被删形（无 inventory 行）refusal 无三键；skip
      形有三键——两形可辨（D3）。
- [x] 2.7 共享消费者格（F2/I8）：retire entry 在位时
...
- [x] 3.3 `openspec validate registry-retire-declaration-channel --strict
      --no-interactive` 通过。
- [x] 3.4 JSON Schema 校验链绿（结论已定：CI json-schema-validate 覆盖，
