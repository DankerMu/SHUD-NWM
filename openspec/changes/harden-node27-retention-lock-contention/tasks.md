...
- [ ] 1.1 在 `_QUERY_TIMEOUT_MS` / `_DROP_TIMEOUT_MS`（`:113-114`）旁新增
...
- [ ] 1.2 `RetentionConfig` 冻结 dataclass 末尾新增
...
- [ ] 1.4 `_default_drop_chunk`（`:670`）在 `SET statement_timeout`（`:685`）**之后、
...
- [ ] 2.1 新增 `LOCK_CONTENTION_PGCODES: frozenset[str] = frozenset({"55P03", "40P01"})`。
      注释写明 design D4：分类段插在 `<hypertable_schema>.<chunk_name>:` **之后**，
      wire code + chunk 名前缀 byte-unchanged（#1213 的 operator-grep 契约）。
- [ ] 2.2 在 `:954` 的 `except Exception as error:` 里用 `getattr(error, "pgcode", None)`
...
- [ ] 2.3 `_redact_error_text` 不动；分类段是纯 ASCII 常量，不经过驱动文本。

## 3. 每块 drop 计时（design D3a，stderr，无 schema 变更）

- [ ] 3.1 drop 循环（`:950-971`）对每块记录 `time.monotonic()` 起止，
      成功与失败**两条路径都**向 stderr 输出一行诊断，含 chunk 的
      `qualified_name` 与毫秒级 elapsed。沿用 `:655` 既有诊断的 JSON 行形状。
- [ ] 3.2 **不动** `schemas/timeseries_retention_receipt.schema.json`；
      `dropped_chunks[]` 的键集保持不变。
- [ ] 3.3 计时诊断不经过 `_redact_error_text`（纯数字 + chunk 名，无凭据面），
...
- [ ] 4.1 T1/T2：**合成异常类**（真 psycopg2 异常的 `pgcode` 只读且客户端构造为 `None`，
...
- [ ] 4.2 T3：`pgcode="57014"` → **不含** `lock-contention`，与既有 `:968` 的形状逐字一致。
- [ ] 4.3 T4：无 `pgcode` 属性的裸 `RuntimeError` → 不含 `lock-contention`。
- [ ] 4.4 T5：`LOCK_TIMEOUT_MS` = `0` / `-1` / `300000` / `300001` / `"abc"`
      → 全部 `RetentionConfigError`，且断言**零 DB 调用**。
- [ ] 4.5 T5b：`LOCK_TIMEOUT_MS = ""` → 视同缺失，取默认值。
- [ ] 4.6 T6：未设 env → `config.lock_timeout_ms == 240_000`。
- [ ] 4.7 **T6b（反空转）**：env 取**非默认**值 `"2000"` → `config.lock_timeout_ms == 2000`。
      没有这条，`config_from_args` 校验了 env 却忘记传进 dataclass 时 T5/T6 会双双保持绿色。
- [ ] 4.8 T6c：断言 `0 < _DEFAULT_LOCK_TIMEOUT_MS < _DROP_TIMEOUT_MS`（spec 要求默认值
      本身满足同一严格边界）。
- [ ] 4.9 **T7（反空转，承重）**：用既有 `_install_fake_drop_psycopg2` /
...
- [ ] 4.10 T8（告警 wrapper 行为）：新建测试，用 fake sendmail 捕获 stdin。
...
- [ ] 4.11 把 `NODE27_TIMESERIES_RETENTION_LOCK_TIMEOUT_MS` 加进 H13 catalogue 断言
      `test_env_example_lists_all_h13_keys`（`tests/test_node27_timeseries_retention.py:2063-2076`），
      使 example ↔ 代码的耦合被钉住。

## 5. 失败可见性（systemd + wrapper）

- [ ] 5.1 新增 `infra/systemd/nhms-node27-unit-failure-alert@.service`（design D6）：
      ...
      `TimeoutStartSec=120`。
- [ ] 5.2 新增 `scripts/node27_unit_failure_alert_once.sh`（`set -euo pipefail`，`chmod +x`）：
...
- [ ] 5.3 `infra/systemd/nhms-node27-timeseries-retention.service` 的 `[Unit]` 段加
      `OnFailure=nhms-node27-unit-failure-alert@%n.service`。**只改这一个单元**。
- [ ] 5.4 `infra/env/node27-timeseries-retention.example` 增
      `#NODE27_TIMESERIES_RETENTION_LOCK_TIMEOUT_MS=240000`，注释写明上界断言与 D3 依据。

## 6. 文档

- [ ] 6.1 runbook §8.2（`docs/runbooks/tier-node27-timeseries-storage.md:2581-2594` 邻域）：
      ...
      本 change 无离线证据）。
- [ ] 6.2 §8.6 新增「被锁失败的判据与升级阈值」：单趟 `lock-contention(*)` refused =
...
- [ ] 6.3 §8.6 记 `OnFailure=` 告警入口（template unit 名、复用的邮件通道、部署步骤）。
- [ ] 6.4 **点名修正两处会因分类前缀而失准的句子**：
...
- [ ] 6.5 §8.6 的 per-chunk 计时说明（`:3019-3021` 此前声称存在、实际不存在的
...
- [ ] E1 本地 `uv run pytest -q tests/test_node27_timeseries_retention.py` 全绿，
      且收集用例数 **> 0**（防 `no tests ran` 的零断言冒烟）。
- [ ] E2 本地 `uv run ruff check .` 零告警。
- [ ] E3 本地 `openspec validate harden-node27-retention-lock-contention --strict --no-interactive`。
