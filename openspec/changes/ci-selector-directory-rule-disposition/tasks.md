...
- [ ] 1.1 Re-derive the 9-directory gap map at HEAD (inverted-index
      form); record the map in the PR body (design table is context
      only).
- [ ] 1.2 Measurement table: each addition-candidate gap suite run
...
- [ ] 2.1 Rule additions per design decisions 3-4: narrow per-module
      ...
      recorded.
- [ ] 2.2 `INTENTIONAL_RULE_GAP_EXCLUSIONS` table with tokens
      {fn-gated, redirect, edge-consumer, runtime-budget}; block
      comments carry rationale (+ measured number for
      runtime-budget).
- [ ] 2.3 Every one of the derived pairs dispositioned (guard-green
...
- [ ] 2.4 Positive-selection floor (design decision 4b): explicit
...
- [ ] 3.1 Disposition guard (inverted index, < 5 s): domain = every
...
- [ ] 3.2 Guard red evidence via injectable selection/exclusion
      seams — constructed rule-list/table inputs (parameter seams;
      zero tracked mutation).
- [ ] 3.3 Order-audit pins: narrow-rule targets provably present in
      `select_tests([module])` for modules coexisting with stop
      rules.
- [ ] 3.4 Domain note vs the PR #1486 closure guard recorded in the
      guard comment (direct importers here; one-hop only for
      GUARDED_MODULE_CLOSURES).

## 4. Spec delta

- [ ] 4.1 ADDED requirement (disposition guard, first-line SHALL,
...
- [ ] 5.2 Red evidence per task 3.2 (honest labels for
      naturally-green pins).
- [ ] 5.3 `git ls-files '*.py' | xargs uv run ruff check` clean.
- [ ] 5.4 `openspec validate ci-selector-directory-rule-disposition
...
- [ ] 5.6 Runtime accounting per design decision 6 (heaviest module
...
- [ ] 5.7 Issue #1455 Verification commands green.
