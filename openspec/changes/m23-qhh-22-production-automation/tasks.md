...
- Zero-canonical-rows-vs-incomplete distinction (scheduler wiring closure, branch `feat/m23-255-generic-canonical-ingestion`): the "incomplete -> submits no forcing/SHUD candidate" floor above applies to a cycle that **already has canonical rows** (`candidate_row_count > 0`) failing identity/variable/lead checks. A cycle with **zero** canonical rows and a non-empty expected horizon is a fresh ingestion: the generic daemon admits a full-chain cohort (no restart stage) so the Slurm chain runs download->convert->forcing->forecast->parse->frequency->publish. An empty expected horizon (`no_expected_leads`) or provider-unavailable readiness keeps the hard block and is never reclassified as fresh. See `issue-255-worklog.md`.
...
- [x] 7.6 Verify with `openspec validate m23-qhh-22-production-automation --strict --no-interactive`, focused backend tests, docs/static checks if present, and an opt-in node-22 live E2E command that reports PASS or BLOCKED with evidence under `artifacts/` or `/scratch/frd_muziyao`. (local: ruff + 16 passed + openspec valid; node-22 真库 5-file suite re-verify at e2a653b; opt-in live E2E via `NHMS_RUN_22_NODE_E2E=1` test_two_node_22_e2e.py)
