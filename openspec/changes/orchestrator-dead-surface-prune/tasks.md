...
- [x] `uv run pytest -q tests/test_select_ci_tests.py` green (new test file added)
...
- [x] 1.2 Drop the kwarg at the five direct callers (`:2397 :3690 :4052 :4215 :4903`) and at the `_materialize_latest_unlocked` routing arm (`:9951`); `_materialize_latest_unlocked`'s own parameter and routing unchanged, and its three callers at `:3771 :4111 :4323` keep `include_direct_jobs=False` verbatim (those kwargs are the materialiser's, not the reducer's)
...
- `_materialize_latest_unlocked`'s three callers keep `include_direct_jobs=False` (`:3775 :4115 :4327`); they are not #1661's dead surface.
