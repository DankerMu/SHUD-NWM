...
- [x] `uv run pytest -q tests/test_select_ci_tests.py` green (new test file added; the first head fc74f888 failed its directory-rule importer audit on CI and node-27 — seven `services/orchestrator/*.py -> tests/test_hydro_status_set_parity.py` pairs undispositioned — fixed in fix pass 1 by adding the suite to the `services/orchestrator/**` selector rule; the implementer's "460 passed" claim at fc74f888 was wrong)
