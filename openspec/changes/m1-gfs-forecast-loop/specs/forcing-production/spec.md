# Capability Spec: forcing-production

## Context

Forcing production is the final data preparation step before hydrological model execution. It reads canonical meteorological products, interpolates gridded data to model-specific meteorological station locations, and generates SHUD-readable forcing files. The system loads station definitions from `met.met_station` (bound to `basin_version_id`, NOT `model_id`; geometry via `geom` PostGIS Point SRID 4490, elevation via `elevation_m`). IDW interpolation weights are stored in `met.interp_weight` (keyed by `source_id`, `grid_id`, `model_id`, `station_id`, `variable`, `grid_cell_id`). The producer generates 6 SHUD forcing variables: PRCP, TEMP, RH, wind (= `sqrt(wind_u_10m² + wind_v_10m²)`), Rn (net radiation derived from shortwave_down), and Press. Output includes `.tsd.forc` files (SHUD binary-text format) and CSV debug files, stored at `forcing/{source}/{cycle_time}/{basin_version_id}/{model_id}/`. A `met.forcing_version` record (`forcing_version_id`, `model_id`, `source_id`, `cycle_time`, `start_time`, `end_time`, `station_count`, `forcing_package_uri`, `checksum`, `lineage_json`) with component lineage via `met.forcing_version_component` (`canonical_product_id`, `variable`, `valid_time_start`, `valid_time_end`, `role`) tracks provenance. Per-station per-variable per-timestep values are persisted in LONG form in `met.forcing_station_timeseries`. The CLI entry point is `nhms-forcing produce --source --cycle --model-id`.
