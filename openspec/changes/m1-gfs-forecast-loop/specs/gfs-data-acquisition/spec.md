# Capability Spec: gfs-data-acquisition

## Context

The NHMS system requires operational GFS (Global Forecast System) meteorological data as its primary forcing input. A GFS adapter implementing the `DataSourceAdapter` interface SHALL discover available forecast cycles, build download manifests, execute downloads, and verify integrity. GFS data is published by NOAA/NCEP in GRIB2 format across four daily cycles (00/06/12/18 UTC) with 7 required variables: tmp2m, apcp, rh2m, u10m, v10m, pressfc, dswrf. Raw files are stored at `raw/{source}/{cycle_time}/` in S3-compatible object storage. Metadata is written to the `met.data_source` (PK `source_id`) and `met.forecast_cycle` (PK `cycle_id`, FK `source_id`) database tables. Cycle status follows ENUM `met.cycle_status`. Errors are tracked via `retry_count`, `error_code`, and `error_message` columns on `met.forecast_cycle`.
