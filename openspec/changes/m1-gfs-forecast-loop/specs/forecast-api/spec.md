# Capability Spec: forecast-api

## Context

The NHMS M1 milestone exposes a FastAPI-based REST API under the `/api/v1/` prefix to serve forecast query results to the frontend and external consumers. The API covers river segment forecast time series, run management, data source/cycle metadata, and meteorological station queries. All responses use ISO 8601 UTC timestamps. Error responses follow a unified envelope with `request_id` and structured error codes. The forecast-series endpoint is the primary data contract between the backend and the MapLibre/ECharts frontend.
