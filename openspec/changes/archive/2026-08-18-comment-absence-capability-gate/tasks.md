...
- [x] 1.1 `services/orchestrator/reconcile.py`：新增
...
- [x] 1.2 `default_comment_sacct_querier` 加 `comment_storage_probe` 可注入参数
...
- [x] 1.3 `docs/runbooks/failed-basin-retry.md` 新增「comment_accounting_unproven
...
- [x] 1.4 既有 15 处实构 querier 用例注入 `comment_storage_probe=lambda: True`
...
- [x] 2.1 探针解析/日志向量：`AccountingStoreFlags    = (null)`（node-22 实测
...
- [x] 2.4 存 comment 集群零变化锁：probe True 下 owned_match 绑定与
      global_absence 过 grace 判 reservation_lost 与现状逐字一致
- [x] 2.5 既有 comment_query 可调用级 fake 用例与 1.4 迁移后的 15 处实构用例
      全绿
- [x] 2.6 抛点优先级断言：comment probe False + visibility False →
...
- [x] 3.1 红证记录：2.3 在改动前红（形状：reservation_lost 被判出，非 TypeError）
