...
- **显式定性（fixture review 两轮重裁终态：unsafe 时两腿弃权）**：
  ...
  预算内且无更高 rung claim 的瞬时失败保留自动重试；被 fail-open 遮蔽
  的 forecast-restart / permanent（含 remedy-permitted）/ cancelled /
  预算耗尽几何去遮蔽到既有梯子终态（见 design D2 终态列）。**这是对
  issue 解决思路「降级为带 reason 的
