...
- [x] Red proof (#1691): all 27 tests the gate turns green are red on pre-change source — the 15 non-reference shapes and the 5 wrapper-around-existing-ref cases qualify (DID NOT RAISE), the 5 wrapper-around-missing-ref cases escape with `BoundedEvidenceError` instead of `EvidenceError`, the `main()` pin returns 0, and the schema-description test fails; `27 failed` → `485 passed` after (test run output pasted in the implementer report; independently reproduced by the round-1 verifier on `git archive HEAD^`). After round 1 the `main()` pin was parametrized over all five slots (31 gate tests, module `489 passed`).
...
- [x] 2.2 Vault `scripts/loop_log_audit.py`: `rotation_attribution` skips only non-compliant catches (same definition) and returns the skipped count; a missing `lens` is no longer counted as rotated. `main` scans the catches of **all** entries (not only `multiround`), prints `NOTE non-compliant catches skipped: <n> in <k> entry(ies) (pr …)` when `n > 0`, and appends `skipped=<n>` to the rotation attribution line (the same whole-log `n` as the NOTE — every entry, while `core`/`rotated` span the multi-round subset). Tests: skipped surfaced with count and PR; `skipped=0` when compliant; an entry without a `round_lenses` key but with non-compliant catches is still reported; a `lens`-less `round: 2` catch is skipped, not rotated. Existing `core=8 rotated=8` assertion stays green.
