...
could not see the rows-6/7 channels at review time — neither contained
the `OUT_OF_MEMORY` literal or a direct `classify_failure` call,
reaching classification one level down through `_failure_policy_payload`
(`:84-109`). (Post-fix, the row-6 channel now carries the literal in
its refusal arm at `:917`; the row-7 pair remains literal-free.) Any future audit of a code's retry semantics must also
...
- db-free scheduler plane: `_failure_policy_payload` (`:84-109`) derives
  ...
  row-5 AND row-6 blocks, neither the downstream-resume overwrite nor
  the model-package-refresh channel rescues OOM — verdicts flip to
  `manual_retry_required` in BOTH geometries (durable output present or
  not), except the row-4 recompute channel and the pre-existing
  raw-manifest repair channels (D2 row 7 — post-change the sole
  remaining auto-retry sources for OOM in their geometry; kept with
  recorded tension, routed at tasks 5.0(c)).
...
- Missing-forecast-output recompute channel: unchanged (D2 row 4);
  raw-manifest repair channels: unchanged, kept with recorded tension
  (D2 row 7, tasks 5.0(c)).
