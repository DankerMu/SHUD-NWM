...
- [x] 2.1 completion verdict保留#1775 terminal-first顺序；仅在terminal-success且strict resolution ready并命名canonical state id时通过optional full accessor读取observed identity。得到mapping时override hydro evidence，method缺失/返回None时回退原 `terminal_evidence["hydro_run"]`；eligible foreign provider异常响亮传播。
...
- [x] 2.3 pre-change/no-identity cohort继续`absent`：successor ready→complete，successor absent/unready→gap；strict not-ready由#1775 closed allowlist分类且不读accessor，ready-but-no-state cold-start仍gap且不读accessor，candidate strict wrapper不变。
...
- [x] 3.3 production scheduler truth table覆盖cohort conflict/match/absent及successor三态；real accessor mapping而非仅fake hydro evidence；#1775交界sentinel分别证明strict not-ready与ready-but-no-state cold-start均不调用accessor，eligible ready+named-state provider异常仍传播。
...
- [x] 4.6 Scope/structure gate：前置#1799的 `scheduler_discovery_evidence.py`、owner-compat tests与 `.large-file-guard.json` 零diff；`scheduler_discovery.py` final≤1000行，且Child B diff只含cohort identity产品语义、fixture与要求驱动tests。
