...
- [x] T7-post node-27 post-merge cutover (§9.4, same session after merge): stop compression/retention/autopipe timers; b26post-check enumeration (relkind v/m = 0 at pre-merge); full-mode provision (ownership transfer + relacl diff + trailing strict audit); TEMP revoked from PUBLIC / re-granted to display; per-component run; per-unit env switch receipts; timers restarted; audit-only before/after every superuser write. Recorded in the post-merge PR. Done 2026-09-03 08:36-08:43 CST on node-27 at master 57ddc545 (receipts `/home/nwm/tmp/receipts-2328/{b26post,b26post-lanes,b26post-env}`): full-mode provision RC 0, sweep `163 scanned, 21 distinct, 0 untrusted`, audit OK with no owner drift; `datacl` after the run `{=c/nhms,nhms=CTc/nhms,nhms_display_ro=Tc/nhms}` (PUBLIC `TEMP` revoked, display re-granted); both write roles `has_temp` = f and `create_on_schemas` = `(none)`; `nhms_cold` absent on node-27, so the conditional tablespace CREATE grant was skipped. Per-component runs from a detached worktree with scratch env copies: autopipe `--limit 0` as `nhms_ingest_rw` rc 0 completed (stats guard `not_triggered`, authority leg completed); retention through its wrapper rc 0 but in ENFORCE mode with `dropped_chunks: []`; download through its wrapper as `nhms_download_rw` rc 0 (a real cycle, GFS/IFS `already_done`); the compression wrapper REFUSED at the sequential-budget preflight (`invalid lane env pair`), so there is no compression receipt under the new role (deviation 4); strict audit-only after the lanes rc 0. Env cutover download -> ingest -> compression -> retention with `*.env.pre-1774` backups (mode 600) and timers restarted; live units after the cutover: download Finished x2, autopipe Finished x2, `pg_stat_activity` sample `nhms_ingest_rw | nhms-autopipe | active`; the redacted env grep shows no `nhms:` DSN outside the compression-replay and archive-rebuild-drill exceptions.
...
4. **T7-post: no per-component compression receipt under the new role.** The
   compression wrapper refused at the sequential-budget preflight with
   `invalid lane env pair` because `infra/env/node27-cold-residency.env` does
   not exist on node-27. The refusal is pre-existing and role-independent — the
   live unit failed identically as `nhms` on 2026-09-02 12:25 CST, before any
   cutover — so it is reported, not fixed here (#1894 tracks the cold-residency
   env/install); ownership is now `nhms_ingest_rw`, so `compress_chunk` runs as
   the owner once that install lands. Two further lane notes from the same
   session: the retention wrapper ran in **ENFORCE** mode despite `--dry-run`
   because the live `node27-timeseries-retention.env` sets
   `NODE27_TIMESERIES_RETENTION_ENFORCE=1`, which the H13 toggle lets override
   the flag (`dropped_chunks: []` — nothing was dropped); and the download
   wrapper has no dry-run mode, so its receipt is a real download cycle
   (GFS/IFS `already_done`).
