...
- The cross-host exclusion is proven by receipt only; CI and node-27 tests
  ...
  current NFS mount options (`vers=4.2`, `local_lock=none`); a mount with
  `local_lock=flock` or `local_lock=all` makes node-22's `flock` local and
  silently removes the exclusion.
