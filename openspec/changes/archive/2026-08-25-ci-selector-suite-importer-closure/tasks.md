...
- [x] 2.2 Add a recursively derived live-tree guard covering every ordinary owner/importer edge; its effective redirect predicate is shared with production, and one shared offender helper owns the live/constructed recursive domain, dotted-identity authority, default redirect predicate, redirect-skip branch, comparison, and missing-importer message so top-level-only, empty-authority, always-skip, no-skip, and comparator regressions all make a constructed proof red.
- [x] 2.3 Pin all three supported module-scope forms (`import tests.test_owner`, `from tests.test_owner import helper`, and `from tests import test_owner`), one-hop-only behavior, function-local import exclusion, file-level gating exclusion, nested `test_*.py` importers, and nested `*_test.py` importers.
...
- [x] 3.2 `uv run pytest -q tests/test_select_ci_tests.py`: 352 passed in 172.75 seconds after the Round 2 depth-retro comparator and redirect-branch closure on the final local head.
