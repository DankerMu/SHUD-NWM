...
- [x] T6 node-27 (queued session; receipts on 483be5fb 2026-09-02 23:4xZ, `/home/nwm/tmp/receipts-2328/b27pre`, quoted in the PR receipt comment — measured: count `status='parsed' AND parsed_at IS NULL` = 0; old form Seq Scans over compressed chunks vs new form Bitmap Heap Scan on `hydro_run` via `hydro_run_display_product_basin_status_idx` + `Filter: parsed_at IS NOT NULL`; ANALYZE/BUFFERS rows=0, `shared hit=25` (index 8) from 76 dead `status=parsed` index entries, 0.633 ms — #1779 AC2's literal single-digit buffers NOT met because of pre-existing index bloat, recorded as measured; 19 passed across the three shared-seed suites; backfill dry-run `outcome=clean`, `bounds.lock_timeout_ms=5000`, no `lock_contention`): RO `EXPLAIN (COSTS OFF)` of the old EXISTS form and the new form, plus `EXPLAIN (ANALYZE, BUFFERS)` of the new form showing the parsed=0 short-circuit stays at single-digit shared buffers (#1779 AC2) (plans archived under the PR); count query; integration-marked run of the three shared-seed suites; backfill dry-run (non-enforce) from a detached worktree with a scratch receipt path, `bounds.lock_timeout_ms` visible, no `lock_contention`.
- [x] T7 Comment on #1686 (posted 2026-09-02 from PR #1960): publish site closed, copyback pushdown still untriaged.
