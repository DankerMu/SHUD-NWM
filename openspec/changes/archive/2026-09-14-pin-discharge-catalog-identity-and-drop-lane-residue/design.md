...
- A bootstrap catalog **without** a discharge entry (the runless `/api/v1/layers` returns `[]` when no run is display-ready at that instant, while a run-scoped fetch moments later can carry discharge; the backend catalog cache is keyed per `run_id`) keeps today's behaviour: the scoped entry is used, otherwise the whole generation would have no discharge layer. Known residual, pre-existing and not fixed here (same class as #2140, reachable only at first-run-becomes-ready): in that window phase 3 resolved no pair and sent no layer-time request, so (a) without a URL `cycle` the precipitation overlay can sit in `index_pending`, and (b) with a URL `cycle` different from the scoped default the discharge layer itself shows the pending reason with no request in flight. Both last until the next load after the runless `[]` expires from the 60 s frontend cache.
