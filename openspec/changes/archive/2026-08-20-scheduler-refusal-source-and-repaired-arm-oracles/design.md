...
| `global` 改绑一个 AST 记为 **`def`** 的名字 | （未测，隐含红）| **绿——穿过** | 红（交叉核对由名字平集升级成 kind 感知：`def` 名运行时须**可调用**、`class` 名须是 `type`、53 个 import 名**豁免**；HEAD 上零假红）**注**：round-1 交付时钉的是 `FunctionType`，round-2 因假红改钉可调用性（见下 (c) 段）；HEAD 上 59 个 `def` 确实全是 `FunctionType`，那条测量为真，但**不是交付实现的判据**。|
...
**声明的摩擦**：任何**正当**新增模块级常量的改动都要同步更新期望映射。
**同样打红的还有一格，Phase 7 终审 F3 补**：**纯重命名一个消费函数**（不动任何常量）
也会打红——映射的值集合是函数名集合，改名即变（实测 `1 failed, 112 passed`）。
方向是假红不是假绿，失败信息直接显示映射 diff、修复成本一行，但先前三处摩擦声明
都只写了「新增或重命名**常量**」，撞上的人在文本里查不到这一格。这是**有意**的：
...
M10 是**反向**用例：它必须绿。现行字符串扫描在 M10 下是**假绿**源
（任何非逐字节的复发写法都能穿过它），这正是 #1418 要治的第二个病。
**假红方向不可达，这里不得写成「双向」**（Phase 7 终审 F1）：两条被钉的字面量自
`d53cff4a`——就是引入该扫描的那个 commit——起就没在本模块出现过（`grep -c` 均为 0），
`ruff format` 造不出不存在的文本，实测跑完格式化后旧扫描三条断言仍成立。
本 PR round-2 已把 PR body 与测试 docstring 里的「双向」改掉，**这一处漏改**，
是同一条更正漏掉一个承载点——本役该形状的又一次发作。
