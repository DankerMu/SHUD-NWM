...
- [x] T3 Exit contract 0/1/2/3 exactly as design D6, including **zero source keys → exit 3**, with DSN redaction on every operator-visible string.
  - 修正 2026-09-18（#2465）: the shipped exit contract is *not* D6-as-written on one row — an import-time display-module failure exits **2** (config stage), not 3. The code is the deliberate one; D6's row was the drift. See the appended correction note at the end of `design.md`.
