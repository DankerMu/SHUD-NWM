...
9. **Pin the HTTP protocol implementation deterministically.** The module entrypoint calls programmatic `uvicorn.run(..., http="h11")` so the transport cannot drift onto uvicorn's `auto` default, which selects the optional native `httptools` when importable. On the node-22 maintained active Python 3.12.7 environment that optional httptools is live-proven broken (`AttributeError: module 'httptools.parser' has no attribute '__all__'`; gateway exits 1 before binding), and `UVICORN_HTTP=h11` does NOT affect programmatic `uvicorn.run` (live red repro exits 1 under the same environment). This is a release/dependency compatibility pin — no dependency change, no fallback/try-except, and it is NOT a maintenance-window `uv sync` authorization; the maintained active environment stays untouched until its own window.
