...
`chain_repository_state.py:359-403` 循环改动：

- 删原 `:400`（终态已不存在）的 `break`；`repaired_by_failed_job_id` / `repair_events` 的写入改为 **key 已存在则跳过**（`if failed_job_id not in repaired_by_failed_job_id`）。外层 `sorted(..., reverse=True)` 已按 truth key 倒序，first-write-wins == newest-wins：同一 failed 行被多个 retry job 认领时保留最新那次。
...
`chain_repository_state.py:883-916` 结构改动（推荐方案「拆开互斥」，issue 两方案之首选）：
...
| gap：source-cycle 变体（恒无 restart_stage） | `repaired_stage_evidence` 落 state 但 completed-stage 投影被 elif 抑制 | `repaired_stage_evidence` 保留 **且** scan 照跑——两证据共存，`restart_stage` 来自候选自身完成事实 |
| gap 子形状：manual-stage 终段 stage（`_stage_after` 为 None） | 同上被 elif 抑制 | 抑制**改由 terminal-completion 守卫承担**：`_FORECAST_STAGE_ORDER` 内 `_stage_after` 为 None 的只有 `state_save_qc`，而产生该修复的成功 retry 行本身即命中 `_has_terminal_completion_stage_success`，故 scan 恒被短路、`completed_stage_evidence` 不出现（round-1 V7 实证；E10 钉） |
...
- **不新增 Batch S 行扫描臂可达性，论证是单调性**（fixture review P2-1 修正后的口径）：D2 只会**新增**非空
  ...
  第三位读它——多一个非空显式键只会让行扫描更**不**可达。注意 `chain_repository_state.py:825-830`
  分支（`failed_task is None` 且候选零行、仅剩 repaired 证据）下 `stage`/`failed_stage` 本来就是
  None，不能以「五键未清空故非 None」为据。
- **D2 新开几何（须 must-preserve 钉）**：`:825` 分支 + gap 形状 + cycle 内有 succeeded
  ...
  （不做对照组等式：flat `retry_count` 因 `:832-834` fallback 天然与无 source-cycle 对照组不等，
  非本 change 引入，见 tasks 2.11）。

## Must-preserve（评审红线）

1. 单次修复既有场景零行为变化（`tests/test_production_scheduler.py` 中既有的
   `test_projected_repaired_cycle_download_target_refuses_the_pin` /
   `test_projected_unrepaired_cancelled_cycle_download_target_still_pins` /
   `test_projected_repaired_cancelled_candidate_row_reopens_the_only_failure_left_arm` 等
   `_candidate_manual_stage_repair_state` 相关用例全绿。按名索引而非行号——本 change round-1 的
   V6 正是行号漂移立的案）。
...
7. `:817-819` 两变体取值优先级（source-cycle 先于 manual-stage）不动。
8. **gap 形状新组合「repaired 证据在场 + 带 `job_id` 的扫描产 completed 证据」的四个消费面**（fixture review P1-2）：
   - `scheduler_state_manual_retry.py` 的 `_state_completed_stage_evidence_names_job`（定义 `:226`，
     被 #1308 row-absent pin gate 在 `:428` 调用）——**helper 级**判定对 gap 形状 state 须与对照组
     （无 source-cycle 修复行）同值（E11 钉）。gate 整体
     （`_unresolvable_marker_entity_pins_attempt`，`:325` 起，函数体 `:416-437`）对被 repaired 证据
     命名的 entity_id 有 `:426-427` 的先于本 change 的不对称短路，不做 gate 级等式（fixture review
     P2-4）。gate 算法本身仍 out of scope，但其**输入形状**变化由本 change 负责。
     （行号为本 PR 终态；round-1 F5/F6 后已重钉。）
   ...
   - `scheduler_state_manual_retry.py` 两处 docstring（`_state_completed_stage_evidence_names_job` 的 `:227-259`、pin gate 的 `:326-415`）的「repaired-copy 无 `job_id`」陈述在 D2 后不再穷尽，须随实现更新（tasks 3.3）。
9. `:825` 分支 + gap + cohort succeeded 行的新几何（上文 D2 新开几何，E13 钉）。
...
E1-E4 对应 #1460 AC 四项，E5-E9 对应 #1461 AC 五项，E10 钉 manual-stage 终段 gap 形状，E11-E13 钉 must-preserve 8/9 的下游消费面与 `:825` 新几何；红先行腿 E1/E5（master 形状必红），其余构造性/回归性两侧同绿。
