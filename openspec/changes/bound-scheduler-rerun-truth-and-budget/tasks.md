...
- [x] Schema / columns / field names — not selected: the D3 mint floor rides only on the in-memory retry decision evidence (`state_evidence`), never as a new run-manifest top-level key. `schemas/run_manifest.schema.json` has root `additionalProperties: false`, and 3.2 asserts the manifest still validates.
...
- [ ] 4.4 `openspec validate bound-scheduler-rerun-truth-and-budget --strict --no-interactive`
