...
- #2234 probe numbers drifted since filing: selector-source diff now also selects `tests/test_river_segment_write_surface_scan.py` (#2185). Spec text states inclusion facts (no counts, no dates) so future routing growth does not re-drift it; observed counts live only in 2.2.
...
- [x] 1.3 #2047 run_status carriers (`docs/spec/03_database_design.md`, `docs/appendices/C_database_schema_draft.md`, `docs/spec/01_architecture_and_flow.md`, `docs/runbooks/forcing-copyback-backfill.md`) = ledger values in migration order (`pending` between `staged` and `submitted`), plus retired-but-live `frequency_done` note; `docs/modules/14_tile_publication_service_{spec,design}.md` §6.1 state the live q_down publish predicate/product from `services/tile_publisher/publisher.py` (`publish_cycle` → `publish_qdown_cycle` → `_discover_qdown_runs`: `status IN ('succeeded', 'parsed', 'published')` AND `variable_e = 'q_down'`; `_upsert_qdown_layer`) and mark the flood-return-period GeoJSON text retired in `b97c16e2`; architecture doc stage count = `STAGE_NAMES` length; `met.cycle_status` `complete` placed explicitly as a different enum; runbook SQL valid on fresh DB.
...
- [x] 2.1 #2059 `uv run pytest -q tests/test_select_ci_tests.py` (full file) → pass. RED: deleting `NHMS_RUN_INTEGRATION=1`, and separately `NHMS_INTEGRATION_DATABASE_URL`, from the fence makes the meta-test fail each time; re-adding `export DATABASE_URL=...` (pre-fix form) also fails it. Probe (no DB; evidence is the return value of `tests/conftest.py::_integration_skip_reason()`, not per-node collection markers): with only `NHMS_RUN_REAL_BASINS_IMPORT=1 DATABASE_URL=x` it returns the integration skip reason; with the three canonical vars it returns None.
- [x] 2.2 #2234 probe (`select_tests` + `_collection_smoke_required`, `meta_guard_only` = selection equals `[SELECTOR_META_GUARD_TEST]`): `["scripts/select_ci_tests.py"]` → selection includes `tests/test_select_ci_tests.py` plus supplemental invariant suites (`tests/test_river_segment_write_surface_scan.py`, `tests/test_timescale_write_guard_wire_site_invariant.py`), `meta_guard_only=false`, `collection_smoke_required=true`; `["tests/test_select_ci_tests.py"]` → selection is exactly the meta-guard suite, `meta_guard_only=true`, `collection_smoke_required=true`; `["packages/common/node27_container_contract.py"]` → selection includes the contract's dependent closure and every `CORE_SMOKE_TESTS` member, `collection_smoke_required=false` (observed at 27e34b1: 3 / 1 / 16 targets, core-smoke 5 of 5). The spec delta states only these inclusion facts; full `tests/test_select_ci_tests.py` pass (2.1).
...
- [x] 2.5 #2155 `grep -n "_backfill_output_segment_geometry\|_ensure_output_river_segments\|_output_river_segment_rows" openspec/changes/fix-batch2-docs-contract-drift/specs/basins-registry-import/spec.md` → no SHALL-remove context; remaining SHALL-remove tokens `git grep` over repo excluding exactly `openspec/**` and `db/migrations/000037_river_segment_multilinestring.sql` (historical header comment, which still names `gap_split_multilinestring`, `splitPositionsAtGaps`, `backfill_river_segment_multilinestring`) → 0 hits, output pasted; kept requirement already states reach-row source, geom/length_m/Type copy, `only_missing`, `geometry_generation` bump.
