...
- [ ] 1.1 Add a non-raising per-entry normalize-or-skip wrapper (reusing
...
- [ ] 1.2 Pass `forcing_uri` (`manifest["forcing"].get("forcing_uri")`) and
...
- [ ] 1.3 Cover both declaration sources: package-manifest lane and
      run-manifest `_forcing_checksum_entries` fallback lane.
- [ ] 1.4 Update the anchor docstring: shape tolerance, parser reuse,
      skip-not-raise contract.

## 2. Tests (tests/test_shud_runtime.py)

- [ ] 2.1 Dot-prefixed legacy declaration + dual members on disk → legacy
      staged (package-manifest lane).
- [ ] 2.2 Uri-only legacy declaration + dual members → legacy staged
      (package-manifest lane).
- [ ] 2.3 Run-manifest fallback lane: dot-prefixed and uri-only shapes →
      legacy staged.
- [ ] 2.4 Invalid/underivable entry → skipped, staging completes
      canonical-first, no raise.
- [ ] 2.5 Both accepted members declared (mixed shapes) → anchor None →
...
- [ ] 2.6 Existing staging-level regression set untouched and green:
...
- [ ] 4.1 `uv run pytest -q tests/test_shud_runtime.py` green (all new +
      existing).
- [ ] 4.2 Red evidence: new shape tests fail against unmodified source
      (stash-based or pre-implementation run).
- [ ] 4.3 `uv run ruff check .` passes (per issue Verification field).
- [ ] 4.4 `openspec validate manifest-anchor-shape-normalization --strict
      --no-interactive` passes.
- [ ] 4.5 Zero modifications to existing test assertions; DG lane and
