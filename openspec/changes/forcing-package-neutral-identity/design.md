...
2. **staging 文件系统探测**(`_stage_standard_shud_forcing`,:974;**round-1 A1/A2 修正**):双名探测;**歧义 raise 仅限 direct-grid**(`DIRECT_GRID_FORCING_INDEX_AMBIGUOUS`,磁盘层独立裁决——DG staging 本是 manifest 白名单制,双文件意味着更深的污染)。**非 direct-grid 是全前缀递归拷贝**(:1679-1693 无白名单),producer 前缀确定且从不清理(:1970,零 delete 调用),原地再生产会留下孤儿 legacy 成员——双文件在非 DG 属**合法稳态**,按 **manifest 锚定选择**(**round-2 R1/S1 修正锚源**):声明源是**校验和已验证的 package manifest**——`forcing_context.package_manifest["files"]`(下载时经 `_verify_package_manifest` 校验),**不是** run-manifest 的 `forcing.files`(生产装配器 `chain_manifest_contracts.py:15-35` 只发 uri/checksum 字段、从不发 files;仅诊断脚本 `create_qhh_shud_manifest.py:135` 写它)。回退链:package manifest `files` → run-manifest `forcing.files`(诊断 lane)→ canonical 优先兜底(manifest 与 canonical 同笔写入 :2100-2105,manifest-current == 最后一次 produce,legacy-current manifest 不可能伴随新写 canonical;manifest 锚定同时关闭 producer 降级回滚留下 stale canonical 的残洞——盲目 canonical 优先关不掉)。恰一 → 用之;声明**双名**(非 DG)→ canonical 优先选择——**有意与 DG 的恰一 fail-closed 门相异**(round-2 S5:非 DG 是容忍 lane,残余双名属合法稳态,fail-closed 会砖死本可运行的 run);零 → 返回 None(direct-grid 由 :941-946 现有 `DIRECT_GRID_STANDARD_SHUD_FORCING_MISSING` fail-closed,非 direct-grid 的 legacy internal-forcing 回退现状保留)。`_prepare_shud_project_forcing`(:927-940)把上述回退链解出的 checksum entries 传入 staging 调用;**禁用** `_authoritative_package_manifest_checksum_entries`(其在 package_manifest 缺失时抛错,会把非 DG lane 新引入 fail-closed)。
