...
- [ ] 1.1 `has_completed_pipeline`（`file_orchestration_journal.py:558-563`
      合取项）追加 `and not _is_foreign_model_cycle_scope_job(...)`
      （design D1；复用 #1288 谓词，禁复制口径、禁动共享谓词
      `_job_matches_candidate`）。
- [ ] 1.2 改写投影处口径注释（`file_orchestration_journal.py:721-729`）：
...
- [ ] 2.2 终态口径开关形：`NHMS_ORCHESTRATOR_TERMINAL_STAGE=forecast_state_save_qc`
...
- [ ] 2.7 不回归（相邻 gate 面）：同一 fixture 下 `has_active_pipeline` /
...
- [ ] 3.1 `uv run pytest -q tests/test_file_orchestration_journal.py` 通过。
- [ ] 3.2 `uv run pytest -q tests/test_production_scheduler.py
      tests/test_orchestration_chain.py` 通过。
- [ ] 3.3 `uv run ruff check .` 通过。
- [ ] 3.4 spec-compliance 人工证据：delta 契约句 + 新 scenario 与最终实现
