...
- [ ] 2.1 Define the Basins package manifest schema with source path, normalized IDs, required files, GIS sidecars, calibration metadata, forcing metadata, per-file checksums, and package checksum.
- [ ] 2.2 Add explicit `nhms-model publish-basins` command contract and tests for local object-store publication.
- [ ] 2.3 Implement object-store publication for runtime SHUD input packages and selected calibration/GIS metadata, returning stable `model_package_uri` values.
- [ ] 2.4 Keep bulk historical forcing CSVs out of the runtime package by default while recording forcing count, sample header coverage, and aggregate checksum metadata.
- [ ] 2.5 Add explicit historical forcing copy option that writes forcing payloads to a separate object-store prefix and records URI/checksum evidence.
- [ ] 2.6 Make publication idempotent when source file checksums and target version are unchanged, and reject checksum changes for the same version without an explicit new version or force option.
- [ ] 2.7 Implement `nhms-model basins-migration-report` so production evidence fails for symlink targets and passes for real copied directories with file count, byte count, and inventory checksum.
