...
- Must preserve: 有行 mismatch / lineage missing 的既有 reason（`tests/test_canonical_converter.py:512,542,570,599`、`tests/test_gfs_adapter.py:951`、`tests/test_ifs_adapter.py:1186`、`tests/test_orchestration_chain.py:9441,9475`）；`_canonical_evidence_is_fresh_zero_row` 判定与 fresh-zero-row 准入（`scheduler_candidates.py:914-932`）；`status`/`ready`/各计数/`missing_*` 字段
...
- [ ] 1.1 `converter.py`：`identity_mismatch` 在零候选行时恒 False（有行时谓词不变）
- [ ] 1.2 `tests/test_canonical_converter.py` 新增：`products=[]` + policy/object identity + `forecast_hours=(0,3)` → `ready False`、`status canonical_incomplete`、`candidate_row_count 0`、`identity_rejected_row_count 0`、`reason == "missing_canonical_variables"`；改前源码上跑红（红证据随报告）
- [ ] 1.3 同一 evidence 经 `_canonical_evidence_is_fresh_zero_row` 判 True（在 `tests/test_production_scheduler.py` 或 converter 测试内直接断言）；`_fresh_zero_row_readiness_provider` 驱动的既有用例补 reason 断言
- [ ] 1.4 `docs/runbooks/scheduler-dbfree-typed-reasons.md` 新增 `canonical_identity_mismatch` 一节：含义（有行且 identity 对不上）、零行新鲜周期报 `missing_canonical_variables`、现场区分（看 `candidate_row_count` / `identity_rejected_row_count`）、2026090312 旧 index 的冻结假信号不回写
