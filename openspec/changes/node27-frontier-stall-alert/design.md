...
- **告警条件**：`now - last_change_at >= stall_hours（默认 4h）`。阈值依据：全趟 2h16m-3h01m + 落库滞后 15-30min，上界 ~3h30m 留余量（issue 实测口径）。**阈值须大于生产 cycle 节奏**：4h 适配回填期（日多 cycle）；稳态单日 cycle 下 node-27 生产值定为 48h（operator 裁定 2026-08-14，env 侧配置，runbook §10.3 记录），模板/代码默认不动。
...
- **round-5 裁决收编（#1373）**：TLS 必须验证（`ssl.create_default_context()`——`SMTP_SSL` 缺省 context 实测 `CERT_NONE`，未验证隧道使授权码可截获、250 可伪造，"目的方同步 250"承诺失效）；lane `default_sendmail_runner` 成功分支必须把 shim stderr 证据行带进 outbox 记录（否则 `SMTP-ACCEPTED` 生产路径不可观测，receipt 无法与空路由时代区分——这是 lane 侧唯一允许的触碰点）；From↔认证账号 addr-spec 一致性 config 期 fail-closed；8-bit 正文经 `8BITMIME` 协商或显式 CTE，绝不裸推；多收件人部分拒收风暴入文档警示（69-over-0 语义不改）。
