...
- [ ] 1.3 runbook §8.8 收窄（D5）：`..._FAILED` = 纯 pre-commit；
...
- [ ] 2.1 I1 主锚（红-绿）：注入 destination CAS 在 `os.replace` 之后失败
...
- [ ] 2.2 I2：`provider_postread_failed` 一例。**推荐 bootstrap 形**
...
- [ ] 3.1 `uv run pytest -q tests/test_run_tree_copyback.py` 通过。
- [ ] 3.2 `uv run ruff check .` 通过。
- [ ] 3.3 `uv run npx markdownlint-cli2 docs/runbooks/current-production-ops.md`
      通过；runbook 判读口径与实现逐句对读。
- [ ] 3.4 `openspec validate copyback-cas-uncertain-classification --strict
      --no-interactive` 通过。
