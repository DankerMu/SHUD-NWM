...
- [ ] 1.1 新模块 `services/orchestrator/retention_frontier.py`（design D1）：
...
- [ ] 1.2 `cli.py` `_run_cleanup` 接入（design D2/D3）：ok ⇒ 透传
...
- [ ] 1.3 `scripts/node27_raw_retention.py`（design D5）：`enabled`/`dry_run`
...
- [ ] 1.4 env 模板：`infra/env/compute.example` 注释新增
...
- [ ] 2.1 `tests/test_retention_frontier.py`：D3 判定表逐格——ok(bound)/
      ...
      N2）。
- [ ] 2.2 I1 主锚（红-绿）：追赶场景——evidence dir 放新鲜 receipt（bound
...
- [ ] 2.3 I2 主锚（红-绿）：无 receipt（或过旧）+ `--execute` → 强制
      dry-run、目标目录仍在、receipt 顶层 `frontier_blocker.reason` 正确
      （接线前红：无保护真删）。
- [ ] 2.4 I3：新鲜 receipt bound=null → 纯墙钟删除照旧（镜像 pass，不比
...
- [ ] 2.5 I4：click 入口与 argparse 入口各至少 1 条端到端（AC3，当前
      `grep _run_cleanup tests` 为空）；两入口对同一夹具行为一致。
- [ ] 2.6 I5 回归：`tests/test_retention.py` 全绿；`run_retention` 既有
      调用面（scheduler pass）零改动（diff 断言级：pass 内文件不动）。
- [ ] 2.7 I6/I7：`tests/test_node27_raw_retention.py`——默认 env 下行为
...
- [ ] 3.3 `openspec validate retention-frontier-out-of-pass --strict
      --no-interactive` 通过。
