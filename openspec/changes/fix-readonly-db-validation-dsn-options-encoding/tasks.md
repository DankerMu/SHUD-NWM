...
- [ ] 1.1 In `_bounded_database_url` (`services/production_closure/readonly_db_route_smoke.py:320-348`) build
...
- [ ] 1.2 Add a unit test that **can go red on the current code**. In
...
- [ ] 1.3 Add a real-PostgreSQL integration test (`pytestmark = pytest.mark.integration`, fixture
...
- [ ] 3.1 `openspec validate fix-readonly-db-validation-dsn-options-encoding --strict --no-interactive` PASS.
- [ ] 3.2 `uv run ruff check .` PASS.
- [ ] 3.3 `uv run pytest -q tests/test_readonly_db_validation_routes.py tests/test_readonly_db_validation.py
  tests/test_readonly_db_validation_probes.py` PASS, including the new case.
- [ ] 3.4 Real-database lane: 1.3's positive run passes and its recorded negative run fails with the FATAL.
- [ ] 3.5 node-27 canonical rerun, read-only: with `NHMS_DISPLAY_READONLY_DATABASE_URL` built in memory from
