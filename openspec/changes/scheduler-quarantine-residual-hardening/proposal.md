...
- env=true strict comparator 逻辑；`active_duplicate_pipeline`；§8/§8.6 predecessor-backfill emitter（#1152 已交付）；journal 写入/删除（§8.7 read-only invariant 必须保持）；cadence 配置本身的变更；PG provider 的 lineage 过滤（design D5 记录理由）；breaker N 的可配置化（YAGNI，常量 N=2）。
