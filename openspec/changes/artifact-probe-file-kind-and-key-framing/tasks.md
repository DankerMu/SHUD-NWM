...
- [ ] 1.1 抽出模块级**纯字符串**归一化函数（prefix 路径段剥离 + `s3://` 支的 unquote），
...
- [ ] 1.2 新增**四态**种类分类器方法（`file|directory|other|absent`），
...
- [ ] 1.3 `exists` 本体**零改动**（diff 中不得出现该方法的任何行）。

## 2. 探针层与消费腿（`services/orchestrator/`）

- [ ] 2.1 在 **`scheduler_state_common.py`**（不是 `scheduler_state_failure.py`——后者 `:19`
...
- [ ] 2.2 新增 sibling 探针，**仅当 `_object_manifest_is_missing` 返回 False 时**被咨询（`:1201`）；
      **只许加一个正向裁决**——absent、不可解析、以及 sibling 的任何其他结局，
      都必须让 `(False, None)` **逐字不变**（design.md D1 约束 2 的加严版）。
      sibling 不入 compat re-export 名单。
- [ ] 2.3 local 腿 `:1231` 在 `path.exists()` 之上加**同一 reason** 的种类判定；symlink 口径不动（D3）。
- [ ] 2.4 `_needs_package_manifest_witness` 改签名为 `(candidate, value)`，**先归一化再问 validator**；
...
- [ ] 2.5 sidecar 腿（`:644-700`）**不改代码**，但确认新 token 落在 `:684` 且 `unsafe_reason`
      随 `:695` 带出（design.md D6 的实现期确认项；若实测不符则回到 D6 重裁）。
- [ ] 2.6 扩 `tests/test_production_scheduler.py:25522` 的 `_RAW_MANIFEST_ABSTENTION_REASONS`
...
- [ ] 3.1 object 腿目录用例：URI **深于 pattern 段数** + root **已配置** + 目录连**整条父链**建
      （`mkdir -p`，否则 O8 抢先）+ root **先 realpath**（否则 macOS `/var` 使 O4/O9 抢先），
      断言 `(True, "artifact_target_not_a_file")`。
- [ ] 3.2 local 腿目录用例：目录坐在**允许收容根之内**（绕开 L5 抢先），断言同一元组。
- [ ] 3.3 seam 契约用例（D1 约束 2 的守门人）：monkeypatch
...
- [ ] 3.4 #1397 回归：`OBJECT_STORE_PREFIX` **带路径段**、forcing FILE key **物理存在**，
      断言分类器答 False 且探针答 `(False, None)`（今日会答 `artifact_probe_error`）。
- [ ] 3.5 裸桶前缀 + 无 percent-encoding 下，分类器答案与改动前**逐字一致**（must-preserve）。
- [ ] 3.6 桶不匹配残留（`s3://other-bucket/...`）：路由与今日一致（D4 末段）。
- [ ] 3.7 symlink-to-file 在 local 腿仍为 present（D3 分歧钉），注释引 **design.md D3**（**无 issue 号**）。
- [ ] 3.8 扰动纯归一化函数时 `normalize_key` **随之变动**的等价性用例（M5 的真实 oracle）。
- [ ] 3.9 #1365 既有目录形状 witness 推导在带路径段 prefix 下仍成立。
- [ ] 3.10 **sidecar 层终局（#1394 AC-2，初稿遗漏）**：派生见证 key
...
- [ ] 3.11 两条 raw-manifest 腿（`:1519`/`:1587`）在新 token 下**abstain**
      （`:1526`/`:1594`），与 2.6 的三项元组一致。
- [ ] 3.12 `normalize_key` 裸 key 支**不 unquote** 的守门用例（1.1 的 must-preserve）。
...
- [ ] 4.1 `specs/job-retry-mechanism/spec.md` delta：拆分"会抛的 probe fault"与"存在但非常规文件"、
...
- [ ] 4.2 `openspec validate artifact-probe-file-kind-and-key-framing --strict --no-interactive` 通过。
