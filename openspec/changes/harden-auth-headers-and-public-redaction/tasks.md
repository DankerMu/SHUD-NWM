...
- [ ] 1.1 Delete `_require_hindcast_access_role`, `_HINDCAST_ACCESS_ROLES`, and the call in
  `get_forecast_series` (`apps/api/routes/forecast.py`). Remove now-unused imports only.
- [ ] 1.2 HTTP tests in `tests/test_forecast_api.py` (store dependency overridden with a recording stub):
...
- [ ] 1.3 Evidence: `rg -n 'X-User-Role' apps packages services --glob '*.py'` shows reads only in
...
- [ ] 2.1 `packages/common/request_auth.py::_internal_live_proof_token_matches`: encode header and token as
  ASCII bytes, `UnicodeEncodeError` → `False`, then `hmac.compare_digest`.
- [ ] 2.2 Delete unreferenced helpers `apps/api/auth.py:143-183` and any symbol ruff then reports unused.
- [ ] 2.3 Route seam `POST /api/v1/runs/{run_id}/cancel` in `tests/test_monitoring_api.py` (same fixture as
...
- [ ] 2.4 Evidence: `rg -n 'Live-Proof.*==|== *configured_token' apps packages/common --glob '*.py'` → no matches.

## 3. #2038 lifecycle/active projection drops mesh_properties_json

- [ ] 3.1 `packages/common/model_registry.py::_model_public_projection` pops `mesh_properties_json`.
- [ ] 3.2 Remove the field (and asymmetry docstring) from `apps/api/openapi_restored_schemas.py`;
...
- [ ] 3.3 Regression (R6/R7): a projection-level test feeding a row whose `mesh_properties_json` carries
...
- [ ] 3.4 R8 unchanged: preflight lineage redaction assertion and GET detail/list tests stay green.
...
- [ ] 4.1 `apps/api/routes/pipeline.py`: retry 503 `error_message`, `_basin_result.error_message`,
...
- [ ] 4.2 Route test (R9) in `tests/test_retry.py`: DB lane, gateway raises
  ...
  assertions green.
- [ ] 4.3 R10: the same gateway text through the file lane's retry 503 yields the identical
  `error_message` `sbatch: error: cannot open [local-path] for writing` (assert equality to the literal on both lanes).
- [ ] 4.4 R11: persisted DB-lane event `message` / `details.error_message` keep raw text (existing
  assertions stay green).
- [ ] 4.5 R12: `GET /api/v1/jobs` with a job whose `error_message` is
...
- [ ] 4.6 R12b: `GET /api/v1/pipeline/stages` (`_stage_summaries` → `_basin_result`) with a stage job carrying the
...
- [ ] 5.1 `services/orchestrator/public_evidence.py::_sanitize_public_path_or_uri_scalar`: anchored
...
- [ ] 5.2 Rewrite pinned asserts in `tests/test_file_orchestration_journal.py` (`"s3://bucket/my key"` →
...
- [ ] 5.3 R14: prose assertion stays; implementer temporarily applies the "`://` anywhere → whole" mutant
...
- [ ] 5.4 R15: literal fixtures `workspace_dir="file:///srv/nhms data/ws"`,
...
- [ ] 5.5 R16: parity corpus adds spaced URIs; parity compares full scalar sanitizer outcome, not only classifier.

## 6. Verification (Evidence Floor)

- [ ] 6.1 Local: `uv run ruff check .`; `openspec validate harden-auth-headers-and-public-redaction --strict --no-interactive`;
  `cd apps/frontend && pnpm check:api-types`.
- [ ] 6.2 Local focused pytest (non-DB): `uv run pytest -q tests/test_forecast_api.py tests/test_monitoring_api.py
...
- [ ] 6.4 red-proof: new-behavior tests fail against pre-change source (batched, reported by implementer).
