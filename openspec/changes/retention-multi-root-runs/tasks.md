...
- [x] 1.1 `services/orchestrator/retention.py`：`plan_retention` 增可选入参
...
- [x] 1.3 `RetentionTarget` / `_target_payload` / skipped / failed 条目携带 `root`（resolve 后绝对路径字符串）。
- [x] 1.4 `to_dict()`：`schema_version` → `nhms.production_scheduler.retention.v2`；新增顶层
...
- [x] 1.4b **`extra_roots` 必须进 evidence 压缩白名单**：把它加入
      ...
      D3 升 v2 的收益归零。
- [x] 1.5 `RetentionConfig.from_env()` 增 `extra_roots_enabled`（`NHMS_RETENTION_EXTRA_ROOTS_ENABLED`，默认 `false`）
...
- [x] 1.6 `run_retention` 计算第二个 cutoff 并转发；闸门关闭时 `runs_only_roots` 传空元组。
- [x] 1.7 调用点转发：`services/orchestrator/scheduler_runtime.py::_run_retention` 传
      `workspace_root` 与 copyback 根；`services/orchestrator/scheduler_core.py` 同步签名。
- [x] 1.8 copyback 根来源：**消费既有的 `SchedulerConfig.object_store_copyback_root`**
...
- [x] 1.9 `services/orchestrator/cli.py::_run_cleanup`（`:121-152`）：额外根的取用与窗口语义必须**显式钉死**，
      两点各自裁定并写进 PR 描述：
...
- [x] 1.9b 额外根删除安全（design.md D6）：额外根的 `runs/` 若为 symlink → 跳过该根并记入 `skipped`
...
- [x] 1.10 `infra/env/compute.example` 与 `infra/env/compute.scheduler-dbfree.env.example` 增两个旋钮
...
- [x] 2.1 额外根回收：workspace 根 `runs/fcst_gfs_<老 cycle>_m`（含 `input/`/`output/`/`logs/`/
      `state_checkpoint_recovery/`）→ 进入 `planned` 且被删除，条目 `root` == workspace 根。
- [x] 2.2 **runs-only 钉死**：额外根下同时放 `raw/gfs/<老 cycle>`、`canonical/gfs/<老 cycle>`、
      `forcing/gfs/<老 cycle>` → `planned` 中**零条**来自这三个前缀（断言按 `root` + `key` 前缀过滤）。
- [x] 2.3 双窗口独立：同一 cycle 同时存在于主根与额外根，cycle 早于主 cutoff、晚于额外 cutoff
      → 主根条目在 `planned`，额外根条目在 `skipped` 且 `reason == "within_retention_window"`。
- [x] 2.4 闸门关闭：`extra_roots_enabled=False` 且额外根有大量老 run
      → `planned`/`deleted`/`freed_bytes` 与不传额外根时**逐 key 一致**，额外根未被扫描。
- [x] 2.5 同根去重：三根传同一路径 → 每个目标在 `planned` 中恰好一次，`freed_bytes` 不翻倍。
- [x] 2.6 缺失根：额外根不存在 / 存在但无 `runs/` → 无条目、不抛异常。
- [x] 2.6b **空根不得解析成 CWD**：`NHMS_OBJECT_STORE_COPYBACK_ROOT=""`（及未设）+ CWD 下存在
      ...
      （额外根仍被扫描并产生目标）。
- [x] 2.7 非 canonical 名字：额外根 `runs/not-a-run-id` → `skipped` 且 `reason == "unparseable_run_cycle"`，
      永不进入 `planned`。
- [x] 2.8 前沿豁免在额外根生效：cycle 早于额外 cutoff 但 >= `active_lower_bound`
      → `skipped` 且 `reason == pipeline_frontier_exempt`。
- [x] 2.9 保护语义：额外根下 `runs/` 内的 `published_artifact_root` 指向目标 → `protected_path` 跳过。
- [x] 2.9b **symlink 逃逸**：额外根的 `runs/` 是指向根外目录的 symlink（根外目录含老 run）
      → 该根零目标，`planned` 中不出现任何根外路径，根外目录在磁盘上仍存在；该根记入 `skipped`。
- [x] 2.9c **containment**：额外根下某 `runs/<run_id>` 内含指向根外的 symlink 子树
      → 删除后根外目标仍存在（`rmtree_no_follow` 不跟随）。
- [x] 2.9d **frontier 块保真**：v2 receipt 仍含 `frontier`（`active_lower_bound`/`source`/`protected_count`），
      形状与 v1 一致 —— `services/orchestrator/retention_frontier.py:133-134` 是既有消费者。
- [x] 2.10 receipt v2 形状：`schema_version == "nhms.production_scheduler.retention.v2"`；
      `extra_roots` 块四个键齐全；`planned` 每条含 `root`。
- [x] 2.10b **压缩后 `extra_roots` 存活**：输入 = 额外根产生数千条 `planned`、触发
...
- [x] 2.11 更新 `tests/test_production_scheduler.py:18601/18647/18704` 三处 `schema_version` 断言至 v2。
- [x] 2.11b **CLI 窗口作用域（期望值必须偏离默认，否则测不出接线 bug）**：
...
- [x] 2.12 删除失败不中断（**异常类型必须钉死**）：输入 = 额外根下某 `runs/<run_id>` 删除时
...
- [x] `uv run pytest -q tests/test_retention.py`
- [x] `uv run pytest -q tests/test_production_scheduler.py -k retention`
- [x] `uv run ruff check .`
- [x] `openspec validate retention-multi-root-runs --strict --no-interactive`
- [x] PR 描述记录：receipt v1→v2 迁移说明、闸门默认关、两步上线（22 dry-run 审清单 → enforce）
