...
- [x] 2.1 Add `tests/test_role_boundary_static.py` covering explicit boundary scenarios:
  ...
  - standalone gateway route scope: input `services.slurm_gateway.app:create_gateway_app()` -> expected `/api/v1/slurm/*` only, excluding forecast/model/pipeline/static/frontend business routes.
  - QHH diagnostic exclusion: input production orchestrator source scan -> expected no `run_qhh_*` or diagnostic manifest-builder tokens.
  - temporary API import allowlist: input `packages/common`, `services/orchestrator`, and `workers/**` Python imports -> expected exactly these (`path`, `module`) pairs and no other `apps.api.*` imports:
...
- [x] 2.2 Reference and run existing `tests/test_runtime_mode.py`, `tests/test_two_node_docker_runtime.py`, and `tests/test_qhh_scripts_static.py` rather than modifying them or duplicating their full logic.
...
- [x] 2.4 Verify `uv run pytest -q tests/test_runtime_mode.py tests/test_two_node_docker_runtime.py tests/test_qhh_scripts_static.py tests/test_role_boundary_static.py tests/test_retry_cancel_consistency.py tests/test_monitoring_api.py`.
...
- [x] 3.3 Add the shared auth/policy extraction issue as a dependency for any future hard-gate that fails `apps.api.*` imports outside the API layer.
