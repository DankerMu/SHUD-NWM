...
- The standalone Slurm gateway app continues to expose only `/api/v1/slurm/*` plus framework docs/openapi surfaces, not forecast/model/pipeline/static/frontend routes.
...
- Static tests that fail on display compute-control leakage, standalone gateway business-route leakage, QHH diagnostic token leakage into production orchestrator code, and new non-allowlisted `apps.api.*` imports from shared/workers/orchestrator paths.
...
- Public routes/entrypoints: full API app, standalone Slurm gateway app, Make/pytest validation entrypoints.
...
- Standalone Slurm gateway app -> route inventory contains `/api/v1/slurm/*` and excludes forecast/model/pipeline/static/frontend business routes.
- Production orchestrator scan -> no `scripts/run_qhh_*` or `create_qhh_shud_manifest` diagnostic token appears.
- Shared/workers/orchestrator import scan -> only the seven #361 allowlisted (`path`, `apps.api.auth`) pairs are present; any new `apps.api.*` import fails.

Boundary-surface checklist:

- Shared helper roots: `packages/common`, `services/orchestrator`, `workers/**`; #360 only scans and allowlists, #361 extracts.
