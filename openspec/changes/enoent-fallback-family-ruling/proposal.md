...
- `services/orchestrator/scheduler_config/path_modes.py:117-119` 的注释补 ADR 指针。

## Non-Goals

- **不改任何站点的运行时行为**。裁定结论是零错位站点（design D2），没有要对齐的对象。
- 不并入 EACCES 跨版本分歧（#1554 / #1623）——issue 明确另一条轴。
- 不处理 `.resolve()` 的 152 个函数面（design D5 report-only）。
