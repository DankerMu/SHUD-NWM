...
- [x] 1.4 env 模板：`infra/env/display.example` 加注释 + `NHMS_DISPLAY_CACHE_WARM_TOKEN=change-me-cache-warm-token`；`infra/env/node27-ingest.example` 在 `AUTOPIPE_MVT_PREWARM_*` 旁加同名键与「必须与 display.env 同值；缺失则 prewarm 不发头并 warning」注释。
