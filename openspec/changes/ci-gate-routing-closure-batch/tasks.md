...
- [x] 2.9 Depth-retro closure: complete every row of the finite load-bearing branch inventory in `.workplans/pr-1834/review/round-2-branch-completeness-inventory.md`. One collection-consumer positive helper must own condition key/true sense, targeted-before-collection ordering, scoped collect command, truthful label, and fail-closed exit; live state must have no violations and five independent scoped workflow mutants must each yield a named violation. The structured real-DB helper must likewise retain standing named mutants for all seven rows, including a B1 `needs: changes` mutant scoped only to the extracted `real-db-integration` job plus service image and named step/command. Update the persisted audit matrix to claim only standing mutants actually present.
