...
- [x] 2.9 First depth-retro closure: complete the nominal A1-A5/B1-B7 finite inventory with one live positive helper and one direct scoped mutant per listed predicate, including B1 confinement to the extracted `real-db-integration` job. Round 3 later proved this lexical/nominal proof model insufficient; task 2.10 supersedes its clean claim without erasing the historical work.
- [x] 2.10 Round-3 depth redesign: make A1-A5 properties of a full-workflow effective-execution contract. Pin the targeted step to an independently authored audited `run` identity and exact `TARGETED_TESTS_JSON` binding; reject targeted/real-DB step or job conditions, continuation, custom shell, working-directory, and inherited workflow/job `defaults.run` metadata that can skip an oracle, change checkout identity, or make failure non-blocking. Never execute arbitrary PR-editable payload: execute only the trusted canonical fixture and finite test-owned semantic variants in a bounded new process group, with all descendants cleaned on success/error/timeout. The trusted behavior matrix must prove exact C0-C3 command sequences, labels/stdout/stderr/summary, collection log, success/failure status, and the existing comment/quote/no-op/dead/reordered/unreachable mutants. Retain B1-B7 and effective step env/condition checks. Add valid full-workflow metadata mutants plus arbitrary-payload rejection and descendant-cleanup proof; then run a fresh Phase 6.2 audit and keep the Round-2 lexical audit explicitly superseded.
