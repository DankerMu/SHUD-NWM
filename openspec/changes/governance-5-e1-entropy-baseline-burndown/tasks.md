...
- [ ] 5.1 Run `uv run --no-sync pytest -q tests/test_entropy_audit_script.py`.
- [ ] 5.2 Run `PYTHONDONTWRITEBYTECODE=1 uv run --no-sync python scripts/governance/audit_repo_entropy.py --format json`.
- [ ] 5.3 Run `PYTHONDONTWRITEBYTECODE=1 uv run --no-sync python scripts/governance/audit_repo_entropy.py --format markdown`.
- [ ] 5.4 Run `PYTHONDONTWRITEBYTECODE=1 uv run --no-sync python scripts/governance/audit_repo_entropy.py --mode hard-gate --format json` and confirm JSON remains parseable, hard-gate counts only gate-eligible findings, and `.entropy-baseline/latest.json` is not written.
- [ ] 5.5 Confirm `.github/workflows/governance.yml` does not use `--mode hard-gate`.
- [ ] 5.6 Run `openspec validate governance-5-e1-entropy-baseline-burndown --strict --no-interactive`.
