...
- [x] E6 Carrier sweep, re-run after **every** fix commit. Two tiers, because the code comments cannot carry the full sentence: `pyproject.toml` sets `line-length = 120` with `E` selected, and the canonical sentence is 1032 characters, so a single-line comment would trip E501 while the D5 fence forbids wrapping the verifier diff past three lines. Tier 1 — **verbatim**: the five schema `description`s and the runbook narrative (whitespace-normalized) each contain the canonical sentence byte-for-byte as it appears in design D2. Tier 2 — **load-bearing assertion plus pointer**: each of the three verifier comments names `execution.ledger` as the source the slot is re-derived from and points at `scripts/node27_timeseries_compression_bundle_author.py:21-25`. Plus, in both tiers: the three refuted formulations have zero hits outside D2/proposal's explicit record of them — `输入值被忽略` / `input value is ignored`, `never reaches the terminal state`, and `never parsed` / `content never interpreted` / `constrained only as an artifact reference`.
