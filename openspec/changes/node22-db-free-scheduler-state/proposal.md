...
PostgreSQL data plane, download, ingest, display API, and frontend. The
historical PostgreSQL listener on node-22 `:55433` still cannot be stopped
because the production scheduler continues to read `DATABASE_URL`, uses
`NHMS_SCHEDULER_LOCK_BACKEND=postgres`, and records live evidence with
