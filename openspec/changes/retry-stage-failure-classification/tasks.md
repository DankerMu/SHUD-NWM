...
- [ ] 1.1 `NON_TRANSIENT_ERROR_CODES` (services/orchestrator/
...
- [ ] 1.2 Prove behavior invariance on the final head: grep shows
...
- [ ] 2.1 13 codes join the "Non-transient error codes block
...
- [ ] 2.2 Exemplar-rot repair (P1-1): MODIFIED "Pre-Guard Evidence
...
- [ ] 3.1 `_UNLISTED_PRODUCTION_ERROR_CODES`
      ...
      alone).
- [ ] 3.2 New derivation pin: the stage family
...
- [ ] 3.3 Verify the spec-driven parametrize
...
- [ ] 3.4 #1419 pins green UNMODIFIED: dual-absence
...
- [ ] 4.1 `uv run pytest -q tests/test_retry.py
      ...
      expected 125 → 150 per fixture-review Note-2, file-journal
      +26; the spec-driven parametrize's growth is the structural
      red/green).
- [ ] 4.2 `git ls-files '*.py' | xargs uv run ruff check` clean.
- [ ] 4.3 `openspec validate retry-stage-failure-classification
...
- [ ] 4.4 Diff = retry.py + tests/test_retry.py + the two-capability
      ...
      production file.
- [ ] 4.5 Issue #1462 acceptance mapped in the PR body, including
