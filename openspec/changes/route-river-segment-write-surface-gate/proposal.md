...
- **D15 (Phase 7 and its follow-up)**: the fixture used two `path:line` reference frames without saying so. Four citations were flagged; measured, they were two defects (one genuine merge drift, one plain off-by-one that was never right in any tree) and two refutations (both correct at base, flagged because the reviewer measured them at the branch tip). One of the two refuted citations was "corrected" to the tip value in the same commit that declared the base frame — the follow-up review called that a P1 and it was reverted. The frame is now declared at the top of `tasks.md`, and Evidence Floor 4.12b and 4.12c close the gap mechanically — 4.12b for citations whose line still resolves but no longer holds the cited content, 4.12c for the bare `:NNN` continuations neither 4.12a nor 4.12b can see.
