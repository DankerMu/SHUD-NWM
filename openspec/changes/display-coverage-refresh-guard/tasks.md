...
- [x] T3 (#1446) CLI `--force`; `--run-id` refusal -> exit 3 + `DISPLAY_COVERAGE_REFRESH_REFUSED run_id=… existing_segment_count=… advice=…` on stderr (test); `--all` report prints `refused`; docstring; runbook :126-142 rewritten as guard-backed with the D4 cost and the two remedies.
