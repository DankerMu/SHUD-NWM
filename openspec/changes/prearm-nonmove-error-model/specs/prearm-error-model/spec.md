...
directory creation, associations directory creation, collision
probing, and manifest writes, in addition to the already-hardened
moves — as a `pre-arm reset refused: ` exit-1
...
- **WHEN** creating the archive directory fails with a filesystem
  error before any move
...
- **WHEN** a non-move operation fails after at least one move
  completed (associations directory creation, or collision-candidate
  exhaustion)
