...
only archive-dir creation (:430) happens before any move; that case
asserts a recursively byte-identical workdir via `_refusal_case`
(:164). The associations-dir creation (:630) is POST-move: no
`_refusal_case`; it is covered by the post-move forensics axis (1).
...
- [ ] 1. `scripts/node27_timeseries_compression_prearm.py`: wrap the
  non-move FS sites in the unified error model —
  `_create_archive_dir` (:428-438), associations `os.makedirs`
  (:630), and the TERMINAL `_write_manifest` call site (:643).
  ...
  already moved".
- [ ] 2. Post-move refusal forensics — applies to ANY refusal raised
...
- [ ] 3. Tests (append-only; existing 38 untouched):
