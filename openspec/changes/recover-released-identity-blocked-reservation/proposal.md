...
`file_orchestration_journal.py:3285-3288` promises otherwise:
...
`chain_forecast_orchestrator_cycle.py:206-215` and
`file_orchestration_journal.py:7918` and `:7984` — are gated on
`should_auto_retry` (directly or through the caller at `:7928-7929`),
which is false by construction for this shape: the release transition
deliberately withholds `error_code` (`file_orchestration_journal.py:3331-3334`),
...
`file_orchestration_journal.py:8863` never classifies it as a retry source, so
`failed_job` resolves to `None` and `:8271` raises `RetryNotFoundError`.

## What Changes

- Add an **operator-gated** recovery path that mints the next
  `_retry_<n>` identity for a released cohort master via the existing
  `_next_current_master_retry_identity` helper, **without** consulting
  `should_auto_retry` and **without** stamping any `error_code`.
- Make the released terminal **announce itself**: both reservation-release write
  points emit a queryable operator-visible record carrying a searchable token,
...
- Cover both **prior-state shapes** a released row can arrive in — a fresh
  ...
  (`release_identity_blocked_reservation`, `file_orchestration_journal.py:3267`,
  decision constructed at `:3338`), so the signal is emitted once, there.
...
- **No automatic retry for this shape.** `AccountingStoreFlags = (null)` on this
  ...
  `file_orchestration_journal.py:3331-3334` and re-open the duplicate-submission
