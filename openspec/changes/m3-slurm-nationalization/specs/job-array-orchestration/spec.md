# Capability Spec: job-array-orchestration

## Context

M1 使用单流域线性链（一次仅调度一个流域），M3 需要支持全国 ≥10 个流域并行执行。Slurm job array（`--array=0-N%M`）可将多个流域作为同一作业的不同 task 并行调度。每个 array task 通过 `SLURM_ARRAY_TASK_ID` 索引 manifest 中对应的流域参数。

Resource profile 按 YAML 配置提供默认值和 per-model 覆盖，控制 partition、nodes、cpus_per_task、memory_gb、walltime、max_concurrent 等调度参数。
