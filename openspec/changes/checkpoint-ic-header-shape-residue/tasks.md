...
- [ ] 1.1 A 面 `packages/common/state_cli.py` `_normalized_checkpoint_ic_file`：
...
- [ ] 1.2 B 面 `_checkpoint_header_minute`：同判据同位置，不合法返回 `None` +
...
- [ ] 2.2b minute_index None 类（空/单 token/非数字尾）与 ≥5 数字 token 类：
