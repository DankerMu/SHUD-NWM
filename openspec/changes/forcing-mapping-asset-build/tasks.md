...
- [ ] 2.1 Implement `workers/mapping_builder/algorithm.py` `nearest_cell_barycenter_geodesic_v1`: element barycenter `(v1+v2+v3)/3` in package CRS transformed to WGS84, nearest registered cell by `pyproj` geodesic distance, and per-element distance/tie/candidate recording.
