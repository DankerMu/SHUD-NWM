...
父PR #1753因breadth gate拆分并关闭未合并。前置 #1799 / PR #1803 已把source-discovery evidence helper行为中性下沉并完整锁定owner compatibility；latest master上的 `scheduler_discovery.py` 为984行。本 successor 不重做该拆分、不修改 `scheduler_discovery_evidence.py` 或guard配置，且产品接线后owner仍必须不超过1000行。
...
- 不修改#1799已合并的discovery evidence extraction、`scheduler_discovery_evidence.py`、`.large-file-guard.json`或owner compatibility tests；`scheduler_discovery.py` final仍须≤1000行。
...
### D3: terminal-first verdict只在strict selection可比较时消费完整identity

#1775要求completion先判断terminal decision，再处理strict/successor admission。terminal-success后，只有strict resolution为ready、`candidate_state`是mapping且包含canonical state id时，discovery才通过 `getattr(active_repository, "completed_pipeline_init_state_identity", None)` 读取完整mapping。若得到mapping，作为 `terminal_init_state_match` 的observed；method缺失或返回None时，继续使用 `decision.evidence["hydro_run"]`，因此非-journal repository与legacy per-basin row行为不变；在这个有可比较selection的合法调用面，foreign provider异常继续响亮传播，不能伪装为absence。

strict not-ready时由#1775 closed allowlist直接分类为`unverifiable`或`conflict`；ready但不命名state的cold-start shape仍为`conflict`。这两类都没有可比较selection，因此不得调用full-identity accessor：provider失败不能否决terminal-first结果。共享helper仍是纯present-field compare；candidate-side strict wrapper不改；accessor不成为新的admission shortcut。
