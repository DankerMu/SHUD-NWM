...
- [ ] 1.1 `_job_is_cycle_scope_row` 下沉 `scheduler_state_rows.py`（语义逐字节不变），
      `scheduler_state_manual_retry.py` re-export 兼容
- [ ] 1.2 (#1298) `_state_retry_attempt` 非 canonical 第三臂：原始 stage 名相等匹配 +
      cycle-scope 行排除 + `effective_retry_attempt` 同口径；`stage=None` 臂与
      canonical 臂逐字节不变；docstring 记录"非 canonical 臂无 floors 载带（窗口敏感）"
      边界
- [ ] 1.3 (#1299) `_job_is_live_candidate_scope_failure` 行身份前置判据（无
...
- [ ] 1.4 (#1300) `_candidate_failed_stage` 新增（显式键同 `_failed_stage`；行扫描跳过
...
- [ ] 1.5 spec 三处挂账措辞随修复改写（经本 change delta；archive 时并入主 spec）

## 2. Evidence Floor（tests/test_production_scheduler.py，全部红先行或变异咬红）

- [ ] E1 (#1298 主判别，红先行): 唯一活失败 = model-scoped `cancelled` 的
      `download_retry_4` 行、顶层 `retry_count=0`、无 `failed_stage` →
      `_manual_retry_new_attempt` 1→**5**
- [ ] E2 (#1298 对照): 同形 canonical `forecast_retry_4` 仍 5；round-4 判别形
      （stage-blind 8 / 无 floor 1 / 正确 3）不回归（既有腿复跑确认未改）
- [ ] E3 (#1298): `stage=None` 臂逐字节不变——evidence-owner / manual-retry 无 stage
      消费者既有腿全绿 + 非 canonical 行存在时 stage-less 读数不变的判别断言
- [ ] E4 (#1298 scope 纪律，变异咬红): model-less `cycle_` run-id 的 cohort download 行
      `retry_count=7` 与候选 model-scoped `download_retry_4` 并存 → 新臂读 4 不读 7；
      变异（删 cycle-scope 排除）该腿红
- [ ] E5 (#1298 边界钉): 非 canonical 最大 attempt 行截出 `job_limit` 窗外 → 新臂读窗内
      值（floors 不覆盖非 canonical——现状钉，docstring 边界的活体证据）
- [ ] E6 (#1299 主表格，红先行): 顶层 `pipeline_status` ∈ {cancelled, failed,
      permanently_failed} × `pipeline_jobs` ∈ {缺失, []} + marker `retry_count=5` →
      `_manual_retry_new_attempt` 全部 1→**5**；{succeeded, 缺失} 两行保持 5
- [ ] E7 (#1299 回归): 携 `job_id` 的真实 `cancelled` 行仍判活失败（arm-2 拒钉不变，
      #1287 语义）
- [ ] E8 (#1299 形状锚): single-mapping（内嵌 id）与顶层平铺**带 id** 形状的 arm-2 取值
      改前改后一致（fail-open 依赖钉）；无 id 合成行不再向 `_restarted_stage_family`
      贡献 stage（侧翼钉）
- [ ] E9 (#1300 主判别，红先行): cohort `convert` 行 `retry_count=7` + 其上 marker
...
- [ ] E10 (#1300 auto-retry 腿): 同形去 marker、`retry_limit=3` →
      `_failure_policy_payload(state)["limit_exhausted"] is False`、决策非
      `retry_limit_exhausted`
- [ ] E11 (#1300 回归四联): download 列仍 3（#1287 AC）；候选有存活行时顶层 `stage`
...
- [ ] E12 (#1300 可达形状): repaired-stage-evidence 分支（顶层 stage 置 None + 行保留）
...
- [ ] E13 (#1298/#1300 交互): `_candidate_failed_stage` 解析出候选自身 model-scoped
      download 失败 → 分类/攻击面读到 download 真值（新臂）且不吃 cohort（E4 同几何
      对照）
- [ ] E14 (#1298 AC-4 活体证据): `scheduler_candidates` strict-warm-start 预算读点
