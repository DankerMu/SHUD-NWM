...
- [ ] 1.1 `services/orchestrator/scheduler_config.py:928-930`（design D2）：
...
- [ ] 2.1 helper 判定表（design D4 六格）：真实目录 / ENOENT / 自指环 /
      环后缀 `loopdir/a/tail`（helper 直测） /
      `<gone>/../<loopdir>/<loop-leaf>`（helper 直测，P2-7 差分形） /
      db-free 臂对照——全部版本无关断言。
- [ ] 2.2 构造 seam 红-绿主锚：环路 `OBJECT_STORE_ROOT`（db-backed）⇒
...
- [ ] 3.3 `openspec validate config-dbbacked-resolve-loop --strict
      --no-interactive` 通过。
- [ ] 3.4 issue Verification 步骤 4（≤3.12 臂缺陷消解）：隔离环境 3.11 下
