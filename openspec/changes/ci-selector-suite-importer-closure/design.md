...
- Synthetic tree mutation: added and rewritten importer files invalidate discovery/parse state; unchanged files reuse parse work. The ordinary missing-importer proof runs the same offender helper and its default recursive/dotted/redirect authorities as the live guard, injecting only the omitted actual selection; a separate redirected-owner proof injects an active redirect and proves the helper's skip branch cannot be removed.
