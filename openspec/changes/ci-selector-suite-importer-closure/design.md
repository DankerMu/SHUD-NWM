...
4. Apply the closure only in the existing ordinary self-selection branch. Explicit `CHANGED_TEST_FILE_RULES` redirects retain their focused behavior, and the disjoint support-module branch remains untouched.
...
- Mechanically derived live-tree invariant: every non-gated module-scope importer of each ordinary owner suite is selected.
...
- [Redirected suites unexpectedly broaden] → run closure only after no redirect matched and pin redirect compatibility.
