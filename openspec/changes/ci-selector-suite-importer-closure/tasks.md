...
- [x] 2.2 Add a recursively derived live-tree guard covering every ordinary owner/importer edge; its effective redirect predicate is shared with production, and one shared offender helper owns the live/constructed domain, dotted-identity authority, comparison, and missing-importer message so top-level-only, empty-authority, and comparator regressions all make the constructed proof red.
...
- [x] 3.2 `uv run pytest -q tests/test_select_ci_tests.py`: 351 passed in 169.80 seconds after the Round 2 depth-retro comparator closure on the final local head.
