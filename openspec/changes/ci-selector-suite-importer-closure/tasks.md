...
- [x] 2.2 Add a recursively derived live-tree guard covering every ordinary owner/importer edge; its effective redirect predicate is shared with production so conditional redirect owners remain guarded when their activation surface is absent, and constructed evidence names an omitted importer.
...
- [x] 3.2 `uv run pytest -q tests/test_select_ci_tests.py`: 351 passed in 161.69 seconds after Round 1 evidence-invariant closure.
