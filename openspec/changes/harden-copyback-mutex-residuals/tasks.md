...
- [ ] T1 `copyback_guard.resolve_copyback_lock_timeout_seconds`: explicit branch
      refuses non-finite values (D7).
- [ ] T2 `copyback_guard`: `primitive` keyword (`"flock"` default, `"posix"`) on
...
- [ ] T3 `copyback_guard`: `CopybackLockBudgetExhausted(CopybackLockError)`,
...
- [ ] T4 `copyback_guard` module comment: waiter accounting lists node-22 pass,
      operator `cleanup`, node-27 raw retention; names the unbounded term (D8 iv).
- [ ] T5 `run_tree_copyback._replace_tree` and `_replace_file`: D1 lifecycle and
      `OBJECT_STORE_COPYBACK_BACKUP_RETAINED`; correct the `_replace_tree`
      docstring's "no data was lost" claim.
- [ ] T6 `forcing_copyback_backfill`: D2 lock scope in `--apply`,
...
- [ ] T7 `node27_raw_retention.run_retention`: canonical lane under
...
- [ ] T8 `retention._delete_entry` / `_remove_tree_under_copyback_mutex`: raise
...
- [ ] T9 Docs: `docs/runbooks/forcing-copyback-backfill.md` closure criterion
...
- [ ] T10 New tests live in new files (large-file guard): 
...
- [ ] T11 `scripts/select_ci_tests.py` selects the new test files for diffs
...
- [ ] EF-1 guard: explicit `timeout_seconds` of `inf`, `-inf`, `nan` →
      `CopybackLockError`, lock file absent afterwards. Red for `inf`/`nan`.
- [ ] EF-2 guard `posix` (holder and probe in separate subprocesses): process A
...
- [ ] EF-3 guard: `EACCES` from `lockf` is treated as busy (poll continues until
      deadline → `CopybackLockTimeout`), not as unsafe.
- [ ] EF-4 classifier: timeout / budget-exhausted / other lock error → the three
      kinds; `CopybackLockBudgetExhausted` is caught by `except CopybackLockError`.
- [ ] EF-5 `_replace_tree` and `_replace_file` each: (a) promote fails + restore
...
- [ ] EF-5b `copyback_run_trees` on a re-copyback of an existing run with
...
- [ ] EF-6 backfill `--apply`: competitor holds the lock, promotes a
...
- [ ] EF-7 backfill `--apply`: during `_inspect_existing_target` a second
...
- [ ] EF-8 backfill plan mode: zero acquisitions, no lock file, every package
...
- [ ] EF-9 lock overhead, measured where the backfill runs: on node-22, read-only,
      ...
      deviation.
- [ ] EF-10 node-27 raw retention (lock holders and probes in subprocesses):
...
- [ ] EF-11 node-27 raw retention: lock held by another process past the budget →
...
- [ ] EF-16 existing suites green: `tests/test_copyback_guard.py`,
...
- [ ] EF-19 cross-host receipt: `evidence/lock-interop-20260914.md` (done before
      implementation).
- [ ] EF-20 `openspec validate harden-copyback-mutex-residuals --strict
