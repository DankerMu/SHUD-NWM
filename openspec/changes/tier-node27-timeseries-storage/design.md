...
- **(2026-09-02, superseded by `compressed-chunk-guard-except-split` / `#1785`) Base-arm contract at the producer, parser and both CLIs** — The intro clause at `:1711` ("every code routes to runbook §4.3 decompress procedure"), the bullets at `:1713-1715` (`OUTPUT_PARSE_COMPRESSED_CHUNK_BLOCKED` / `FORCING_PRODUCE_COMPRESSED_CHUNK_BLOCKED` / `FORCING_COMPRESSED_CHUNK_BLOCKED` as the single wire code per route), the shape cheatsheet at `:1780-1781` (parser and producer stamping the `_BLOCKED` code on the base-class arm), the four-code list at `:1790-1794` and the test-name list at `:1795` (the four non-apply tests were renamed by the split into one case per arm: `test_compressed_chunk_{write,guard}_error_sets_{blocked,guard_failed}_error_code` in `tests/test_output_parser.py`, `..._forcing_error_code` in `tests/test_forcing_producer.py`, `test_forcing_{click,argparse}_leg_emits_{compressed_chunk_blocked,guard_failed}_prefix_and_{exit_1,rc_1}` and `test_output_{click,argparse}_leg_emits_…[shud-output|parse]`; only `test_compressed_chunk_write_error_produces_dedicated_reason_code` survives) describe the pre-split state and are historical. `#1785` split each of those eight arms into `except CompressedChunkWriteError` (subclass, `_BLOCKED` codes unchanged) followed by the base-class arm stamping `FORCING_COMPRESSED_CHUNK_GUARD_FAILED` / `OUTPUT_PARSE_COMPRESSED_CHUNK_GUARD_FAILED` and the CLI prefixes `FORCING_PRODUCE_COMPRESSED_CHUNK_GUARD_FAILED:` / `OUTPUT_PARSE_COMPRESSED_CHUNK_GUARD_FAILED:`, mirroring the `#1781` apply-layer split. The current contract is `openspec/changes/compressed-chunk-guard-except-split/` and runbook §4.3.1; the history above is left intact.
...
Retention drops chunks from `hydro.river_timeseries` (source=`runs`) and `met.forcing_station_timeseries` (forcing recovery); the drill receipt PASS branch declares `coverage[]` tuples `(source ∈ {forcing, runs, db-export}, window)`. Runbook §7.5 already declares the rule the runner MUST byte-for-byte enforce: the `runs` UNION and the forcing-recovery UNION span the drop window, where forcing recovery is the UNION of verified `forcing` product tuples plus verified `db-export` tuples. `db-export` remains independently required iff the completeness receipt reports any `coverage=db-export` verdict overlapping the drop window — but its required span is each salvage-backed window (complete `coverage=db-export` subject windows ∩ drop window, per H9), NOT the whole drop window; portions backed by product archives carry no db-export requirement, and overlapping db-export subjects with no `complete` verdict fail closed (#1162). The drill emits per-cycle 24 h tuples, so a 14 d drop window is normally covered by ~14 daily tuples whose union spans it — no single tuple is expected to individually contain the drop window. A first physical Timescale chunk whose range begins before truthful completeness bounds is deferred rather than globally blocking later fully evidenced chunks; exact per-chunk `drop_chunks(older_than, newer_than)` bounds prevent retirement from cascading through that protected chunk. Before ANY coverage leg is consulted, the gate enforces the drill's DECLARED judgment span (#1207): the receipt's recorded `salvage_derivation.drop_window` (#1206) MUST contain the retention drop window, closed-interval with equality passing, else `DRILL_DERIVATION_WINDOW_TOO_NARROW`. Evidence from a run that declared it judged less than this drop never counts, because coverage tuples carry no subject identity and one subject's wide tuple would otherwise vouch for another subject the narrowed drill never derived; a present-but-unusable section refuses with the same code, a receipt with no `salvage_derivation` section keeps pre-#1207 behavior, and a recorded `null` window (drill ran un-narrowed) passes. Inside the db-export leg, after the empty-derivation refusal and before any per-target tuple union, the gate additionally binds its requirement set to the drill's completeness snapshot (#1220): the drill records the db-export universe of the receipt it consumed at `salvage_derivation.db_export_windows` (unfiltered by any drop window, same normalization as the gate's salvage-backed-window derivation), and every salvage-backed target derived from the gate-time completeness receipt MUST be an exact `{start, end}` member of that recorded set, else `DRILL_COMPLETENESS_SNAPSHOT_UNBOUND`. The completeness receipt is rewritten in place daily while a drill receipt is valid for up to `NODE27_TIMESERIES_RETENTION_DRILL_MAX_AGE_DAYS`, so without this binding a db-export subject added after the drill enters the requirement set never having been restore-verified and an older subject's identity-less tuple vouches for it. Binding is judged over the drop-filtered TARGETS only (a new subject outside the drop window is not a requirement of this tick) and at window granularity (the gate's own requirement granularity); requirement SHRINK passes, a present-but-unusable record refuses with the same code, and receipts without the section or without the field keep pre-#1220 behavior. `salvage_derivation.completeness_generated_at` is recorded for diagnostics and is NEVER a refusal input — a newer-snapshot or digest-inequality refusal would refuse at virtually every tick and collapse the 30 d drill budget to under a day. Refusal is per-shortfall — distinct wire codes so operators see which source blocked. §7.5 uses UNION wording aligned with §8.2 wire codes (`DRILL_DERIVATION_WINDOW_TOO_NARROW`, `DRILL_COVERAGE_FORCING_MISSING`, `DRILL_COVERAGE_RUNS_MISSING`, `DRILL_COMPLETENESS_SNAPSHOT_UNBOUND`, `DRILL_COVERAGE_DB_EXPORT_MISSING`) so all three surfaces — H2 here, runbook §7.5, and runbook §8.2 — share the same byte-identical semantic.
...
Receipt schema requires `dropped_chunks[]{name, freed_bytes: integer, minimum: 0}`. Measurement path (back-filled by #1125): `SELECT total_bytes FROM chunks_detailed_size('<hypertable_schema>.<hypertable_name>'::regclass) WHERE chunk_schema = ... AND chunk_name = ...` per selected chunk BEFORE the corresponding `drop_chunks` call; recorded in a local dict keyed by fully-qualified chunk name; attached to `dropped_chunks[]` on success. Post-drop measurement is impossible (chunk gone). The original wording here named `pg_total_relation_size(<schema>.<chunk_name>::regclass)`, which sizes only the main chunk relation and therefore under-reported every **compressed** chunk by the size of its `_timescaledb_internal.compress_hyper_*` sibling — proven by the 2026-07-25 first live enforce (receipt sum 17,403,371,520 B vs `pg_database_size` delta 19,097,174,016 B; see #1125). Reuse of the compression `_default_measure_chunk_bytes` pattern is now **scoped to the connection/failure shape only** (per-chunk fresh connection, 60 s statement timeout, per-chunk failure → `0` → continue, no `after=True` branch): retention **deliberately diverges** from the compression sibling's private-catalog join (`_COMPRESSED_SIBLING_QUERY` over `_timescaledb_catalog.chunk.compressed_chunk_id`) because retention needs one total-reclaim number, which the public `chunks_detailed_size` returns directly and which the live evidence validated byte-exactly against the DB-size delta. This divergence is intentional and must not be "fixed" back to the sibling's query.
...
- `DRILL_DERIVATION_WINDOW_TOO_NARROW` — the drill receipt's recorded `salvage_derivation.drop_window` does not contain (closed-interval; equality passes) the retention drop window, so the drill declared a narrower judgment span than this drop and its identity-less coverage tuples cannot vouch for it (#1207); also emitted when the `salvage_derivation` section is present but unusable (not an object, `drop_window` key missing, unparseable, or `end` before `start` — fail-closed). A receipt with no `salvage_derivation` section keeps pre-#1207 behavior; a recorded `drop_window` of `null` passes.
...
- `DRILL_COMPLETENESS_SNAPSHOT_UNBOUND` — the drill receipt records `salvage_derivation.db_export_windows` (the normalized db-export universe of the completeness snapshot the drill consumed) and at least one salvage-backed window derived from the gate-time completeness receipt is not an exact `{start, end}` member of it, so the gate-time requirement set drifted past the drill's snapshot and the drifted window was never restore-verified (#1220); judged over the drop-window-filtered targets only and BEFORE any db-export coverage tuple is consulted. Also emitted when the recorded set is present but unusable (not an array, an entry that is not a window object, missing/non-string endpoints — fail-closed). A receipt with no `salvage_derivation` section, or with the section but no `db_export_windows` field, keeps pre-#1220 behavior; `salvage_derivation.completeness_generated_at` is diagnostics only and never a refusal input.
- `DRILL_COVERAGE_DB_EXPORT_MISSING` — completeness has `coverage=db-export` subject overlap but the UNION of the drill's `source=db-export` tuples fails to cover at least one salvage-backed window intersected with the drop window (per-window judgement, NOT the whole drop window — see runbook §7.5); also emitted when the overlapping db-export subject derives no salvage-backed window at all (fail-closed), and when a salvage-backed window's intersection with the drop window is inverted (a corrupt subject window whose `end` precedes its `start`; fail-closed guard, #1162). The emitted `refusal_reason` carries a detail suffix localizing the shortfall (#1175), the bare code staying the registered `WIRE_CODES` token and a strict prefix of both forms: `:<clipped_start>/<clipped_end>` — the FIRST uncovered salvage-backed window in ascending window order, CLIPPED to the drop window and serialized with the runner's UTC `Z` ISO-8601 rendering (an inverted clip is rendered verbatim), single-shortfall early return preserved — and `:no-derivable-window` for the empty-derivation branch, where no interval exists to name.
