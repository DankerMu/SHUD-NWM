...
- **`.large-file-guard.json` exclude-list extension** — The design did not touch large-file plumbing. The PR extended the large-file guard's exclude list for one file (verified as a documented exception, not a bypass); this is orthogonal to the guard semantics and does not affect the invariant matrix. Recorded here for evidence completeness.

Additional post-review corrections applied on top of head `830218fb` (R1 fix pass):

- **Boundary predicate `range_start <= %s`** — The catalog query is `range_start <= batch_max AND range_end > batch_min`. TimescaleDB chunks are `[range_start, range_end)`; the earlier `range_start < %s` missed the boundary case where a batch's max valid_time equals a compressed chunk's range_start (in which case the INSERT lands inside that chunk). Regression-tested by `test_boundary_range_start_inclusive_blocks_write` with a predicate-aware fake cursor.
- **`HYPERTABLES_GUARDED` runtime enforcement** — The guard now refuses any `(hypertable_schema, hypertable_name)` not in `HYPERTABLES_GUARDED` before any SQL runs, so a wire-site typo cannot silently permit a write. Unit-tested by `test_unknown_pair_refuses_at_guard_entry`.
- **`REASON_APPLY_COMPRESSED_CHUNK_BLOCKED` reason code + parser `OUTPUT_PARSE_COMPRESSED_CHUNK_BLOCKED` error code + CLI structured error** — The compressed-chunk write guard now surfaces via a dedicated caller-observable contract at all three paths (apply, parser, CLI), so operators route on the runbook decompress procedure rather than the generic SQL/runtime failure buckets. Tested by `test_compressed_chunk_write_error_produces_dedicated_reason_code` and `test_compressed_chunk_guard_error_sets_dedicated_error_code`.
...
- **AST-scan wire-site invariant** — `tests/test_timescale_write_guard_wire_site_invariant.py` enforces "every module in workers/**, packages/common/** that DELETEs from a guarded hypertable MUST also call `check_batch_targets_uncompressed`" — parametrized over `sorted(HYPERTABLES_GUARDED)` — so a fourth wire site cannot slip in unwired.
