...
- Publish boundary: validated receipts use same-directory mode-0600 temporary files plus atomic replace, mandatory directory fsync, and post-replace parent-FD identity verification. Pre-replace blockers preserve the previous receipt and clean temporary residue; after-replace durability/namespace failures are indeterminate, fail closed, and cannot be used as fresh retention evidence. The configured parent is operator-controlled and non-rotating during publication. Product/archive deletion and other mutations remain out of scope.
