...
- Regression: #848 discovers forcing leaf, manifest-bound flat run tree, and provider/legacy physical state valid-time units without DB access or clone-target fabrication; exact-cutoff, malformed, ambiguous and unreadable candidates fail closed.
- Regression: archive tar verification proves exact safe regular-member path/size/sha bijection in addition to tarball sha; staged tar+manifest publish and corrupt-final quarantine move whole leaf directories on one device.
- Regression: source retirement revalidates the complete pinned preimage before and after same-device tombstone rename, then no-follow deletes only that tree; observed pathname/content drift preserves source/tombstone, while post-final-check open-FD writes remain an explicit immutable-producer contract violation outside the rename protocol.
