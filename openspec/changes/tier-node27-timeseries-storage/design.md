...
- `DRILL_COVERAGE_DB_EXPORT_MISSING` — completeness has `coverage=db-export` subject overlap but the UNION of the drill's `source=db-export` tuples fails to cover at least one salvage-backed window intersected with the drop window (per-window judgement, NOT the whole drop window — see runbook §7.5); also emitted when the overlapping db-export subject derives no salvage-backed window at all (fail-closed).
