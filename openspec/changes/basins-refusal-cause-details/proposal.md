...
- reingest 透传（`basins_reingest.py:104-125`）无辜：上游修好自动生效。
- 真实踩坑在案：`docs/runbooks/receipts/reach-geom-ingest-20260620.json:181-187`
  （tailanhe）payload 只有泛化文案，runbook `:94` 运维只能写猜测。
- 对照组：`scripts/publish_scheduler_file_registry.py:675/:769/:900` 同语义已带
...
- 两个 raise 点传拒因 details，键集为**四键**：
  ...
  `publish_scheduler_file_registry.py:675` 一致（不新造别名）；第四键是 **#1553 口径
...
- 验收 1-4 → tasks 1.1-1.3 + 2.1（payload 断言 + 口径一致锁）；验收 5 → 2.2
  （IC 头部畸形 → 拒因见 `*.cfg.ic` 文件名，扩 `tests/test_basins_package_publication.py:1059`
  附近既有断言点）；验收 6 → 2.3（reingest 透传链）；验收 7 → 2.4（旧键完整锁）。
