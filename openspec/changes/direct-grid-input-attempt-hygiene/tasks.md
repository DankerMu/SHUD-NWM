...
- [ ] 1.1 `_stage_direct_grid_directory_artifact` 入口（写入循环前）加
...
- [ ] 1.2 删除失败重编码：捕获 `SafeFilesystemError`/`OSError` → 抛
...
- [ ] 2.2 AC2 污染锚：manifest 自身声明两个 index 成员 → 仍抛
      `DIRECT_GRID_FORCING_INDEX_AMBIGUOUS`（manifest 门，不被卫生掩盖；
      若既有测试已覆盖则指认并保持绿）。
- [ ] 2.3 AC3 删除失败锚：残留成员 unlink 失败——**目录形与 symlink 形**
...
- [ ] 2.4 AC4 车道隔离 + legacy 回归：非 direct-grid staging 后残留成员
      仍存活（卫生未泄漏）且 manifest 锚定解析场景既有测试保持绿；
      legacy-only 包（声明集合={legacy}）staging 行为不变、无删除副作用。
- [ ] 2.5 纵深防御锚：卫生后文件系统仍并存双成员（构造带外写入形）→
...
- [ ] 2.6 删除范围合同锚：`shud/` 下上一 attempt 的 station CSV 与无关
      文件在卫生后存活（只删 index 成员补集）。
- [ ] 2.7 零声明锚（design D6）：direct-grid manifest 不声明任何 index
...
- [ ] 3.1 `uv run pytest -q tests/test_shud_runtime.py` 通过。
- [ ] 3.2 `uv run ruff check .` 通过。
- [ ] 3.3 `openspec validate direct-grid-input-attempt-hygiene --strict
      --no-interactive` 通过。
- [ ] 3.4 spec-compliance 人工证据：D5 两分场景与实现逐句对读；歧义门
      消息与 spec 措辞一致。
