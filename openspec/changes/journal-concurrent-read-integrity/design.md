...
5. `_cycle_rows_by_model_unlocked`（`:4233-4237`）的 `fingerprint=None` 存入
   **经复核不是缺陷，原拟的 issue 已撤销**（tasks 4.5 相应改为「不立单」）。
   我先后写错过两版：先说「一段永远命不中的缓存写入」，再说「窗外也存」——两句都假。
   实测：它的 6 个调用点（`:1581`/`:2618`/`:2970`/`:3307`/`:6853`，
   其中 `:6853` 在 `_materialize_latest_unlocked:6841`，其 6 个调用者亦全部在窗内）
   **无一在窗外**；这些 entry 会被 owner 后续的同 key 窗内 `_cycle_rows` 命中，
   **这正是它的用途**——函数自己的 docstring（`:4173-4179`）写着
   「The caller holds the cycle write lock; the populated model caches therefore remain
   authoritative until an append invalidates them.」
   即：**有意的窗内 memoization，按构造不可校验，只经快路径命中，出口 clear 即失效。**
   它是 D2 那条「入口 clear 是正确性前提」的成因之一（这半句 D2 说对了），
   但本身无需修、无需立单。**若后来者重新「发现」它，回读本条，不要立单。**
...
| 4.5 写成「永远命不中」，改后又写成「窗外也存」 | 两版都假。它是有意的窗内 memoization（docstring 明说），6 个调用点无一在窗外——**这条 issue 已整个撤销**，见 D10.5。把假前提写进 follow-up issue 是 D2 刚踩过的坑，我在同一份文件里踩了第二次 |
...
**教训 2（更贵）**：4.5 那条我改了两版、两版都假，最后正确答案是**它根本不是缺陷**。
两次都是先断定「这是个缺陷」再去补理由，而没有先问「它为什么会这么写」——
答案就在函数自己的 docstring 里。**立 follow-up issue 前必须先读被指控代码的 docstring**；
一个基于假前提的 issue 会让后来者删掉一个真优化，比不立单坏得多。
