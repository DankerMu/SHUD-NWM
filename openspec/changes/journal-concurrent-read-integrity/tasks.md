...
- [ ] 1.4 更新 `:4098-4102` 的注释：写清楚免指纹的两条前提（cycle flock + append hook）
  ...
  是这条快路径的正确性前提**——窗内 entry 带 `fingerprint=None`（`:4162`/`:4233-4237`/`:6839`），
...
- [ ] 3.1 两线程共享**同一** repository 实例，按以下**严格顺序**（顺序本身是断言的一部分）：
...
- [ ] 3.3a 从 `_locked_cycle_write` 的 **yield body** 抛异常后，
  断言 `repository._cycle_write_owner is None`。
- [ ] 3.3b 从**进入路径**抛异常：monkeypatch `_ensure_root_unlocked`
...
- [ ] 3.4 断言：cohort X 的写不会让 **另一线程（非 owner）** 读到错值，
...
- [ ] 3.5 把判据改回 `self._write_lock.locked()`，3.1 必须转红。
- [ ] 3.5b 把标记的置位从 `try:` 内第一条挪到 `_ensure_root_unlocked()` 之前，3.3b 必须转红。
...
- [ ] 3.6 **红证（确定性）**：monkeypatch `os.open`（或等价注入点），
...
- [ ] 3.10 目标是 `tests/test_file_orchestration_journal.py:8048-8055` 那个 carve-out：
...
- [ ] 3.11 AST 守卫：`_cycle_write_owner` 的**写入**只允许出现在 `_locked_cycle_write`
...
- [ ] 4.4 `_locked_cycle_write` **出口** `.clear()`（`:6957-6958`）的性能收窄——立 issue。
...
- [ ] 5.4 变异矩阵，逐个给出**实测**红/绿（凡填「预期红」而未实测的格子必须标明是推断）：
...
- [ ] 6.1 每一处与本 tasks/design 的 departure 写入 PR 的 `偏离记录` 段（what/why/impact）；
  无偏离也要显式写"无偏离"。
- [ ] 6.2 design D1（收窄到单站点 + keyed 三元组标记 + 置于 `try` 内首条）、
  D2（两次 clear 都保留；入口 clear 是正确性前提）、D3（重试落调用方）、
  D5（3 次无退避）四条裁定若在实现中被推翻，**停下来报告**，不要自行改道。
- [ ] 6.3 本 fixture 经两路只读评审后已修正 2 P1 + 6 P2 + 6 P3（记录在 design D11）。
