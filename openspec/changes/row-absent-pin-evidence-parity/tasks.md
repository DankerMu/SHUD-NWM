...
- [x] 1.1 `file_orchestration_journal.record_manual_repair`：details 追加
...
- [x] 1.2 `scheduler_state_identity_filter` retry-event details 白名单
      （:487-505 carve-out）追加同名 8 键；锚定 0/False 透传
      （`not in (None, "")` 语义）。
- [x] 1.3 `_unresolvable_marker_entity_pins_attempt` 重构（design D3）：
      ...
      语义映射声明、B token 上限。
- [x] 1.4 twin 与路由器**逻辑/行为 diff 为零**的证明义务（实现报告）；
      ...
      **必须随之改写**指向新 delivered domain（P2-2——散文更新不算
      逻辑 diff，报告中单列）。
- [x] 1.5 D4 第 5 类（写入后重新激活）可达性核实：追 typed API
...
- [x] 2.5 D4 永久限定成对披露锚：winner-eviction 形（写入后成功被更晚
      stage 挤出）与「写入后才 succeeded 的队列 stage」形各一对——
      row-present refuse / row-absent pin **钉现状**，docstring 指向
      spec 限定条款（无 red-proof 要求，钉值锚）。
- [x] 2.6 legacy backstop 回归：无 `target_*` 键的 marker 在既有矩阵
      全部格 + #1292 交付锚下判决逐位不变。
- [x] 2.7 B 合同：stage-less legacy marker 显式判决用例 + `token != row
...
- [x] 2.8 写读全链路锚（integration）：真实 `record_manual_repair` →
      真实投影 + identity filter → 决策态：新键存活、判决按 D3；同锚
      反向验证既有白名单键行为不变。
- [x] 2.9 D6 compaction 锚：完成阶段 model-less 队列 marker 事件投影后
...
- [x] 3.2 `uv run pytest -q tests/test_file_orchestration_journal.py` 通过。
- [x] 3.3 `uv run ruff check .` 通过。
- [x] 3.4 spec-compliance 人工证据：delta 的 delivered-domain 改写句、
