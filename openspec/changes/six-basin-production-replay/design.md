...
**首时次判定不改**:070500 pass 中,状态链已被 D4 清场 → `exists_any_generation=False` → 变更 1 契约自然给出 `PACKAGED_IC_BOOTSTRAP`(两级资格 tier-b object probe,12/12 已实证 qualified)→ runtime 消费包 IC(`init_mode=3`)。replay 模式对首时次分支零介入——这是本设计的核心验收点:新路径的实弹首验。
...
2. tile 失效(新 `scripts/node27_invalidate_tiles.py`,dry-run 默认):删除范围收窄为 **(source_id, valid_time ∈ 回放窗口)** 的 `map.tile_cache` 行 + 对应文件缓存条目(r1 P2-9:仅按 source_id 会波及 12 个域外流域的缓存行,与 must-preserve 1 冲突;`map.tile_cache` 无 basin 维度,valid_time 窗口是最细可用刀);receipt 记删除行数。执行于 re-ingest 完成后。窗口内瓦片含域外流域信号的部分将由后续请求按新旧混合数据重建——域外流域行数据未变,重建后字节等价,负验证以 DB 行断言而非缓存行断言。
