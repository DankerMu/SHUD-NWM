...
- [ ] 1.1 `workers/output_parser/parser.py`：`load_run_context` 追加
...
- [ ] 1.2 `upsert_river_timeseries` INSERT 列表 +7 列；行元组 arity
      10→17（ENUM 三值与文本同源 append，无 cast 无模板改动，design
      D2）；`ON CONFLICT DO UPDATE SET` 追加镜像三列（design D3，文本
      SET 列表逐字不变）
- [ ] 1.3 `db/seeds/seed_demo.py`：`_build_river_timeseries_rows` 加键
...
- [ ] 1.4 write-guard：`packages/common/timescale_write_guard.py` 零触碰；
