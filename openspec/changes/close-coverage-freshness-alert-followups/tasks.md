...
- `scripts/node27_coverage_freshness_alert.py` — **comment and docstring text only**, the D3 journal-budget comment at `:109-117` and the module docstring's D3 bullet at `:56-58` (the same framing-line count, stated a third time)
- `openspec/changes/archive/2026-09-18-node27-coverage-freshness-alert/{design.md,tasks.md}` (dated correction notes)

## Must preserve

- `scripts/node27_coverage_freshness_alert.py` — every **executable** line unchanged. Its exit-2-on-import-failure behaviour is the deliberate one (`config_from_env`'s docstring records why); #2465 is documentation drift, not a code defect. The permitted edits are the D3 journal-budget comment at `:109-117` (see T5) and the module docstring's D3 bullet at `:56-58`, which restates the same framing-line count; `git diff -- scripts/node27_coverage_freshness_alert.py` must touch comment and docstring lines only. Proof form: raw `ast.dump` equality holds for the comment edit but **not** for the docstring one (a docstring is an AST node), so the executable-parity proof is `ast.dump` over the docstring-stripped tree.
...
- [x] T5 Runbook §11.2, same paragraph, two measured corrections carried in because they are in the text being edited.
  ...
  - (b) The journal-budget figure must be stated **by category, not as one lump**: how many lines systemd itself frames a failing tick with, and whether this lane adds a structured stderr line on that exit path. The prior draft's single "5" was an attribution error — master's comment block (`scripts/node27_coverage_freshness_alert.py:107-110` on master) enumerates *four* systemd framing lines plus *one* lane stderr line, whereas the node-27 measurement on the **exit-1** path shows five systemd framing lines (`Starting…`, `Main process exited…`, `Failed with result…`, `Failed to start…`, `Triggering OnFailure= dependencies.`) and **no** stderr line, because the alert path calls `_emit` without `structured=`. Both compositions total 5, so `24 + 5 = 29 ≤ 30` holds either way — but only one of them is true.
  - Therefore the runbook sentence **and** that comment must both be rewritten to whatever E7 measures, with the two categories named separately and the exit path (1 vs 2/3) stated. Writing the corrected figure in only one of the two places is not acceptable: it would move the drift from doc↔doc to code↔doc. If E7 cannot be run, T5(b) is dropped whole — do not ship an unmeasured number in either place.
- [x] T6 Runbook §11.3: a third branch for "A 和 B 都查空" — coverage rows populated with `segment_count > 0` but the catalog still refuses the cycle (window columns inconsistent, or the hourly grid off the 3 h phase). Give the check (inspect `river_valid_time_start/end`, `min/max_lead_time_hours`, `river_sample_count` against `segment_count * lead_count`, the `(end - start) = (lead_count - 1) * 3600` span, and the `(start - cycle) % 3600` phase for the source's newest cycles) and the remediation (re-run `scripts/node27_refresh_coverage.py --run-id <run_id>`, which rewrites the window columns whenever the new scan is non-empty; `--force` is **not** the remediation — it zeroes the row, so it belongs only to the operator-confirmed empty-scan exception behind #1446's rc=3 refusal). Note the class has live writers so it is reachable, not hypothetical. Two cross-references in the same edited text go stale on this task and must move with it: §11.3's own heading 「处置：两个真分支 + 一个"看不见"分支」 and §11.2's 「走 §11.3 两个分支」.
...
- [x] E6c T6: the new §11.3 branch names its entry condition ("A 和 B 都查空") and the bare `--run-id` remediation; `grep -n -- "--force" docs/runbooks/current-production-ops.md` shows `--force` inside §11.3 **only as the operator-confirmed empty-scan exception**, never as the remediation line itself.
...
- **T6's remediation shipped narrower than this file originally worded it.** T6 was written as "re-run `scripts/node27_refresh_coverage.py` with `--force`"; what shipped is the bare `--run-id <run_id>` refresh, with `--force` demoted to the operator-confirmed empty-scan exception. Reason: `--force`'s action is to **zero** the row (`scripts/node27_refresh_coverage.py`'s overwrite-guard docstring), so making it the default remediation would hand the operator a lights-out command for a run the national layer may be serving; the non-empty rescan already overwrites the window columns without it. T6 and E6c above are reworded to what shipped. Two further widenings of branch C beyond T6's wording: its diagnostic `LEFT JOIN`s `hydro.run_display_coverage` (T6 assumed rows populated with `segment_count > 0`, which branch B cannot actually establish — B's numerator never joins that table), so the `no_coverage_row` rc=0 silent path surfaces as a NULL row instead of vanishing; and it dedups to the newest run per `(network, cycle)` the way `_national_discharge_coverage_rows` does — ranking the same way, but **diverging exactly where the `LEFT JOIN` is aimed**: the catalog filters `rdc.segment_count > 0` *inside* its ranked subquery, so when the newest run has no populated row it falls back to an older run on that cycle, while the diagnostic ranks all display-ready runs so the missing row can surface at all. The runbook prose carries that divergence (it sends the operator back to the earlier `run_id`); this sentence must not be read as claiming identity. The refresh command also carries the **writer** DSN (`infra/env/node27-ingest.env`): the refresh is an `INSERT … ON CONFLICT DO UPDATE`, and the alerting env's `nhms_display_ro` cannot execute it.
- **T5 is carried by neither issue.** The §11.2 paragraph is already under the pen for T4, and its current 「不会被截掉」 is contradicted by `build_report`'s own bounded guarantee; leaving a false absolute inside the routing table being fixed for routing accuracy would be incoherent. Recorded here and in the PR body as an addition, not smuggled in. T5(b) additionally widens the change surface to non-executable text in `scripts/node27_coverage_freshness_alert.py` — first the D3 comment block, then (after review round 1 found the figure stated a third time) the module docstring's D3 bullet at `:56-58`. The Non-goal below forbids changing that script's *behaviour*, and neither a comment nor a docstring edit does; correcting the runbook while leaving either of them stating a different composition would only relocate the drift. Because a docstring **is** an AST node, the executable-parity proof for this change is `ast.dump` over the docstring-stripped tree, not raw `ast.dump` and not a `#:`-filtered `git diff`.
