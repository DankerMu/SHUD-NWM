# worktree-local-large-file-guard Specification

## Purpose

Bind large-file guard policy, Git inspection, filesystem reads, merge metadata, and diagnostics to the worktree that owns the operation while preserving legal path bytes and merge attribution.
