...
The batch per-model row reducer SHALL have no flag governing direct-record participation: it never includes direct records and never stores into the cycle-rows cache, and its signature SHALL be exactly the keyword-only parameters `source_id`, `cycle_time` and `model_ids`, pinned by a signature test so that a re-added flag turns the suite red. The "any flag that governs whether direct records participate SHALL retain its meaning unchanged in the narrowed read" clause of the cycle-scoped lookup requirement binds the narrowed single-row replay, not the batch reducer; the reducer's only ever-executed meaning (no direct records) is what remains. The latest-view materialiser's own routing flag SHALL keep its meaning (`True`, the default, reads through the fingerprinted cycle-rows path; `False` reads through the batch reducer); the materialiser callers that pass `False` today SHALL keep passing it, and the callers that take the default `True` SHALL be unchanged. The sequence-floor computation SHALL be exposed only as the unlocked variant that write lanes call while holding the write lock; no lock-taking wrapper SHALL exist for it.
...
#### Scenario: Reducer signature carries no flag
