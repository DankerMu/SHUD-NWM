# review-governance-records Specification

## Purpose
Tracked, CI-executed tooling over the repository's review-governance records: the cross-PR round-ceiling memory `.review-gate-issues.json` (written only through `scripts/review_gate.py`) and the frozen review-loop ledger `docs/review-loop-log.jsonl` (audited by `scripts/governance/loop_log_audit.py`). The installed subagent-workflow no longer carries either tool, so the repository owns them.
