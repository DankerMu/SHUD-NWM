# mvt-tile-cache-lifecycle Specification
...
`infra/systemd/nhms-node27-mvt-cache-retention.service` SHALL declare `ExecStartPre=/usr/bin/mkdir -p /home/nwm/node27-mvt-cache-retention-logs` before `ExecStart`, append-mode `StandardOutput`/`StandardError` under that directory, and `ExecStart=/home/nwm/NWM/scripts/node27_mvt_cache_retention_once.sh`; `infra/systemd/nhms-node27-mvt-cache-retention.timer` SHALL fire daily with `Persistent=true`. The wrapper SHALL refuse an env file that is missing, a symlink, or not mode 0600, SHALL hold a non-blocking `flock` so overlapping ticks skip, and SHALL propagate the runner's exit code unchanged. Every path the wrapper itself writes after sourcing the env file — the summary sink (`NODE27_MVT_CACHE_RETENTION_SUMMARY_PATH`), the log file (`NODE27_MVT_CACHE_RETENTION_LOG_FILE`) and the flock path (`NODE27_MVT_CACHE_RETENTION_LOCK_PATH`) — SHALL be absolute: a relative value is refused with `SUMMARY_PATH_NOT_ABSOLUTE`, `LOG_FILE_NOT_ABSOLUTE` or `LOCK_PATH_NOT_ABSOLUTE` (exit 2) before the lock is taken, the runner is invoked, or any file is created, because the wrapper `cd`s into the repository and a relative sink would land in its git work tree. `infra/env/node27-mvt-cache-retention.example` SHALL carry `NHMS_MVT_FILE_CACHE_DIR` with the warning that it must equal the display PROCESS's value (the `nhms-display-api.service` ExecStart default `/home/nwm/.cache/nhms/mvt`, not `display.example`'s `/tmp/...`).
...
#### Scenario: Wrapper refuses a relative summary, log or lock path

- **WHEN** `NODE27_MVT_CACHE_RETENTION_SUMMARY_PATH`, `NODE27_MVT_CACHE_RETENTION_LOG_FILE` or `NODE27_MVT_CACHE_RETENTION_LOCK_PATH` is a relative path
- **THEN** the wrapper exits 2 with the matching `*_NOT_ABSOLUTE` blocked reason, the runner is not invoked, and no file is created under the repository root
