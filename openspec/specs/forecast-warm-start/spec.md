# forecast-warm-start Specification
...
For every basin × source whose generation history is empty at candidate planning time and whose model carries a published package-manifest reference (registry `resource_profile.manifest_uri`), the scheduler SHALL make exactly one machine-decidable initial-state decision on the generation-aware path before submission. Qualification has two sources, in order: (a) when the referenced manifest carries an `included_files` inventory, the inventory decides — exactly one `*.cfg.ic` entry, canonical (`<shud_input_name>.cfg.ic` when the manifest names the SHUD input directory, otherwise a top-level path), `sha256` differing from the empty-file digest and positive `size_bytes` → qualified; (b) when the manifest is readable but carries no `included_files` inventory (direct-grid variant shape), the registry row's `shud_input_name` and `model_package_uri` SHALL derive the canonical packaged-IC object uri, and a bounded no-follow stat+digest probe of that single object decides — present and non-empty → qualified with the probed `sha256`; missing or empty → unqualified; probe read failure → unreadable; row missing `shud_input_name` or `model_package_uri` → unqualified with a distinct reason. Then: (1) qualified → the packaged initial condition SHALL be selected and consumed (`PACKAGED_IC_BOOTSTRAP`); (2) unqualified (entry missing, non-canonical, ambiguous — more than one `*.cfg.ic` anywhere in the inventory — or empty) or unreadable (manifest or probe cannot be fetched or parsed) → the candidate SHALL be blocked with a typed reason — an unreadable manifest SHALL NOT be treated as absence of an IC. Candidates that reach planning without a qualification signal SHALL retain today's behavior unchanged, in the shape that path produces today: a registry row that publishes no package-manifest reference yields the existing labeled `COLD_NEW_MODEL` decision (evidence mode `db_free_cold_new_model`, reason `no_prior_history`); the two named gate-bypass paths (registry entry without package checksum and declaration, and state-index-unavailable legacy fallback) yield the pre-existing legacy evidence shape, which carries no transition block and no packaged signal — strict mode blocks such first-cycle candidates, while non-strict mode may still reach the legacy `cold_start_no_state` selection fallback (a named residual, not new behavior). The decision SHALL be recorded in the candidate state evidence, the run manifest initial-state block, and the orchestration journal run row. The decision contract SHALL hold identically under strict and non-strict warm-start modes.

#### Scenario: Qualified packaged IC is selected and consumed

- **WHEN** a first-cycle basin's package manifest lists exactly one `*.cfg.ic` entry, that entry is the canonical `<shud_input_name>.cfg.ic` (or a top-level path when the manifest names no SHUD input directory), with `sha256` differing from the empty-file digest and positive `size_bytes`, and the candidate is planned
...
#### Scenario: Inventory-less variant manifest qualifies via the canonical object probe

- **WHEN** a first-cycle basin's referenced manifest is readable but carries no `included_files` inventory, and the registry row's `shud_input_name` and `model_package_uri` locate a non-empty canonical `*.cfg.ic` object in the package
- **THEN** the decision is `PACKAGED_IC_BOOTSTRAP` carrying the probed object `sha256`; a missing or empty canonical object blocks the candidate, and a probe read failure blocks as unreadable rather than being classified as having no IC
