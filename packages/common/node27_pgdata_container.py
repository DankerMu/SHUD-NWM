"""Exact container snapshot normalization and inert PGDATA recreation argv."""
...
PINNED_IMAGE_ID = str(
    json.loads(Path(__file__).with_name("node27_external_contract_snapshot.json").read_text(encoding="utf-8"))[
        "host_context"
    ]["nhms_db_image_id"]["value"]
)
...
class ContainerSnapshot:
    ...
    def config_payload(self) -> dict[str, Any]:
        """Every field reproduced by ``serialize_container_argv`` except runtime ID."""
...
def serialize_container_argv(
    ...
    *,
    ...
    This pure serializer adds no deployment identity or mount. A private env
    file avoids exposing credentials in argv.
    """
