...
----------------------
Constants (``PRJ_WKT``, ``NODE_STEP_X`` / ``NODE_STEP_Y``, ``GRID_LON0``,
``NODE_LON_COUNT``, etc.) are declared at module scope so peer modules
that ``from tests.fixtures.mapping_builder.keliya import build`` receive
the design constants without triggering fixture regeneration. All
file-writing statements are inside :func:`main`, which fires ONLY under
``python tests/fixtures/mapping_builder/keliya/build.py`` (the ``__name__
== "__main__"`` gate). Importing this module never mutates the
checked-in fixture bytes.
"""
...
# Center it at (lon=100.25, lat=36.15) so it straddles the boundary between
# lat rows 1 and 2 and lon columns 2 and 3.
#
# Node grid: 23 lon x 12 lat = 276 nodes -> 22 x 11 quads = 484 triangles.
# Non-square node steps NODE_STEP_X = 1600m and NODE_STEP_Y = 2000m in the
# package CRS give:
#   mesh footprint = 22*1600m x 11*2000m = 35.2km x 22km
# which is exactly 4 cells wide x 2 cells tall in the 0.1 deg WGS84 grid at
# lat 36N, producing exactly 8 unique used cells under nearest-cell mapping.
...
def main() -> None:
    """Regenerate the checked-in keliya fixture files.

    Import-time side effects are intentionally excluded so peer modules
    can ``from tests.fixtures.mapping_builder.keliya import build`` for
    the module-level constants above without overwriting the fixture
    bytes. This entrypoint is only fired under the ``__name__ ==
    "__main__"`` gate at the bottom of this file.
    """
    ...
    transformer_to_pkg = pyproj.Transformer.from_crs(
        "EPSG:4326", pyproj.CRS.from_wkt(PRJ_WKT), always_xy=True
    )
    basin_center_lon, basin_center_lat = 100.25, 36.15
    basin_center_x, basin_center_y = transformer_to_pkg.transform(
    ...
    # referenced. In the variant .sp.att the FORC values will be rewritten
    # to 1..8 (the shud_forcing_index of the 8 used cells).
    att_lines: list[str] = []
    n_att_cols = 9
    ...
    for i, (lon, lat) in enumerate(STATIONS_LONLAT, start=1):
        x, y = transformer_to_pkg.transform(lon, lat)
        tsd_lines.append(
