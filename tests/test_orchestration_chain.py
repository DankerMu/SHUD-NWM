...
def test_parse_success_copybacks_active_run_trees(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from services.orchestrator import chain_forecast_execution
    ...
    assert event["entity_id"] == "gfs_2026050100"
...
def _seed_canonical_precip_tree(
    ...
    *,
    ...
    grid_key = f"canonical/{storage_source}/grid/{storage_source.lower()}_0p25"
    grid_payload = json.dumps({"grid_id": f"{storage_source.lower()}_0p25"}).encode("utf-8")
    grid_path = object_store_root / grid_key / "grid.json"
...
def _precip_context(
    *,
    source_id: str = "IFS",
    cycle_id: str = "ifs_2026050100",
    cycle_time: str = "2026-05-01T00:00:00Z",
    active_basins: list[dict[str, Any]] | None = None,
) -> CycleOrchestrationContext:
    basins = active_basins if active_basins is not None else []
    return CycleOrchestrationContext(
        source_id=source_id,
        cycle_time=_dt(cycle_time),
        cycle_id=cycle_id,
        run_id=f"cycle_{cycle_id}",
        all_basins=list(basins),
        active_basins=list(basins),
    )


def _precip_mirror_events(repository: FakeCycleRepository) -> list[dict[str, Any]]:
    return [event for event in repository.events if event["event_type"] == "canonical_precip_mirror"]


def test_db_free_terminal_stage_mirrors_canonical_precip_and_records_a_receipt(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from services.orchestrator import chain_forecast_execution
    ...
    monkeypatch.setattr(
        ...
        "copyback_run_trees",
        lambda **kwargs: {"status": "copied", "run_ids": list(kwargs["run_ids"])},
    )
    ...
    # Entry names, not `Path.exists()`: macOS is case-insensitive, so an
    # `exists()` probe for `canonical/ifs` answers True for `canonical/IFS`.
    # The directory name carries the `IFS` spelling; `cycle_id` is `ifs_...`.
    ...
    # `redact_payload` blanks any key matching /token/, which is why the cycle
    # key is `cycle`.
    assert "cycle_token" not in json.dumps(event["details"])
    assert "[redacted]" not in json.dumps(event["details"])


def test_canonical_precip_mirror_identity_comes_from_the_context_not_the_cycle_id(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """`cycle_id` lowercases the source; deriving identity from it would write
    `canonical/ifs/2020010100` and the read side would 404."""
    ...
    # Entry names, not `Path.exists()`: macOS is case-insensitive.
    assert [entry.name for entry in (copyback_root / "canonical").iterdir()] == ["IFS"]
    assert sorted(entry.name for entry in (copyback_root / "canonical/IFS").iterdir()) == [
        "2026050100",
        "grid",
    ]
...
def test_canonical_precip_mirror_runs_for_every_terminal_status_but_reconcile_unverified(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    result_status: str,
) -> None:
...
def test_canonical_precip_mirror_does_not_run_on_reconcile_unverified(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    repository = FakeCycleRepository()
...
def test_canonical_precip_mirror_ignores_the_active_basins_gate(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    copyback_root = tmp_path / "shared-object-store"
    monkeypatch.setenv("NHMS_OBJECT_STORE_COPYBACK_ROOT", str(copyback_root))
    run_tree_calls: list[dict[str, Any]] = []
    monkeypatch.setattr(
        chain_forecast_execution,
        "copyback_run_trees",
        lambda **kwargs: run_tree_calls.append(dict(kwargs)),
    )
    _seed_canonical_precip_tree(
        Path(orchestrator.config.object_store_root),
        storage_source="IFS",
...
def test_canonical_precip_mirror_exception_is_swallowed_and_recorded(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from services.orchestrator import chain_forecast_execution
    ...
    client = FakeCycleSlurmClient()
    orchestrator = _orchestrator(tmp_path, repository, client, terminal_stage="forecast_state_save_qc")
    monkeypatch.setenv("NHMS_OBJECT_STORE_COPYBACK_ROOT", str(tmp_path / "shared-object-store"))
    ...
    assert mirror["error"] == "mirror exploded"
    assert mirror["error_type"] == "RuntimeError"
    assert mirror["storage_source"] == "IFS"
    assert mirror["cycle"] == "2026050100"


def test_canonical_precip_mirror_failure_receipt_keeps_the_cycle_token_unredacted(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Persisted round trip on the failure branch. The inline failure summary
    spells the cycle key `cycle`, never `cycle_token`: `redact_payload` blanks
    any key matching /token/, so the `cycle_token` spelling would land in the
    journal as `[redacted]` and destroy the cycle identity of the one receipt
    that says the mirror failed."""
    ...
    # No seeded tree: the publisher explodes before it reads the source root.
    monkeypatch.setattr(chain_forecast_execution, "TilePublisher", ExplodingPublisher)
    ...
    # lowercase source in the cycle id, and rotates into `<cycle>.<n>.jsonl`.
    segments = sorted((tmp_path / "journal" / "journal" / "IFS").glob("2026050100*.jsonl"))
    assert segments
    text = "\n".join(path.read_text(encoding="utf-8") for path in segments)
    # The spelling assertions come first: they name the defect directly, rather
    # than surfacing it as a downstream KeyError on the `cycle` lookup.
    assert "[redacted]" not in text
    assert "cycle_token" not in text
    records = [
        json.loads(line)
    ...
    events = [
        ...
        and record["payload"].get("event_type") == "canonical_precip_mirror"
    ]
    assert len(events) == 1
    assert events[0]["status_to"] == "failed"
...
@pytest.mark.parametrize("error_type", ["journal", "orchestrator"])
def test_canonical_precip_mirror_receipt_write_failure_is_swallowed(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    error_type: str,
) -> None:
    """`FileOrchestrationJournalError` is a sibling of `OrchestratorError`, not a
    subclass, so a swallow narrowed to either one lets a journal fault abort the
    terminal stage."""
    ...
    raised = (
        FileOrchestrationJournalError("journal is gone", field="details")
        if error_type == "journal"
        else OrchestratorError("PIPELINE_EVENT_FAILED", "event write failed")
    )
    assert not isinstance(raised, OrchestratorError) or error_type == "orchestrator"

    def failing_insert_pipeline_event(**kwargs: Any) -> dict[str, Any]:
...
def test_canonical_precip_mirror_receipt_survives_the_journal_sanitizer(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    absent: str,
) -> None:
    """Persisted round trip. `_public_pipeline_event_payload` renders every
    `_path`/`_root` key as `[local-path]`, so the two absent-source states are
    told apart by `missing_object_key`, which is relative and survives."""
    ...
    _seed_canonical_precip_tree(object_store_root, storage_source="IFS", cycle_token="2026050100")
    if absent == "prcp":
        shutil.rmtree(object_store_root / "canonical/IFS/2026050100/prcp_rate_or_amount")
        expected_object_key = "canonical/IFS/2026050100/prcp_rate_or_amount"
    else:
        shutil.rmtree(object_store_root / "canonical/IFS/grid")
    ...
    assert mirror["storage_source"] == "IFS"
    assert mirror["cycle"] == "2026050100"
    assert mirror["missing_object_key"] == expected_object_key
    # The two states are distinguishable only because of that key: the sanitizer
    # collapses both absolute paths to the same placeholder.
    assert mirror["missing_path"] == "[local-path]"
    assert mirror["root"] == "[local-path]"


def test_canonical_precip_mirror_persisted_success_renders_object_keys_verbatim(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    assert [event["status_to"] for event in events] == ["ok", "skipped"]
    mirror = events[0]["details"]["precip_mirror"]
    assert mirror["status"] == "ok"
    assert mirror["storage_source"] == "IFS"
    assert mirror["cycle"] == "2026050100"
    ...
    assert {tree["action"] for tree in mirror["trees"]} == {"copy"}
    assert mirror["root"] == "[local-path]"
    # The duplicate records `skipped`, because the mirror is idempotent. Its
    # `reason` must render verbatim too -- the sanitizer touches neither key.
    duplicate = events[1]["details"]["precip_mirror"]
...
# --------------------------------------------------------------------------- #
# #2070 / #2076: the mirror runs after the `convert` entry's cycle-status write,
# and `_run_cycle_chain` re-runs it once at chain exit for a pass that skipped
# `convert` (downstream restart) or whose mirror recorded `failed`. Every test
# below drives `orchestrate_cycle` / `_after_cycle_stage_terminal` and reads the
# receipts; none calls `_mirror_canonical_precip` directly.
# --------------------------------------------------------------------------- #
_PRECIP_HOOK_MESSAGE = "Canonical precipitation mirror ran after convert."
_PRECIP_RECOVERY_MESSAGE = "Canonical precipitation mirror recovered at chain exit."


def _downstream_restart_basins(restart_stage: str = "forecast") -> list[dict[str, Any]]:
    basins = _basins(2)
    for basin in basins:
...
def _precip_chain_orchestrator(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    repository: FakeCycleRepository,
    client: FakeCycleSlurmClient,
    *,
    copyback_root: Path | None,
) -> ForecastOrchestrator:
    """node-22's live shape: the chain ends at `state_save_qc`, whose sibling
    run-tree copyback is stubbed so only the canonical lane touches the root."""

    from services.orchestrator import chain_forecast_execution

    orchestrator = _orchestrator(tmp_path, repository, client, terminal_stage="forecast_state_save_qc")
    if copyback_root is None:
        monkeypatch.delenv("NHMS_OBJECT_STORE_COPYBACK_ROOT", raising=False)
    else:
        monkeypatch.setenv("NHMS_OBJECT_STORE_COPYBACK_ROOT", str(copyback_root))
    monkeypatch.setattr(
...
def _mirrored_files(copyback_root: Path) -> dict[str, bytes]:
    return {
        str(path.relative_to(copyback_root)): path.read_bytes()
        for path in copyback_root.rglob("*")
        if path.is_file() and path.name != COPYBACK_BATCH_LOCK_NAME
    }


def test_canonical_precip_mirror_downstream_restart_pass_recovers_a_missing_mirror_at_chain_exit(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """#2076 scenario 1: a `restart_stage == "forecast"` pass skips `convert` at the
    `stage_index < start_stage_index` guard, so the terminal hook never fires; the
    chain-exit recovery is the only thing that can mirror the cycle."""
    ...
    assert result.status == "succeeded"
    # `convert` was neither submitted nor resumed on this pass.
    assert "convert" not in {submission["stage"] for submission in client.submissions}
    assert "convert" not in {stage.stage for stage in result.stages}
    assert not any(job.get("stage") == "convert" for job in repository.jobs.values())
    ...
    assert events[0]["entity_id"] == "gfs_2026050100"
    mirror = events[0]["details"]["precip_mirror"]
    assert set(events[0]["details"]) == {"precip_mirror"}
    assert mirror["status"] == "ok"
    assert mirror["storage_source"] == "gfs"
    assert mirror["cycle"] == "2026050100"
...
def test_canonical_precip_mirror_downstream_restart_pass_skips_an_identical_mirror_without_rewriting(
    ...
    """The accepted per-pass cost (#2076): an already-identical tree still yields one
    `skipped` (`trees_already_mirrored`) receipt, and no destination byte is rewritten."""
    ...
    # Both trees -- the cycle and the grid -- or the grid tree plans a copy and the
    # receipt reads `ok`.
    shutil.copytree(object_store_root / "canonical", copyback_root / "canonical")
    before = {
        key: ((copyback_root / key).stat().st_ino, (copyback_root / key).stat().st_mtime_ns)
        for key in payloads
    }
    ...
    assert result.status == "succeeded"
    events = _precip_mirror_events(repository)
    assert [(event["status_to"], event["message"]) for event in events] == [("skipped", _PRECIP_RECOVERY_MESSAGE)]
    mirror = events[0]["details"]["precip_mirror"]
    assert mirror["reason"] == "trees_already_mirrored"
    assert {tree["action"] for tree in mirror["trees"]} == {"skip"}
    after = {
        key: ((copyback_root / key).stat().st_ino, (copyback_root / key).stat().st_mtime_ns)
        for key in payloads
    }
...
def test_canonical_precip_mirror_failed_hook_is_healed_by_a_later_downstream_restart_pass(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """#2076 scenario 2. Pass 1 runs the full chain with the mirror faulted: the
    `convert` hook records `failed` and its same-pass chain-exit retry fails too.
    Pass 2 enters downstream of `convert` after the fault clears and heals it."""
    ...
    class FlakyPublisher(real_publisher):  # type: ignore[misc, valid-type]
        def copyback_canonical_precip(self, source: str, cycle: str) -> dict[str, Any] | None:
            if fault["active"]:
                raise OSError("copyback mount stalled")
            return super().copyback_canonical_precip(source, cycle)

    monkeypatch.setattr(chain_forecast_execution, "TilePublisher", FlakyPublisher)

    first = orchestrator.orchestrate_cycle("gfs", "2026050100", _basins(2))

    assert first.status == "succeeded"
    assert "convert" in {submission["stage"] for submission in client.submissions}
    first_events = _precip_mirror_events(repository)
    assert [(event["status_to"], event["message"]) for event in first_events] == [
        ("failed", _PRECIP_HOOK_MESSAGE),
        ("failed", _PRECIP_RECOVERY_MESSAGE),
    ]
    assert _mirrored_files(copyback_root) == {}

    fault["active"] = False
    submissions_before = len(client.submissions)
    second = orchestrator.orchestrate_cycle("gfs", "2026050100", _downstream_restart_basins())

    assert second.status == "succeeded"
    assert "convert" not in {submission["stage"] for submission in client.submissions[submissions_before:]}
    events = _precip_mirror_events(repository)
    assert len(events) == 3
    healed = events[2]
    assert healed["message"] == _PRECIP_RECOVERY_MESSAGE
    assert healed["status_to"] == "ok"
    assert _mirrored_files(copyback_root) == payloads


def test_canonical_precip_mirror_full_chain_ok_hook_runs_no_chain_exit_recovery(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    repository = FakeCycleRepository()
...
def test_canonical_precip_mirror_convert_reached_without_a_terminal_entry_runs_no_recovery(
    ...
    """The pass reached `convert` but its submit failed before the hook call site
    (`submission_failed`), so nothing ran and there is nothing to retry."""
    ...
    real_submit_job = client.submit_job

    def failing_convert_submit(payload: dict[str, Any]) -> dict[str, Any]:
        if payload["manifest"]["stage"] == "convert":
            raise RuntimeError("convert submission failed")
        return real_submit_job(payload)

    monkeypatch.setattr(client, "submit_job", failing_convert_submit)

    result = orchestrator.orchestrate_cycle("gfs", "2026050100", _basins(2))

    assert result.status == "failed"
    assert [stage.status for stage in result.stages] == ["submission_failed"]
    assert _precip_mirror_events(repository) == []
    assert not (copyback_root / "canonical").exists()
...
@pytest.mark.parametrize(
    "case",
    ["tree_absent", "symlinked_leaf", "symlinked_ancestor", "no_copyback_root"],
)
def test_canonical_precip_mirror_chain_exit_recovery_needs_a_local_unsymlinked_tree_and_a_copyback_root(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    case: str,
) -> None:
    """The recovery predicate refuses a symlink at ANY component of
    `canonical/<S>/<cycle>/prcp_rate_or_amount` (not just the leaf), treats an
    absent tree as nothing to recover (a retention-pruned cycle), and is fenced by
    `NHMS_OBJECT_STORE_COPYBACK_ROOT` -- in every case no receipt is appended."""

    repository = FakeCycleRepository()
    client = FakeCycleSlurmClient()
    copyback_root = tmp_path / "shared-object-store"
    orchestrator = _precip_chain_orchestrator(
        monkeypatch,
        tmp_path,
        repository,
        client,
        copyback_root=None if case == "no_copyback_root" else copyback_root,
    )
    object_store_root = Path(orchestrator.config.object_store_root)
    cycle_dir = object_store_root / "canonical/gfs/2026050100"
    if case == "tree_absent":
        # Grid only: the source tree the recovery keys on was pruned.
        _seed_canonical_precip_tree(object_store_root, storage_source="gfs", cycle_token="2026050100")
        shutil.rmtree(cycle_dir)
    elif case == "symlinked_leaf":
        outside = tmp_path / "outside"
        _seed_canonical_precip_tree(outside, storage_source="gfs", cycle_token="2026050100")
        _seed_canonical_precip_tree(object_store_root, storage_source="gfs", cycle_token="2026050100")
        shutil.rmtree(cycle_dir / "prcp_rate_or_amount")
        (cycle_dir / "prcp_rate_or_amount").symlink_to(outside / "canonical/gfs/2026050100/prcp_rate_or_amount")
        ...
        _seed_canonical_precip_tree(object_store_root, storage_source="gfs", cycle_token="2026050100")
        shutil.rmtree(cycle_dir)
        cycle_dir.symlink_to(outside / "canonical/gfs/2026050100")
        # The leaf itself is a real directory; only its parent is a link.
        assert not (cycle_dir / "prcp_rate_or_amount").is_symlink()
        assert (cycle_dir / "prcp_rate_or_amount").is_dir()
    else:
        _seed_canonical_precip_tree(object_store_root, storage_source="gfs", cycle_token="2026050100")
    from packages.common import safe_fs
...
def _downstream_restart_outcome(result: Any) -> tuple[str, tuple[tuple[str, str], ...]]:
    return result.status, tuple((stage.stage, stage.status) for stage in result.stages)


@pytest.mark.parametrize("fault", ["publisher_raises", "mirror_raises", "receipt_write_raises"])
def test_canonical_precip_mirror_chain_exit_recovery_is_fail_open(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    fault: str,
) -> None:
    """#2076 scenario 4: whatever the recovery's mirror or receipt write raises,
    `_run_cycle_chain` returns what it would have returned without the recovery."""

    from services.orchestrator import chain_forecast_execution

    baseline_repository = FakeCycleRepository()
    baseline = _precip_chain_orchestrator(
        monkeypatch, tmp_path / "baseline", baseline_repository, FakeCycleSlurmClient(), copyback_root=None
    ).orchestrate_cycle("gfs", "2026050100", _downstream_restart_basins())

    repository = FakeCycleRepository()
    copyback_root = tmp_path / "faulted" / "shared-object-store"
    orchestrator = _precip_chain_orchestrator(
        monkeypatch, tmp_path / "faulted", repository, FakeCycleSlurmClient(), copyback_root=copyback_root
    )
    _seed_canonical_precip_tree(
        Path(orchestrator.config.object_store_root), storage_source="gfs", cycle_token="2026050100"
    )
    attempts: list[str] = []
    if fault == "publisher_raises":
        ...
        monkeypatch.setattr(chain_forecast_execution, "TilePublisher", ExplodingPublisher)
    elif fault == "mirror_raises":

        def exploding_mirror(self: Any, context: Any, **kwargs: Any) -> None:
            attempts.append(str(kwargs.get("message")))
            raise RuntimeError("mirror entry exploded")

        monkeypatch.setattr(chain_forecast_execution, "_mirror_canonical_precip", exploding_mirror)
    else:
        real_insert = repository.insert_pipeline_event
    ...
    assert _downstream_restart_outcome(result) == _downstream_restart_outcome(baseline)
    # The recovery really ran into the fault -- otherwise a green here proves nothing.
    assert len(attempts) == 1
    if fault == "publisher_raises":
        events = _precip_mirror_events(repository)
        assert [(event["status_to"], event["message"]) for event in events] == [
            ("failed", _PRECIP_RECOVERY_MESSAGE)
        ]
        assert events[0]["details"]["precip_mirror"]["error_type"] == "RuntimeError"
    elif fault == "receipt_write_raises":
        assert attempts == [_PRECIP_RECOVERY_MESSAGE]
        assert _precip_mirror_events(repository) == []
        assert (copyback_root / "canonical/gfs/2026050100/prcp_rate_or_amount").is_dir()


def test_canonical_precip_mirror_chain_exit_recovery_runs_when_a_downstream_stage_raises(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Every exit counts: a stage raising `OrchestratorError` on a downstream-restart
    pass still gets the recovery, and the original exception is what propagates."""
    ...
    raised = OrchestratorError("STAGE_EXPLODED", "forecast stage exploded")
    stages_entered: list[str] = []

    def exploding_stage(stage: StageDefinition, context: Any, **kwargs: Any) -> Any:
        del context, kwargs
        stages_entered.append(stage.stage)
        raise raised

    monkeypatch.setattr(orchestrator, "_submit_and_wait_cycle_stage", exploding_stage)

    with pytest.raises(OrchestratorError) as error_info:
        orchestrator.orchestrate_cycle("gfs", "2026050100", _downstream_restart_basins())

    assert error_info.value is raised
    assert stages_entered == ["forecast"]
    events = _precip_mirror_events(repository)
    assert [(event["status_to"], event["message"]) for event in events] == [("ok", _PRECIP_RECOVERY_MESSAGE)]
    assert _mirrored_files(copyback_root) == payloads


class _CallOrderCycleRepository(FakeCycleRepository):
    """`events` and `cycle_statuses` are separate lists on the base fake, so their
    relative order is unrecoverable; this one writes both into one recorder."""

    def __init__(self, calls: list[tuple[str, str]], *, fail_status_write: bool = False) -> None:
        super().__init__()
        self.calls = calls
        self.fail_status_write = fail_status_write

    def update_forecast_cycle_status(self, **kwargs: Any) -> dict[str, Any]:
        self.calls.append(("cycle_status", str(kwargs["status"])))
        if self.fail_status_write:
            raise _CycleStatusWriteFailed("cycle status write failed")
        return super().update_forecast_cycle_status(**kwargs)


class _CycleStatusWriteFailed(Exception):
    pass


def _record_mirror_start(monkeypatch: pytest.MonkeyPatch, calls: list[tuple[str, str]]) -> None:
    from services.orchestrator import chain_forecast_execution

    real_publisher = chain_forecast_execution.TilePublisher

    class RecordingPublisher(real_publisher):  # type: ignore[misc, valid-type]
        def copyback_canonical_precip(self, source: str, cycle: str) -> dict[str, Any] | None:
            calls.append(("mirror_start", cycle))
            return super().copyback_canonical_precip(source, cycle)

    monkeypatch.setattr(chain_forecast_execution, "TilePublisher", RecordingPublisher)


@pytest.mark.parametrize("result_status", ["succeeded", "failed"])
def test_canonical_precip_mirror_runs_after_the_entry_cycle_status_write(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    result_status: str,
) -> None:
    """#2070: a stalled copyback mount must not delay the `convert` entry's own
    cycle-status write, so that write is recorded before the mirror starts -- on
    `succeeded` and on the failure tail alike."""

    calls: list[tuple[str, str]] = []
    repository = _CallOrderCycleRepository(calls)
    orchestrator = _orchestrator(tmp_path, repository, FakeCycleSlurmClient(), terminal_stage="forecast_state_save_qc")
    copyback_root = tmp_path / "shared-object-store"
    monkeypatch.setenv("NHMS_OBJECT_STORE_COPYBACK_ROOT", str(copyback_root))
    _seed_canonical_precip_tree(
        Path(orchestrator.config.object_store_root), storage_source="IFS", cycle_token="2026050100"
    )
    _record_mirror_start(monkeypatch, calls)

    orchestrator._after_cycle_stage_terminal(
        _PRECIP_CONVERT_STAGE, _precip_context(), result_status, {"status": result_status}, None
    )

    expected_status = (
        _PRECIP_CONVERT_STAGE.success_cycle_status
        if result_status == "succeeded"
        else _PRECIP_CONVERT_STAGE.failure_cycle_status
    )
    assert calls == [("cycle_status", expected_status), ("mirror_start", "2026050100")]
    assert [event["status_to"] for event in _precip_mirror_events(repository)] == ["ok"]


@pytest.mark.parametrize("result_status", ["succeeded", "failed"])
def test_canonical_precip_mirror_still_runs_when_the_entry_cycle_status_write_raises(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    result_status: str,
) -> None:
    calls: list[tuple[str, str]] = []
    repository = _CallOrderCycleRepository(calls, fail_status_write=True)
    orchestrator = _orchestrator(tmp_path, repository, FakeCycleSlurmClient(), terminal_stage="forecast_state_save_qc")
    copyback_root = tmp_path / "shared-object-store"
    monkeypatch.setenv("NHMS_OBJECT_STORE_COPYBACK_ROOT", str(copyback_root))
    payloads = _seed_canonical_precip_tree(
    ...
    with pytest.raises(_CycleStatusWriteFailed, match="cycle status write failed"):
        orchestrator._after_cycle_stage_terminal(
            _PRECIP_CONVERT_STAGE, _precip_context(), result_status, {"status": result_status}, None
        )

    assert [name for name, _ in calls] == ["cycle_status", "mirror_start"]
    assert [event["status_to"] for event in _precip_mirror_events(repository)] == ["ok"]
    assert _mirrored_files(copyback_root) == payloads
...
def test_chain_http_slurm_gateway_client_legacy_subclass_uses_owner_module() -> None:
    ...
    assert client._validated_submit_response(
        {"job_id": "mock_1", "status": "submitted"}
    ) == {"job_id": "mock_1", "status": "submitted"}
...
@pytest.mark.parametrize(
    ...
    [
        {"job_id": "mock_1"},
        {"job_id": "mock_1", "status": None},
        {"job_id": "mock_1", "status": ""},
        {"job_id": "mock_1", "status": "pending"},
        {"job_id": "mock_1", "status": "queued"},
        {"job_id": "mock_1", "status": "unknown"},
        {"job_id": "mock_1", "status": ["submitted"]},
    ],
)
def test_round10_http_submit_response_rejects_status_outside_gateway_contract(
    response: dict[str, Any],
) -> None:
    from services.orchestrator.chain import HttpSlurmGatewayClient, SlurmClientError
    from services.orchestrator.chain_config import SubmitDisposition

    client = HttpSlurmGatewayClient("http://gateway.example")
    with pytest.raises(SlurmClientError) as exc_info:
        client._validated_submit_response(response)
    assert exc_info.value.error_code == "SLURM_GATEWAY_INVALID_RESPONSE"
    assert exc_info.value.submit_disposition == SubmitDisposition.AMBIGUOUS
...
def test_round10_http_submit_response_accepts_all_gateway_statuses(status: str) -> None:
    from services.orchestrator.chain import HttpSlurmGatewayClient

    response = {"job_id": "mock_1", "status": status}
    assert HttpSlurmGatewayClient("http://gateway.example")._validated_submit_response(
        response
    ) == response
...
def test_array_stage_submission_threads_idempotency_comment(tmp_path: Path) -> None:
    ...
    observed_reservations: dict[str, dict[str, Any] | None] = {}
    ...
    class _ArrayCommentCapturingClient(FakeCycleSlurmClient):
        def submit_job_array(
            ...
            *,
            ...
            array_comments[stage_name] = comment
            key = idempotency_key_from_comment(comment)
            observed_reservations[stage_name] = repository.query_candidate_state(key)
    ...
    for stage_name in array_stages:
        ...
        reservation = observed_reservations.get(stage_name)
        assert reservation is not None, f"array stage {stage_name} submitted before reservation"
        assert reservation["status"] == "reserved"
        assert reservation["slurm_job_id"] is None

        job = repository.jobs[f"job_cycle_gfs_2026050100_{stage_name}"]
        assert job["idempotency_key"] == f"cycle_gfs_2026050100:{stage_name}"
        assert job["slurm_job_id"] is not None


def test_chain_stage_execution_internal_calls_preserve_legacy_override_surface(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    repository = StoreBackedCycleRepository(_pipeline_store())
    client = FakeCycleSlurmClient()
    orchestrator = _orchestrator(tmp_path, repository, client)
    cycle_time = _dt("2026-05-01T00:00:00Z")
    basins = orchestrator._normalize_cycle_basins(_basins(1), "gfs", cycle_time)
    ...
    def _slurm_submission_manifest(manifest: Mapping[str, Any]) -> dict[str, Any]:
        manifest_calls.append(str(manifest["stage"]))
        submission = dict(manifest)
        submission["legacy_override_marker"] = True
        return submission
    ...
    assert result.status == "succeeded"
    assert aggregation is not None
    assert array_calls == ["forcing"]
    assert manifest_calls == ["forcing"]
    ...
    def _poll_cycle_stage_until_terminal(
        *,
        ...
        poll_calls.append(str(initial_job["job_id"]))
        terminal = dict(initial_job)
        terminal.update(
            {
                "status": "succeeded",
                "finished_at": _fmt(_dt("2026-05-01T00:02:00Z")),
                "accounting": {"elapsed_raw": "00:01:00"},
            }
        )
        return TerminalJobObservation(job=terminal)

    monkeypatch.setattr(orchestrator, "_poll_cycle_stage_until_terminal", _poll_cycle_stage_until_terminal)
...
def test_round10_forecast_poll_timeout_stops_chain_as_reconciling(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from services.orchestrator.file_orchestration_journal import FileOrchestrationJournalRepository

    class Client(FakeCycleSlurmClient):
        def _submit(
            self, stage: str, run_id: str, model_id: str, payload: dict[str, Any]
        ) -> dict[str, Any]:
            response = super()._submit(stage, run_id, model_id, payload)
            if stage != "forecast":
                response["status"] = "succeeded"
                response["finished_at"] = "2026-05-01T00:02:00Z"
                response["exit_code"] = 0
                self.jobs[response["job_id"]].update(response)
            return response

    repository = FileOrchestrationJournalRepository(tmp_path / "journal")
    client = Client(never_terminal_stage="forecast")
    orchestrator = _orchestrator(tmp_path, repository, client)
    orchestrator.config = OrchestratorConfig(
        workspace_root=tmp_path / "workspace",
        object_store_root=tmp_path / "object-store",
    ...
    assert result.stages[-1].status == "reconcile_unverified"
    assert [submission["stage"] for submission in client.submissions] == ["forecast"]
    job = repository.get_pipeline_job(result.stages[-1].pipeline_job_id)
    assert job["status"] == "reconcile_unverified"
    ...
    assert any(
        event["event_type"] == "reconcile_unverified"
        and event["status_to"] == "reconcile_unverified"
        for event in events
    )
...
def test_round10_forecast_unknown_submit_status_is_ambiguous_and_unbound(
    ...
    class Client(FakeCycleSlurmClient):
        def submit_job_array(self, *args: Any, **kwargs: Any) -> dict[str, Any]:
            response = super().submit_job_array(*args, **kwargs)
            if response["stage"] == "forecast":
...
def _file_journal_failed_master_at_attempt(
    tmp_path: Path,
    *,
    error_code: str,
    attempt: int,
) -> tuple[Any, dict[str, Any]]:
    """The same master, reserved under a production ``_retry_<n>`` identity."""

    from tests.test_file_orchestration_journal import _retry_attempt_failed_cohort_master

    return _retry_attempt_failed_cohort_master(tmp_path, error_code=error_code, attempt=attempt)
...
def test_cycle_retry_decline_marks_an_exhausted_transient_master_row(tmp_path: Path) -> None:
    ...
    "No budget left" is expressed the way production expresses it (#1312
    round-4): the third attempt is a ``..._forecast_retry_2`` reservation whose
    durable ``retry_count`` stays 0, and ``_retry_job_for_stage_result``
    resolves the effective attempt from that id suffix — a ``max_retries=0``
    budget would exhaust on attempt 0 and never exercise the suffix path.
    """

    repository, record = _file_journal_failed_master_at_attempt(
        tmp_path,
        error_code="NODE_FAILURE",
        attempt=2,
    )
    retry_service = _file_journal_retry_service(repository, max_retries=2)
    orchestrator = _orchestrator(tmp_path, repository, FakeCycleSlurmClient(), retry_service=retry_service)
    result = _forecast_stage_run_result(str(record["job_id"]))
...
def test_persistent_file_journal_oom_cohort_stays_permanently_failed_across_two_passes(
    ...
    """#1312 seam 9 (design D9): the end-to-end invariants across two passes.

    What this pins is the *pass-level* contract: a real ``orchestrate_cycle``
    lands the mark on pass one, a second pass over the same journal raises
    nothing, the row is still ``permanently_failed`` when re-read from a freshly
    opened repository, exactly one ``permanently_failed`` event exists, and both
...
def test_a_failed_permanent_failure_mark_is_re_attempted_on_the_next_pass(tmp_path: Path) -> None:
    """#1312 round-4 (e): the write-failure fallback's own promise, end to end.

    The spec's write-failure THEN says the pass degrades *because* "the
    idempotent mark is re-attempted on a later pass".  The unit tests pin the
    degrade half (no raise, row unchanged, operator event); nothing pinned the
    recovery half, so a fallback that quietly parked the row on ``failed``
    forever would still have been green.

    Pass one's mark raises and the row stays ``failed`` — which is exactly why
    pass two does NOT take the already-terminal defer branch the sticky e2e
    takes: it re-projects, re-declines, and re-marks.  The mark's idempotency
    then has to hold across the two attempts: exactly one
    ``permanently_failed`` event, not one per pass.
    """
    ...
    class _MarkFailsOnceJournalRepository(FileOrchestrationJournalRepository):
        mark_attempts = 0

        def mark_pipeline_job_permanently_failed(self, job_id: str, **kwargs: Any) -> Any:
            type(self).mark_attempts += 1
            if type(self).mark_attempts == 1:
                raise OrchestratorError(
                    "FILE_JOURNAL_WRITE_FAILED",
                    "failed to append file orchestration journal records",
                    {"job_id": job_id},
                )
            return super().mark_pipeline_job_permanently_failed(job_id, **kwargs)
    ...
    repository = _MarkFailsOnceJournalRepository(tmp_path / "journal")
    object_store = LocalObjectStore(tmp_path / "object-store", "s3://nhms")
    client = _OutOfMemoryForecastArrayClient(
        object_store=object_store,
        run_ids=[str(basin["run_id"]) for basin in basins],
        array_results_by_stage={"forecast": [["failed", "failed"], ["failed", "failed"]]},
    )
    orchestrator = _orchestrator(
        tmp_path,
        repository,
    ...
    assert _permanently_failed_master_events(repository, after_first) == []
    mark_failures = [
        event
        for event in _master_events(repository, after_first)
    ...
    assert after_second["status"] == "permanently_failed"
    assert reopened["status"] == "permanently_failed"
    assert len(_permanently_failed_master_events(repository, after_second)) == 1
...
def test_cycle_survives_a_failed_permanent_failure_mark_and_records_it(
    tmp_path: Path,
    error_kind: str,
) -> None:
    ...
    Both halves of the catch tuple are injected (#1312 round-4, mutant o2): the
    typed mark raises ``OrchestratorError`` from the append/durability layer and
    ``FileOrchestrationJournalError`` from its own record validators, so pinning
    only one class leaves narrowing the tuple to the other one undetected.
    """
...
# ---------------------------------------------------------------------------
# #1410: the cohort projection receives the master Slurm identity explicitly.
# ---------------------------------------------------------------------------


_COHORT_JOB_ID = "job_cycle_gfs_2026050100_forecast_cohort_fixture_forecast"


def _forecast_cohort_basins() -> list[dict[str, Any]]:
    basins = _basins(2)
    for index, basin in enumerate(basins):
        basin.update(
            {
...
def _spy_cohort_projections(repository: Any) -> list[dict[str, Any]]:
    ...
    real = repository.project_forecast_cohort_tasks
    calls: list[dict[str, Any]] = []

    def spy(job_id: str, **kwargs: Any) -> dict[str, int]:
        result = real(job_id, **kwargs)
        calls.append({"job_id": job_id, "result": result, **kwargs})
        return result

    repository.project_forecast_cohort_tasks = spy  # type: ignore[method-assign]
    return calls


def test_submit_leg_projects_the_cohort_with_the_gateway_master_slurm_id(tmp_path: Path) -> None:
    """#1410 sibling leg: submit/poll keeps handing the projection the gateway id."""
    ...
    master = repository.get_pipeline_job(_COHORT_JOB_ID)
    assert master is not None
    assert [call["master_slurm_job_id"] for call in calls] == [str(master["slurm_job_id"])]
    assert str(master["slurm_job_id"]).isdigit()


def test_resume_of_a_terminal_cohort_projects_with_the_real_master_slurm_id(tmp_path: Path) -> None:
    """#1410: the already-terminal resume leg must not sniff the pipeline job id.
...
class _CohortProjectionOnlyRepository:
    """Minimal cohort repository: the projector must never be reached."""
    ...
    def project_forecast_cohort_tasks(self, job_id: str, **kwargs: Any) -> dict[str, int]:
        self.calls.append({"job_id": job_id, **kwargs})
        return {"total": 1, "pipeline_status": 1, "pipeline_event": 0}
...
@pytest.mark.parametrize("master_slurm_job_id", ["", "job_cycle_x"])
def test_cohort_override_fails_closed_without_a_numeric_master_slurm_id(
    master_slurm_job_id: str,
) -> None:
    """#1410: an empty or non-numeric master identity is attributable, not silent."""

    from services.orchestrator.chain_array_accounting import record_cycle_stage_status_override
    from services.orchestrator.chain_types import ArrayAggregation, ArrayTaskResult

    repository = _CohortProjectionOnlyRepository()
    orchestrator = types.SimpleNamespace(repository=repository)
    context = CycleOrchestrationContext(
        source_id="gfs",
        cycle_time=_dt("2026-05-01T00:00:00Z"),
        cycle_id="gfs_2026050100",
    ...
    aggregation = ArrayAggregation(
        ...
        failed=0,
        cancelled=0,
        task_results=(ArrayTaskResult(task_id=0, slurm_job_id="2001_0", status="succeeded"),),
    )
    ...
    with pytest.raises(OrchestratorError) as error:
        record_cycle_stage_status_override(
            ...
            context,
            _COHORT_JOB_ID,
            {"job_id": _COHORT_JOB_ID, "status": "succeeded"},
            aggregation,
            None,
            master_slurm_job_id=master_slurm_job_id,
        )

    assert error.value.error_code == "SLURM_MASTER_IDENTITY_UNAVAILABLE"
...
def test_canonical_precip_mirror_lock_timeout_records_a_failed_receipt_and_does_not_raise(
    ...
    """E11: the cycle survives with a `failed` receipt; the hook itself does not retry.
    ...
    Only a cycle that receives no further pass still needs
    `scripts/canonical_precip_copyback_backfill.py`
    (`docs/runbooks/current-production-ops.md` §5.3). Saying it "defers to the
    next cycle" would still be false: the retry rides passes for THIS cycle.
    This test calls the hook directly, so it sees only the hook's own receipt.
    """
...
def test_canonical_precip_mirror_lock_timeout_still_advances_the_cycle_stage(
    ...
    """The canonical mirror's timeout does NOT hold the stage back -- the retry lives elsewhere.
    ...
    the `failed` mirror is instead retried by the chain-exit recovery (#2076) --
    once at the exit of the same pass and on every later downstream-restart pass
    for the cycle -- and the manual backfill CLI is only for a cycle that gets no
    further pass.
