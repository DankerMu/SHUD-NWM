...
def test_run_identity_pushdown_returns_byte_identical_rows_on_every_forecast_shape(
    ...
    emptying the four pushdown fragments — the same monkeypatch idiom this module
    already uses on ``_QHH_LATEST_CANDIDATE_RUNS_SQL`` — so both sides run the
    identical outer layer and differ only in what was pushed into the branches.
    """
    _prepared_database(throwaway_database_url)
    connection = _connect(throwaway_database_url)
    try:
