...
def test_authority_stats_hygiene_migration_splits_into_the_expected_statements() -> None:
    """000052 mixes a DO block with two CONCURRENTLY statements (issue #1468).

    ``split_sql_statements`` is the splitter ``packages.common.migrate`` applies
    migrations with, so it is the authoritative statement oracle. Two things
    ...
    * the ``$$`` body survives whole -- the DO block carries five internal
      ...
      broken fragments;
    * no statement is wrapped in a transaction and the two CONCURRENTLY
      statements arrive as their own top-level statements -- ``CREATE/DROP
      INDEX CONCURRENTLY`` is rejected inside a transaction block, and
    ...
    assert len(executables) == 7, f"expected seven executable statements, got {len(executables)}"

    do_block = executables[0]
    assert do_block.startswith("DO $$"), do_block
    assert do_block.endswith("$$;"), do_block
    # The whole body, semicolons and all, is inside statement 0.
    ...
    assert executables[1].startswith(
    ...
    assert "GIN (lower(river_segment_id) gin_trgm_ops)" in executables[1], executables[1]
    assert executables[2] == (
        "DROP INDEX CONCURRENTLY IF EXISTS core.river_segment_id_trgm_idx_legacy;"
    ), executables[2]

    assert [
        re.sub(r"\s+", " ", statement).strip() for statement in executables[3:]
    ] == [
