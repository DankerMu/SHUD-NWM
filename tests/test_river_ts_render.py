...
# ---------------------------------------------------------------------------
# PostgreSQL exact lower-case quoted identifier references (#2086)
# ---------------------------------------------------------------------------
# Bare/bare references remain the #2053 unquoted grammar's responsibility; this
# extension accepts a qualified reference only when at least one token is exact quoted.


def _quoted_text_identity_refusal(sql: str, *, entry: str) -> str | None:
    """Return the narrow refusal so one defect probe observes every public seam."""
    try:
...
def _assert_exact_quoted_text_identity_refusal(message: str | None, *, entry: str, column: str) -> None:
...
def _assert_qualified_quoted_text_identity(
    sql: str,
    *,
    entry: str,
    column: str,
...
@pytest.mark.parametrize("column", TEXT_IDENTITY_COLUMNS)
@pytest.mark.parametrize(
    ("label", "alias"),
    [
        ("lower-bare-alias", "rt"),
...
def test_exact_quoted_text_identity_column_through_a_bare_fact_alias_is_attributed_and_refused(
    column: str,
    label: str,
    alias: str,
) -> None:
    """A bare alias folds, while the quoted column must be the exact lower-case member."""
    entry = f"quoted-bare-{label}-{column}"
    sql = f'SELECT {alias}.value FROM hydro.river_timeseries {alias} WHERE {alias}."{column}" = :value'

    assert fact_table_attribution(sql).aliases == frozenset({"rt"})
    _assert_qualified_quoted_text_identity(sql, entry=entry, column=column)


@pytest.mark.parametrize("column", TEXT_IDENTITY_COLUMNS)
@pytest.mark.parametrize(
    ("label", "uppercase_column"),
    [
        ("lower-bare-column", False),
        ("upper-bare-column", True),
    ],
)
def test_exact_quoted_fact_alias_reference_with_a_bare_column_is_attributed_and_refused(
    column: str,
    label: str,
    uppercase_column: bool,
) -> None:
    """Only the bare column folds; the exact quoted qualifier names the bare declaration."""
    entry = f"quoted-qualifier-{label}-{column}"
    bare_column = column.upper() if uppercase_column else column
    sql = f'SELECT RT.value FROM hydro.river_timeseries RT WHERE "rt".{bare_column} = :value'
...
@pytest.mark.parametrize("column", TEXT_IDENTITY_COLUMNS)
def test_exact_quoted_fact_alias_and_column_are_attributed_and_refused(column: str) -> None:
    entry = f"quoted-pair-{column}"
    sql = f'SELECT RT.value FROM hydro.river_timeseries RT WHERE "rt"."{column}" = :value'
...
@pytest.mark.parametrize("column", TEXT_IDENTITY_COLUMNS)
def test_exact_quoted_text_identity_column_on_an_unaliased_fact_read_is_attributed_and_refused(column: str) -> None:
    """The guarded helper owns unaliased attribution; the alias helper stays alias-scoped."""
    entry = f"quoted-unaliased-{column}"
    sql = f'SELECT value FROM hydro.river_timeseries WHERE "{column}" = :value'
    message = _quoted_text_identity_refusal(sql, entry=entry)
...
def test_an_unaliased_exact_quoted_column_uses_the_existing_comparison_operator_family(
    operator: str,
    value: str,
) -> None:
    entry = f"quoted-unaliased-operator-{operator.lower().replace(' ', '-')}"
    sql = f'SELECT value FROM hydro.river_timeseries WHERE "variable" {operator} {value}'
    message = _quoted_text_identity_refusal(sql, entry=entry)

    assert fact_table_text_identity_columns(sql, entry=entry) == {"variable"}
    assert text_fact_columns(sql, "rt") == set()
    _assert_exact_quoted_text_identity_refusal(message, entry=entry, column="variable")


@pytest.mark.parametrize(
    ("label", "alias", "reference"),
    [
        ("lower-bare-qualifier-uppercase-column", "rt", 'rt."VARIABLE"'),
        ("upper-bare-qualifier-uppercase-column", "RT", 'RT."VARIABLE"'),
        ("lower-bare-qualifier-mixed-column", "rt", 'rt."Variable"'),
        ("quoted-qualifier-uppercase-column", "rt", '"rt"."VARIABLE"'),
        ("quoted-qualifier-mixed-column", "rt", '"rt"."Variable"'),
        ("uppercase-quoted-qualifier", "rt", '"RT"."variable"'),
    ],
)
def test_nonexact_quoted_identifiers_remain_unattributed_at_qualified_comparison_positions(
    label: str,
    alias: str,
    reference: str,
) -> None:
    sql = f"SELECT {alias}.value FROM hydro.river_timeseries {alias} WHERE {reference} = :value"
...
def test_a_quoted_text_identity_prefix_with_an_escaped_suffix_remains_unattributed(reference: str) -> None:
    sql = f"SELECT rt.value FROM hydro.river_timeseries rt WHERE {reference} = :value"

    assert fact_table_text_identity_columns(sql, entry="quoted-suffix") == set()
    assert text_fact_columns(sql, "rt") == set()
...
def test_an_unaliased_exact_quoted_column_does_not_match_a_qualified_other_relation_column() -> None:
    ...
    assert fact_table_text_identity_columns(sql, entry="quoted-unaliased-dot-guard") == set()
    assert text_fact_columns(sql, "rt") == set()
    assert render_river_ts_sql(sql, "narrow", entry="quoted-unaliased-dot-guard").sql == sql
...
def test_a_complete_quoted_identifier_body_is_not_an_unquoted_alias_column_pair(quoted_identifier: str) -> None:
    """A complete quoted body is one identifier in both SELECT and predicate positions."""
    for sql in (
        f"SELECT {quoted_identifier}, rt.value FROM hydro.river_timeseries rt WHERE rt.run_key = :run_key",
        f"SELECT rt.value FROM hydro.river_timeseries rt WHERE {quoted_identifier} = :value",
    ):
        assert fact_table_text_identity_columns(sql, entry="quoted-body") == set()
        assert text_fact_columns(sql, "rt") == set()
        assert render_river_ts_sql(sql, "narrow", entry="quoted-body").sql == sql


@pytest.mark.parametrize("sibling", ["variable_e", "unit_e", "quality_flag_e"])
@pytest.mark.parametrize(
    ("label", "template"),
    [
        ("bare-qualifier", 'SELECT rt.value FROM hydro.river_timeseries rt WHERE rt."{sibling}" = :value'),
        ("quoted-qualifier", 'SELECT rt.value FROM hydro.river_timeseries rt WHERE "rt"."{sibling}" = :value'),
        ("unaliased", 'SELECT value FROM hydro.river_timeseries WHERE "{sibling}" = :value'),
    ],
)
def test_quoted_enum_key_siblings_remain_unattributed(
    sibling: str,
    label: str,
    template: str,
) -> None:
    sql = template.format(sibling=sibling)
    entry = f"quoted-enum-key-{label}-{sibling}"
...
@pytest.mark.parametrize("reference", ['rs."variable"', '"rs"."variable"'])
def test_quoted_text_identity_columns_of_another_relation_remain_unattributed(reference: str) -> None:
    sql = (
        "SELECT rt.value FROM hydro.river_timeseries rt JOIN core.river_segment rs "
        f"ON rs.river_segment_key = rt.river_segment_key WHERE {reference} = :value"
    )

    assert fact_table_text_identity_columns(sql, entry="quoted-other-relation") == set()
    assert text_fact_columns(sql, "rt") == set()
    assert render_river_ts_sql(sql, "narrow", entry="quoted-other-relation").sql == sql


def test_an_authority_scalar_subquery_with_an_exact_quoted_text_column_remains_unattributed() -> None:
    sql = (
        "SELECT rt.value FROM hydro.river_timeseries rt "
        "WHERE rt.run_key = (SELECT run_key FROM hydro.hydro_run WHERE \"run_id\" = :run_id)"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "ordinary-doubled-quote",
            "SELECT rt.value FROM hydro.river_timeseries rt "
            "WHERE rt.note = 'it''s rt.\"variable\" = :value' AND rt.run_key = :run_key",
        ),
        (
            "upper-escape-literal",
            r"""SELECT E'it\'s rt."variable" = :value' AS note, rt.value FROM hydro.river_timeseries rt """
            "WHERE rt.run_key = :run_key",
        ),
        (
            "lower-escape-literal",
        ...
        (
            "line-comment",
            "SELECT rt.value -- rt.\"variable\" = :value\n"
            "FROM hydro.river_timeseries rt WHERE rt.run_key = :run_key",
        ),
        (
            "nested-block-comment",
            "SELECT rt.value /* outer /* rt.\"variable\" = :value */ tail */ "
            "FROM hydro.river_timeseries rt WHERE rt.run_key = :run_key",
        ),
    ],
)
def test_quoted_reference_bytes_in_non_code_remain_unattributed(label: str, sql: str) -> None:
    assert fact_table_text_identity_columns(sql, entry=label) == set()
    assert text_fact_columns(sql, "rt") == set()
    assert render_river_ts_sql(sql, "narrow", entry=label).sql == sql
...
@pytest.mark.parametrize(
    ("alias", "reference"),
    [
        ("r", 'hydro.river_timeseries AS "r"'),
        ("r", 'hydro.river_timeseries "r"'),
        ("rt", 'hydro.river_timeseries AS "rt"'),
        ("rt", 'hydro.river_timeseries "rt"'),
    ],
)
...
# bodies. ``outer_predicates`` and unguarded ``text_fact_columns`` stay
# outer-query-only; the three whole-statement doors refuse instead of answering
# a member set. Unaliased/unqualified names remain #2148.

_CORRELATED_SCALAR_TEXT_IDENTITY_COLUMNS = (
    "run_id",
    "river_network_version_id",
    "variable",
    "basin_version_id",
...
_CORRELATED_SCALAR_REASON = (
    "correlated outer fact-alias reference inside comparison-position scalar subquery"
)
_CORRELATED_SCALAR_AUTHORITY_OUTER = (
    "SELECT rt.value FROM hydro.river_timeseries rt WHERE rt.run_key ="
)
_CORRELATED_SCALAR_LATER_INDEPENDENT_SQL = (
    "SELECT rt.value FROM hydro.river_timeseries rt WHERE rt.run_key = "
...
_CORRELATED_SCALAR_NESTED_ONLY_SQL = (
    "SELECT rt.value FROM hydro.river_timeseries rt WHERE rt.run_key = "
    "(SELECT max(hr.run_key) FROM hydro.hydro_run hr WHERE hr.run_key = "
    "(SELECT max(hr2.run_key) FROM hydro.hydro_run hr2 WHERE rt.variable = :v))"
)


def _correlated_scalar_authority_sql(reference: str) -> str:
    """Cardinality-safe authority scalar skeleton from the #2114 red proof."""
    return (
        "SELECT rt.value\n"
        "FROM hydro.river_timeseries rt\n"
        "WHERE rt.run_key = (\n"
...
def _assert_correlated_scalar_refusal(sql: str, *, entry: str) -> None:
    """The three guarded doors refuse; the unguarded alias helper stays empty."""
    assert fact_table_attribution(sql).aliases == frozenset({"rt"})
    assert text_fact_columns(sql, "rt") == set()
    assert text_fact_columns(sql, "RT") == set()
...
def _assert_correlated_scalar_clean(sql: str, *, entry: str) -> None:
    """Authority/other/data controls keep the pre-#2114 guarded-empty answer."""
    assert fact_table_text_identity_columns(sql, entry=entry) == set()
    assert text_fact_columns(sql, "rt") == set()
    assert text_fact_columns(sql, "RT") == set()
    legacy = render_river_ts_sql(sql, "legacy", entry=entry)
...
def test_correlated_scalar_member_enumeration_matches_the_shared_identity_contract() -> None:
    assert _CORRELATED_SCALAR_TEXT_IDENTITY_COLUMNS == TEXT_IDENTITY_COLUMNS


@pytest.mark.parametrize("column", _CORRELATED_SCALAR_TEXT_IDENTITY_COLUMNS)
@pytest.mark.parametrize(
    ("label", "reference_template"),
    [
        ("bare-bare", "rt.{column}"),
...
def test_correlated_scalar_direct_member_token_forms_fail_closed(
    column: str,
    label: str,
    reference_template: str,
) -> None:
    """Seven members × four direct/exact-quoted spellings are one correlated candidate."""
    entry = f"correlated-scalar-direct-{label}-{column}"
    sql = _correlated_scalar_authority_sql(
        reference_template.format(column=column, bare_column=column.upper())
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tabs", "rt\t.\tvariable"),
        ("newlines", "rt\n.\nvariable"),
        ("nested-block-comment", "rt /* outer /* inner */ tail */ . variable"),
        ("line-comment", "rt -- separator\n . variable"),
    ],
)
def test_correlated_scalar_separator_forms_fail_closed(label: str, reference: str) -> None:
    """#2092 scanner-normalised separators remain visible after the body is restored."""
    entry = f"correlated-scalar-separator-{label}"
...
@pytest.mark.parametrize(
    ("label", "reference"),
    [
        ("bare-alias-bare-column", "(rt).variable"),
        ("bare-alias-quoted-column", '(rt)."variable"'),
        ("quoted-alias-bare-column", '("rt").VARIABLE'),
        ("quoted-alias-quoted-column", '("rt")."variable"'),
    ],
)
def test_correlated_scalar_exact_parenthesized_forms_fail_closed(label: str, reference: str) -> None:
    """#2112 exact one-token groups inside a scalar body take the correlated-scalar reason."""
    entry = f"correlated-scalar-paren-exact-{label}"
    sql = _correlated_scalar_authority_sql(reference)
...
@pytest.mark.parametrize(
    ("label", "reference"),
    [
        ("function-call", "f(rt).variable"),
        ("nested-group", "((rt)).variable"),
    ],
)
def test_correlated_scalar_representative_unsupported_parenthesized_forms_fail_closed(
    label: str,
    reference: str,
) -> None:
    """Representative #2112 unsupported forms are refused as correlated, not outer-unmodelled."""
    entry = f"correlated-scalar-paren-unsupported-{label}"
    sql = _correlated_scalar_authority_sql(reference)
...
def test_correlated_scalar_later_independent_body_fails_closed() -> None:
    """First scalar is authority-clean; only the later body carries ``rt.variable``."""
    sql = _CORRELATED_SCALAR_LATER_INDEPENDENT_SQL
    entry = "correlated-scalar-later-independent"

    assert "rt.variable" not in outer_predicates(sql)
    _assert_correlated_scalar_refusal(sql, entry=entry)


def test_correlated_scalar_nested_only_body_fails_closed() -> None:
    """Only the inner nested scalar carries the candidate, so top-level-only walks fail."""
    sql = _CORRELATED_SCALAR_NESTED_ONLY_SQL
...
def test_correlated_scalar_local_alias_shadow_is_conservatively_refused() -> None:
    """A body-local relation reusing the outer alias is not guessed to be local."""
    sql = (
        "SELECT rt.value\n"
        "FROM hydro.river_timeseries rt\n"
        "WHERE rt.run_key = (\n"
        "  SELECT rt.run_key\n"
        "  FROM hydro.hydro_run rt\n"
        "  WHERE rt.variable = :v\n"
        "  LIMIT 1\n"
        ")"
    )
    _assert_correlated_scalar_refusal(sql, entry="correlated-scalar-local-shadow")


def test_correlated_scalar_inner_fact_reread_keeps_count_delta_precedence() -> None:
    """An inner fact-table read keeps the older comparison-position sub-select reason."""
    entry = "correlated-scalar-inner-fact-reread"
    sql = (
        "SELECT rt.value FROM hydro.river_timeseries rt "
        "WHERE rt.run_key = ("
        "SELECT r2.run_key FROM hydro.river_timeseries r2 "
        "WHERE rt.variable = :v LIMIT 1)"
    )
...
@pytest.mark.parametrize(
    ("label", "sql"),
    [
        ("authority-run-id", _correlated_scalar_authority_sql("hr.run_id")),
        ("other-relation-variable", _correlated_scalar_authority_sql("hr.variable")),
        ("enum-variable", _correlated_scalar_authority_sql("rt.variable_e")),
        ("enum-unit", _correlated_scalar_authority_sql("rt.unit_e")),
        ("enum-quality", _correlated_scalar_authority_sql("rt.quality_flag_e")),
        ("nonexact-quoted-column", _correlated_scalar_authority_sql('rt."Variable"')),
        ("multipart", _correlated_scalar_authority_sql("other.rt.variable")),
        (
            "ordinary-literal",
            "SELECT rt.value FROM hydro.river_timeseries rt "
            "WHERE rt.run_key = ("
            "SELECT hr.run_key FROM hydro.hydro_run hr "
            "WHERE hr.run_id = :run_id AND hr.note = 'rt.variable' LIMIT 1)",
        ),
        (
            "upper-escape-literal",
            "SELECT rt.value FROM hydro.river_timeseries rt "
            "WHERE rt.run_key = ("
            "SELECT hr.run_key FROM hydro.hydro_run hr "
            r"WHERE hr.run_id = :run_id AND hr.note = E'rt.variable' LIMIT 1)",
        ),
        (
            "lower-escape-literal",
            "SELECT rt.value FROM hydro.river_timeseries rt "
        ...
        (
            ...
            "WHERE rt.run_key = ("
            "SELECT hr.run_key FROM hydro.hydro_run hr "
            "WHERE hr.run_id = :run_id -- rt.variable = :v\n"
            "  AND hr.run_key IS NOT NULL LIMIT 1)",
        ),
        (
            "nested-block-comment",
            "SELECT rt.value FROM hydro.river_timeseries rt "
            "WHERE rt.run_key = ("
            "SELECT hr.run_key FROM hydro.hydro_run hr "
            "WHERE hr.run_id = :run_id /* outer /* rt.variable */ tail */ "
            "AND hr.run_key IS NOT NULL LIMIT 1)",
        ),
        (
            "complete-quoted-identifier",
            _correlated_scalar_authority_sql('"rt.variable"'),
        ),
    ],
)
def test_correlated_scalar_authority_other_enum_and_data_controls_remain_clean(
    label: str,
    sql: str,
) -> None:
    _assert_correlated_scalar_clean(sql, entry=f"correlated-scalar-control-{label}")
...
def test_correlated_scalar_functional_notation_and_star_remain_routed(
    label: str,
    reference: str,
) -> None:
...
def test_correlated_scalar_outer_predicates_and_alias_helper_stay_outer_only() -> None:
    """Public outer-only contracts stay byte-identical on the original scalar skeleton."""
    sql = _correlated_scalar_authority_sql("rt.variable")
...
# renderer accepts a declared subset of §4.1 and refuses everything else. What is
# refused is `$` and any non-ASCII byte IN CODE, and (round 5, L2) a numeric
# constant glued to an `E'` escape prefix — measured 0/20 over the registry (all
# 10 `$` occurrences, in 2 entries, are the closing anchor of a `'^[0-9]+$'`
# literal; all 6 `—` occurrences, in 4 entries, sit inside `--` comments; no
# entry glues a digit to a quote anywhere in its RAW source, measured), so
# nothing the readers use is lost. The pins
# below are the subset's contract: what is refused, what is NOT refused, and that
# the refusal happens before any traversal can mis-lex the statement.
# ---------------------------------------------------------------------------
...
def _assert_refused_as_outside_the_subset(
    ...
    def _assert_public_door(raised: pytest.ExceptionInfo[RiverTemplateError]) -> None:
        message = str(raised.value)
        assert _SUBSET_REFUSAL in message
        assert entry in message
...
def test_a_dollar_sign_inside_a_literal_or_comment_stays_inside_the_subset() -> None:
    ...
    Two of the twenty registered templates contain a ``$`` — 10 occurrences
    between them, every one the closing anchor of a ``'^[0-9]+$'`` regex literal
    — and this is why decision 18 costs the readers nothing. A subset check spelled over the RAW text instead of the
...
def test_non_ascii_text_inside_a_comment_or_literal_stays_inside_the_subset() -> None:
    ...
    The registry's 6 non-ASCII occurrences, spread over those 4 entries, are all
    ``—`` characters inside ``--`` comments. Blanked text is what the rule reads, so prose comments and
...
def test_a_numeric_literal_glued_to_an_escape_prefix_is_refused_as_outside_the_lexical_subset(
    ...
    §4.1.2.2 requires the ``E`` of an escape string to be a TOKEN START. After a
    numeric constant it is not one, so an independent §4.1 reference lexer reads
    ``1E'a\'x'`` as an identifier-less escape string while this module's
    ``_opens_escape_string`` sees ``isalnum()`` before the ``E`` and reads a
    PLAIN literal, backslash-blind — the two disagree, and the disagreement is
    the same common-mode one as rounds 1 to 4: the plain literal closes on the
    escaped quote, the next quote opens a phantom literal over the statement's
    own ``FROM`` clause, both counters read 0 and the equality guard is satisfied
    by mutual blindness.

    No PostgreSQL version EXECUTES either answer — a lex error on 15+, a parse
    error on ≤14 (verifier #2018 round-5 L2 (iv), read out of ``scan.l`` and
    explicitly NOT verified against a live server; this loop has none) — so there
    is no ground truth to implement faithfully here, which is exactly the case
    decision 18 exists for: the shape leaves the input domain instead of being
    modelled.

    What each parameter is red for at 03241693, measured rather than assumed:

    * ``select_list`` (verifier L's statement) and ``lowercase_e`` are LIVE
      holes there — ``_lexical_subset_violation`` answers ``None``, occurrences
      0 == walk 0, ``fact_table_text_identity_columns`` answers ``set()``, the
      narrow render SHIPS ``rt.variable`` and the legacy render leaves the fact
      table un-renamed;
    * ``predicate`` is red by MESSAGE: at 03241693 the spans still diverge from
      §4.1 (module ``(89, 93), (94, 112), (114, 117)`` vs reference
      ``(89, 95), (111, 117)``), but the phantom literal opens after the ``FROM``
...
def test_the_glued_escape_prefix_arm_reports_the_whole_numeric_constant() -> None:
    r"""The arm's SHAPE, at the offsets and text the refusal message quotes.

    Two claims that the three-door pins above cannot make, because they only ask
    "was it refused":

    * the arm covers the whole numeric constant, so ``12E'`` and ``1.5e'`` are
      refused as such and the message quotes the constant from its first digit —
      both of those DO diverge from §4.1 with a backslash inside the literal
      (measured: module and reference disagree there);
    * it stops where PostgreSQL's exponent does. ``1.5e3'a\'x'`` and
      ``1EE'a\'x'`` are NOT refused, because the character before the quote is
      not the ``E`` of a prefix at all and module and reference already agree on
      both (measured). Without that boundary the arm would be a digit-anywhere
      rule and would start refusing ordinary numeric SQL.
    """
...
def test_a_digit_before_an_escape_prefix_inside_a_literal_or_comment_stays_inside_the_subset() -> None:
    """Non-vacuity for the second arm: it is asked over BLANKED text, like the first.

    The arm needs the literal's quotes to survive blanking (it is a claim about
    what a quote means), so it reads ``_blank_non_code(sql,
    keep_literal_quotes=True)`` rather than the quote-blanking form the other
    checks read. That is one step closer to the raw text than any other check in
    the guard, so the failure mode to keep out is an arm that reads the RAW
    statement and refuses a template for a ``1E'`` its author wrote inside a
    string or a comment. Both statements below render for both stores.
    """
    literal_sql = "SELECT rt.value, '1E' AS n FROM hydro.river_timeseries rt WHERE rt.run_key = %(k)s"
    comment_sql = (
        "SELECT rt.value -- note 1E'x'\nFROM hydro.river_timeseries rt WHERE rt.run_key = %(k)s"
    )

    for label, sql in (("literal", literal_sql), ("comment", comment_sql)):
...
@pytest.mark.parametrize(
    ("label", "sql", "still_renders_legacy"),
    [
        ("uppercase_e", "SELECT col1E'x' AS n, " + _REAL_READ, True),
        ("lowercase_e", "SELECT x1e'a\\'x' AS n, " + _REAL_READ, False),
    ],
    ids=["uppercase_e", "lowercase_e"],
)
def test_an_identifier_character_before_the_digit_keeps_todays_answer(
    label: str, sql: str, still_renders_legacy: bool
) -> None:
    r"""The arm is scoped to NUMERIC constants, so ``col1E'x'`` is untouched.

    In ``col1E'x'`` and ``x1e'a\'x'`` the digit is inside an IDENTIFIER, the
    ``E`` is not a token of its own by anyone's rule, and the module already
    answers "plain literal" exactly as §4.1 does (verifier L measured both as
    AGREEING at 03241693). Refusing them would be over-refusal bought for
    nothing, so the arm carries ``(?<!\w)`` and these two keep the answers they
    have today:

    * ``col1E'x'`` — occurrences 1 == walk 1, text identity ``{'variable'}``, the
      narrow render refused for THAT reason and the legacy render succeeding
      with the rename;
    * ``x1e'a\'x'`` — the backslash-blind plain literal runs off the end, so the
      UNTERMINATED belt refuses it, at offset 15 and with its own message.

    Either way the message must not be the subset one: a subset refusal here
    would mean the arm had stopped being about numeric constants.
    """
    assert _lexical_subset_violation(sql) is None

    if still_renders_legacy:
        assert fact_table_name_occurrences(sql) == 1
        assert fact_table_attribution(sql).reference_count == 1
        assert fact_table_text_identity_columns(sql, entry=label) == {"variable"}
        assert "hydro.river_timeseries_legacy rt" in render_river_ts_sql(sql, "legacy", entry=label).sql
        ...
        with pytest.raises(
            RiverTemplateError, match="unterminated literal or comment opened at offset 15"
        ) as raised:
            render_river_ts_sql(sql, "narrow", entry=label)
    assert _SUBSET_REFUSAL not in str(raised.value)


def test_a_digit_glued_to_a_double_quoted_identifier_stays_inside_the_subset() -> None:
    r"""The arm's other boundary: ``1E"x"``, the quote form it does NOT cover.

    Decision 18's second arm is spelled for the SINGLE quote only. Measured at
    03241693 and here: on ``1E"x"`` the module and the §4.1 reference lexer
    already give the same answer — no span at all, because a double-quoted run is
    an identifier in both — so there is nothing for a refusal to close, and
    widening the arm to ``["']`` would refuse a statement the differential
    compares happily. Today's answer, pinned: the statement is inside the subset,
    reads the fact table once, and is refused by the NARROW check on
    ``rt.variable`` rather than by the subset.
    """
    sql = 'SELECT 1E"x" AS n, ' + _REAL_READ
...
_ADVERSARIAL_CORPUS: tuple[tuple[str, str], ...] = (
    ...
    # The `_opaque_runs` discriminator for its QUOTED-IDENTIFIER half (round-5
    # L3). The `)` has to sit inside a quoted identifier INSIDE the stripped
    # sub-select: only there does the reference's own `skip_balanced` walk over
    # it, so a reference that stopped skipping quoted identifiers would close the
    # group one character early and strip a different span. The same identifier
    # in the SELECT list — outside every stripped group — changes nothing and
    # leaves the mutant alive (measured: 167/167 green with `AS "a(b"` there).
    (
        "quoted_identifier_holding_an_unbalanced_paren_inside_a_stripped_group",
        'SELECT rt.value FROM hydro.river_timeseries rt WHERE rt.run_key = (SELECT run_key AS "a)b" '
        "FROM hydro.hydro_run WHERE run_id = :r) AND rt.valid_time = :t",
    ),
)
