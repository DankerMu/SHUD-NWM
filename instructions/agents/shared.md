...
- **PR 上后端只跑定向测试，与 draft/ready 无关**：
  ...
  - **Unit Tests 的第三种模式（塌缩）**：当定向选择塌缩为只剩 selector meta-guard 套件（`meta_guard_only=true`——删掉一个 `tests/test_*.py`、只改 `tests/` 支持模块如 `conftest.py`，或改 selector 自身的 PR），该套件照常执行断言，**并额外**跑一次全树 `pytest tests/ -q --collect-only`；这一分支的 `::warning` / step summary **不**声称"0 断言执行"（定向那次确实执行了）。此类 PR 的绿只等于"meta-guard 断言通过 + 全树 import/语法通过"，**不**代表被删/被改模块的下游断言跑过。
