...
- node-27 DB 热数据在 `pg_default`（宿主 `/data/GHDC/nhms-primary/pgdata`，容器 bind `/home/postgres/pgdata/data`）；工作集峰值必须对配置 PGDATA 的实际目标设备余量判定，不能借用 `/home`，冷层仍未启用；容量核查一律 `df -h / /home /data/GHDC` + `psql` 实测，不引用文档里的历史数字（`/` 也要看：#1765 的 `/tmp/pytest-of-nwm` 把根卷塞满过，node-27 上跑 pytest 前先
  `mkdir -p /home/nwm/tmp && export TMPDIR=/home/nwm/tmp`）。容器 `nhms-db` 由裸 `docker run` 创建（无 compose/systemd unit），重建流程与存储分层历史见 `docs/runbooks/tier-node27-timeseries-storage.md` 与 ADR 0002。
...
- **node-22 例外（维护窗口前）**：`/scratch/frd_muziyao/NWM` 的活动 checkout 在运维批准的维护窗口前保持 Python 3.12.7（共享 `.venv` 被在线服务占用，而新 pin 是 3.11——在此窗口前**禁止** `uv sync`，也禁止裸 `uv run` / 任何会隐式重建环境的 `uv run` 变体，否则 uv 会在 3.11 pin 下自动删除并重建 3.12 的 `.venv`，已实测会打断成半拆状态；**系统 Python 也不是替代**）。必须执行的活动操作使用 checked-in wrapper 或精确活动解释器 `/scratch/frd_muziyao/NWM/.venv/bin/python`（控制台入口用 `-m`）；解释器缺失即 fail-closed，绝不触发环境创建。若需维护窗口前的只读 Python 观察，只用已验证的 `uv run --no-sync ...`——它执行的是仍活动的 3.12.7 环境、**不代表 3.11 pin 生效**；`--active` 并非安全替代（同样会重建）。切换仅在维护窗口内执行（部署与安装单位落地由 #1831 跟踪，不在本 PR 完成）：停用占用 `.venv` 的进程 → `uv sync --all-extras --dev` → 断言 `uv run python -V` 为 3.11.x → 按各自 runbook 重启并验证服务。该 pin 不含任何 Slurm 运行时验证。
...
- 跨 PR 轮次上限记忆 `.review-gate-issues.json` 只经 `scripts/review_gate.py` 写：`fix_gate.py open --pr <PR>` 旁跑 `uv run python scripts/review_gate.py check --issue <N> --pr <PR>`（exit 2 = 该 issue 已在别的 PR 撞过硬停，先要用户决定）；该 issue 的 PR **每次关闭**都跑 `uv run python scripts/review_gate.py record --issue <N> --pr <PR> --rounds <R> --outcome <OUTCOME>`（`merged` 在 post-merge archive commit 里；`superseded-by-split` / `abandoned` / `descoped` 在跑 `fix_gate.py close` 时一并 commit；fix_gate 在该 PR 锁过必加 `--ceiling`，否则后继 PR 的 `check` 查不到）。loop-log 审计按需 `uv run python scripts/governance/loop_log_audit.py`。
