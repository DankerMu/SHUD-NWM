...
| 3.13 | `#744 -> this closeout PR pending` - Two-node group verification and evidence closeout | Inventory closeout map and `tests/test_two_node_e2e_evidence.py` closeout guard | `uv run pytest -q tests/test_two_node_e2e_evidence.py -k "closeout or inventory"` plus OpenSpec and diff-check. | Final map records tasks 3.1-3.13 and explicit closeout non-goals; no product, topology, DB, Slurm, API, frontend, deployment, station-MVT, or new lane semantics change. |
