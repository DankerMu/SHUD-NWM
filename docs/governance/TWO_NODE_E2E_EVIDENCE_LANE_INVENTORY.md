...
```bash
rg -n "FINAL_REQUIRED_LANES|LaneEvaluation|_load_lane_documents|_evaluate_docker_preflight|_evaluate_docker_security|_evaluate_readonly_db|_evaluate_source_lane|_evaluate_manual_ops|_evaluate_cross_plane|_source_scope_results|_final_status" services/production_closure/two_node_e2e_evidence.py
rg -n "TWO_NODE_E2E_[A-Z0-9_]+" services/production_closure/two_node_e2e_evidence.py
rg -n "docker_preflight|docker_security|readonly_db|api|browser|logs|manual_ops|source_scope_results|lane_summaries" tests/test_two_node_e2e_evidence.py
```
...
Run metadata must declare source scope and strict identities. The shared
identity tuple is `run_id`, `source`, `cycle_time`, and `model_id`; log-like
proof additionally requires `job_id`. API, browser, logs, readonly DB route
proof, manual ops receipts, and cross-plane records must match the metadata
identity for each declared source. `cycle_time` comparison uses timestamp
semantics, so equivalent compact and ISO UTC encodings may match.
