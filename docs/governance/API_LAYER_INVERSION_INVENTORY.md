...
| Scripts | `scripts/governance/entropy_audit/check_paths_and_api.py` (the scan itself; `scripts/governance/audit_repo_entropy.py` is its compatibility facade since #1842), `scripts/validate_two_node_docker_runtime.py` | Scan implementation and expected-entrypoint validation text. Not counted as layer inversion findings. |
