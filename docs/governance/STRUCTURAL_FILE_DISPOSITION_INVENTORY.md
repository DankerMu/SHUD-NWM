...
```bash
wc -l services/orchestrator/scheduler.py services/orchestrator/scheduler_preflight.py services/orchestrator/chain.py services/orchestrator/chain_analysis.py services/orchestrator/chain_source_cycle.py services/orchestrator/chain_runtime_utils.py services/production_closure/two_node_e2e_evidence.py services/production_closure/readiness_validation.py apps/api/main.py apps/frontend/src/components/map/M11MapLibreSurface.tsx
...
| `services/orchestrator/chain.py` | 6680 |
...
| `services/orchestrator/chain.py` | 6680 | P1 | `compute_control` orchestration chain facade and aggregation surface; source-cycle repair owner is `services/orchestrator/chain_source_cycle.py`; runtime utility owner is `services/orchestrator/chain_runtime_utils.py`; analysis method owner is `services/orchestrator/chain_analysis.py` | Compatibility facade retained while source-cycle repair helpers moved to `chain_source_cycle.py`, runtime utility helpers moved to `chain_runtime_utils.py`, and analysis method bodies moved to `chain_analysis.py` under `facade-shrink-scheduler-chain`. | #670, #671, `facade-shrink-scheduler-chain` | Retain legacy chain private names until callers migrate to owner modules or stable public orchestration APIs. New source-cycle, runtime, or analysis helper exposure must update `docs/governance/CHAIN_COMPATIBILITY_INVENTORY.md` with owner, retention, removal condition, and focused verification. | `wc -l`; `uv run pytest -q tests/test_analysis_pipeline.py tests/test_orchestration_chain.py -k "analysis or manifest"`; `uv run pytest -q tests/test_entropy_audit_script.py -k "compatibility_facade or structural_file_budget or chain"`; `uv run ruff check services/orchestrator/chain.py services/orchestrator/chain_analysis.py services/orchestrator/chain_source_cycle.py services/orchestrator/chain_runtime_utils.py`; `openspec validate facade-shrink-scheduler-chain --strict --no-interactive`; `git diff --check`. |
