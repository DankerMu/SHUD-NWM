...
```bash
wc -l apps/frontend/src/components/map/M11MapLibreSurface.tsx apps/frontend/src/components/map/m11MapBuilders.ts apps/frontend/src/components/map/m11MapPrimitives.tsx apps/frontend/src/components/map/m11MapInteractions.ts apps/frontend/src/components/map/m11MapRuntime.tsx
...
| Path | Current lines |
|---|---:|
| `apps/frontend/src/components/map/M11MapLibreSurface.tsx` | 373 |
...
| `apps/frontend/src/components/map/M11MapLibreSurface.tsx` | 373 | P2 | `display_readonly` frontend map facade; pure builder owner is `apps/frontend/src/components/map/m11MapBuilders.ts`; MapLibre primitive owner is `apps/frontend/src/components/map/m11MapPrimitives.tsx`; interaction owner is `apps/frontend/src/components/map/m11MapInteractions.ts`; runtime camera/error/status owner is `apps/frontend/src/components/map/m11MapRuntime.tsx` | M11 pure map builders extracted in #761, MapLibre primitives extracted in #762, interaction dispatch extracted in #763, and camera/error/status runtime helpers extracted in #764. Keep the display map facade stable while deterministic builder logic, source/layer JSX primitives, event dispatch, and map runtime state are owned by focused modules. | #687, #761, #762, #763, #764 | `apps/frontend/src/components/map/m11MapBuilders.ts` owns registered overlay construction, vector source keys, GeoJSON collections, filters, label anchors, geometry budget summaries, unavailable reasons, and national-river filtering. `apps/frontend/src/components/map/m11MapPrimitives.tsx` owns national river, basin boundary, basin label, basin-river, registered overlay, selected-segment, and station cluster primitives plus source/layer ID constants, paint/layout, promote IDs, selected/hovered filters, and station GeoJSON cluster rendering. `apps/frontend/src/components/map/m11MapInteractions.ts` owns interactive layer ID ordering, hover/click dispatch, rendered-feature fallback, station cluster expansion, cursor state, feature property extraction, and `M11MapOverlayInteraction` payload shape. `apps/frontend/src/components/map/m11MapRuntime.tsx` owns `m11MapStyleUrls`, Tianditu `MapStyle` construction, initial camera state, fit/fly effect de-dupe keys, source-error reset and glyph warning downgrade, and loading/unavailable status overlay rendering. `M11MapLibreSurface.tsx` retains query-derived composition, popup state/rendering, selected-segment/station data attributes, map primitive composition, and legacy facade imports used by pages/tests. Future extraction requires preserving source/layer IDs, source keys, tile URL templates, GeoJSON feature properties, budget skip semantics, unavailable reason text, interaction priority, callback payloads, camera command semantics, source-error visibility, station-MVT non-closure, display identity, and focused frontend verification. No popup behavior change, backend station-MVT endpoint completion, generated API type change, or visual redesign is changed by #761-#764. | #761-#764 verification: focused M11 shell builder/source/geometry/unavailable/selected-segment, primitive layer/source/cluster/highlight/hover, interaction priority/click/hover/cluster/rendered-cluster/river-segment, and camera/fit/source-error/glyph/unavailable/loading regressions; floating controls regression; frontend build; entropy script tests; OpenSpec single-change/all validation; and diff-check. |
...
The map surface remains the display-readonly facade after #761 through #764.
...
downgrade, and loading/unavailable status overlay rendering. The facade still
re-exports the legacy helpers imported by M11 pages/tests and retains
query-derived composition plus popup/selection state for later scoped issues.
