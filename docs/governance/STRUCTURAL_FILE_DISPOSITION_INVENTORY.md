...
```bash
wc -l services/orchestrator/scheduler.py services/orchestrator/scheduler_preflight.py services/orchestrator/chain.py services/orchestrator/chain_analysis.py services/orchestrator/chain_repository.py services/orchestrator/chain_repository_state.py services/orchestrator/chain_source_cycle.py services/orchestrator/chain_runtime_utils.py services/production_closure/two_node_e2e_evidence.py services/production_closure/readiness_validation.py apps/api/main.py apps/frontend/src/components/map/M11MapLibreSurface.tsx
...
| `services/orchestrator/chain.py` | 5435 |
...
| `services/orchestrator/chain.py` | 5435 | P1 | `compute_control` orchestration chain facade and aggregation surface; source-cycle owner is `services/orchestrator/chain_source_cycle.py`; runtime owner is `services/orchestrator/chain_runtime_utils.py`; analysis owner is `services/orchestrator/chain_analysis.py`; repository body owner is `services/orchestrator/chain_repository.py` with candidate-state assembly in `services/orchestrator/chain_repository_state.py` | Compatibility facade retained while source-cycle repair helpers moved to `chain_source_cycle.py`, runtime helpers moved to `chain_runtime_utils.py`, analysis method bodies moved to `chain_analysis.py`, and `PsycopgOrchestratorRepository` method bodies moved to repository owner modules under `facade-shrink-scheduler-chain`. | #670, #671, `facade-shrink-scheduler-chain` | Retain legacy chain private names until callers migrate to owner modules or stable public orchestration APIs. New source-cycle, runtime, analysis, or repository helper exposure must update `docs/governance/CHAIN_COMPATIBILITY_INVENTORY.md` with owner, retention, removal condition, and focused verification. | `wc -l`; `uv run pytest -q tests/test_orchestration_chain.py -k "persistence_repository_compat or candidate_state"`; `uv run pytest -q tests/test_gateway_reconcile.py -k "reserve_pipeline_job_sql_absorbs_all_unique_conflicts"`; `uv run pytest -q tests/test_entropy_audit_script.py -k "compatibility_facade or structural_file_budget or chain"`; `uv run ruff check services/orchestrator/chain.py services/orchestrator/chain_analysis.py services/orchestrator/chain_repository.py services/orchestrator/chain_repository_state.py services/orchestrator/chain_source_cycle.py services/orchestrator/chain_runtime_utils.py`; `openspec validate facade-shrink-scheduler-chain --strict --no-interactive`; `git diff --check`. |
