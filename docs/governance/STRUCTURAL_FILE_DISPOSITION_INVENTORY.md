...
| Path | Current lines |
|---|---:|
| `services/orchestrator/scheduler.py` | 3731 |
...
|---|---:|---|---|---|---|---|---|
| `services/orchestrator/scheduler.py` | 3731 | P1 | `compute_control` scheduler/orchestrator facade; active model discovery/coercion owner is `services/orchestrator/scheduler_models.py`; candidate quality/output owner is `services/orchestrator/scheduler_candidate_quality.py`; Slurm/preflight helper owner is `services/orchestrator/scheduler_preflight.py`; scheduler pass runtime owner is `services/orchestrator/scheduler_runtime.py`; cancel-requested active Slurm owner is `services/orchestrator/scheduler_cancellation.py`; candidate manifest owner is `services/orchestrator/scheduler_candidate_manifest.py`; candidate execution/model-run evidence owner is `services/orchestrator/scheduler_candidate_execution_evidence.py` | Compatibility facade retained while active model discovery/coercion helpers moved to `scheduler_models.py`, candidate quality/output helpers moved to `scheduler_candidate_quality.py`, Slurm/preflight helper bodies moved to `scheduler_preflight.py`, scheduler runtime/cancellation method bodies moved to `scheduler_runtime.py` / `scheduler_cancellation.py`, candidate manifest helpers moved to `scheduler_candidate_manifest.py`, and candidate execution/model-run evidence helpers moved to `scheduler_candidate_execution_evidence.py` under `facade-shrink-scheduler-chain`. Keep `_slurm_preflight`, `_slurm_gateway_check`, `_default_gateway_probe`, and `_slurm_gateway_backend` facade-local because legacy tests patch those names through `scheduler.py`; keep `uuid4` and `MAX_EVIDENCE_BYTES` facade monkeypatch behavior for scheduler-pass tests. | #669, #671, `facade-shrink-scheduler-chain` | Retain legacy scheduler private names until callers migrate to owner modules or stable public scheduler APIs. New model discovery, candidate quality/output, preflight, runtime, cancellation, candidate manifest, or candidate execution evidence helper exposure must update `docs/governance/SCHEDULER_COMPATIBILITY_INVENTORY.md` with owner, retention, removal condition, and focused verification. | `wc -l`; `uv run pytest -q tests/test_production_scheduler.py -k "candidate_quality or output_evidence or resource_summary or residual_blockers or output_river or model_run_evidence or candidate_evidence"`; `uv run pytest -q tests/test_entropy_audit_script.py -k "compatibility_facade or structural_file_budget or scheduler"`; `uv run ruff check services/orchestrator/scheduler.py services/orchestrator/scheduler_candidate_quality.py`; `openspec validate facade-shrink-scheduler-chain --strict --no-interactive`; `git diff --check`. |
