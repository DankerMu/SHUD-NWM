...
| Path | Current lines |
|---|---:|
| `services/orchestrator/scheduler.py` | 2796 |
...
|---|---:|---|---|---|---|---|---|
| `services/orchestrator/scheduler.py` | 2796 | P1 | `compute_control` scheduler/orchestrator facade; gateway/preflight orchestration owner is `services/orchestrator/scheduler_gateway.py`; runtime root/config helper owner is `services/orchestrator/scheduler_runtime_roots.py`; type owner is `services/orchestrator/scheduler_types.py`; config owner is `services/orchestrator/scheduler_config.py`; active model discovery/coercion owner is `services/orchestrator/scheduler_models.py`; candidate quality/output owner is `services/orchestrator/scheduler_candidate_quality.py`; Slurm/preflight helper owner is `services/orchestrator/scheduler_preflight.py`; scheduler pass runtime owner is `services/orchestrator/scheduler_runtime.py`; cancel-requested active Slurm owner is `services/orchestrator/scheduler_cancellation.py`; candidate manifest owner is `services/orchestrator/scheduler_candidate_manifest.py`; candidate execution/model-run evidence owner is `services/orchestrator/scheduler_candidate_execution_evidence.py` | Compatibility facade retained while Slurm preflight/gateway orchestration moved to `scheduler_gateway.py`, scheduler runtime-root preflight and config path/env helper bodies moved to `scheduler_runtime_roots.py`, scheduler result/model/candidate dataclasses moved to `scheduler_types.py`, `ProductionSchedulerConfig` moved to `scheduler_config.py`, active model discovery/coercion helpers moved to `scheduler_models.py`, candidate quality/output helpers moved to `scheduler_candidate_quality.py`, Slurm/preflight helper bodies moved to `scheduler_preflight.py`, scheduler runtime/cancellation method bodies moved to `scheduler_runtime.py` / `scheduler_cancellation.py`, candidate manifest helpers moved to `scheduler_candidate_manifest.py`, and candidate execution/model-run evidence helpers moved to `scheduler_candidate_execution_evidence.py` under `facade-shrink-scheduler-chain`. Keep gateway facade forwarders monkeypatch-compatible because legacy tests patch `_default_gateway_probe` and `_slurm_gateway_backend` through `scheduler.py`; keep `uuid4` and `MAX_EVIDENCE_BYTES` facade monkeypatch behavior for scheduler-pass tests. | #669, #671, `facade-shrink-scheduler-chain` | Retain legacy scheduler private names until callers migrate to owner modules or stable public scheduler APIs. New gateway, runtime-root, config helper, type, config, model discovery, candidate quality/output, preflight, runtime, cancellation, candidate manifest, or candidate execution evidence helper exposure must update `docs/governance/SCHEDULER_COMPATIBILITY_INVENTORY.md` with owner, retention, removal condition, and focused verification. | `wc -l`; `uv run pytest -q tests/test_production_scheduler.py tests/test_cli_publish_qdown.py tests/test_run_qhh_continuous.py -k "slurm_gateway or slurm_preflight or grib_env or database_url or database_host"`; `uv run pytest -q tests/test_entropy_audit_script.py -k "compatibility_facade or structural_file_budget or scheduler"`; `uv run ruff check services/orchestrator/scheduler.py services/orchestrator/scheduler_gateway.py`; `openspec validate facade-shrink-scheduler-chain --strict --no-interactive`; `git diff --check`. |
