...
```bash
wc -l services/orchestrator/scheduler.py services/orchestrator/scheduler_preflight.py services/orchestrator/chain.py services/orchestrator/chain_source_cycle.py services/production_closure/two_node_e2e_evidence.py services/production_closure/readiness_validation.py apps/api/main.py apps/frontend/src/components/map/M11MapLibreSurface.tsx
...
| `services/orchestrator/chain.py` | 7497 |
...
| `services/orchestrator/chain.py` | 7497 | P1 | `compute_control` orchestration chain facade and aggregation surface; source-cycle repair helper owner is `services/orchestrator/chain_source_cycle.py` | Compatibility facade retained while source-cycle repair, retry provenance, repaired-stage evidence, sort-key, task identity, and bounded candidate-state helper bodies moved to `chain_source_cycle.py` under `facade-shrink-scheduler-chain`. | #670, #671, `facade-shrink-scheduler-chain` | Retain legacy chain private names until callers migrate to owner modules or stable public orchestration APIs. New source-cycle helper exposure must update `docs/governance/CHAIN_COMPATIBILITY_INVENTORY.md` with owner, retention, removal condition, and focused verification. | `wc -l`; `uv run pytest -q tests/test_orchestration_chain.py -k "source_cycle or retry_provenance or candidate_state or repaired or repair"`; `uv run pytest -q tests/test_entropy_audit_script.py -k "compatibility_facade or structural_file_budget or chain"`; `uv run ruff check services/orchestrator/chain.py services/orchestrator/chain_source_cycle.py`; `openspec validate facade-shrink-scheduler-chain --strict --no-interactive`; `git diff --check`. |
