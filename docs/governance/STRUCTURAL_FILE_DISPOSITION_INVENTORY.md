...
| `services/orchestrator/chain.py` | 3506 |
...
| `services/orchestrator/chain.py` | 3506 | P1 | `compute_control` orchestration chain facade and aggregation surface; source-cycle owner is `services/orchestrator/chain_source_cycle.py`; runtime owner is `services/orchestrator/chain_runtime_utils.py`; analysis class owner is `services/orchestrator/chain_analysis_orchestrator.py`; analysis method owner is `services/orchestrator/chain_analysis.py`; forecast owners are `services/orchestrator/chain_forecast_control.py`, `services/orchestrator/chain_forecast_cycle.py`, `services/orchestrator/chain_forecast_trigger.py`, `services/orchestrator/chain_forecast_execution.py`, `services/orchestrator/chain_forecast_state.py`, `services/orchestrator/chain_forecast_submission.py`, and `services/orchestrator/chain_forecast_templates.py`; workspace/log owner is `services/orchestrator/chain_workspace.py`; HTTP Slurm client owner is `services/orchestrator/chain_slurm_client.py`; repository body owner is `services/orchestrator/chain_repository.py` with candidate-state assembly in `services/orchestrator/chain_repository_state.py` | Compatibility facade retained while source-cycle repair helpers moved to `chain_source_cycle.py`, runtime helpers moved to `chain_runtime_utils.py`, `AnalysisOrchestrator` moved to `chain_analysis_orchestrator.py`, analysis method bodies moved to `chain_analysis.py`, forecast control methods moved to `chain_forecast_control.py`, forecast cycle normalization/identity helpers moved to `chain_forecast_cycle.py`, forecast trigger/auto-trigger helpers moved to `chain_forecast_trigger.py`, forecast execution/state/submission/template method bodies moved to forecast owner modules, workspace/log helper bodies moved to `chain_workspace.py`, HTTP Slurm client body moved to `chain_slurm_client.py`, and `PsycopgOrchestratorRepository` method bodies moved to repository owner modules under `facade-shrink-scheduler-chain`. | #670, #671, `facade-shrink-scheduler-chain` | Retain legacy chain private names until callers migrate to owner modules or stable public orchestration APIs. New source-cycle, runtime, analysis, forecast, workspace/log, HTTP Slurm client, or repository helper exposure must update `docs/governance/CHAIN_COMPATIBILITY_INVENTORY.md` with owner, retention, removal condition, and focused verification. | `wc -l`; `uv run pytest -q tests/test_analysis_pipeline.py tests/test_orchestration_chain.py -k "analysis or manifest"`; `uv run pytest -q tests/test_entropy_audit_script.py -k "compatibility_facade or structural_file_budget or chain"`; `uv run ruff check services/orchestrator/chain.py services/orchestrator/chain_analysis_orchestrator.py`; `openspec validate facade-shrink-scheduler-chain --strict --no-interactive`; `git diff --check`. |
