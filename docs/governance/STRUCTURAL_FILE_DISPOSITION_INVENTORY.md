...
Governance-8 owner-module updates as follow-up extractions land, and
Governance-8 issue #767 inventory synchronization evidence. This inventory is
evidence-only. It records the current disposition for the mandatory-governance
source files called out by the structural entropy budget and the owner modules
introduced to reduce those facades; it does not remove compatibility surfaces,
enable a hard gate, or write a baseline.
...
| Path | Current lines |
|---|---:|
| `services/orchestrator/scheduler.py` | 6815 |
| `services/orchestrator/chain.py` | 8222 |
