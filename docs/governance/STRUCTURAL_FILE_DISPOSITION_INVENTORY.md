...
```bash
wc -l apps/frontend/src/components/map/M11MapLibreSurface.tsx apps/frontend/src/components/map/m11MapBuilders.ts apps/frontend/src/components/map/m11MapPrimitives.tsx apps/frontend/src/components/map/m11MapInteractions.ts
...
| Path | Current lines |
|---|---:|
| `apps/frontend/src/components/map/M11MapLibreSurface.tsx` | 499 |
...
| `apps/frontend/src/components/map/M11MapLibreSurface.tsx` | 499 | P2 | `display_readonly` frontend map facade; pure builder owner is `apps/frontend/src/components/map/m11MapBuilders.ts`; MapLibre primitive owner is `apps/frontend/src/components/map/m11MapPrimitives.tsx`; interaction owner is `apps/frontend/src/components/map/m11MapInteractions.ts` | M11 pure map builders extracted in #761, MapLibre primitives extracted in #762, and interaction dispatch extracted in #763. Keep the display map facade stable while deterministic builder logic, source/layer JSX primitives, and event dispatch are owned by focused modules. | #687, #761, #762, #763 | `apps/frontend/src/components/map/m11MapBuilders.ts` owns registered overlay construction, vector source keys, GeoJSON collections, filters, label anchors, geometry budget summaries, unavailable reasons, and national-river filtering. `apps/frontend/src/components/map/m11MapPrimitives.tsx` owns national river, basin boundary, basin label, basin-river, registered overlay, selected-segment, and station cluster primitives plus source/layer ID constants, paint/layout, promote IDs, selected/hovered filters, and station GeoJSON cluster rendering. `apps/frontend/src/components/map/m11MapInteractions.ts` owns interactive layer ID ordering, hover/click dispatch, rendered-feature fallback, station cluster expansion, cursor state, feature property extraction, and `M11MapOverlayInteraction` payload shape. `M11MapLibreSurface.tsx` retains query-derived composition, popup state/rendering, fit/fly camera effects, map source-error handling, Tianditu style wiring, loading/unavailable rendering, and legacy facade imports used by pages/tests. Future extraction requires preserving source/layer IDs, source keys, tile URL templates, GeoJSON feature properties, budget skip semantics, unavailable reason text, interaction priority, callback payloads, station-MVT non-closure, display identity, and focused frontend verification. No popup behavior change, camera/error behavior change, backend station-MVT endpoint completion, generated API type change, or visual redesign is changed by #761-#763. | #761-#763 verification: focused M11 shell builder/source/geometry/unavailable/selected-segment, primitive layer/source/cluster/highlight/hover, and interaction priority/click/hover/cluster/rendered-cluster/river-segment regressions; floating controls regression; frontend build; entropy script tests; OpenSpec single-change/all validation; and diff-check. |
...
The map surface remains the display-readonly facade after #761 through #763.
...
interactive layer ID ordering, cursor state, and interaction payload shape. The
facade still re-exports the legacy helpers imported by M11 pages/tests and
retains popup/selection state, camera/error handling, loading/unavailable
rendering, and Tianditu basemap style wiring for later scoped issues.
