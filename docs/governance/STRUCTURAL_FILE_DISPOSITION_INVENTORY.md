...
| Path | Current lines |
|---|---:|
| `services/orchestrator/scheduler.py` | 5830 |
| `services/orchestrator/scheduler_preflight.py` | 734 |
| `services/orchestrator/chain.py` | 4668 |
...
|---|---:|---|---|---|---|---|---|
| `services/orchestrator/scheduler.py` | 5830 | P1 | `compute_control` scheduler/orchestrator facade; Slurm/preflight helper owner is `services/orchestrator/scheduler_preflight.py` | Compatibility facade retained while Slurm/preflight helper bodies moved to `scheduler_preflight.py` under `facade-shrink-scheduler-chain`. Keep `_slurm_preflight`, `_slurm_gateway_check`, `_default_gateway_probe`, and `_slurm_gateway_backend` facade-local because legacy tests patch those names through `scheduler.py`. | #669, #671, `facade-shrink-scheduler-chain` | Retain legacy scheduler private names until callers migrate to owner modules or stable public scheduler APIs. New preflight helper exposure must update `docs/governance/SCHEDULER_COMPATIBILITY_INVENTORY.md` with owner, retention, removal condition, and focused verification. | `wc -l`; `uv run pytest -q tests/test_production_scheduler.py -k "slurm_gateway or slurm_preflight or grib_env or database_url or database_host"`; `uv run pytest -q tests/test_entropy_audit_script.py -k "compatibility_facade or structural_file_budget or scheduler"`; `uv run ruff check services/orchestrator/scheduler.py services/orchestrator/scheduler_preflight.py`; `openspec validate facade-shrink-scheduler-chain --strict --no-interactive`; `git diff --check`. |
| `services/orchestrator/chain.py` | 4668 | P1 | `compute_control` orchestration chain facade and aggregation surface; source-cycle owner is `services/orchestrator/chain_source_cycle.py`; runtime owner is `services/orchestrator/chain_runtime_utils.py`; analysis owner is `services/orchestrator/chain_analysis.py`; forecast execution owners are `services/orchestrator/chain_forecast_execution.py`, `services/orchestrator/chain_forecast_submission.py`, and `services/orchestrator/chain_forecast_templates.py`; repository body owner is `services/orchestrator/chain_repository.py` with candidate-state assembly in `services/orchestrator/chain_repository_state.py` | Compatibility facade retained while source-cycle repair helpers moved to `chain_source_cycle.py`, runtime helpers moved to `chain_runtime_utils.py`, analysis method bodies moved to `chain_analysis.py`, forecast execution/submission/template method bodies moved to forecast owner modules, and `PsycopgOrchestratorRepository` method bodies moved to repository owner modules under `facade-shrink-scheduler-chain`. | #670, #671, `facade-shrink-scheduler-chain` | Retain legacy chain private names until callers migrate to owner modules or stable public orchestration APIs. New source-cycle, runtime, analysis, forecast, or repository helper exposure must update `docs/governance/CHAIN_COMPATIBILITY_INVENTORY.md` with owner, retention, removal condition, and focused verification. | `wc -l`; `uv run pytest -q tests/test_orchestration_chain.py -k "stage_execution or submit_and_wait or poll_until_terminal or partial_array or template_export or submission_failed or duplicate_submission or pipeline_logs or retry"`; `uv run pytest -q tests/test_pipeline_logs_artifacts.py`; `uv run pytest -q tests/test_entropy_audit_script.py -k "compatibility_facade or structural_file_budget or chain"`; `uv run ruff check services/orchestrator/chain.py services/orchestrator/chain_forecast_execution.py services/orchestrator/chain_forecast_submission.py services/orchestrator/chain_forecast_templates.py services/orchestrator/chain_repository.py services/orchestrator/chain_repository_state.py`; `openspec validate facade-shrink-scheduler-chain --strict --no-interactive`; `git diff --check`. |
