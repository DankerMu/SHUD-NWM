...
| Path | Current lines |
|---|---:|
| `services/orchestrator/scheduler.py` | 4214 |
...
|---|---:|---|---|---|---|---|---|
| `services/orchestrator/scheduler.py` | 4214 | P1 | `compute_control` scheduler/orchestrator facade; Slurm/preflight helper owner is `services/orchestrator/scheduler_preflight.py`; scheduler pass runtime owner is `services/orchestrator/scheduler_runtime.py`; cancel-requested active Slurm owner is `services/orchestrator/scheduler_cancellation.py`; candidate manifest owner is `services/orchestrator/scheduler_candidate_manifest.py`; candidate execution/model-run evidence owner is `services/orchestrator/scheduler_candidate_execution_evidence.py` | Compatibility facade retained while Slurm/preflight helper bodies moved to `scheduler_preflight.py`, scheduler runtime/cancellation method bodies moved to `scheduler_runtime.py` / `scheduler_cancellation.py`, candidate manifest helpers moved to `scheduler_candidate_manifest.py`, and candidate execution/model-run evidence helpers moved to `scheduler_candidate_execution_evidence.py` under `facade-shrink-scheduler-chain`. Keep `_slurm_preflight`, `_slurm_gateway_check`, `_default_gateway_probe`, and `_slurm_gateway_backend` facade-local because legacy tests patch those names through `scheduler.py`; keep `uuid4` and `MAX_EVIDENCE_BYTES` facade monkeypatch behavior for scheduler-pass tests. | #669, #671, `facade-shrink-scheduler-chain` | Retain legacy scheduler private names until callers migrate to owner modules or stable public scheduler APIs. New preflight, runtime, cancellation, candidate manifest, or candidate execution evidence helper exposure must update `docs/governance/SCHEDULER_COMPATIBILITY_INVENTORY.md` with owner, retention, removal condition, and focused verification. | `wc -l`; `uv run pytest -q tests/test_production_scheduler.py -k "model_run_evidence or candidate_evidence or preflight_blocked or resource_profile or partial_cycle or forcing_ready or forcing_blocked or evidence_write"`; `uv run pytest -q tests/test_entropy_audit_script.py -k "compatibility_facade or structural_file_budget or scheduler"`; `uv run ruff check services/orchestrator/scheduler.py services/orchestrator/scheduler_preflight.py services/orchestrator/scheduler_runtime.py services/orchestrator/scheduler_cancellation.py services/orchestrator/scheduler_candidate_manifest.py services/orchestrator/scheduler_candidate_execution_evidence.py tests/test_production_scheduler.py`; `openspec validate facade-shrink-scheduler-chain --strict --no-interactive`; `git diff --check`. |
