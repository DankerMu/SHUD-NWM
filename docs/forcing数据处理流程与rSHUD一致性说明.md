...
> **目的**：说明本项目（NHMS/NWM 业务化系统）如何为「已建模流域的固定站点」生产 SHUD forcing，并逐项对照 rSHUD/AutoSHUD 的离线建模 forcing 流程，论证本项目处理的正确性。
...
本项目产出的 SHUD forcing 文件，在 **文件格式、变量集、单位、时间语义、站点↔网格(mesh)耦合关系** 五个维度上与 rSHUD/AutoSHUD **完全一致**；二者的差异**只**在于：
...
> 一句话给 leader：**我们没有改动 rSHUD/AutoSHUD 建好的模型（mesh、河网、参数、站点覆盖都原样复用），只是把"喂给固定站点的数值"从 LDAS 再分析换成了 IFS/GFS 实时预报，并保证换源后写出的 forcing 文件与 rSHUD 写出的在格式与单位上逐字节同构。**

### 0.1 维度速查对照表

| 维度 | rSHUD / AutoSHUD（离线建模基线） | 本项目（业务化、固定站点刷新） | 是否一致 |
|---|---|---|---|
| 系统定位 | 建模 **+** forcing 一体 | **仅** forcing，模型复用 | 不同（定位差异） |
...
| 站点↔三角元耦合 | `ForcingCoverage` 现算 Voronoi/Thiessen | **复用原模型**已有覆盖（不重算） | **一致**（且不重算） |
| 格点→站点取值 | 站点≈格点中心，最近邻 | **IDW**（k=4, p=2, haversine） | 方法不同，重合时退化一致 |
...
**本项目** 处理的是**另一个阶段**：在 rSHUD/AutoSHUD **已经建好**的流域上，**只为其固定站点刷新 forcing 时间序列**，并不重建模型。这一点在仓库的架构决策中被明确固化：
...
**关键不变量**：本项目复用原模型的 mesh、河网、率定参数，以及**最重要的——forcing 站点 ↔ 三角元的覆盖（Thiessen）映射**（固化在模型 `.att` 文件的 `FORC` 列中，运行时由 `workers/shud_runtime/runtime.py:1638` `_remap_sp_att_forcing()` 校验/重映射）。**从 SHUD 视角，模型结构丝毫未变，只是每个站点 CSV 里的数字换成了 IFS/GFS。** 这正是"对已建模流域做 forcing"安全的根本原因。
...
IFS/GFS GRIB2 ──①获取──▶ 规范化 NetCDF ──②解码映射──▶ 单位换算/物理反演 ──③──▶
   canonical 场 ──④IDW 插值到固定站点──▶ 站点时间序列 ──⑤写 SHUD 包──▶ .tsd.forc + 每站 CSV ──⑥入库
```

![本项目 forcing 处理流程示意图](images/forcing-pipeline.png)

图注：本项目以 IFS/GFS GRIB2 为输入，经 `cfgrib`/`xarray` 解码规范化、单位换算、去累积与按源湿度反演后，用 IDW 插值到原模型固定 forcing 站点，再写出 SHUD 包（`.tsd.forc` + 每站 CSV）并入库 PostGIS/TimescaleDB。图内仅表达六阶段结构；精确字段与契约以正文为准，包括 IFS 字段 `2t`/`ssr`/`tp`、风速公式 `√(u²+v²)`、单位 `mm·day⁻¹`/`°C`/`W·m⁻²`，以及 `.tsd.forc` 表头 `ID Lon Lat X Y Z Filename`。
...
### 4.4 站点 ↔ mesh 耦合：复用且不重算

本项目通过 `workers/model_registry/qhh_production_bootstrap.py:328-473` `read_qhh_tsd_forc()` **读取原模型的 `.tsd.forc`**，原样保留每个站点的 `forcing_index`、坐标与文件名（`station_id = {project}_forc_{index:03d}`），写入 `met.met_station.properties_json.shud_forcing_index`。
运行时 `workers/shud_runtime/runtime.py:1638-1648` 据此校验 `.att` 的 `FORC` 列。
**站点↔三角元的 Thiessen 覆盖关系是 rSHUD 建模时算好的，本项目不重算、不改动。**

![站点与三角元 Thiessen 覆盖复用示意图](images/forcing-coverage-reuse.png)

图注：rSHUD 在建模阶段通过 `ForcingCoverage` 计算 forcing 站点对 mesh 三角元的 Thiessen/Voronoi 覆盖，并把覆盖关系固化到模型 `.att` 的 `FORC` 列；本项目复用这份站点↔三角元耦合关系，不重算覆盖，只刷新每个站点 CSV 中的时序数值。站点索引仍遵循 `.tsd.forc` 中 `ID Lon Lat X Y Z Filename` 的固定契约。
...
图注：上泳道表示 rSHUD/AutoSHUD 的 LDAS 再分析离线流程：逐步速率/均值经一次乘法换算，在建模时计算 Thiessen 覆盖并由 `write_forc` 写出；下泳道表示本项目的 IFS/GFS 业务预报流程：累积场先去累积，湿度按源使用原生 RH 或露点反演，再复用既有 Thiessen 覆盖并写 SHUD 包。两条流程最终收敛到同一 SHUD 契约：5 个变量 `Precip`/`Temp`/`RH`/`Wind`/`RN`，单位 `mm·day⁻¹`/`°C`/`[0,1]`/`m·s⁻¹`/`W·m⁻²`，索引文件为 `.tsd.forc`。
...
- **格式/变量/单位/时间/站点耦合**：本项目与 rSHUD/AutoSHUD **完全一致**，写出的 `.tsd.forc` 与每站 CSV 与 rSHUD `write_forc` 同构，SHUD 可无差别读入。
- **差异**：仅在数据源（IFS/GFS 预报 vs LDAS 再分析）及由此必需的预处理（去累积、按源反演湿度）；这些差异是业务化的物理必然，且本项目在风速、辐射处理上更严格。
- **不改模型**：mesh、河网、率定参数、站点↔三角元覆盖全部复用 rSHUD 建模产物，本项目只刷新固定站点的时间序列数值。
...
| IDW 插值 | `workers/forcing_producer/producer.py:1579-1641` |
