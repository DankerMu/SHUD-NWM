...
3. Inspect the evidence artifact printed by the wrapper. Every admitted repair
   ...
   | `artifact_target_not_a_file` | The probe **did** run and found something at `probe_key` that is not a regular file — almost always a directory squatting on a file key (a leftover placeholder, an interrupted writer, a rsync that created the path as a directory). Applies to the object leg and the local leg alike | No — the rebuild would have to write a file where a directory stands (`IsADirectoryError`). Inspect it first (`ls -ld "$OBJECT_STORE_ROOT/$PROBE_KEY"`), remove the placeholder once you are sure it holds no wanted data, then let the next pass re-probe or re-run the rebuild |
