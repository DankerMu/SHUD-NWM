...
| `key_consistency_drift` | 新 run 的 `river_network_version_id` / 输出段数 / 输出文件清单与旧半不一致。清单轴比较前会**两侧同时排除** checkpoint IC 归一化侧车(`state_checkpoints/` 下、以 `.` 起、以 `.normalized` 止且中间非空的那个文件,由 run **自己的 `state_save_qc`** 写出),排除内容记在行内 `key_consistency.excluded_normalization_sidecars`;`prior/new_output_file_names` 仍是全量清单 | 严重(R3):旧键行不会被 parser 删除条件覆盖,会残留。先对比该行的 `prior_output_file_names` / `new_output_file_names`:若唯一差异就是那个 `.{ic}.normalized` 侧车,说明用的是修复前的驱动器(design v9 前),升级后重跑即可,**不是**真漂移——该文件**只出现在其 IC 需要归一化(header 重定时或有负残差被 clamp)的那一半,不是消费痕迹**,新旧半哪边有都可能;其余任何差异(缺文件、多出非侧车文件、真实 checkpoint `*.cfg.ic.update` 名字变了、段数或 river network 变了)= 真漂移,停止,人工介入;**不要**自动删旧键行,也**不要**手工删/改 run 树里的输出文件去凑一致 |
