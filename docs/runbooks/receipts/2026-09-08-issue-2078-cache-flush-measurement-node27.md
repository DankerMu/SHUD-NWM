...
**版本限定**：这两组数字量自 `5a86841c` 的生产代码路径；master/PR 代码的 `/api/v1/layers` 在同一 DB 上因缺 000057 迁移（#2145）返回 500（§2 T1、§7.5），所以 layers 的冷 miss 没有 master 同版本基线，由它推出的回放百分比上界待 #2145 落地后重测；`/api/v1/runs` 的 master 侧读数见 §5/§6（36–38 ms/条）。
