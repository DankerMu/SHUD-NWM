...
| Q214-OSPEC-01 | OpenSpec validation | static validation | `openspec validate m21-qhh-hydro-met-ops-mvp --strict --no-interactive` | `.codex/evidence/issue-214/openspec-validate.log` | Follow-up passed locally: change is valid (historical #214 receipt; the change is since archived and the by-name command no longer resolves — re-validate with `openspec validate --all --strict --no-interactive`) | Proves the M21 OpenSpec change is internally valid. It does not prove runtime behavior. |
