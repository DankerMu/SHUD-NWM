...
### C1. 实机部署 receipt
- [ ] 27 用 `infra/compose.display.yml` 起 display API：`NHMS_SERVICE_ROLE=display_readonly`、`NHMS_REQUIRE_SERVICE_ROLE=true`。
- [ ] 证明 27 无 Slurm CLI/config/socket、无 Docker socket、无禁止 mount/env、`/api/v1/slurm/*` 404、published 只读、`GET /api/v1/runtime/config` 返回 `display_readonly`（对应 §10.1，跑 `scripts/validate_two_node_docker_runtime.py` + 实机探测）。
