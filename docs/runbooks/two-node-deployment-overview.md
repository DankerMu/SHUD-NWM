...
>   （本机 PG `:55433` 已 archived/stopped，仅作显式 rollback archive）
...
jq -c 'select(.record_type == "pipeline_event"
       ...
       | .payload.details.precip_mirror' \
  "$NHMS_SCHEDULER_JOURNAL_ROOT"/journal/IFS/2026050100*.jsonl
```

判读：`status` 为 `ok` **只表示拷贝当刻目的地与源一致**，不表示该周期产物完整——门挂在 `convert`
上，而 orchestrator 在任何失败尾都**不会 scancel**（唯一 `cancel_job` 调用点是运维入口）。最尖锐的一臂是轮询超时——
它不是独立状态，而是带 `error_code == "SLURM_JOB_TIMEOUT"` 的 `failed`；此时 gateway 仍报作业 running、
sbatch 可能仍在写，镜像会忠实地拷走半棵树并记 `ok`。所以同一条回执若对应的 stage 结果不是
...
`infra/env/compute.scheduler-dbfree.env.example` 两者本应分别是 NFS（`:60`）与本地 `/scratch`（`:87`）。
...
重试一次）；且本地 `prcp_rate_or_amount/` 仍在（retention 剪掉后不补、不发回执）。补漏回执记
`skipped` 且 `reason == "trees_already_mirrored"` 表示 NFS 树已与源一致、没重写任何文件——下游 restart pass
每次都会留一条，属预期开销不是故障。并发 pass（如 `skipped_duplicate_submission`）的补漏若撞上持锁者，
**不会立刻失败**：先等批锁最多 `DEFAULT_COPYBACK_LOCK_TIMEOUT_SECONDS`（900 s，可由
`NHMS_OBJECT_STORE_COPYBACK_LOCK_TIMEOUT_SECONDS` 覆盖），超时才以 `error_type == "CopybackLockTimeout"`
记一条 `failed`。hook 因锁超时记了 `failed` 时，同 pass 的出口重试还会再等满一个超时，所以该 pass
最坏要多耗两个超时才退出。下一个经过该 cycle 的下游 restart pass 会自愈，无需处置；只有此后该 cycle 再无 pass 时才手工 backfill（`current-production-ops.md`
...
- `DATABASE_URL` 只供 `compute-api` / rollback 使用；`scheduler-once` runtime env 中必须缺席。
- `NHMS_SCHEDULER_DB_FREE_REQUIRED=true`，scheduler state/lock/registry/readiness/journal/state-index selectors
  都是 `file`，对应 manifest/index/journal paths 非空且位于允许根内。
