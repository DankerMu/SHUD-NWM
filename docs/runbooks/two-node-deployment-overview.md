...
- `scheduler-once`：手工 profile，用于一次性运行生产计划入口。

22 的环境文件需要配置 writer DB、workspace、Basins/model assets、published artifact 写入路径、Slurm Gateway 和资料源参数。
...
- workspace 和 published artifact host root 可写。
