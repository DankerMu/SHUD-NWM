...
- 27 节点是只读展示面，负责面向用户提供 `/hydro-met`、`/ops` 和查询 API，只读取数据库与已发布产物。
- 两个节点之间不通过 27 调 22 控制接口完成生产操作；共享边界只有 PostgreSQL 和 published artifacts。
...
资料源 GFS/IFS
  ...
  -> 27 只读查询 PostgreSQL 和 published artifacts
  -> 用户在 /hydro-met 和 /ops 查看结果、日志、异常和人工处理建议
```
...
- 将展示需要的结果、manifest、日志、诊断证据写入 published artifacts。
...
`/ghdc`。因此 `/ghdc/data/nwm/published` 是 22 与 27 之间的发布共享面，不应被默认当成 Slurm
作业运行目录。Slurm 作业应使用计算节点可见的 workspace/object-store 路径运行；任务完成后，由 22
侧 publish/copyback 步骤把展示产品、manifest、日志和诊断证据写入 `/ghdc/data/nwm/published`。
...
- 只读读取 published artifacts 中的展示产品、manifest 和日志。
...
| published artifacts | 写 `/ghdc/data/nwm/published` | 只读读 `/home/ghdc/nwm/published` | `/hydro-met`、`/ops`、日志弹窗和诊断 |
| 对象存储或发布目录 | 写入或发布 | 只读凭证或只读挂载 | 跨节点展示产物交付 |
...
lock/evidence/runtime/temp roots、service role、source/model/basin filters、interval/max-pass bounds、Basins/model
assets、Slurm Gateway 和资料源参数。
...
27 的环境文件需要配置 readonly DB、readonly published artifact root、只读对象存储凭证、CORS 和端口。不能配置 Slurm Gateway、workspace、Basins/model assets、Docker socket 或其他计算控制能力。
...
5. 27 读取 readonly DB 和 published artifacts。
6. 用户访问 `/hydro-met` 查看水文气象结果，访问 `/ops` 查看运行状态和日志。
...
- `NHMS_SCHEDULER_RUNTIME_ROOT`、`NHMS_SCHEDULER_TEMP_ROOT` 和 `NHMS_SCHEDULER_ALLOWED_ROOTS` 与实际挂载一致。
