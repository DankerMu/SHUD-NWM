# M10 Production Closure Validation

Current validation authority for the M10 #147-#152 production-closure lanes, moved out
of the current validation matrix [`docs/VALIDATION.md`](../VALIDATION.md) so both documents
satisfy the 1,000-line structural guard. The matrix links here from each `## M10 ...`
heading stub and owns every other lane. Procedure text, commands and acceptance
evidence below are the moved M10 family unchanged except for the #1912 six-file Basins
publication command expansion and the self-lint paths that now name this file.
