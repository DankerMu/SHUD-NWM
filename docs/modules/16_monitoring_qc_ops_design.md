...
监控资料、作业、模型运行、入库、瓦片发布和数据质量，支持告警、审计和重跑。
...
| 上游 | 所有模块日志、指标、QC 结果。 |
| 下游 | 运维人员、调度器、前端状态提示。 |
| 主要数据表/存储 | `ops.pipeline_event`, `ops.quality_check`, `ops.audit_log`, `ops.alert` |
...
- GFS/IFS 周期超时可告警。
- SHUD 失败可定位到 run_id 和日志。
- active model 切换有审计记录。
- QC 不通过的产品不发布给 viewer。
