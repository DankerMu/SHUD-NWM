...
> 当前规模口径（2026-07-18）：node-22 共有 18 个业务流域（12 旧 + 6 新），每个
> 流域分别绑定 GFS `gfs_0p25` 与 IFS `ifs_0p25` 的 source-scoped variant，生产
> registry 共 36 行且全部为 `direct_grid`。附录 A 的 13 模型数据只是历史审计样本，
> 不再作为迁移数量或生产状态依据；当前 authority 以
...
| 默认 GFS/IFS 共用 0.25° binding | 当前 contract 以单一 `grid_id` 为身份字段；即使两源有序坐标签名相同，`gfs_0p25` 与 `ifs_0p25` 仍分别登记快照并构建 source-scoped variant。当前 18 个流域因此是 36 个 variant，不共用 manifest |
...
├── Legacy mapping variant（只作历史/恢复基线，不进入当前生产 registry）
├── GFS direct-grid mapping variant（grid_id=gfs_0p25）
└── IFS direct-grid mapping variant（grid_id=ifs_0p25）
```
...
## 2.2 Source-specific 原则

每个不同的规范化 source/grid 身份必须对应独立 mapping variant。`grid_signature`
钉住有序 `(grid_cell_id, lon@12dp, lat@12dp)`，`grid_id` 钉住 source 的网格契约；
两者都必须匹配。当前 manifest 只有一个 `grid_id` 字段，因此不能用签名相等绕过
`gfs_0p25` / `ifs_0p25` 的 source-scoped 身份。
...
当前生产固定分别构建：GFS variant 的 `applicable_source_ids=["gfs"]`、
`grid_id="gfs_0p25"`；IFS variant 的 `applicable_source_ids=["IFS"]`、
`grid_id="ifs_0p25"`。两个 snapshot 当前实算签名相同，只能说明有序坐标一致，
不能合并 source scope 或模型身份。未来若 contract 升级为显式多 source/grid 映射，
必须先补 parser、producer、registry 与双源实算证据，不能仅改文档宣称共享。

per-source 的 mapping binding、forcing package、scheduler 路由与 warm state
（`hydro.state_snapshot` 唯一键含 `source_id`）全部独立。当前 contract 是 manifest
级单一 `grid_id` 和 `grid_signature`，不支持 station 级 source variation。
...
不得继续使用 IFS `.sp.att`，同时混入 GFS 格点值；签名相同也必须切换到 GFS 的
source-scoped variant。
...
SHUD-NWM 为兼容旧资产，在 contract 缺失时仍保留 IDW 实现；生产 route 设置
`NHMS_SCHEDULER_REQUIRE_DIRECT_GRID=true`，发布和加载两端都会拒绝 legacy 行。
...
无论 signature 是否一致，都按 source/grid identity 分别构建 binding。
...
zhaochen_wem（1 cell）与 zhaochen_mc（4 cells）必须走 §6.5 小流域科学特批；当前
生产已记录 operator approval 并生成 source-scoped direct-grid variant，不保留 IDW
业务路由。
...
| IFS direct-grid mapping variant | 是，若 IFS 为业务 source；始终独立于 GFS binding（§2.2） |
...
IFS direct        = 独立 IFS source-scoped binding（不得与 GFS manifest 共用，见 §2.2）
```

新流域的科学验证（G11）相应简化：direct-grid 率定期 replay 与率定结果的一致性 + 预报期 forcing 合理性检查即可；**不要求** vs-IDW 的运营 A/B——没有需要保护的旧运营基线。

例外：used-cell 数 < 4 的小流域（§6.5）——direct-grid 单/寡格点驱动对其是退化，
必须取得科学特批才能注册 direct-grid；未获批时该流域保持未上线，不能改走 legacy IDW
业务路由。
...
- direct-grid variant 注册：新 `core.model_instance` 行（新 model_id），粒度为 basin × source/grid；GFS/IFS 即使签名相同也分别注册，`applicable_source_ids` 只含对应 source，legacy 行保留作 calibration/replay；
...
8. GFS/IFS 按 §2.2 分别绑定 `gfs_0p25`、`ifs_0p25`，不得共用 manifest；
...
- `ifs_0p25` 与 `gfs_0p25` 的 `grid_signature` 实测一致（`6c008901b8b7…`），但 `grid_id` 与 source scope 不同——这是 §2.2 要求两个 source-scoped variant 的直接依据。
