...
| 默认 GFS/IFS 共用 0.25° binding | 共用与否由 `grid_signature` 等值判定（producer 同一算法实算，见 §2.2）；现网实测 `ifs_0p25` 与 `gfs_0p25` 签名已一致，验证通过后应共用单一 binding，避免 13×2 份几何相同的重复资产 |
...
| 只考虑文件回滚 | 必须同时解决 scheduler 路由、warm-state 兼容和 cycle-boundary 回滚 |
...
| Migration Evidence Package | ownership 表、diff、距离、QA、审批、回滚信息 | 与 mapping variant 同寿命且不可变 |
...
不满足上述任一条 → 分别构建。若共用通过，13 流域只需 13 份 binding 而非 26 份几何相同的重复资产。
...
- 下载 bbox 来自环境变量 `NHMS_DOWNLOAD_BBOX_*`（默认 63–145°E / 8–64°N）。`grid_cell_id` 是 bbox 裁剪后的**扁平索引字符串**（如 `"36268"`），**bbox 一变全体 cell id 漂移**。registry 必须把 bbox 作为 grid snapshot 身份的一部分钉死；生产部署 env 值与 registry 值不一致时 fail closed。现网 13 流域（73–119°E / 28–43°N）均在默认覆盖内。
...
3. model CRS 权威 = package 内 `gis/*.prj`（checksum-bound）。`.sp.mesh` 本身**不含任何 CRS 元数据**；现网 13 流域是 12 套参数各异的自定义 Albers + 1 套 Transverse Mercator（qhh），全部 `PROJCS["unknown"]` 无 EPSG 码——CRS 必须逐包读取并写入 evidence，禁止任何全局 CRS 假设。
...
- startdate 异质性登记：现网 13 流域 `.tsd.forc` startdate 横跨 1951–2024，禁止任何“统一历史起点”假设；
...
- 三角形节点合法（非退化）；
- CRS 来自 package 内 `gis/*.prj` 且 checksum-bound（现网 13 流域全部 `PROJCS["unknown"]` 无 EPSG 码，禁止全局 CRS 假设）；
- element count 一致；
...
1. legacy → direct state transfer；
2. direct forcing production；
3. runtime execution；
4. direct → legacy rollback；
5. rollback 后 state continuity；
...
3. **流量展示的水文连续性**：river network 与 hydrologic core 不变（INV-2），流量图层结构上无感；但没有 Change 5 的 state clone，direct-grid 首轮只能冷启动，spin-up 期流量曲线会出现肉眼可见的失真/断裂。**cycle-boundary cutover + 兼容 state 转移是流量展示无感的硬前提**，不是可选优化。
...
回滚必须在 cycle boundary 执行：

1. route 切回 legacy model variant（CMFD 或其他建模期驱动）；
2. 使用该 variant 重新生产 forcing；
3. 选择兼容 state；
4. 生成新的 run manifest；
5. 保留失败 direct-grid package和证据；
6. 不修改失败资产；
7. 不通过 producer 内部 fallback 实现回滚。

## 11.3 State rollback

优先顺序：

1. 使用最新、core fingerprint 兼容的 state；
2. 使用最后一个 legacy state；
3. 从已知稳定 checkpoint replay；
4. 冷启动仅作为明确降级策略。
...
存量 13 流域仍按 Phase 0–8 走完整迁移流程；本节默认规则只适用于新增流域。
...
| rollback record | target model/state |
...
- CMFD/GFS/IFS model variants；
- scheduler pin；
- required mapping mode；
- whole-run source fallback；
- 禁止 mixed-grid。
...
- station-MVT / 气象代站图层按 active mapping variant 过滤（消除双轨混显）；
- shadow 期 direct-grid station mirror 的 `active_flag` 策略与 cutover 单事务原子翻转；
- display / 前端 active model_id 动态解析（禁止 pin 旧 model_id）；
- 历史 cycle 按旧 variant 查询的回归验证；
- node-27 live receipt：站点图层 + 气象代站时序弹窗 + 流量曲线跨 cutover 连续性。
...
**最终实施顺序应是：先平台 readiness，再 GFS/IFS grid registry，再单一 source pilot，再解决 state 和 route 切换，最后才进行流域批量迁移。不得先批量重写** `.sp.att`**。**

---

# 附录 A：13 流域现网基线核查（2026-07-06，node-27 实测）

本规范多处规则的现实依据。数据来自 node-27 primary PG 与 NFS 上的模型包实测。
