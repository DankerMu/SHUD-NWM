...
M18 模型资产生命周期操作仅通过受保护 API/服务执行，支持 activate、deactivate、
switch version、rollback version、supersede、deprecate。每次 mutation 前执行 preflight，
成功、阻断、重复 already-current 和 rollback 都写入审计证据；审计写入失败时不得提交状态
变化。同一 `basin_version_id` 操作域最多一个 active model；`active_flag` 保留给既有只读消费者，
canonical 状态为 `inactive`、`active`、`deprecated`、`superseded`。deactivate 若会留下缺失
active model 默认阻断，只有 `sys_admin` 带显式 override 和 reason 才可放行。M18 不实现任意
模型包上传，也不执行生产对象存储 delete/upload。
