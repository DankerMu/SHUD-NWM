...
一处**曾具名的缺口（已由 #2460 关闭）**：`basins_discovery.py::_safe_resolve_under_root` 所在模块
曾超过仓库行数守卫的上限（该守卫判的是绝对行数而非增量），为在站点写下标记而一度加入豁免。
**#2460** 已把该模块拆到上限以下（不调用 `os.path.realpath` 的文件系统原语迁至
`basins_discovery_fs.py`；该成员连同其标记留在原模块，普查键不变）并移除了该豁免。
**标记本身照常写在站点上——守卫断言二不为它开洞。**
