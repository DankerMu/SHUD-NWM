...
PR #1939 adds a direct datum: of its three round-2 catches, one (an over-general
...
本节初稿写的是「这个比值既不能支持 keep 也不能支持 cut」。**那句话是错的**：两处缺口都是系统性压低
轮换的，修正后 keep 的证据只会变强不会变弱。

第一次更正给出的数字是 `122 PR / core=80 / rotated=237 / 74.8%`。**那个数字也是错的**，因为它没排掉
下面这条第五处缺口。以脚本实际打印的为准：

| 口径 | 分母 | core | rotated | final_review | rotated 占比 |
|---|---|---|---|---|---|
| 现行（有缺口） | 168 | 161 | 302 | 混在 rotated 里 | 65.2% |
| 本文件第一次更正（错） | 122 | 80 | 237 | 56 | 74.8% |
| **修正后（记录在案）** | **77** | **80** | **105** | **33** | **56.8%** |

第五处缺口是实现时才挖出来的，比前四处都大：**45 条记账的 `round_lenses` 是扁平字符串列表**
（每轮一个镜名，如 `["binding-soundness+zero-behavior-change", "final-review"]`），不是列表的列表。
`set("correctness")` 得到的是**字符**集合，于是 core 结构上恒为 0，这 45 条把 **163 条** later-round
catch 强行记成 rotated。它们占 rotated 总数的一半以上，而且方向与前两处相反——是系统性**抬高**轮换的。

三处缺口方向相抵之后，结论仍是 keep，但强度从 65.2% 落到 **56.8%**，比现行口径**更弱**，不是更强。
本文件前两版对强度的判断（一次说不可用、一次说变强）都不准确，以本表最后一行为准。

方向没变（rotated 仍过半），所以 **keep 维持**，无行为变更。但余量已经不大：56.8% 距 50% 的翻转线只有
不到 7 个百分点，下一次 revisit 应当认真看，而不是照抄结论。

第四项（零发现轮不可见）仍然是可识别性上限而非偏置：按 catch 计数的口径永远看不见「轮进来的镜把一条
昂贵假设证伪」这类价值，本 PR 的 `review-correctness` 就是实例。不修，见 #2036 的 won't-fix 建议。
...
## Revisit 2026-09-04（PR #2027 / issue #2007 合并后 audit 再次 DECIDABLE）

样本 **171** 个多轮 merged PR（较上条 +1），later-round catches **core=166 vs rotated=304 / skipped=15**
（较上条 core +5、rotated +2）。方向未变，**keep rotation 维持**（autonomous default-keep，翻转仍须
maintainer 裁定），无行为变更。

本条值得单独记一句，因为它是本 ADR 尾部第四项「计数口径的可识别性上限」目前最干净的一个实例：

issue #2007 跑了**四轮**交叉审查、11 个 lens，在生产代码里**一个功能缺陷都没有**。四轮的产出全部是
oracle 覆盖与文书。轮换进来的 lens 确实在贡献 catch（round-4 的 `review-invariant-state` 抓到
`cache_key` 的 x/y/valid_time 无 oracle，`review-security-perf` 提出的压缩 chunk 暴露面候选被实机
**证伪**），所以计数器会把这轮记成「rotation 有效」。但真正把这个 issue 从「看起来对」推到「证明对」
的不是任何一个 lens，而是**突变测试**——把谓词删掉再跑真实 DB，看套件是否变红。四轮里每一轮的
决定性证据都是这个动作，而它在本文件的三个计数器里**不可见**：它既不是 catch，也不属于任何 lens。

同样不可见的是「证伪」的价值。round-4 的 `review-security-perf` 提了一条 P1 候选，实测（旧路由 2.82 s
vs 新路由 3.15 s，同一批压缩 chunk）判定该暴露面**改前就存在**。按 catch 计数它是噪音；按工程价值它
省掉了一次没必要的设计改动。这与本文件此前记的「昂贵假设证伪」是同一类。
...
### Revisit 2026-09-13（post PR #2292 / issue #2290）

追加后实测 645 行、635 merged、10 terminal；211 个多轮样本，
later-round catches core=270、rotated=270、phase=57、skipped=15。
本单 high 两轮；两项 contract / compatibility 发现均在首轮，第二轮和最终
...
### Revisit 2026-09-13（post PR #2289 / issues #2041 #2146 #1926 #2075）

追加后实测 647 行、637 merged、10 terminal；212 个多轮样本，
later-round catches core=270、rotated=270、phase=57、skipped=15。
本单 high 五轮（12/6/5/3/0 verified），经三轮门 breadth retro（用户裁定缩范围至 #2294）
...
### Revisit 2026-09-13（post PR #2305 / issues #2081 #2169 #2038 #1975 #1976）

追加后实测 650 行、640 merged、10 terminal；213 个多轮样本，
later-round catches core=270、rotated=270、phase=57、skipped=15。
本单 high 单轮（四席 clean、0 候选），Phase 7 gap sweep clean；单轮 PR 不进入轮换归因样本，
不能据此判断轮换收益。记录 deferral：keep/cut 仍待维护者决策；本次合并预授权
不含审核策略调整。现行 keep、座位上限不变。
...
产品源码字节一致不能替代最终 tracked tree 的实际 audit；扫描输入的
CI reader-edge 缺口另由 #2323 跟踪。
...
前两条 Revisit 都把 keep/cut 推迟到「#2477 落地、重算比例之后」。本条就是那次重算。#2036 修复后的审计已从 `002ba4b59^` 恢复为被跟踪的 `scripts/governance/loop_log_audit.py`，这次在 `docs/review-loop-log.jsonl` 上实跑，日志共 690 行，其中 677 行 merged、13 行 terminal。

- **结果：不再是 DECIDABLE，退出码 0。** 231 个多轮已合并 PR 中，128 个被排除在轮换分母之外：79 个只收窄、没有新增 lens；45 个 round-1 lens 集合是 non-list；1 个是 empty（PR 1788）；3 个声明为 incidental。剩下 103 个发生轮换的 PR，later-round catches 为 **core=146 / rotated=173 / final_review=43**，另有 15 个不可归属。rotated 份额 **54.2%**，与已记录的 `keep` 一致。用 `--rotation-decision none` 做无条件判定时同样指向「集中在 rotated-in lenses」。
- **与上两条的差异完全来自算法。** 上两条的 core=388 / rotated=275 / phase=74、rotated 份额 41.5%，是修复前算法算出来的，偏差正是 #2036 修掉的那几项：子集收缩的轮次被算进分母；扁平字符串被逐字符拆开；Phase 7 的 catch 被记成 rotated。所以前两条里「判据不成立、只靠 default-keep」的判断不再成立。
- **样本已冻结，问题的对象也已不在。** 2026-09-22 安装的 `subagent-workflow` 已不再写这份日志，最后一行是 2026-09-18 的 PR #2496。它的 re-review 固定用一个 `correctness+test-evidence` 席，不再做 lens 轮换。因此这个 ADR 所裁决的行为在当前工作流里已经不存在。

处置：**keep 维持**，这次有修复后算法的实数支撑。本 ADR 关闭为历史决定：只有日志写入步骤回到工作流、或者轮换重新被引入时才需要再审。审计工具留在 `scripts/governance/loop_log_audit.py`，需要时手动运行。
